...
pub use crate::special::{Date, TimeOfDay, Timestamp};
use bytes::BytesMut;
...
impl<'a> FromSql<'a> for SystemTime {
    fn from_sql(_: &Type, raw: &'a [u8]) -> Result<SystemTime, Box<dyn Error + Sync + Send>> {
        ...
        if matches!(time, i64::MIN | i64::MAX) {
            return Err(
                "timestamp infinity cannot be decoded as SystemTime; use Timestamp<SystemTime>"
                    .into(),
            );
        }
        ...
        .ok_or("value too large to decode as SystemTime")?;
...
impl ToSql for SystemTime {
    fn to_sql(&self, _: &Type, w: &mut BytesMut) -> Result<IsNull, Box<dyn Error + Sync + Send>> {
        ...
        let to_usec = |d: Duration| -> Result<u64, Box<dyn Error + Sync + Send>> {
            d.as_secs()
                .checked_mul(USEC_PER_SEC)
                .and_then(|usec| usec.checked_add(u64::from(d.subsec_nanos()) / NSEC_PER_USEC))
                .ok_or_else(|| "value too large to transmit".into())
        };

        let time = match self.duration_since(epoch) {
            Ok(duration) => {
                i64::try_from(to_usec(duration)?).map_err(|_| "value too large to transmit")?
            }
            Err(e) => i64::try_from(to_usec(e.duration())?)
                .ok()
                .and_then(i64::checked_neg)
                .ok_or("value too large to transmit")?,
        };
        // PostgreSQL reserves the signed endpoints for infinity. A finite
        // SystemTime must never silently turn into that different value.
        if matches!(time, i64::MIN | i64::MAX) {

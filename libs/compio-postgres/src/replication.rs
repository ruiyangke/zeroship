...
mod tests {
    ...
    /// The tag-dispatch test above rules on all 19 legal tags with well-formed
    /// bodies. This sweeps the FIELD readers behind them: a legal tag followed
    /// by a body of the wrong length, truncated mid-cstring, or carrying
    /// plausible-but-wrong values.
    ///
    /// Worth sweeping because `decode_frame` is the largest hand-rolled parser
    /// here and one arm reads with `Buf::get_u64`, which panics rather than
    /// erroring when short. Its exact-length guard is all that stands between a
    /// truncated StreamAbort tail and a panicked replication task.
    ///
    /// Body lengths are biased onto the field boundaries the arms demand; a
    /// uniform length would be refused by the first `read_u32` and sweep
    /// nothing behind it.
    #[test]
    fn hostile_pgoutput_frames_are_refused_rather_than_panicking() {
        struct Rng(u64);
        impl Rng {
        ...
        // Every legal tag, plus bytes that are not tags at all.
        const TAGS: &[u8] = b"BCOIUDTRYMASEcp\x00\xff";
        // 8/16/24 are LSN and timestamp boundaries; 25 and 29 sit either side
        // of StreamAbort's optional 16-byte tail after its two xids.
        const LENS: &[usize] = &[0, 1, 4, 8, 12, 16, 20, 24, 25, 28, 29, 32, 36, 40];

        let mut rng = Rng(0x9017_0007_0000_0001);
        let (mut decoded, mut refused) = (0u32, 0u32);
        let mut decoder = pgoutput::Decoder::new();
        for _ in 0..40_000 {
            let mut frame = vec![TAGS[rng.below(TAGS.len())]];
            for _ in 0..LENS[rng.below(LENS.len())] {
                frame.push(rng.byte());
            }
            // Recycle the decoder so its stream state varies without one poisoned
            // decoder dominating the sweep.
            if rng.below(64) == 0 {
        ...
        assert!(
            decoded > 100,
            "no generated frame decoded, so the field readers were never swept: decoded={decoded}"
        );
        assert!(refused > 1_000, "no frame was refused: refused={refused}");
    }

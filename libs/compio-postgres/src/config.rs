...
mod dsn_fuzz {
    ...
    /// Metacharacters the DSN grammar assigns meaning to, plus multibyte text
    /// so a byte-offset slice lands mid-character if one is computed wrong.
    const PIECES: &[&str] = &[
        "=",
        "'",
        "\\",
    ...
    #[test]
    fn a_hostile_dsn_is_refused_rather_than_panicking() {
        let mut rng = Rng(0x0D5F_F0FF_1234_5678);
        let (mut ok, mut err) = (0u32, 0u32);
        for _ in 0..40_000 {
            let mut dsn = String::new();
            for _ in 0..=rng.below(12) {
                dsn.push_str(PIECES[rng.below(PIECES.len())]);
            }
            match Config::from_str(&dsn) {
                Ok(_) => ok += 1,
                Err(_) => err += 1,
            }
        }
        eprintln!("DSN fuzz: ok={ok} err={err}");
        assert!(
            ok > 100,
            "the generator never produced a parseable DSN, so it proved nothing: ok={ok}"
        );
        assert!(
            err > 100,

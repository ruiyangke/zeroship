...
Our compio async-fn loop doesn't naturally express "park and come back". The naive drain `while let Some(msg) = receiver.next().await` blocks the entire connection task and reproduces the deadlock hazard. The fix is a local `select_biased!` that races `receiver.next()` against `read_backend(&mut self.stream)`:

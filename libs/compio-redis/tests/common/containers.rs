//! Owned Docker fixtures shared by Redis driver, KV, and V8 integration tests.

use super::redis_types;
...
#[derive(Default)]
pub struct Fixtures {
    redis: OnceLock<Container<GenericImage>>,
    cluster: OnceLock<ClusterFixture>,
}

struct ClusterFixture {
    config: redis_types::RedisConfig,
    _cluster: Vec<Container<GenericImage>>,
}

impl ClusterFixture {
    fn start(redis: &Container<GenericImage>) -> Self {
        ...
        let seeds = cluster
            ...
            .collect::<Vec<_>>();
        Self {
            config: redis_types::RedisConfig::new(redis_types::Topology::Cluster { seeds }),
            _cluster: cluster,
        }
    }
}
...
impl Fixtures {
    pub fn redis_url(&self) -> String {
        let redis_types::Topology::Standalone { endpoint } = self.redis_config().topology else {
            unreachable!("standalone fixture")
        };
        format!("redis://{endpoint}")
    }

    pub fn cluster_urls(&self) -> Vec<String> {
        let redis_types::Topology::Cluster { seeds } = self.cluster_config().topology else {
            unreachable!("cluster fixture")
        };
        seeds
            .iter()
    ...
    pub fn redis_config(&self) -> redis_types::RedisConfig {
        standalone(self.redis.get_or_init(start_redis))
    }

    pub fn cluster_config(&self) -> redis_types::RedisConfig {
        self.cluster
            .get_or_init(|| ClusterFixture::start(self.redis.get_or_init(start_redis)))
            .config
            .clone()
    }
}
...
pub fn standalone(container: &Container<GenericImage>) -> redis_types::RedisConfig {
    redis_types::RedisConfig::new(redis_types::Topology::Standalone {

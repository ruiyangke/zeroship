...
-- relax zeroship.events.sandbox_id to NULLable. Transcribed from
-- crates/sandbox/migrations/0005_events_sandbox_id_nullable.sql
-- (sandbox.* → zeroship.*).
--
-- Pre-fix, `events.sandbox_id` was `NOT NULL` with a CHECK requiring the
...
-- never matched a real sandbox row but DID land in `idx_events_sandbox_ts`,
...
--changeset zeroship-sandbox:sandbox-events-sandbox-id-nullable splitStatements:false
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
        ...
        EXECUTE 'ALTER TABLE zeroship.events ALTER COLUMN sandbox_id DROP NOT NULL';
    END IF;
END
$$;
--rollback SELECT 1;

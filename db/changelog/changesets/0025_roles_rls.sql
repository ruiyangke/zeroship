...
--changeset zeroship:rls-app-secrets splitStatements:true
...
--rollback DROP POLICY IF EXISTS tenant_isolation ON zeroship.app_secrets;
--rollback ALTER TABLE zeroship.app_secrets NO FORCE ROW LEVEL SECURITY;
--rollback ALTER TABLE zeroship.app_secrets DISABLE ROW LEVEL SECURITY;

-- ─── RLS: gateway_sessions (key: app_id UUID; GUC zeroship.tenant_app) ────
--changeset zeroship:rls-gateway-sessions splitStatements:true
...
CREATE POLICY tenant_isolation ON zeroship.gateway_sessions
    USING (app_id = current_setting('zeroship.tenant_app', true)::uuid)
    WITH CHECK (app_id = current_setting('zeroship.tenant_app', true)::uuid);
--rollback DROP POLICY IF EXISTS tenant_isolation ON zeroship.gateway_sessions;
--rollback ALTER TABLE zeroship.gateway_sessions NO FORCE ROW LEVEL SECURITY;
--rollback ALTER TABLE zeroship.gateway_sessions DISABLE ROW LEVEL SECURITY;

-- ─── RLS: app_session_anchors (key: app_id UUID; GUC zeroship.tenant_app) ─
--changeset zeroship:rls-app-session-anchors splitStatements:true
...
CREATE POLICY tenant_isolation ON zeroship.app_session_anchors
    USING (app_id = current_setting('zeroship.tenant_app', true)::uuid)
    WITH CHECK (app_id = current_setting('zeroship.tenant_app', true)::uuid);
--rollback DROP POLICY IF EXISTS tenant_isolation ON zeroship.app_session_anchors;
--rollback ALTER TABLE zeroship.app_session_anchors NO FORCE ROW LEVEL SECURITY;
--rollback ALTER TABLE zeroship.app_session_anchors DISABLE ROW LEVEL SECURITY;
...
--changeset zeroship:rls-app-user-identities splitStatements:true
...
--rollback DROP POLICY IF EXISTS tenant_isolation ON zeroship.app_user_identities;
--rollback ALTER TABLE zeroship.app_user_identities NO FORCE ROW LEVEL SECURITY;
--rollback ALTER TABLE zeroship.app_user_identities DISABLE ROW LEVEL SECURITY;

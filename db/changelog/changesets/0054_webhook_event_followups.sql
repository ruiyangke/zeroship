...
--changeset zeroship:refund-status-add-terminal-failure splitStatements:true
...
--rollback ALTER DOMAIN zeroship.refund_status DROP CONSTRAINT refund_status_check;
--rollback ALTER DOMAIN zeroship.refund_status ADD CONSTRAINT refund_status_check CHECK (VALUE IN ('pending','issued'));

--changeset zeroship:refunds-failed-at-column splitStatements:true
...
--rollback ALTER TABLE zeroship.refunds DROP COLUMN failed_at;

--changeset zeroship:refunds-resolve-by-cash-ref splitStatements:false
...
END;
$fn$ LANGUAGE plpgsql;
--rollback DROP FUNCTION IF EXISTS zeroship.refunds_immutable() CASCADE;

--changeset zeroship:refunds-over-refund-exclude-failed splitStatements:false
...
END;
$fn$ LANGUAGE plpgsql;
--rollback DROP FUNCTION IF EXISTS zeroship.refunds_no_over_refund() CASCADE;

--changeset zeroship:credit-entry-kind-add-refund-clawback splitStatements:true
...
--rollback ALTER DOMAIN zeroship.credit_entry_kind DROP CONSTRAINT credit_entry_kind_check;
--rollback ALTER DOMAIN zeroship.credit_entry_kind ADD CONSTRAINT credit_entry_kind_check CHECK (VALUE IN ('grant','promo','goodwill','refund_to_credit','consumed','void_reversal'));

--changeset zeroship:credit-ledger-refund-clawback-constraints splitStatements:false
...
--rollback DROP INDEX IF EXISTS zeroship.credit_ledger_refund_clawback_note_idx;
--rollback ALTER TABLE zeroship.credit_ledger DROP CONSTRAINT credit_ledger_grant_ref;
--rollback ALTER TABLE zeroship.credit_ledger ADD CONSTRAINT credit_ledger_grant_ref CHECK ((kind IN ('consumed','void_reversal') AND consumed_from_grant_id IS NOT NULL) OR (kind NOT IN ('consumed','void_reversal') AND consumed_from_grant_id IS NULL));
--rollback ALTER TABLE zeroship.credit_ledger DROP CONSTRAINT credit_ledger_kind_sign;
--rollback ALTER TABLE zeroship.credit_ledger ADD CONSTRAINT credit_ledger_kind_sign CHECK ((kind = 'consumed' AND amount_cents < 0) OR (kind <> 'consumed' AND amount_cents > 0));

-- ─────────────────────────────────────────────────────────────────────────────
-- (2) payout.failed — a creator's connected-account payout bounced
-- ─────────────────────────────────────────────────────────────────────────────

--changeset zeroship:payout-failures splitStatements:true
...
--rollback DROP INDEX IF EXISTS zeroship.payout_failures_creator_idx;
--rollback DROP TABLE zeroship.payout_failures;

--changeset zeroship:payout-failures-immutable splitStatements:false
...
--rollback DROP TRIGGER IF EXISTS payout_failures_immutable_trg ON zeroship.payout_failures;
--rollback DROP FUNCTION IF EXISTS zeroship.payout_failures_immutable();

-- ─────────────────────────────────────────────────────────────────────────────
-- (3) payment_intent.payment_failed — an end-user's Connect checkout charge failed
-- ─────────────────────────────────────────────────────────────────────────────

--changeset zeroship:connect-checkout-failures splitStatements:true
...
--rollback DROP INDEX IF EXISTS zeroship.connect_checkout_failures_creator_idx;
--rollback DROP TABLE zeroship.connect_checkout_failures;

--changeset zeroship:connect-checkout-failures-immutable splitStatements:false
...
--rollback DROP TRIGGER IF EXISTS connect_checkout_failures_immutable_trg ON zeroship.connect_checkout_failures;
--rollback DROP FUNCTION IF EXISTS zeroship.connect_checkout_failures_immutable();

-- ─────────────────────────────────────────────────────────────────────────────
-- Notification kinds for the two new creator-facing events
-- ─────────────────────────────────────────────────────────────────────────────

--changeset zeroship:billing-notification-kind-add-webhook-followups splitStatements:true
...
--rollback ALTER DOMAIN zeroship.billing_notification_kind DROP CONSTRAINT billing_notification_kind_check;
--rollback ALTER DOMAIN zeroship.billing_notification_kind ADD CONSTRAINT billing_notification_kind_check CHECK (VALUE IN ('payment_failed','past_due','suspended','recovered','invoice_finalized','refunded','disputed','spend_warn','spend_degrade','spend_block'));
...
DO $g$ BEGIN
  ...
  END IF;
END $g$;
--rollback DO $rb$ BEGIN IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='zeroship_control') THEN EXECUTE 'REVOKE ALL ON zeroship.payout_failures FROM zeroship_control'; EXECUTE 'REVOKE ALL ON zeroship.connect_checkout_failures FROM zeroship_control'; END IF; END $rb$;

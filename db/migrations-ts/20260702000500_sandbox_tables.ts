  ...
  schema() {
    table("deleted_sandboxes", { schema: "zeroship" }).create({
      columns: {
        ...
        sandbox_id: t.text().required(),
        user_id: t.text().required(),
        deleted_at: t.timestamp().required().default(now()),
      },
      primaryKey: ["id"],
    });
    ...
    table("hosts", { schema: "zeroship" }).create({
      columns: {
        id: t.bigInt().required().identity(),
        host_id: t.text().required(),
        boot_id: t.text().required(),
        hostname: t.text().required(),
        region: t.text().required(),
        backend: t.text().required(),
        started_at: t.timestamp().required().default(now()),
        last_heartbeat: t.timestamp().required().default(now()),
        status: t.text().required().default("alive"),
        drain_started_at: t.timestamp(),
        version: t.text().required().default(""),
    ...
    table("sandbox_events", { schema: "zeroship" }).create({
      columns: {
        ...
        ts: t.timestamp().required().default(now()),
        data: t.json().required().default({}),
      },
      primaryKey: ["id"],
    });
    ...
    table("sandboxes", { schema: "zeroship" }).create({
      columns: {
        ...
        agent_url: t.text(),
        host_id: t.text().required(),
        generation: t.bigInt().required().default(0),
        status: t.text().required().default("starting"),
        key_fp: t.text().required(),
        created_at: t.timestamp().required().default(now()),
        started_at: t.timestamp(),
        stopped_at: t.timestamp(),
        last_used_at: t.timestamp().required().default(now()),
        ...
        idle_snapshot_count_long_poll: t.boolean().required().default(false),
        last_drain_failure_at: t.timestamp(),
        drain_failure_count: t.int().required().default(0),
      },
      primaryKey: ["id"],
    });
    ...
    table("shares", { schema: "zeroship" }).create({
      columns: {
        id: t.bigInt().required().identity(),
        token_id: t.text().required(),
        sandbox_id: t.text().required(),
        port: t.int().required(),
        scope: t.text().required(),
        secret_version: t.int().required(),
        issued_at: t.timestamp().required().default(now()),
        expires_at: t.timestamp().required(),
        revoked_at: t.timestamp(),
        use_count: t.bigInt().required().default(0),
    ...
    table("wake_jobs", { schema: "zeroship" }).create({
      columns: {
        ...
        agent_url: t.text(),
        lessee: t.text().required(),
        lessee_updated_at: t.timestamp().required().default(now()),
      },
      primaryKey: ["id"],
    });

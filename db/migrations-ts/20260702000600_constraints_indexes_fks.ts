...
export function up() {
  ...
  pgTable("app_session_anchors", { schema: "zeroship" }).index("app_session_anchors_user_idx").add({ on: ["app_id", "global_user_id"], where: (col) => col("revoked_at").isNull() });
  table("app_user_identities", { schema: "zeroship" }).index("app_user_identities_pairwise_sub_idx").add({ on: ["pairwise_sub"] });
  pgTable("app_user_identities", { schema: "zeroship" }).index("app_user_identities_relay_active_idx").add({ on: ["relay_email"], unique: true, where: (col) => col("relay_email").isNotNull().and(col("revoked_at").isNull()) });
  ...
  pgTable("gateway_sessions", { schema: "zeroship" }).index("auth_gateway_sessions_app_sid_idx").add({ on: ["app_id", "sid"], where: (col) => col("sid").isNotNull().and(col("revoked_at").isNull()) });
  pgTable("gateway_sessions", { schema: "zeroship" }).index("auth_gateway_sessions_idle_idx").add({ on: ["idle_expires_at"], where: (col) => col("revoked_at").isNull() });
  pgTable("magic_completions", { schema: "zeroship" }).index("auth_magic_completions_expires_idx").add({ on: ["expires_at"], where: (col) => col("consumed_at").isNull() });
  ...
  pgTable("users", { schema: "zeroship" }).index("auth_users_deletion_due_idx").add({ on: ["deletion_scheduled_for"], where: (col) => col("deletion_scheduled_for").isNotNull().and(col("anonymized_at").isNull()) });
  table("authz_decisions", { schema: "zeroship" }).index("authz_decisions_occurred_idx").add({ on: [{ column: "occurred_at", order: "desc" }] });
  pgTable("authz_decisions", { schema: "zeroship" }).index("authz_decisions_user_idx").add({ on: ["actor_user_id"], where: (col) => col("actor_user_id").isNotNull() });
  table("billing_disputes", { schema: "zeroship" }).index("billing_disputes_invoice_idx").add({ on: ["invoice_id"] });
  pgTable("billing_metrics", { schema: "zeroship" }).index("billing_metrics_owner_app_idx").add({ on: ["owner_app"], where: (col) => col("owner_app").isNotNull() });
  ...
  pgTable("billing_reconciliation_findings", { schema: "zeroship" }).index("billing_reconciliation_findings_open_idx").add({ on: ["detected_at"], where: (col) => col("resolved_at").isNull() });
  ...
  pgTable("credit_ledger", { schema: "zeroship" }).index("credit_ledger_idempotency_key_idx").add({ on: ["idempotency_key"], unique: true, where: (col) => col("idempotency_key").isNotNull() });
  pgTable("credit_ledger", { schema: "zeroship" }).index("credit_ledger_refund_clawback_note_idx").add({ on: ["note"], unique: true, where: (col) => col("kind").cast({ to: "text" }).eq("refund_clawback") });
  pgTable("credit_ledger", { schema: "zeroship" }).index("credit_ledger_refund_to_credit_note_idx").add({ on: ["note"], unique: true, where: (col) => col("kind").cast({ to: "text" }).eq("refund_to_credit") });
  pgTable("device_grants", { schema: "zeroship" }).index("device_grants_client_id_idx").add({ on: ["client_id"], where: (col) => col("client_id").isNotNull() });
  table("device_grants", { schema: "zeroship" }).index("device_grants_expires_at_idx").add({ on: ["expires_at"] });
  pgTable("device_grants", { schema: "zeroship" }).index("device_grants_provider_pending_user_code_idx").add({ on: ["provider", "user_code"], where: (col) => col("status").eq("pending") });
  ...
  pgTable("billing_notifications", { schema: "zeroship" }).index("idx_billing_notifications_pending").add({ on: ["claimed_at"], where: (col) => col("status").cast({ to: "text" }).eq("pending") });
  ...
  pgTable("creator_account_history", { schema: "zeroship" }).index("idx_creator_account_history_one_open").add({ on: ["creator_id"], unique: true, where: (col) => col("unlinked_at").isNull() });
  ...
  pgTable("creator_billing_status", { schema: "zeroship" }).index("idx_creator_billing_status_past_due").add({ on: ["past_due_since"], where: (col) => col("state").cast({ to: "text" }).eq("past_due") });
  ...
  pgTable("sandbox_events", { schema: "zeroship" }).index("idx_sandbox_events_ts_brin").add({ on: ["ts"], using: "brin", with: { pagesPerRange: 32 } });
  ...
  pgTable("sandboxes", { schema: "zeroship" }).index("idx_sandboxes_active_user_project").add({ on: ["user_id", "project_id"], unique: true, where: (col) => col("deleted_at").isNull().and(col("status").in(["starting", "running", "recreating"])) });
  table("sandboxes", { schema: "zeroship" }).index("idx_sandboxes_created_at").add({ on: ["created_at"] });
  pgTable("sandboxes", { schema: "zeroship" }).index("idx_sandboxes_host_id_status").add({ on: ["host_id", "status"], where: (col) => col("deleted_at").isNull() });
  pgTable("sandboxes", { schema: "zeroship" }).index("idx_sandboxes_status_last_used").add({ on: ["status", "last_used_at"], where: (col) => col("deleted_at").isNull() });
  pgTable("sandboxes", { schema: "zeroship" }).index("idx_sandboxes_user_id").add({ on: ["user_id"], where: (col) => col("deleted_at").isNull() });
  pgTable("shares", { schema: "zeroship" }).index("idx_shares_expires_at").add({ on: ["expires_at"], where: (col) => col("deleted_at").isNull().and(col("revoked_at").isNull()) });
  pgTable("shares", { schema: "zeroship" }).index("idx_shares_iss_issued_at").add({ on: ["iss", "issued_at"], where: (col) => col("deleted_at").isNull().and(col("iss").isNotNull()) });
  pgTable("shares", { schema: "zeroship" }).index("idx_shares_sandbox_id_port").add({ on: ["sandbox_id", "port"], where: (col) => col("deleted_at").isNull() });
  ...
  pgTable("invoice_payments", { schema: "zeroship" }).index("invoice_payments_charge_provider_ref_key").add({ on: ["invoice_id", "provider_ref"], unique: true, where: (col) => col("kind").cast({ to: "text" }).eq("charge") });
  pgTable("invoice_payments", { schema: "zeroship" }).index("invoice_payments_dispute_provider_ref_key").add({ on: ["invoice_id", "provider_ref", "kind"], unique: true, where: (col) => col("kind").cast({ to: "text" }).in(["dispute_debit", "dispute_reversal"]) });
  table("invoice_payments", { schema: "zeroship" }).index("invoice_payments_invoice_idx").add({ on: ["invoice_id"] });
  pgTable("invoices", { schema: "zeroship" }).index("invoices_active_period_claim").add({ on: ["creator_id", "period"], unique: true, where: (col) => col("status").cast({ to: "text" }).ne("void") });
  ...
  pgTable("oauth_refresh_tokens", { schema: "zeroship" }).index("oauth_refresh_tokens_idem_reap_idx").add({ on: ["idem_expires_at"], where: (col) => col("idem_response_enc").isNotNull() });
  pgTable("oauth_refresh_tokens", { schema: "zeroship" }).index("oauth_refresh_tokens_one_active_per_family").add({ on: ["refresh_family_id"], unique: true, where: (col) => col("rotated_at").isNull().and(col("revoked_at").isNull()) });
  ...
  pgTable("pending_disputes", { schema: "zeroship" }).index("pending_disputes_charge_idx").add({ on: ["charge"], where: (col) => col("charge").isNotNull() });
  pgTable("pending_disputes", { schema: "zeroship" }).index("pending_disputes_payment_intent_idx").add({ on: ["payment_intent"], where: (col) => col("payment_intent").isNotNull() });
  pgTable("permission_tokens", { schema: "zeroship" }).index("permission_tokens_owner_active_idx").add({ on: ["owner_id"], where: (col) => col("revoked_at").isNull() });
  ...
  pgTable("sandboxes", { schema: "zeroship" }).index("sandboxes_idle_snapshot_idx").add({ on: ["last_used_at"], where: (col) => col("status").eq("running").and(col("idle_snapshot_opted_in")) });
  pgTable("sandboxes", { schema: "zeroship" }).index("sandboxes_status_lessee_idx").add({ on: ["status", "lessee_updated_at"], where: (col) => col("status").in(["snapshotting", "restoring", "restoring_cold"]) });
  ...
  pgTable("wake_jobs", { schema: "zeroship" }).index("wake_jobs_lessee_idx").add({ on: ["lessee_updated_at"], where: (col) => col("state").notIn(["ok", "failed"]) });
  table("wake_jobs", { schema: "zeroship" }).index("wake_jobs_sandbox_idx").add({ on: ["sandbox_id"] });
  pgTable("wake_jobs", { schema: "zeroship" }).index("wake_jobs_sandbox_pending_uniq").add({ on: ["sandbox_id"], unique: true, where: (col) => col("state").notIn(["ok", "failed"]) });
  pgTable("wake_jobs", { schema: "zeroship" }).index("wake_jobs_state_idx").add({ on: ["state"], where: (col) => col("state").notIn(["ok", "failed"]) });

...
export function up() {
  ...
  raw({ sql: "ALTER TABLE ONLY zeroship.app_oauth_clients\n    ADD CONSTRAINT app_oauth_clients_client_id_fkey FOREIGN KEY (client_id) REFERENCES zeroship.oauth_clients(client_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.app_session_anchors\n    ADD CONSTRAINT app_session_anchors_client_id_fkey FOREIGN KEY (client_id) REFERENCES zeroship.oauth_clients(client_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.app_user_identities\n    ADD CONSTRAINT app_user_identities_app_client_id_fkey FOREIGN KEY (app_client_id) REFERENCES zeroship.oauth_clients(client_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.billing_customer_refs\n    ADD CONSTRAINT billing_customer_refs_creator_id_fkey FOREIGN KEY (creator_id) REFERENCES zeroship.creator_billing(creator_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.billing_line_provider_refs\n    ADD CONSTRAINT billing_line_provider_refs_line_fk FOREIGN KEY (invoice_id, app_id, segment_no) REFERENCES zeroship.invoice_lines(invoice_id, app_id, segment_no) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.billing_notifications\n    ADD CONSTRAINT billing_notifications_creator_id_fkey FOREIGN KEY (creator_id) REFERENCES zeroship.creator_billing(creator_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.connect_checkout_failures\n    ADD CONSTRAINT connect_checkout_failures_creator_id_fkey FOREIGN KEY (creator_id) REFERENCES zeroship.creator_accounts(creator_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.creator_billing_status\n    ADD CONSTRAINT creator_billing_status_creator_id_fkey FOREIGN KEY (creator_id) REFERENCES zeroship.creator_billing(creator_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.creator_billing_status_history\n    ADD CONSTRAINT creator_billing_status_history_creator_id_fkey FOREIGN KEY (creator_id) REFERENCES zeroship.creator_billing(creator_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  ...
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.credit_ledger\n    ADD CONSTRAINT credit_ledger_creator_id_fkey FOREIGN KEY (creator_id) REFERENCES zeroship.creator_billing(creator_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });
  // TODO(dsl-v2): multi-column or non-id foreign keys remain raw until the FK lowerer accepts the full constraint model
  raw({ sql: "ALTER TABLE ONLY zeroship.device_grants\n    ADD CONSTRAINT device_grants_client_id_fkey FOREIGN KEY (client_id) REFERENCES zeroship.oauth_clients(client_id) ON DELETE CASCADE", reason: "the current basic FK path is limited to single-column references to id" });

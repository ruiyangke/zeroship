...
export function up() {
  ...
  pgTable("credit_ledger", { schema: "zeroship" }).index("credit_ledger_refund_clawback_note_idx").add({ on: ["note"], unique: true, where: (c) => c("kind").cast("text").eq("refund_clawback") });
  pgTable("credit_ledger", { schema: "zeroship" }).index("credit_ledger_refund_to_credit_note_idx").add({ on: ["note"], unique: true, where: (c) => c("kind").cast("text").eq("refund_to_credit") });
  ...
  pgTable("billing_notifications", { schema: "zeroship" }).index("idx_billing_notifications_pending").add({ on: ["claimed_at"], where: (c) => c("status").cast("text").eq("pending") });
  ...
  pgTable("creator_billing_status", { schema: "zeroship" }).index("idx_creator_billing_status_past_due").add({ on: ["past_due_since"], where: (c) => c("state").cast("text").eq("past_due") });
  ...
  pgTable("invoice_payments", { schema: "zeroship" }).index("invoice_payments_charge_provider_ref_key").add({ on: ["invoice_id", "provider_ref"], unique: true, where: (c) => c("kind").cast("text").eq("charge") });
  pgTable("invoice_payments", { schema: "zeroship" }).index("invoice_payments_dispute_provider_ref_key").add({ on: ["invoice_id", "provider_ref", "kind"], unique: true, where: (c) => c("kind").cast("text").in(["dispute_debit", "dispute_reversal"]) });
  table("invoice_payments", { schema: "zeroship" }).index("invoice_payments_invoice_idx").add({ on: ["invoice_id"] });
  pgTable("invoices", { schema: "zeroship" }).index("invoices_active_period_claim").add({ on: ["creator_id", "period"], unique: true, where: (c) => c("status").cast("text").ne("void") });

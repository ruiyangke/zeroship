...
export default {
  ...
  schema() {
    ...
    // the feed it already owns. The worker is denied by default in the platform
    // schema (db/migrations-ts/20260818000200_worker_database_authority.ts
    // revokes future tables through ALTER DEFAULT PRIVILEGES), so it needs no
    // revoke here.
    grant({

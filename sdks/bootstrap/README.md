...
- The Vite plugin (`@zeroship/vite-plugin`), via `import` in
  `src/dev-bootstrap/index.ts` so dev-mode dispatch + schema install
...
- `install-schema.ts` — `installSchema(schema, env) → { collections, ready }`,
  the orchestrator behind `export default { schema }`. Walks the schema
  map, calls `registerModel` in topo order, plants typed Collection
...
- `runtime-entry.ts` — TLA orchestrator the runtime crate
  ...
  evaluation gates on DDL settling.

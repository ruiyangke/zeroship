/*
 * Capture-tabs-evidence: mirrors capture-toggle-evidence.mjs.
 * Walks each Tabs story; writes a 2x PNG per story under both both Crystal themess.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      // Wait for any Tabs root to mount before snapping.
      await page.locator(".zs-tabs").first().waitFor({
        state: "visible",
        timeout: 5000,
      });
      // 250ms settle covers Base UI's indicator measure pass — the
      // indicator hides until layout settles + posts position vars.
      await page.waitForTimeout(250);
      const file = join(outDir, `${theme.value}-tabs-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "tabs-evidence.json"),

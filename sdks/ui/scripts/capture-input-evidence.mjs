/*
 * Capture-input-evidence: spins up an http-server over storybook-static,
 * loads each Input story under both both Crystal themess, and writes a 2x PNG
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-input").first().waitFor({ state: "visible", timeout: 5000 });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-input-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "input-evidence.json"),

/*
 * Capture-accordion-evidence: walks each Accordion story; writes a 2x
 * PNG per story under both both Crystal themess. Mirrors capture-tabs-evidence.mjs.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      // Wait for any Accordion root to mount before snapping.
      await page.locator(".zs-accordion").first().waitFor({
        state: "visible",
        timeout: 5000,
      });
      // 250ms settle covers any opening-panel transition.
      await page.waitForTimeout(250);
      const file = join(outDir, `${theme.value}-accordion-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "accordion-evidence.json"),

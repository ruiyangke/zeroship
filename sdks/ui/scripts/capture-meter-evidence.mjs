/*
 * Capture-meter-evidence: mirrors capture-slider-evidence.mjs.
 * Walks each Meter story; writes a 2x PNG per story under both both Crystal themess.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-meter").first().waitFor({
        state: "visible",
        timeout: 5000,
      });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-meter-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "meter-evidence.json"),

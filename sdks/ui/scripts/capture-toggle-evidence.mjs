/*
 * Capture-toggle-evidence: mirrors capture-radio-evidence.mjs.
 * Walks each Toggle story; writes a 2x PNG per story under Crystal.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      // Either a standalone .zs-toggle or a .zs-toggle-group must mount
      // before the capture — the group stories don't paint a bare
      // toggle until Base UI's <button> rows render.
      await page.locator(".zs-toggle, .zs-toggle-group").first().waitFor({
        state: "visible",
        timeout: 5000,
      });
      // Same 200ms settle the other capture scripts use — covers Base UI's
      // initial press-state hydration and the focus-visible reset after
      // navigation.
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-toggle-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "toggle-evidence.json"),

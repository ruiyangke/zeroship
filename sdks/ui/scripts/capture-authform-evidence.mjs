...
const stories = [
  ...
  "blocks-authform--sign-up",
  "blocks-authform--with-error",
  "blocks-authform--loading",
  "blocks-authform--with-social",
];
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-auth-form").first().waitFor({ state: "visible", timeout: 5000 });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-authform-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "authform-evidence.json"),

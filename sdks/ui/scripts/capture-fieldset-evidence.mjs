...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-fieldset").first().waitFor({ state: "visible", timeout: 5000 });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-fieldset-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "fieldset-evidence.json"),

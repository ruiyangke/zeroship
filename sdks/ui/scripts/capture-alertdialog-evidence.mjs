/*
 * Capture-alertdialog-evidence: spins up an http-server over
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      const file = join(outDir, `${theme.value}-alertdialog-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "alertdialog-evidence.json"),

/*
 * Capture-separator-evidence: mirrors capture-progress-evidence.mjs.
 * Walks each Separator story; writes a 2x PNG per story under both both Crystal themess.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      // The Separator's "main axis" dimension is 0 (border carries the
      // line), which Playwright reads as hidden. Wait for the DOM node
      // to be attached + give the storybook frame a beat to paint.
      await page.locator(".zs-separator").first().waitFor({
        state: "attached",
        timeout: 5000,
      });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-separator-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "separator-evidence.json"),

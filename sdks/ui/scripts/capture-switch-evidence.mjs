/*
 * Capture-switch-evidence: mirrors capture-checkbox-evidence.mjs.
 * Walks each Switch story; writes a 2x PNG per story under both both Crystal themess.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-switch").first().waitFor({ state: "visible", timeout: 5000 });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-switch-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "switch-evidence.json"),

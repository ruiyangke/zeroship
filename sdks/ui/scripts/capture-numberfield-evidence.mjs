/*
 * Capture-numberfield-evidence: mirrors capture-toggle-evidence.mjs.
 * Walks each NumberField story; writes a 2x PNG per story under both both Crystal themess.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      // NumberField always mounts as .zs-number-field.
      await page.locator(".zs-number-field").first().waitFor({
        state: "visible",
        timeout: 5000,
      });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-numberfield-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "numberfield-evidence.json"),

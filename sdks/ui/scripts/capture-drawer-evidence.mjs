 ...
 *
 * Every Drawer story starts CLOSED with a real Trigger button. This
 * script clicks the per-story trigger testid before screenshotting so
...
// click the canonical "md" representative so the docs page renders
// cleanly without flicker.
const stories = [
  {
    id: "components-drawer--basic",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--left-side",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--top",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--bottom",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--sizes",
  ...
  {
    id: "components-drawer--with-form",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--with-long-content",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--nested",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--rtl",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
  {
    id: "components-drawer--close-as-child",
    triggers: ['[data-testid="drawer-trigger"]'],
  },
];
...
try {
  ...
  for (const theme of themes) {
    for (const { id: storyId, triggers } of stories) {
      ...
      // Wait for the most recently-opened Content to be visible.
      await page.locator(".zs-drawer-content").last().waitFor({
        state: "visible",
        timeout: 5000,
      });
      await page.waitForTimeout(400);
      const file = join(outDir, `${theme.value}-drawer-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "drawer-evidence.json"),

/*
 * Capture-form-evidence: walks each Form story under both both Crystal themess
 * and writes a 2x PNG to storybook-static/theme-evidence/.
 * Mirrors capture-checkbox-evidence.mjs in shape.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-form").first().waitFor({ state: "visible", timeout: 5000 });
      // The validation-driven stories (WithValidation, ActionsRefValidate)
      // need a beat for any post-mount state machine flush; the rest pay
      // a small settle cost (acceptable for offline capture).
      await page.waitForTimeout(300);
      const file = join(outDir, `${theme.value}-form-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "form-evidence.json"),

...
/**
 * Build a module exporting { fetch?, rpc, workflows }. RPC values are actual
 ...
 * original receiver through a bound function.
 */
...
for (const _zsName of Object.keys(_zsUser)) {
  ...
  if (typeof _zsFn !== "function" || isWorkflowClass(_zsFn)) continue;
  const _zsConfig = _zsFn.config;
  const _zsId = (_zsConfig && typeof _zsConfig.id === "string" && _zsConfig.id) || _zsName;
  _zsRpc[_zsId] = _zsFn;
}
${exportEntry}`;
}

...
// `crates/zeroship-migrate-node` and type its exposed surface.
//
...
// `#[napi]` source (`crates/zeroship-migrate-node/src/wire.rs` + `bridge.rs`) into
// `crates/zeroship-migrate-node/index.d.ts` — the SINGLE source of truth. We import
 ...
 *  2. the `zeroship-migrate-node` package: its generated loader (`index.js`) picks the
 *     correct prebuilt binary for `process.platform`/`arch`/libc, whether that is a
 *     sibling `.node` (monorepo dev) or one of the `zeroship-migrate-node-<triple>`
 *     optionalDependencies (published install).
 *
 * `zeroship-migrate-node` is a real `dependency` of this package, so the platform
 * resolution and the "npm optional-deps bug" guidance all live in its generated
 * loader; we never re-implement triple math here.
 *
 * @throws if neither the override nor `zeroship-migrate-node` yields a usable addon.
 */
export function loadAddon(): MigrateAddon {
  if (cached) return cached;
  const require = createRequire(import.meta.url);
  const override = process.env.ZERO_MIGRATE_ADDON_PATH;
  const target = override ?? "zeroship-migrate-node";
      ...
      `zero-migrate-cli: could not load the native addon from ${JSON.stringify(target)}: ` +
        ...
        (override
          ...
          : "Install the platform binary (it ships as an optionalDependency of " +
            "zeroship-migrate-node), or set ZERO_MIGRATE_ADDON_PATH to a `napi build` output."),
    );
  }
  if (!mod || typeof mod.irVersion !== "function") {
    throw new Error(
      `zero-migrate-cli: the native addon loaded from ${JSON.stringify(target)} is missing ` +
        "irVersion(): it is not a zeroship-migrate-node build.",
    );
  }
  cached = mod;
  return mod;

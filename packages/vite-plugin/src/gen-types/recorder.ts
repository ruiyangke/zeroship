 ...
 * (`{ ir_version, name, ops }`) using the `@zeroship/migrate` recorder
 * (`@zeroship/migrate/internal/recorder`). No CLI subprocess, no `zeroship-runtime`
 ...
 * its `@zeroship/migrate` DSL import must resolve to the SAME
 ...
 * `@zeroship/migrate` at import time). The bundled module is then imported and
 * drained through `buildEnvelope`.
 */
...
export async function recordMigration(tsPath: string): Promise<IrEnvelope> {
  ...
  // of the EXTERNAL `@zeroship/migrate` at import time walks up to the one installed
  ...
  try {
    await build({
      ...
      // to ONE installed `@zeroship/migrate` instance (shared op-recorder singleton).
      external: ["@zeroship/migrate", "@zeroship/migrate/*"],

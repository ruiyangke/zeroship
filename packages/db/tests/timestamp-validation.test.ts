...
import { validateDoc, isTimestampValue } from "../../../crates/zeroship-data-v8/js/runtime/validate.js";
...
test("timestamps accept integral Unix milliseconds, Dates, and real ISO instants", () => {
  const schema = fieldsOf({ instant: t.timestamp() });
...
test("timestamps reject impossible dates, times, offsets, and unrepresentable instants", () => {
  const scalar = fieldsOf({ instant: t.timestamp() });
  const array = fieldsOf({ instants: t.array(t.timestamp()) });
  for (const instant of [

/*
 * Capture-button-evidence: spins up an http-server over storybook-static,
 * loads each Button story under both both Crystal themess, and writes a 2x PNG
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      // Wait for fonts to load and for at least one button to render.
      await page.evaluate(() => document.fonts && document.fonts.ready);
      await page.locator(".zs-button").first().waitFor({ state: "visible", timeout: 5000 });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-button-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "button-evidence.json"),

/*
 * Capture-progress-evidence: mirrors capture-slider-evidence.mjs.
 * Walks each Progress story; writes a 2x PNG per story under both both Crystal themess.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-progress").first().waitFor({
        state: "visible",
        timeout: 5000,
      });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-progress-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "progress-evidence.json"),

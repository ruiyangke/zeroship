/*
 * Capture-avatar-evidence: mirrors capture-progress-evidence.mjs.
 * Walks each Avatar story; writes a 2x PNG per story under both both Crystal themess.
 */
...
try {
  ...
  for (const theme of themes) {
    for (const storyId of stories) {
      ...
      await page.locator(".zs-avatar").first().waitFor({
        state: "visible",
        timeout: 5000,
      });
      await page.waitForTimeout(200);
      const file = join(outDir, `${theme.value}-avatar-${storyId}.png`);
  ...
  await writeFile(
    join(outDir, "avatar-evidence.json"),

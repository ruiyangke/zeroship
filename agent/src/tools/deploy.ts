/**
 * deploy_app tool — deploys a JS bundle to the appbase platform.
 */
...
export const deployApp = tool(
  async ({ app_id, server_js }) => {
    // Ensure app exists
    const checkRes = await fetch(`${APPBASE_URL}/api/apps/${app_id}`, {
      headers: { Authorization: `Bearer ${APPBASE_KEY}` },
    });

    if (checkRes.status === 404) {
      // Create app
      const createRes = await fetch(`${APPBASE_URL}/api/apps`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${APPBASE_KEY}`,
    ...
    // Deploy
    const res = await fetch(`${APPBASE_URL}/api/apps/${app_id}/deploy`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${APPBASE_KEY}`,
    ...
    return `Deployed app "${app_id}" — version ${data.version}. Live at ${APPBASE_URL}/rpc with header X-App-Id: ${app_id}`;
  },
  {
    name: "deploy_app",
    description:

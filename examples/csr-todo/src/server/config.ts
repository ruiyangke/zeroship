...
// gateway. Opt these two read-only procedures into anonymous so the public SPA
// reaches them through the gateway. `publiclyAccessible: true` is the deliberate
// confirmation the manifest validator requires alongside `auth: "anonymous"`.
export default defineApp({
  resources: {
    "rpc:listTodos": { auth: "anonymous", publiclyAccessible: true },
    "rpc:searchTodos": { auth: "anonymous", publiclyAccessible: true },
  },
});

...
// committed `zeroship.jsonc` carries. That is what makes the deploy gate and the
// runtime reach the schema `zeroship migrate` filled, so it is asserted on the
// packed artifact rather than on the file the packer read.
...
//
// `sql` below throws on a non-zero exit, which is right for every statement this
// fixture expects to succeed. The tenant fence is asserted the other way round:
// the shared worker login must be REFUSED, and a helper that threw would make
// the refusal indistinguishable from a broken fixture.
...
export class Platform {
  ...
  async start(): Promise<Target[]> {
    ...
    const organization = await this.api("POST", `${control.url}/api/organizations`, { name: "DB todos fixture", slug: `db-todos-${randomBytes(6).toString("hex")}` });
    ...
    assert.equal(await this.sql(
      `SELECT count(*) FROM pg_tables WHERE schemaname='${schema}' AND tablename='todos'`), "1",
    ...
    const created = await this.api("POST", `${control.url}/api/apps`, { name: "dbtodos", project_id: project.id });
    ...
    // THE DEPLOYED APP REACHES ITS DATA THROUGH THE BINDING, and only through
    // it. Both arms run on the SAME login the worker connects as and differ in
    // one statement: the `SET ROLE` the data plane issues per transaction.
    // Without it PostgreSQL refuses `42501`; with it the deployed tier's own
    // table is readable. The count is compared to the superuser's reading of the
    // same table, so neither arm can pass over a schema with nothing in it.
    const owned = await this.sql(`SELECT count(*) FROM "${schema}".todos`);
    const unnarrowed = await this.workerSql(`SELECT count(*) FROM "${schema}".todos`);
    ...
    const narrowed = await this.workerSql(`SELECT count(*) FROM "${schema}".todos`, bindingRole);
    assert.equal(narrowed.exitCode, 0, `the binding role must reach this database: ${narrowed.output}`);
    assert.equal(lastLine(narrowed), owned,
      "the binding role reads the table the migration created in this database");
    ...
    const capability = await this.workerSql(
      `SELECT has_column_privilege('${schema}.todos', 'title', 'SELECT')::text,
              has_column_privilege('${schema}.todos', 'title', 'INSERT')::text`,
      bindingRole);

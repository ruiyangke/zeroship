...
import { mutation, query, stream } from "@zeroship/server";
...
export const health = query(async (_input: Record<string, never>) => {
  return { ok: true, runtime: "zeroship", backend: "sqlite" };
});

export const seedDemo = mutation(async (_input: Record<string, never>) => {
...
export const upsertWorkspace = mutation(async ({
  slug,
  name,
  tier,
  region,
...
export const tasksWithRelations = query(async ({
  workspaceId,
}: {
  workspaceId: WorkspaceId;
}) => {
...
export const updateTaskVersioned = mutation(async ({
  id,
  version,
  title,
}: {
...
export const softDeleteTask = mutation(async ({ id }: { id: TaskId }) => {
...
export const restoreTask = mutation(async ({ id }: { id: TaskId }) => {
...
export const purgeTask = mutation(async ({ id }: { id: TaskId }) => {
...
export const transactionShowcase = mutation(async ({
  workspaceId,
  ownerId,
}: {
  workspaceId: WorkspaceId;
...
export const searchShowcase = query(async ({
  workspaceId,
}: {
  workspaceId: WorkspaceId;
}) => {
...
export const securityShowcase = mutation(async ({ userId }: { userId: UserId }) => {
...
health.config = { id: "db-e2e.health" };
seedDemo.config = { id: "db-e2e.seed-demo" };
createTask.config = { id: "db-e2e.create-task" };
upsertWorkspace.config = { id: "db-e2e.upsert-workspace" };
getTask.config = { id: "db-e2e.get-task" };
queryShowcase.config = { id: "db-e2e.query-showcase" };
tasksWithRelations.config = { id: "db-e2e.tasks-with-relations" };
updateTaskVersioned.config = { id: "db-e2e.update-task-versioned" };
softDeleteTask.config = { id: "db-e2e.soft-delete-task" };
restoreTask.config = { id: "db-e2e.restore-task" };
purgeTask.config = { id: "db-e2e.purge-task" };
transactionShowcase.config = { id: "db-e2e.transaction-showcase" };
searchShowcase.config = { id: "db-e2e.search-showcase" };
securityShowcase.config = { id: "db-e2e.security-showcase" };
liveTasks.config = { id: "db-e2e.live-tasks" };

...
export class Platform {
  ...
  static async create(): Promise<Platform> {
    ...
    const work = await mkdtemp(join(tmpdir(), "zeroship-kv-dashboard-"));
  ...
  async start(): Promise<Target[]> {
    ...
    console.info("KV fixture: build platform binaries and dashboard");
    ...
    console.info("KV fixture: start backing containers and apply platform migrations");
    const postgres = await this.container(new GenericContainer("postgres:18")
      .withExposedPorts(5432)
      .withEnvironment({ POSTGRES_PASSWORD: "kv-fixture-password", POSTGRES_DB: "kv_fixture" })
    ...
    const authority = `${postgres.getHost()}:${postgres.getMappedPort(5432)}/kv_fixture`;
    const dsn = `postgres://postgres:kv-fixture-password@${authority}`;
    ...
    console.info("KV fixture: start platform services");
    ...
    console.info("KV fixture: create and deploy the app");
    const created = await fetch(`${control.url}/api/apps`, {
      method: "POST", headers: { authorization: `Bearer ${bearer}`, "content-type": "application/json" },
      body: JSON.stringify({ name: "kvdash" }), signal: AbortSignal.any([processes.signal, AbortSignal.timeout(10_000)]),
    });
    ...
    console.info("KV fixture: start local Vite and wait for app dispatch");
    ...
    processes.start("dev", process.execPath, [vite, "--host", "127.0.0.1", "--port", `${ui.number}`, "--strictPort"], app, {
      KV_DASHBOARD_API_PORT: `${dev.number}`, ZEROSHIP_BIN: binary("zeroship"),
    });
    const targets = [
    ...
    for (const target of targets) await this.waitFor(target.name, () => this.httpReady(`${target.apiUrl}/__zeroship/v1/kv.snapshot`, {
  ...
  close(): Promise<void> {
    return this.closing ??= (async () => {
      ...
      if (errors.length) throw new AggregateError(errors, "Failed to clean up dashboard fixture");
    })();
  }
}

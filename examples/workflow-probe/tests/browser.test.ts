  ...
  try {
    for (const target of targets()) {
      ...
      try {
        ...
        await expect.poll(() => page.locator("#out").textContent()).toContain("workflow-probe");
        const result = await page.evaluate(async () => {
          const response = await fetch("/__zeroship/v1/wf.ping", {
            method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ json: {} }),
          });
          if (!response.ok) throw new Error("Browser RPC failed: " + response.status + ": " + await response.text());
          return response.json();
        });
        expect(result.json).toMatchObject({ ok: true, cases: ["basic", "child", "compensate", "signal", "sleep"] });

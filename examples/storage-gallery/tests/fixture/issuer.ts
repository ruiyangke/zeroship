...
export function issuer() {
  ...
  const jwks = { keys: [{ ...publicKey.export({ format: "jwk" }), alg: "EdDSA", use: "sig", kid: "storage-gallery-acceptance" }] };
  return {
    ...
    bearer(url: string, owner: string): string {
      ...
      const body = `${encode({ alg: "EdDSA", typ: "at+jwt", kid: "storage-gallery-acceptance" })}.${encode({

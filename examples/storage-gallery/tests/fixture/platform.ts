...
export class Platform {
  ...
  static async create(): Promise<Platform> {
    ...
    const work = await mkdtemp(join(tmpdir(), "zeroship-storage-gallery-"));
  ...
  async start(): Promise<Target[]> {
    ...
    const created = await fetch(`${control.url}/api/apps`, {
      ...
      body: JSON.stringify({ name: "gallery" }), signal: AbortSignal.any([processes.signal, AbortSignal.timeout(10_000)]),
    });
    ...
    processes.start("dev", process.execPath, [vite, "--host", "127.0.0.1", "--port", `${ui.number}`, "--strictPort"], app, {
      STORAGE_GALLERY_API_PORT: `${dev.number}`, ZEROSHIP_BIN: binary("zeroship"),
    });
    const targets = [
    ...
    for (const target of targets) await this.waitFor(target.name, () => this.httpReady(`${target.apiUrl}/__zeroship/v1/gallery.list`, {

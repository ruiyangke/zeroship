...
export class Platform {
  ...
  static async create(): Promise<Platform> {
    ...
    const work = await mkdtemp(join(tmpdir(), "zeroship-workflows-order-"));
  ...
  async start(): Promise<Target[]> {
    ...
    const signerId = typedIdFromStableSeed("wjs", "workflows-order-fixture-signer");
    ...
    const created = await fetch(`${control.url}/api/apps`, {
      ...
      body: JSON.stringify({ name: "orders" }), signal: AbortSignal.any([processes.signal, AbortSignal.timeout(10_000)]),
    });

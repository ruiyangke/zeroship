...
| [`storage-probe/`](./storage-probe/) | `@zeroship/storage` with no login: put/get for text and bytes, overwrite, delete-then-read, content-type round trip, prefix listing and pagination, and a 1 MiB streaming put and get. Every procedure returns plain JSON so `tests/e2e_dev_vs_deployed_storage.sh` can diff dev (LocalFs) against deployed (S3) |

...
The storage and serving layer that backs `.zship`. Content-addressed bytes, edge-cached on the gateway, served zero-copy where possible.
...
The trait operates on a **single global keyspace** (`blobs/<hash>`). Cross-tenant dedup is safe by construction under two protocol constraints (see `docs/reference/zship.md` for the full argument):
...
- `crates/control/src/deploy.rs` — `.zship` ingestion. Verifies hashes, deduplicates against `has_blob`, calls `put_blob` for new content, writes the manifest via `put_manifest`.

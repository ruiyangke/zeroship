...
5. **Minimal** — no metadata, no source maps, no JSON; just an index and data
...
│  Format version: u32 little-endian (4 bytes) │
│  SHA-256 hash of all bytes after (32 bytes)  │
├──────────────────────────────────────────────┤
│ Index Section                                │
│  Module count: u32 little-endian (4 bytes)   │
...
| 4 | Compressed size |
| 4 | Original size (for pre-allocating decompression buffer) |

Fixed fields per entry: **15 bytes** + specifier length.
...
Starts immediately after the last index entry. Contains concatenated zstd-compressed frames in index order:

```
[module 0 source] [module 1 source] [module 2 source] ...
```
...
2. Read `compressed_size` bytes from `data_section_start + data_offset`
3. zstd decompress into pre-allocated buffer of `original_size` bytes
4. Return as UTF-8 string (or raw bytes for `Data` type)
...
**Pass 1 — compress:**
1. For each module: zstd compress source
2. Record compressed and original sizes

**Pass 2 — compute offsets and write:**
1. Compute data offsets: cumulative sum of compressed sizes
...
| Item | Where it lives |
|------|---------------|
| App name, version | Database |
| Exports list | Database (informational) or discovered by evaluation |
| Compiler version | Database |
| API key | Database |
| Source maps | Separate file in object storage (`v3.appbundle.map`) |

## Storage layout

```
Object storage (R2/S3):
  bundles/{app_id}/v3.appbundle        ← runtime loads this
  bundles/{app_id}/v3.appbundle.map    ← CLI/error reporting loads this (optional)

Database:
  id, version, content_hash, bundle_url, bundle_size, entry, name, ...
```
...
**Level 1 — lazy decompression:** The index is read eagerly (~15 bytes per module + specifier). Module sources stay as compressed bytes. Only decompressed when the runtime needs them.
...
|-------|----------------:|-------:|
| After index parse | 0 of 200 | ~4KB (index) + blob |
...
| App type | Modules | Uncompressed | .appbundle size |
|----------|--------:|-------------:|----------------:|
| Simple API | 1 | 5 KB | ~2 KB |
| Medium app | 10 | 50 KB | ~18 KB |
| Large (bundled) | 1 | 500 KB | ~110 KB |
| Large (unbundled) | 200 | 2 MB | ~520 KB |

...
| D8 | **test-only Hydra-public env naming collided with the production naming shape** — fixtures/tests referenced a Hydra-public env name that did not match the production-side spelling, masking the D9 drift | tests | Fold into the D9 unification: the single `hydra_public_url` field is the only Hydra-public name in code *and* tests, so the test naming can no longer diverge from production |

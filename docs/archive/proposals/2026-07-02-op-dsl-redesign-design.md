...
| K6 | Safe online migrations first-class: `render/expand_contract.rs`, online renameColumn dual-write, PR9a pending-contract interlock, crash-safe cursor backfill, split statement_timeout(60s)/lock_timeout(3s) envelope with checksummed overrides, production-gate pin `crates/zeroship-control/tests/deploy_migrate_test.rs:643` | Kept; extended with NOT VALID→VALIDATE pairing, online type change, CONCURRENTLY step class, per-phase version_keys (§9). Interlock additionally hoisted to validate time (§9.3). |
...
(`crates/zeroship-control/tests/deploy_migrate_test.rs:643`) stays until that track lands.

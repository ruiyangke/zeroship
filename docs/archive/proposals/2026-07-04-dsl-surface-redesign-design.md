...
**P0 — PostgreSQL is first-class** *(the governing stance — reframes P5)* <!-- Added 2026-07-06 (operator direction): PG-first supersedes the /pg two-tier boundary. -->. The platform targets **PostgreSQL first**; the core authoring surface **is** PostgreSQL-shaped, and the platform is **not bent toward a lowest-common-denominator portable core** for other vendors' sake. PG-specific constructs — vector index methods (`hnsw`/`ivfflat`), `gin`/`gist`/`brin`/`spgist`, `EXCLUDE` constraints, `ON CONFLICT` upsert, `~` regex, `pg_column_size`, `current_setting`, RLS, roles/grants, PG `EXTRACT` fields, etc. — are **directly usable on the core surface**, with **no `@zeroship/migrate/pg` import and no `c.pg.` casting**. Portability to SQLite/MySQL is an **explicit, opt-in** concern, expressed with `dialect({ pg, sqlite, mysql })` at the exact value/op that diverges; where a construct has no native realization on a target and no `dialect()` leg, the engine **fails closed with an error at that target** (the existing validate-time `DIALECT_UNSUPPORTED` gate) — never silently degraded, and the author is **never forced into a vendor tier to use a PG feature**. **This reframes P5**: the `/pg` two-tier split is dropped as an *ergonomic/type* gate. The engine's per-op/per-node/per-option **dialect table** + validate-time fail-closed gate + the `dialect()` escape are the *sufficient* portability mechanism (they already exist). `c.pg.*` / `pgTable`-only exports are no longer required to *use* a PG feature; a distinct `/pg` home survives only where a capability is genuinely PG-private *and* needs isolation (`raw` stays a P11 counted debt instrument by its `reason`/ratchet — that is P11's gate, not a `/pg` import). Default: **PG feature → core surface, fail-closed elsewhere, `dialect()` to port.**

### The twelve principles (P1–P12 — read all under P0)

...
**Scope:** add `grant_type=refresh_token` to the self-contained OP (`crates/zeroship-auth/src/op/`) and the `zeroship.oauth_refresh_tokens` family store, for the **CLI / programmatic face only** — the `zeroship login` device-token deploy flow and confidential OAuth clients that request `offline_access`. The **browser / console face gets NO refresh family** (OQ-1 → Option B, see Revision log). The browser face is a bounded, server-authoritative gateway session with just-in-time (JIT) access-token re-mint; its contract is stated here in §8 and its implementation is deferred to **P5c** (the gateway re-home).
**Author seam:** `crates/zeroship-auth/src/op/refresh.rs` (new), `crates/zeroship-auth/src/op/authorization_code.rs` (issuance + `TokenResponse`), `db/migrations-ts/20260702000300_auth_oauth_tables.ts`. The gateway's `do_refresh` is **removed**, not swapped (§8).
...
<!-- Corrected in round 5 (MINOR-1): the column is `sub`, not `token_subject`. Verified against db/migrations-ts/20260702000300_auth_oauth_tables.ts and the live writer crates/zeroship-control/src/oauth_grants_handlers.rs:240. -->
```
...
- `crates/zeroship-gateway/src/auth_token.rs::do_refresh` and the Hydra `refresh_token_public` call are **removed** — there is no browser OAuth refresh to perform. (This is why round-1 CRITICAL #2 dissolves: there is no public-vs-confidential `do_refresh` to reconcile, and no per-app `client_secret` to custody.)
- `crates/zeroship-gateway/src/anchors.rs` (the `app_session_anchors` rotation store, single-flight, breadcrumb) is **retired for the browser face**, per P0 §551. No "anchor death ⇒ OP `/revoke`" wire is needed (round-1 #7) because there is no anchor and no OP family for this face.
...
| `crates/zeroship-auth/src/op/refresh.rs` (`exchange_refresh_token`) | **net-new** | Rotation + reuse-detection + bounded idempotency + family advisory lock. |
| `with_xact_advisory_lock2(conn, ns, key)` in `crates/zeroship-auth/src/advisory_lock.rs` | **net-new helper (round-5 MINOR-2)** | Two-arg, **xact-scoped** `pg_advisory_xact_lock(int4, int4)` on the **writing** connection (auto-released at COMMIT/ROLLBACK). The existing helper is session-scoped single-i64-key and cannot express the `(NS, hashtext)` two-namespace keyspace §5.4 needs. Used by rotation, issuance, `/revoke`, bulk revoke, and the sweeper. |
...
| `crates/zeroship-gateway/src/anchors.rs` + `auth_token.rs::do_refresh` | **removed (P5c)** | Browser OAuth-refresh anchor retired (P0 §551); not swapped. |
...
- Internal code: `crates/zeroship-auth/src/sessions/login.rs` (30m idle/12h absolute), `crates/zeroship-auth/src/store/sessions.rs::validate`, `crates/zeroship-auth/src/op/authorization_code.rs`, `crates/zeroship-auth/src/op/issuer.rs`, `db/migrations-ts/20260702000300_auth_oauth_tables.ts`

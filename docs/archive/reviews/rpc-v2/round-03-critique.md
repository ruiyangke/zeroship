...
| Feasibility | 78 | Phases are now LOC-and-dependency-tagged, with phase 0 (native `RpcError`) cleanly separable from the rest. But phase 0 + phase 1 ship Rust crates that do not yet exist (`crates/zeroship-runtime/src/rpc/{dispatch,error,superjson}.rs`) and the LOC estimates are by-eye. Phase 5's "gateway-side SETNX flow with Lua-script release" is real but "build runs a wire-compat check at deploy time" (§13) is not in any phase. Phase 4's "CHWBL routing" is already shipped — so what's the actual delta there? The phase boundaries leak. |
...
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/web/dom/exception.rs` — DOMException reference for `RpcError`
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/web/dom/form_data.rs` — native FormData reference
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/node/async_hooks/als.rs` — ALS reference for §3

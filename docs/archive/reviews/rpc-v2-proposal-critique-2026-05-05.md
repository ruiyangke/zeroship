...
- **What's wrong:** as of ISS-01, `crates/zeroship-runtime/src/node/async_hooks/als.rs` ships native `AsyncLocalStorage` backed by V8's `ContinuationPreservedEmbedderData`, which propagates *automatically* across every await, microtask, `.then`, generator yield. The proposal's `__zs_bind_request_ctx` / `__zs_get_request_ctx` mechanism (§3 line 168) requires the kernel to repaint `executing_request_id` on every V8 entry (see `crates/zeroship-runtime/src/auth.rs:55-59`). Per the inline comment in `auth.rs:13-22`, an earlier `thread_local!` revision was wrong; the current per-request-id approach is correct *but more brittle than ALS*. ALS is wired so that every continuation captures its own context with zero kernel cooperation.
...
| --- | --- | --- |
| §3 line 198: "no AsyncLocalStorage gymnastics" | Native ALS shipped (`crates/zeroship-runtime/src/node/async_hooks/als.rs`), backed by `ContinuationPreservedEmbedderData`, *propagates automatically* | Critical |
...
| §4: "wire is JSON. Plain `JSON.stringify` loses Date, BigInt..." (with superjson as the fix) | True, but the platform has structured cloning via `crates/zeroship-runtime/src/web/structured_clone.rs` for cross-realm transfer, and FormData/Blob/File natively. The "JSON-only" framing forecloses richer wires | Major |
...
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/node/async_hooks/als.rs` — native ALS backing for the `ctx` redesign
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/auth.rs` — current request-id slot mechanism the proposal §3 rests on
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/web/dom/exception.rs` — DOMException pattern for `RpcError` to follow
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/web/dom/form_data.rs` — native FormData for the Binary section to use
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/web/headers.rs:689` — `#[v8_method(fastcall)]` precedent
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/web/fetch/request.rs` — native Request for `request()` helper
...
- `/home/ruiyang/Projects/appbase/crates/zeroship-runtime/src/core/runtime.rs:2629-2691` — RPC fast path, `parse_envelope_body`
- `/home/ruiyang/Projects/appbase/crates/zeroship-gateway/src/idempotency.rs` — gateway store; lock primitive missing

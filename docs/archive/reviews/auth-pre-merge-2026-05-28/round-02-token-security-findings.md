...
**File:** `crates/zeroship-control/src/authz_guard.rs:162-198`

**Severity rationale:** Any hydra-issued access token whose scopes include the appropriate scope strings — regardless of the audience or client it was issued to — is accepted by the control plane. In a deployment with multiple hydra OAuth clients (gateway, builder, app-runtime, future first/third-party clients), a token meant for the gateway or for an app can be replayed against `console.zeroship.ai`/`api.zeroship.ai` control endpoints and yield real CRUD rights up to the scope it carries. Hydra's `/oauth2/introspect` response carries the `aud` claim (and is already deserialized in `crates/zeroship-core/src/hydra.rs:213-218,221-231`); the control plane just throws it away.
...
**File:** `crates/zeroship-gateway/src/wrapper_token.rs:166-229` and `crates/zeroship-gateway/src/router/auth.rs:184-322`
...
**File:** `crates/zeroship-core/src/dpop.rs:930-969`
...
**File:** `crates/zeroship-auth/src/identity/magic_link.rs:149-197`
...
**File:** `crates/zeroship-control/src/token_handlers.rs:104-107`
...
- Hydra-client (`crates/zeroship-auth/src/hydra_client/*.rs`) — not reviewed for admin-API authentication / TLS pinning.

...
**File:** `crates/zeroship-authz/src/lower.rs:7-12` (lowering) and `crates/zeroship-control/src/token_handlers.rs:27-32, 205-274` (consumer).
...
**File:** `policies/platform/audit_locked.cedar:7-8`, `crates/zeroship-authz/src/entities.rs:207-211` (the producer that never emits the attr), `crates/zeroship-auth/src/store/migrations.rs:252-279` (no schema column).
...
**File:** `crates/zeroship-authz/src/resource.rs:13-19`, `crates/zeroship-authz/src/lower.rs:59-89`.
...
**File:** `crates/zeroship-auth/src/ui/consent.rs:371-409`, `crates/zeroship-control/src/token_handlers.rs:364-416`, `crates/zeroship-authz/src/eval.rs:135-141`.
...
**File:** `crates/zeroship-authz/src/lower.rs:135` and `crates/zeroship-authz/src/lower.rs:141-151`.
...
**File:** `crates/zeroship-authz/src/entities.rs:152-170`, `crates/zeroship-control/src/registry.rs:97-112` (the actual `apps` table), `policies/platform/suspended_apps.cedar`.
...
**Suggested fix:** Same as C2: add `suspended BOOLEAN NOT NULL DEFAULT FALSE` (and `audit_locked`) to the `apps` migration in `crates/zeroship-control/src/registry.rs:98-108`, replace `load_app_suspended` with a single typed `load_app_flags` query (no relation fallback, no column swallow). Delete the `for table in ["control.apps", "apps"]` loop entirely. Pre-launch, the schema either has the column or the test fails — no detect-and-warn.

## MEDIUM

### M1. Entity cache holds revoked memberships / platform roles up to 30 s
**File:** `crates/zeroship-authz/src/entities.rs:14, 17-22, 269-290`.
...
**File:** `crates/zeroship-authz/src/eval.rs:151-165`, `crates/zeroship-auth/src/store/migrations.rs:274`.
...
**File:** `crates/zeroship-control/src/token_handlers.rs:370-373`.
...
**File:** `crates/zeroship-authz/src/action.rs:21,44,67,108`.
...
**File:** `crates/zeroship-control/src/authz_guard.rs:81-89, 152-159, 190-197`, `crates/zeroship-authz/src/eval.rs:111`.
...
- **PAT JWT mint/verify** (`crates/zeroship-control/src/token_handlers.rs:104-162`). Ed25519, requires `typ=pat+jwt`, kid matched against the issuer, issuer + audience pinned, no `alg=none` window. JWT carries `policy_hash`, DB lookup re-checks `(id, owner_id, policy_hash, kind='pat', not revoked, not expired)`. Solid.
- **OAuth scope subset comparison** (`crates/zeroship-auth/src/ui/consent.rs:238-249`). Both sides are `sort_dedup`'d before `binary_search`. Subset semantics (`requested ⊆ previously_granted`) is correct; superset requests fall through to the consent UI rather than silently auto-approving. Standard OIDC scopes (`openid`/`email`/`profile`/`offline_access`) are treated as just-another-string in the subset check, which is correct because they're case-sensitive per RFC 6749 and the comparison is exact.

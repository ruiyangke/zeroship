...
in `crates/control`, `crates/zeroship-auth/src/ui/*`, and `crates/zeroship-gateway/src`; the
...
**File:** `crates/zeroship-control/src/api.rs:556-569` (`auth_callback`),
`crates/zeroship-control/src/api.rs:674-700` (`start_oidc_redirect`),
`crates/zeroship-gateway/src/router/dispatch.rs:1282-1296` (gateway callback),
`crates/zeroship-gateway/src/router/dispatch.rs:1325-1358` (gateway start_oidc_redirect).
...
**File:** `crates/zeroship-control/src/oauth_handlers.rs:252-275` (`validate_create_body`),
...
the consent template at `crates/zeroship-auth/src/ui/templates/consent.html:5` inside
...
**File:** `crates/zeroship-auth/src/ui/signup.rs:81, 247-259` (`redirect_to_login`).
...
**File:** `crates/zeroship-auth/src/ui/magic.rs:1179-1182` (`user_agent_str`),
...
**File:** `crates/zeroship-auth/src/ui/signup.rs:108-112`,
`crates/zeroship-auth/src/ui/forgot.rs:75` (via `email_norm`),
`crates/zeroship-auth/src/ui/magic.rs:166` (via `email_norm`),
`crates/zeroship-auth/src/store/users.rs:69-95` (no store-layer cap).
...
**File:** `crates/zeroship-auth/src/ui/signup.rs:159, 165`,
`crates/zeroship-auth/src/store/users.rs:69-95`.
...
**File:** `crates/zeroship-auth/src/ui/me.rs:222-224` (`users::find_by_id`),
`crates/zeroship-auth/src/store/users.rs` (look for the user_id-as-string path).

**Severity rationale:** Where the code uses `Uuid::parse_str` and rejects
non-UUIDs (e.g. `crates/zeroship-control/src/api.rs:100-106`), failures cleanly land
...
`crates/zeroship-control/src/oauth_grants_handlers.rs:84-93` (`client_id` from
...
**File:** `crates/zeroship-control/src/env_store.rs:62` (`MAX_VALUE_BYTES = 64 KiB`),
called from `crates/zeroship-control/src/env_handlers.rs:88-118` (`set_var`/`set_secret`).
...
**File:** `crates/zeroship-control/src/stripe_handlers.rs:436-444` (uses
`sanitize_event_id` already, good), but
`crates/zeroship-control/src/stripe_handlers.rs:419` reflects the serde error message
...
**File:** `crates/zeroship-auth/src/ui/device.rs:42-94`, particularly the
...
**File:** `crates/zeroship-control/src/token_handlers.rs:27-32` (`CreateTokenBody`).
...
- `crates/zeroship-authz/src/scope.rs::parse_scope_string` — closed-vocabulary, rejects
  unknown scopes (line 184); `crates/zeroship-control/src/oauth_handlers.rs:230-250`
  validates the scope list at registration time the same way.
- `crates/zeroship-bundle/src/unpack.rs` — manifest read capped at 1 MiB
...
- `crates/zeroship-control/src/api.rs::deploy` — explicit `PayloadConfig::new(256 MiB)`,
...
- `crates/zeroship-control/src/stripe_handlers.rs::webhook` — 256 KiB body cap
  *before* sig verification; 4 KiB cap on the signature header; HMAC-SHA256
  ...
  doesn't early-exit.
- `crates/zeroship-core/src/dpop.rs::verify` — strict header allowlist (`typ ==
...
- `crates/zeroship-control/src/token_handlers.rs::validate_grant_subset` — caps the
  ...
  fan-out.
- `crates/zeroship-control/src/env_handlers.rs::list_audit` — `query.limit: i64` is
  clamped `1..=500` inside `audit::recent_for_app` (line 91); negative or
  huge values are bounded before the SQL.
- `crates/zeroship-authz/src/entities.rs::cedar_string` — properly escapes `"`,
...
- `crates/zeroship-auth/src/ui/login.rs` — uses `HeaderValue::from_str(&redirect_to)`
...
- `crates/zeroship-control/src/oidc_rp.rs::Stash::decode` — constant-time MAC
  ...
  identical in shape.
- `crates/zeroship-auth/src/ui/{oauth_google,oauth_github}.rs` — both verify the
...
- `crates/zeroship-control/src/internal.rs` — `app_id` parsed with explicit
...
- `crates/zeroship-gateway/src/dpop_exchange.rs::handle` — strict step-by-step
...
- H2 is blocked by the fixer coordination rule: the required validation
  change lands in `crates/zeroship-control/src/oauth_handlers.rs`, which this round's
...
- H2: DEFERRED (`c0cc22e1`) — blocked by the R4/R1 overlap rule for
  `crates/zeroship-control/src/oauth_handlers.rs`.
- M1: CLOSED (`6034de2f`)
- M2: CLOSED (`1437ce29`)
- M3: CLOSED (`660276b2`)
- M4: CLOSED (`3bce2b66`)

...
- `libs/compio-postgres/src/lib.rs` — the existing compio-native pg driver (no tokio).
- `libs/compio-postgres/src/pool.rs` — HikariCP-style single-threaded `Pool` + `PoolConfig` we will drop in.
...
- **`compio-postgres`.** The platform's compio-native pg driver (`libs/compio-postgres/`). Replaces tokio-postgres.
- **Pool.** HikariCP-style single-threaded connection pool inside `compio-postgres` (`libs/compio-postgres/src/pool.rs`).

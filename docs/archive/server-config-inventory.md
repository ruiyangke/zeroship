...
| `REDIS_TEST_URL` | env | URL | skip test if unset | yes (DSN) | no | **(test-only)** gates Redis/KV integration tests | compio-redis/tests/*; kv-v8/tests/redis_backend.rs:16 |
| `DRAGONFLY_CLUSTER_SEEDS` | env | CSV of URLs | skip test if unset | yes (DSNs) | no | **(test-only)** gates live Dragonfly-cluster tests | compio-redis/tests/cluster.rs:14; kv-v8/tests/* |
| `KV_REQUIRE_REDIS` | env | `=="1"` | unset (skip allowed) | no | test PANICS when `=1` but `REDIS_TEST_URL` unset | **(test-only)** forces Redis KV tests in CI | kv-v8/tests/redis_backend.rs:28 |
...
- **`crates/plugin-storage`, `crates/zeroship-kv-v8`, `libs/compio-postgres`, `libs/compio-redis` read NO env in `src/`** — all env usage is in `tests/`. Backend URLs/paths are passed in as constructor args by the CLI/host (`KvBinding::with_backend`, `DbPlugin::new(url)`, `StoragePlugin::new(root)`). `bundle`'s only config input is compile-time `CARGO_PKG_VERSION`.

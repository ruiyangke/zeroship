...
- `crates/zeroship-gateway/src/user_auth.rs` — the existing JWT path (gets deleted)
- `crates/zeroship-gateway/src/router/dispatch.rs:963-971` — the call site that `extract_user` + `encode_user_header` are wired through
- `crates/zeroship-control/src/auth_service.rs`, `auth_handlers.rs`, `oauth.rs` — files that get deleted
...
| U6 | Delete legacy `crates/zeroship-gateway/src/user_auth.rs` + tests | gateway | 20 min |
...
**Files:** `crates/zeroship-gateway/src/router/dispatch.rs` (or wherever the manifest dispatch lives), `crates/zeroship-gateway/src/main.rs` (config to receive hydra public URL).
...
**File:** `crates/zeroship-gateway/src/main.rs` (the config struct)
...
`proxy_to` is the gateway's existing proxy machinery (look at `crates/zeroship-gateway/src/proxy.rs` for the pattern). Reuse.
...
**File:** `crates/zeroship-core/src/oidc_verify.rs` (new), `crates/zeroship-core/src/lib.rs` (export).

- [ ] **Step U2.1.1:** Write the failing test in `crates/zeroship-core/tests/oidc_verify_test.rs`:
...
**File:** `crates/zeroship-gateway/src/oidc_rp.rs` (new), `crates/zeroship-gateway/src/main.rs` (state wiring), `crates/zeroship-gateway/src/lib.rs` (export).
...
- [ ] **Step U3.1.1:** Write `crates/zeroship-gateway/src/oidc_rp.rs`:
...
- [ ] **Step U3.1.2:** Reference shape — `crates/zeroship-auth/tests/common/pkce.rs` (from Phase 2.5 helpers) has the PKCE generator. The gateway can either depend on `zeroship-auth` for that one helper OR copy it. Recommend: extract PKCE generator to `crates/core` (since both auth tests and gateway RP need it).
...
- [ ] **Step U3.2.1:** In `crates/zeroship-gateway/src/oidc_rp.rs`, add cookie helpers:
...
**Files:** `crates/zeroship-auth/src/store/migrations.rs` (add a new table), `crates/zeroship-gateway/src/sessions.rs` (new), `crates/zeroship-gateway/src/main.rs` (wire PG connection).
...
- [ ] **Step U4.1.1:** Add to `crates/zeroship-auth/src/store/migrations.rs::STATEMENTS`:
...
- [ ] **Step U4.2.1:** Write `crates/zeroship-gateway/src/sessions.rs`:
...
**Files:** `crates/zeroship-gateway/src/router/dispatch.rs` (the existing call site at line 963), `crates/zeroship-gateway/src/main.rs` (state wiring + new `/__zs/auth/callback` route).
...
# Unit U6 · Delete legacy `crates/zeroship-gateway/src/user_auth.rs`

After U5 is wired and tests pass, the legacy file is unused. Delete it.

- [ ] **Step U6.1:** `git rm crates/zeroship-gateway/src/user_auth.rs`
- [ ] **Step U6.2:** Remove the `mod user_auth;` declaration from `crates/zeroship-gateway/src/lib.rs` and `main.rs`.
...
**Files:** `crates/zeroship-control/src/oidc_rp.rs` (new), `crates/zeroship-control/src/main.rs` (state + route), `crates/zeroship-control/src/lib.rs` (export).

- [ ] **Step U7.1:** Write `crates/zeroship-control/src/oidc_rp.rs`. Largely mirrors `crates/zeroship-gateway/src/oidc_rp.rs` but:
...
- [ ] **Step U7.2:** Add the table to `crates/zeroship-auth/src/store/migrations.rs`.
...
**Files to delete:**
- `crates/zeroship-control/src/auth_service.rs`
- `crates/zeroship-control/src/auth_handlers.rs`
- `crates/zeroship-control/src/oauth.rs`

**Files to modify:**
- `crates/zeroship-control/src/lib.rs` — remove `mod auth_*; mod oauth;`
- `crates/zeroship-control/src/main.rs` — remove `/auth/*` route registrations; keep just `/auth/callback` (from U7)
- `crates/zeroship-control/src/api.rs` — if it referenced auth handlers, drop those references

- [ ] **Step U8.1:** `git rm` the three files.
- [ ] **Step U8.2:** Remove mod declarations.
- [ ] **Step U8.3:** Update routing.
- [ ] **Step U8.4:** Adjust `crates/zeroship-control/src/main.rs`'s `AppState` to drop the `auth: AuthService` and `google_oauth: Option<GoogleConfig>` fields (the OIDC RP replaces both).
...
**File:** `crates/zeroship-auth/src/store/migrations.rs` (add a one-shot migration block).
...
**File:** `crates/zeroship-auth/tests/e2e_full_stack.rs` (or in a sibling test crate — pick wherever the cross-binary tests live).
...
- §11.1 (retire legacy `crates/zeroship-auth/`): done in Phase 1

...
| zeroship migrate | STAGED | `zeroship migrate [migrations.ir.json] [--app] [--control] [--env] [--config] [--yes]` | `crates/zeroship-cli/src/migrate.rs` | `docs/build-and-deploy-golden-path.md` | `tests/e2e_db_app_end_to_end.sh` | Reuses the control URL. Part B re-keys the route under the edge's `/v1/databases/*` split; Part A alone falls through to control and returns 404. Required after deploy for env.db apps. `"protected": true` needs `--yes`. |

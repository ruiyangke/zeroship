...
Generated by scanning the tree on 2026-08-11 and checked against
`deploy/compose/docker-compose.yml`.

**Counts.** 178 distinct names in Rust (167 reachable in production, 11
test-only), 34 in the JS packages after discarding scan artifacts. Numbers this
precise rot; the regeneration commands are at the bottom so the next reader can
...
| `ZEROSHIP_SCHEME` | `http` | The scheme PUBLIC urls advertise. Behind a TLS-terminating proxy the origin serves http while the issuer must say https. |
...
`OAUTH_AUDIENCE` `ZEROSHIP_GATEWAY_URL` `DEPLOY_TMP_DIR`

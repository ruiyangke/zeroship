...
```ts
t.text().generated((c) => concatWs(" ", c("first"), c("last")))        // STORED; pass { virtual: true } to invert
...
table("users").backfill({
  set: { display_name: (c) => c("nickname").coalesce(c("name")) },
  cursorColumn: "id",
  });
```
...
| Portable expressions (`(c) => …`, chain scalar methods, `concatWs`) | ✅ | ✅ | ✅ |

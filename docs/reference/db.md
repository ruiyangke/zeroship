...
another schema. The host's schema binding and database permissions determine
access; runtime descriptors supply the collection's field metadata.

Migration declarations have separate naming rules for creating schema objects.
The migration engine reserves platform and backend catalog prefixes during
declaration. These rules prevent naming collisions and do not hide existing
customer tables from ORM access.

**Migration type generation validates declarations at build time.** Column names
...
of the classification names (`public`, `pii`, `spi`, `phi`, `pci`,

...
5. **Remove Hydra entirely** — the sidecar, `HydraAdmin`, the login/consent challenge delegation, Hydra introspection, the `oauth_hydra` schema/role, the ory configs, **the entire `crates/zeroship-auth/src/hydra_client/` module** (9 files; enumerated at §6.1(8)/P6), **and** the gateway's Hydra *relying-party* stack (§6).
...
| **Token _verification_ + JWKS _consume_ + provider seam** | ✅ cheap to extend — `AuthProvider` is a 2-variant enum `{Hydra, Supabase}`; adding a `Platform` JWKS arm is a low-invasive change | `crates/zeroship-core/src/auth_provider/mod.rs:20-40`, `core/oidc_verify.rs` (alg-pinned JWKS cache) |
...
4. the **deploy-token `AuthProvider`** path (`crates/zeroship-core/src/auth_provider`).
...
1. **Verifiers learn TWO issuers.** The `AuthProvider` enum (`crates/zeroship-core/src/auth_provider/mod.rs:20`, today `{Hydra, Supabase}`) gains a `Platform` JWKS arm. During migration the four surfaces above accept **both** platform-`iss` (new JWKS) **and** Hydra-`iss` (existing introspection/JWKS). The verifier *selects by `iss`/`kid`*; both are valid in flight.
...
- **Seam survives, arm swapped:** `AuthProvider` verify seam (`crates/zeroship-core/src/auth_provider/mod.rs`) → gains a **`Platform` JWKS arm**; the Hydra-introspection arm is dropped at P5 (after dual-issuer cutover, §6.0).
...
| **P6** | **Flip apps issuer→platform per flag**, then remove Hydra-`iss` recognition + Hydra entirely (sidecar, HydraAdmin, `oauth_hydra` schema/role, ory configs, Hydra arm of `AuthProvider`, **and the entire `crates/zeroship-auth/src/hydra_client/` 9-file module**: `clients.rs`/`consent.rs`/`device.rs`/`jwks.rs`/`login.rs`/`logout.rs`/`mod.rs`/`sessions.rs`/`types.rs`) | Full stack green with no Hydra; dual-issuer arm removed; `grep -r hydra crates/` empty |
...
| 10 | MINOR | JOSE lib listed as open | JOSE uses `jsonwebtoken`/EdDSA; signing-key retention now uses the persisted maximum issued expiry plus the complete JWKS cache and skew budget. | `crates/zeroship-auth/src/oidc/issuer.rs`, `crates/zeroship-auth/src/cron/signing_key_retention.rs` |
...
| 3 | MINOR | `hydra_client/` deletion under-enumerated | §1.5 + §6.1(8) + §9 P6 gate now enumerate the full **`crates/zeroship-auth/src/hydra_client/` 9-file** deletion (`clients/consent/device/jwks/login/logout/mod/sessions/types.rs`) — a second `{login,consent,device}.rs` set distinct from `ui/`; rewrite-vs-delete split made unambiguous. | `crates/zeroship-auth/src/hydra_client/` (9 files) |

...
0. **Capability-kind check at `connect()` (S4 — designed here, not just tested).** The native connect op calls `rpc::capability::current_kind()` **first**. If it returns `Some(Query)` or `Some(Mutation)`, the op refuses with the `capability_violation` envelope from `build_capability_violation` — the same rail by which fetch refuses under `Mutation` and database writes refuse under `Query`. Only `Action`/`Stream`/`Subscription` and trusted transport code running without a kind frame may open sockets. **Caveat made explicit:** the capability kind is defense in depth, not a sandbox boundary. Native `Procedure::invoke` installs the host-managed continuation frame before calling the handler; it does not depend on a mutable JavaScript global. The boundary that stops a hostile isolate is `NetPolicy`, SSRF protection and resource caps. The kind check prevents a read-only procedure compiled without strict TypeScript from casually opening a socket; the design does not treat it as the security boundary.

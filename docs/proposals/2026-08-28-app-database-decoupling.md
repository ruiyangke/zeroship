...
There is no third arm and no message sniffing. A worker composes a role name only when Control has
resolved a binding for the app, so the absent-binding case is refused before any statement is
sent: `env.db` is not minted at all.

`crates/zeroship-data-orm/tests/postgres_binding_fence.rs` measures both arms through the ORM
...
8. **Write the mandatory regression tests.** PARTLY BUILT, in
    ...
    ORM under the binding. (d) one app on two databases holds a transaction on each at once, which
    is the lane key measured as behaviour; a key without the database half turns the second into
    `nested_top_level_transaction`.

    BUILDABLE: (c) rotating the epoch through an apply and reaping `E-1` - the arm that exists
    measures a binding at an epoch the cluster never minted, which reaches the same classifier but
    does not exercise the reap. (e) two databases each declaring `users`, which needs the CDC
    routing key. (f) a deploy naming a database the app holds no active binding to. (g) a migration
    applied to a shared database not failing any bound app's deploy.

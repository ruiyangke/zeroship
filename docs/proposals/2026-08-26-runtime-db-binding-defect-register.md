...
(`crates/zeroship-data-v8/src/lib.rs:998`) - **8 connections per worker thread**, shared by the
...
(`crates/zeroship-data-v8/src/lib.rs:698`) on every deletion driven by the version poller - two
...
(`crates/zeroship-data-v8/src/context.rs:674-686`), which builds a `"{app}:{deploy}:"` prefix and
...
(`crates/zeroship-data-v8/tests/sqlite_integration.rs:4191`). Its fixture updates `{ ssn: ... }` against
...
`support::init_test_tracing` (`crates/zeroship-data-v8/tests/support/mod.rs:25`) now exists and is

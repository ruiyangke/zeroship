...
- `gen-types --from-db <dsn>` → declaration (reusing `introspect_schema`).
  - **Lossy-fidelity subsection (explicit).** Introspection cannot recover everything the snapshot carries:
    ...
    - **The SQLite introspector is a documented follow-up** per the header of `crates/zeroship-data-engine/src/crud/introspect_schema.rs` — Phase 2 ships the PG introspector; SQLite `--from-db` is deferred.

...
> - `bin/zeroship-migrate-js.rs:451` — a `zsv8`/`standalone-cli` binary; its apply path is PG, so it resolves `native-pg` via its feature set (like `migrated`, §5.4).

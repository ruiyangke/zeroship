# JS Portable Migration DSL — Normative Specification
...
Today a creator authors a migration as a `.sql` file in a single dialect (Flyway `V0001__x.sql`, or dbmate `<ts>_x.sql`). That forces a choice of Postgres *or* SQLite, hand-maintenance of two files for portability, and gives no portable path for **data migrations** (DML). This design makes a **JavaScript/TypeScript migration DSL** the primary authoring path. The DSL is consumed as **named ESM imports** from `@zeroship/migrate` (`import { createTable, addColumn, update, … } from "@zeroship/migrate"`); a migration module exports **parameterless `up()` / `down()`** functions that call the imported op-functions directly. At **build/dev time** (Node, not the engine) the module is evaluated into a **checksummed `.ir.json` artifact** — the migration **intermediate representation (IR)**. The Rust engine loads that IR, **lowers it per-dialect into an ordered `AppliedPlan`** (a sequence of the engine's existing phase artifacts — a `Migration`, a `BackfillSpec`, an `ExpandContractPlan`; §2.0), and runs it through the existing guard → least-priv role → immutable journal → apply pipeline. A pure-DDL migration lowers to a single-step plan and applies exactly like a `.sql` migration today; a mixed DDL+DML or online migration lowers to a multi-step plan whose interleave/journal/pending logic is executed by a **single shared plan orchestrator, `apply_plan`** (§2.0, §6).

**The four locked properties (binding).** This design is pinned to four deliberate decisions, each stated below and traced through the spec:
- **(A) No raw SQL.** The DSL is entirely structured: there is **no raw-SQL escape hatch of any kind** — no raw dialect statements and no raw expression text. Every byte of SQL the engine applies is engine-rendered from structured data.
- **(B) Named ESM imports, no `op.` prefix.** The op-functions are **named imports** called directly inside `up()`/`down()`; there is no `op` object and no `op.` prefix. The imported functions **record into an ambient per-migration recorder** the build evaluator installs before calling `up()`/`down()`.
- **(C) All-strings fluent expressions.** Every expression position takes a callback `(c) => Expr` with a **single injected builder handle** `c`: `c("name")` → `ColRef` (the argument is a plain string) and `c.fn.*` is the scalar-function namespace. Chainable operator methods build a **closed AST**, auto-wrapping bare JS values to `Literal`.
- **(D) Structural validator.** The closed expression AST is part of the IR schema (schemars single-source). Validation is **purely structural** — an allow-list check over constructed AST nodes — never a parse of text. There is no lexer, no Pratt/precedence parser, no Rust-vs-JS parser drift, and no differential fuzzer.

**The unsupported-tail tradeoff is deliberate.** The cost of no-raw-SQL is an honest unsupported tail (exotic PG-only functions, subqueries/windows in a transform, cross-table correlation): these are hard `UNSUPPORTED` errors (with `kind:"expr"|"op"`, §8.8), not a raw fallback. The expressible surface *is* the supported surface; portability is by construction. **HIGH-1 (the expression-grammar anti-drift problem earlier readings wrestled with) is ELIMINATED**, not merely mitigated: because the AST is *constructed* in JS and serialized to IR, never *parsed from text*, validation is a structural allow-list check over the same schemars-single-source IR as every other op.

**What the committed scope delivers.** The committed deliverable is **the imperative authoring surface (DDL + DML) consumed as named ESM imports + an all-strings fluent expression surface + online rename + the AI-loop structural expression-AST validator & structured-error envelope + a portable, bi-dialect data path (insert/update/delete + batched backfill on BOTH Postgres and SQLite)**. Portable DDL is a byproduct that substantially overlaps the declarative-diff path Task #46 already shipped; the genuinely non-overlapping value is the imperative surface, online rename, the AI feedback loop, and **portable bi-dialect DML — the primary reason this DSL exists** (one script, both backends, DDL *and* DML; §1.1). The bi-dialect DML executor (the SQLite backfill/DML path) is **committed scope**, not a demand-gated maybe. The "one portable script, DDL+DML, both backends" headline is backed by committed PRs (§10), not aspiration.

**Fully-structured, no-raw-SQL stance (property A, binding).** The DSL is **entirely structured: there is no raw-SQL escape hatch.** Every transform/predicate an author writes — a `set` RHS, a `where`, an `addCheck` body, a partial-index `where:` — is built as a **closed expression AST** through an **all-strings fluent column-accessor builder** (`(c) => Expr`, §3.2/§3.3.1), never as SQL text. The author writes a concise fluent expression (`(c) => c("first").concat(" ", c("last"))`), and **the engine owns 100% of SQL rendering** per dialect. The single injected handle `c` is a column-accessor function whose call `c("first")` produces a `ColRef` node and whose returned value carries chainable operator methods (`.eq`, `.concat`, `.gt`, `.coalesce`, …) that build the AST directly, auto-wrapping bare JS values to `Literal` binds — *not* verbose prefix-builders; the scalar-function namespace hangs off the same handle as `c.fn.*`. Only the operator/function **methods** are non-string (the builder API); every **identifier** is a plain string literal, uniform with `createTable("orders", …)`. This is a deliberate, principled tradeoff: it buys **guaranteed portability by construction** (the expressible surface *is* the supported surface — anything not expressible is a hard structured error, never a silent mis-apply, §9), and it **removes the expression-parser drift surface entirely** — the AST is *constructed* in JS by the fluent builder and serialized to IR, never *parsed from text*, so there is no lexer, no precedence parser, no Rust-vs-JS parser drift, and no differential fuzzer. The cost is an honest unsupported tail (exotic PG-only functions, subqueries/windows in a transform, cross-table correlation): these are hard `UNSUPPORTED` errors (`kind:"expr"|"op"`, §8.8), not a raw fallback. **Bonus:** HIGH-1 — the expression-grammar anti-drift problem earlier readings wrestled with — is *eliminated*, not merely mitigated: validation is now a purely structural allow-list check over the same schemars-single-source IR as every other op.

**Typing stance (binding).** Table and column **names** in migration ops are **plain strings** (`addColumn("users", "last_seen", …)`, `update("orders", …)`, `c("status")`), **never** bound to the live `@zeroship/db` schema. Migrations are immutable historical artifacts; a migration that referenced `users.lastSeen` must still compile after a later migration drops that column. Structural type-safety (op function arg shapes, the `t` column-type lexicon, the fluent-expression node shapes, insert-row value shapes) is preserved; **name existence is validated at apply time against the real DB**, never at `tsc` time against the declared schema (§3.3). This follows industry precedent: Kysely uses `Kysely<any>` in migration files on purpose, Alembic uses string names, Drizzle migrations are generated SQL — none bind migration files to the live schema.
...
The goal: **a creator (or the AI builder) writes one migration; it applies faithfully to Postgres and SQLite, doing both schema changes (DDL) and data migrations (DML)** — portably for the common cases (all DDL, literal insert/delete, the portable update/backfill fluent expressions, and the `c.fn.splitPart` helper within its pinned single-ASCII envelope, §9), and dialect-explicit (with a structured, machine-readable error rather than a silent mis-apply) for the long tail. The headline is "one script, both backends" for the common case, not an unbounded "any transform is portable" — §9 draws the boundary precisely, with the SQLite `split_part` lowering exhibited in-doc and proven byte-identical to PG against real SQLite 3.51.2. The proof requires adding `instr` to the SQLite authorizer allow-list — a deterministic, side-effect-free builtin — as an explicit, separately-reviewed change (§9); a `substr`-only split is mathematically infeasible.

**The AI builder is the dominant author.** Per `AGENTS.md` ("AI builds it"), the AI builder, not a human, authors most migrations. An LLM reaching for a non-portable transform (an exotic PG-only function, a subquery in a `set`) is the common case, not the edge case — and because there is no raw escape, the design must tell the AI loop *structurally* and *early* when a transform is not expressible. The all-strings fluent surface helps here too: the AI emits a chained expression (`(c) => c("total").gt(0)`) whose every node is a closed AST variant, so a non-expressible reach (a window, a cross-table ref) simply has no fluent method to call — the boundary is in the API shape, surfaced as a structured error, not a free-text string that mis-applies. The design is shaped around the AI loop's feedback mechanism throughout:

- **Structured, machine-readable rejections are a load-bearing contract** (§3.3.1 / §8.8): every authoring-time rejection carries `{ code, op_index, ts_location, dialect, reason, suggested_fix? }`, because the AI loop self-corrects on structured payloads, not prose. The human-readable message is the projection of the structured error.
- **The structural expression-AST validation (§3.3.1) and the structured-error envelope land at IR-freeze (PR1)** — when the IR shape is frozen — because they are the AI loop's primary feedback signal and must exist from the first IR PR. The validation is a structural allow-list check over the closed expression AST, not a text parser.
...
So we need an **imperative op surface** (Alembic / Kysely shape) *in addition to* the declarative one. They are not competitors: §7 shows the declarative path can **scaffold** a DSL migration (autogenerate parity), and both paths emit the *same IR*.

### 1.3 What "primary path" means concretely

- New migrations are authored as `.ts` files that import the named op-functions from `@zeroship/migrate` and export `up()`/`down()`. `zeroship-migrate new` scaffolds such a `.ts`, not a `.sql`.
- `zeroship-migrate generate` (declarative diff) emits a `.ts` **scaffold** in the same named-import shape (Alembic-autogenerate parity), which the author reviews/edits and which carries DML hooks the differ left as TODOs.
- The dbmate-style raw-SQL **loader stays** (the engine still ingests hand-written `.sql` for the public dbmate CLI in §7 and the platform Flyway-mode profile), but it is no longer the recommended way to *author* a new migration. **There is no DSL-surface raw-SQL escape** — a creator authoring through the named-import DSL has no way to emit raw dialect SQL; raw `.sql` survives only as a separate, operator/platform-facing loader path (the dbmate CLI and the platform Flyway-mode profile), never reachable from the structured DSL.
...
| `AppliedPlan.flags: MigrationFlags` | `flags`, with unset fields **derived** from the ops (e.g. any `dropColumn`/`del` ⇒ `destructive: true`). Flags are also computed per step where the orchestrator needs them. |
...
| `insert` / `update` / `del` / `backfill` | **always `None`** — no general inverse of a DML statement |

**Composition rule:** a migration's `down` is the reverse-ordered concatenation of each op's `down` only if **every** op is auto-reversible. If any op yields `None` and the author did not provide an explicit module-level `down()`, the whole migration is `down: None` (non-rollbackable). The §3.1 hero example (which contains a `backfill` and a `dropColumn`) therefore **must** hand-write `down()`. An author-supplied `down()` is itself a structured migration — its own op-function calls + fluent transforms — subject to the same down-derivation and lowering, never a raw-SQL string.
...
- **A plan is `rollbackable` only if every step has a `down`.** Computed and surfaced **at plan time** (load): the loader sets `plan.rollbackable: bool`, and `status`/`rollback` shows a plan as non-rollbackable by construction before the operator attempts it. Any plan containing a `Backfill`, a `Dml`, or an in-flight `OnlineRename` is non-rollbackable unless the author supplied a complete module-level `down()` that itself lowers to an all-reversible plan. The recovery posture for a non-rollbackable plan is **roll-forward**.
- **Author-supplied `down()` is its own plan.** It lowers to its own `AppliedPlan` (with its own DDL + reverse-backfill steps), applied as a forward plan via `apply_plan`, subject to this same rollbackability analysis.

### 2.2 The IR stores *ops*, not SQL — rendering is the engine's job

**`.ir.json` NEVER contains SQL.** It contains the logical op (`{ "op": "addColumn", "table": "users", "column": "email", "type": "string", "nullable": false }`) and, for transforms/predicates, the **closed expression AST as data** — the same tree the fluent column-accessor builder constructs (`{ "op": "update", "table": "users", "set": { "full": { "concat": [{"col":"first"}, {"lit":" "}, {"col":"last"}] } }, … }`, which is what `(c) => c("first").concat(" ", c("last"))` records). The engine renders the dialect SQL at load time through `DdlEmitter` (DDL) and the expression renderer (transforms) — the exact same code path the declarative differ uses for DDL (`declarative.rs:3810`). This is what makes one artifact apply to both backends: the artifact is dialect-neutral data; the engine lowers it. **There is no exception** — no op carries dialect-specific SQL text, because there is no raw-statement escape and no raw-expression-text escape (property A, §0). Every byte of SQL the engine applies is engine-rendered from structured data.
...
| `renameColumn` | `table, from, to, type` (the IR carries the neutral type discriminant the public `ColumnDef` reduces to — never a separate public `ColType` param, §3.2) | IrAuthor maps the neutral type→PG type, then online expand-contract — `ExpandContractAuthor` lowers `OnlineIntent::RenameColumn{ty:<pg-type>}` (`expand_contract.rs:70/:82`) | IrAuthor maps the neutral type→SQLite affinity, then **rebuild planner** (NOT `ExpandContractAuthor`) — §2.6 |
...
| `update` (one-shot) | `table, set{}, where?` | NEW assembler → a `PlanStep::Dml` step: `UPDATE … SET … WHERE …`; the `set` values and `where` are **closed expression ASTs** built via the fluent column-accessor builder (`(c) => Expr`, §3.2/§3.3.1), structurally validated (§3.3.1) | portable for in-AST `set`/`where` (§9) |
| `update { batch }` / `backfill` | `table, set{}, where?, cursorColumn?, batchSize?, name` (cursorColumn defaults to the single-col PK; batchSize defaults to the engine's choice — §3.2) | lowers to a `Backfill(BackfillSpec)` plan step (`backfill.rs:76`), run by `run_backfill` (PG) / the new SQLite executor (§2.3.1) | portable on BOTH backends (PG via the existing executor; SQLite via the committed §2.3.1 executor) |
| `del` | `table, where` (mandatory), `{ limit? }` | NEW assembler → a `PlanStep::Dml` step: `DELETE FROM … WHERE …`; `where` is a closed expression AST built via the fluent column-accessor builder (`(c) => Expr`); `destructive: true` derived; `where` required (no accidental full-table delete). (Named `del`, not `delete`, because `delete` is a JS reserved word — §3.2.) | portable |

**No escape hatch.** There is no raw-statement op and no raw-statement variant in the closed set. Anything not expressible as a structured op or a closed expression AST is a hard structured error (`UNSUPPORTED` (`kind:"op"`/`"expr"`), §8.8), never a raw-SQL fallback (§9). The expressible surface is the supported surface; portability is by construction.
...
- **In the `.ir.json`**, an `insert` op carries a structured `values` facet: `{ "op": "insert", "table": "...", "columns": ["a","b"], "rows": [[<lit>,<lit>], …] }`, where each `<lit>` is a typed JSON scalar — never a SQL string. Per the §2.5 constrained numeric domain, a numeric `<lit>` is an exact `i64` integer or a decimal-string (`{ "decimal": "…" }`); a fractional/exponential JS `number` or an integer ≥ `2^53` is **rejected at record time** (use a `bigint` literal or decimal-string). Transform expressions on the RHS of an `update`/`backfill` `set` are **closed expression-AST nodes** built by the fluent column-accessor builder (§3.3.1), whose embedded `Literal` params (bare JS values auto-wrapped by the fluent methods) are themselves typed scalars carried as native binds; `insert` row *values* are always typed scalars.
...
- **Checksum.** The bind values fold into the plan checksum via the canonical IR op list (§2.4) — an `insert`'s `rows` array is canonicalized (stable column order, canonical scalar serialization) and length-prefix-folded (§2.4 point 2), and an `update`/`del`/`backfill` expression AST's typed `Literal` params (auto-wrapped from bare JS values) fold via the expression-node rule (§2.4 point 3, with the same typed-scalar canonicalization as the `rows` facet). So changing a seed value OR an in-AST threshold value is drift, and the checksum is dialect-stable because the AST + typed values are dialect-neutral (per-dialect rendering only affects `$n` vs `?n` and the SQL spelling, which is not hashed). A fixture asserts two migrations differing only in an expression-AST `Literal` param have different `Checksum::of_ir`.
...
2. **The checksum is computed by the engine — not by the JS builder — over a canonical-IR input shape.** The JS builder does not reimplement the SHA-256 length-prefixed folding. It emits ops; the engine computes the checksum. There is exactly one checksum implementation, in trusted Rust. The `.ir.json` may carry a `checksum` field as an integrity hint, but the engine recomputes and is authoritative; the hint is advisory and need not be present.
   ...
   - **`CanonicalOpList` — the typed, dialect-neutral input `Checksum::of_ir` folds (defined here).** `CanonicalOpList` is the named type the golden-fixture value-equality gate (§2.5) hinges on, so it has a concrete shape:
     ...
     3. **Expression-AST nodes (typed `Literal` params fold in).** A `set`/`where` transform is a closed expression AST built via the fluent column-accessor builder (`(c) => Expr`, §3.3.1) — `ColRef | Literal | BinOp | UnaryOp | Case | FnCall | FnSynth | Cast`. Each node serializes through JCS like any other op field (point 1); a `Literal` param node (a bare JS value auto-wrapped by a fluent operator method, e.g. the `0` in `c("total").gt(0)`) carries its typed scalar (canonicalized identically to the §2.3.2 `rows` facet — `i64` / decimal-string / string / bool / null / base64-bytes; never a JS float). So two migrations differing only in a `Literal` value (`c("total").gt(0)` vs `c("total").gt(5)`) have different `Checksum::of_ir` — the "changing a seed/threshold value is drift" guarantee holds for AST-embedded params, not only the `rows` facet. Because the AST is structured data (not text), there is no fragment-template-text to fold separately; the node tree *is* the canonical form.
     4. **The list is folded in op order, each element length-prefixed** (the same length-prefixed-fold discipline `Checksum::of` already uses for `up`/`down`), so reordering or inserting an op changes the checksum. Every op — DDL or DML — contributes only its JCS bytes; no op contributes raw SQL text (there is no raw escape — property A).
...
- **What makes an artifact `PgOnly` in the no-raw model.** Since there is no raw escape, `PgOnly` is never author-declared via a raw statement. It is **derived**: an artifact is `PgOnly` iff it contains a structured op or expression the engine can render on Postgres but which is a hard error on SQLite — the bounded set enumerated in §9 (`insert { onConflict }`, an out-of-envelope `c.fn.splitPart`, the PG-only ANN index methods, and the JSON/date/regex/window/cross-table tail that is simply not expressible at all). An op that is unsupported on *both* dialects is `UNSUPPORTED` (`kind:"op"`/`"expr"`) (not `PgOnly`); `PgOnly` is reserved for the PG-renderable-but-SQLite-unsupported subset.
...
- **Portability is discovered at AUTHOR/CI time against BOTH targets unconditionally** — not deferred to whichever target a given deploy uses. Production is PG (the permissive target): if the only check were per-deploy, a `PgOnly` migration would deploy cleanly to prod and silently break the dev (SQLite) tier. Modeled on validating against the full target matrix at proposal time:
  - The CI gate (the §5.1 consistency gate, extended) lowers every not-yet-applied `.ir.json` with `target_dialect = Postgres` and with `target_dialect = Sqlite`; if the SQLite lowering fails (a PG-renderable-but-SQLite-unsupported op or expression — `insert { onConflict }`, an out-of-envelope `c.fn.splitPart`, a PG-only index method; §9), the artifact is flagged `dialect_scope = PgOnly` at CI/author time and the AI loop is told *then* (a structured `UNSUPPORTED` (`kind:"expr"`/`"op"`)/`DIALECT_SCOPE_PGONLY` payload, §8.8).
...
1. **Authoritative — golden `.ir.json` fixtures, with a named canonicalization and a value-level hard gate.** A corpus of `.ir.json` files is loaded by both the JS builder (it emits JSON for the equivalent DSL source) and the Rust engine (it deserializes + renders + applies on PG and SQLite).
...
The builder emits `renameColumn` carrying a **dialect-neutral column type** (the §3.2 lexicon, e.g. `"string"`/`{ ref }`/`{ vector: n }`), not a raw dialect type string and not the Postgres-specific expand-contract plan. The builder authors no triggers, dual-write functions, or dependency edges; lowering is the engine's job.

**The neutral-column-type → PG-type / SQLite-affinity split.** `OnlineIntent::RenameColumn.ty` is, in today's code, "the Postgres type of the column (emitted verbatim)" (`expand_contract.rs:82`) — a raw PG type string. The IR must not carry that PG string; it carries the dialect-neutral column type, and `IrAuthor` renders it per dialect at lowering, using the same type-mapping the `DdlEmitter`/`t.*` lexicon applies for `addColumn`:
- **PG lowering** ⇒ `IrAuthor` maps the neutral column type to its PG type string and constructs `OnlineIntent::RenameColumn { ty: <pg-type> }`, which `ExpandContractAuthor::author` emits verbatim (unchanged — the neutral→PG mapping happens before it, in `IrAuthor`).
- **SQLite lowering** ⇒ `IrAuthor` maps the neutral column type to a SQLite affinity type and feeds it into the rebuild planner's desired-column set — it never passes a PG type string to the SQLite leg.
...
- `dialect == Postgres` ⇒ map neutral column type→PG type, build `OnlineIntent::RenameColumn{…, ty: <pg-type>}`, call `ExpandContractAuthor::author(…)`, set `online: true` + `phase`.
- `dialect == Sqlite` ⇒ bypass `ExpandContractAuthor` entirely, map neutral column type→SQLite affinity, synthesize the rebuild input the planner consumes (the desired post-rename column set), invoking the SQLite rebuild planner to emit the 12-step sequence (offline, `online: false`).
...
- **Fixture:** an `renameColumn`'s E1..C2 ids (and intra-chain `depends_on`) are byte-equal to the ids the declarative `t.*`-diff path produces for the equivalent rename.
...
- **Faithful-e2e proof (per `feedback_faithful_e2e_tests.md`).** A test applies a single `renameColumn` IR plan on a real SQLite DB and asserts it executes through `rebuild_one` (a row survives the rename, the old column is gone, the journal records the rebuild migration's version) and journals identically to the same rename run through the declarative path now routed through `apply_plan`.
...
- `backfill` emits a `BackfillSpec` (`backfill.rs:76`): `table, cursor_column, batch_size, set_clause, filter?, name` (the `filter` field is the engine-internal name for the rendered predicate; the author writes `where`, §3.2). The `set`/`where` an author writes are fluent `(c) => Expr` callbacks (§3.3.1); the engine renders the closed AST into the spec's `set_clause`/`filter` per dialect — the author never writes raw SQL (property A). The engine owns the cursor loop. On Postgres it assembles the guarded windowed `UPDATE` via the existing writable-CTE executor (`backfill.rs:286`). On SQLite it uses the SQLite backfill executor (§2.3.1) — a plain batched `UPDATE … WHERE rowid IN (SELECT … LIMIT n)` loop. The author never writes the loop SQL.

---

## 3. The TypeScript DSL API surface (named ESM imports)

The DSL is the package `@zeroship/migrate`, consumed as **named ESM imports**. A migration is a `.ts` module that imports the op-functions it uses and exports a single `default { up, down? }` object whose **parameterless `up()` (required) and `down()` (optional)** functions call them directly. There is **no `op` object and no `op.` prefix** (property B, §0). **The DSL is NOT generic over the live schema** (see the typing stance, §3.3): table/column names are plain strings.

The op-functions are:
...
import {
  ...
  insert, update, del, backfill,   // `del`, not `delete` — `delete` is a JS reserved word
  t,                                // `t` = the fluent column-type lexicon (shared with @zeroship/db)
} from "@zeroship/migrate";
```

There is **no importable `fn`**: the scalar-function namespace is reached through the single expression-builder handle as `c.fn.*` (§3.3.1), so an author never imports it and the hero example has no imported-but-unused symbol.
...
export default {
  name: "split_name_column",   // optional; defaults to the filename label ("split_name")

  up() {
    addColumn("users", "first_name", t.text());   // nullable by default — the safe additive column
    addColumn("users", "last_name", t.text());
    backfill("users", {
      // cursorColumn defaults to the single-column PK ("id"); batchSize defaults to the
      // engine's chosen size — both are overridable, neither is boilerplate here.
      // PORTABLE form (recommended, AI-scaffolded): the scalar-function namespace `c.fn`
      // (reached off the single builder handle, never imported) exposes the engine-
      // synthesized helper `c.fn.splitPart` (§9), which emits split_part on PG and a
      // PINNED, exhibited, authorizer-legal instr+substr expression on SQLite (proven
      // byte-identical to PG against SQLite 3.51.2; instr is added to the SQLite
      // allow-list for this engine-owned lowering). The " " delimiter is a single
      // ASCII char and n is a positive literal (1..8), so this call is squarely IN
      // the helper's proven envelope (§9) and is portable across both backends.
      // A multi-char/empty/non-ASCII delimiter, n<=0, or n>8 is a hard SQLite error
      // (EXPR_NOT_PORTABLE), not a silent mis-split. There is no raw escape:
      // c.fn.splitPart builds an FnSynth AST node, never SQL text. Every identifier is
      // a string: c("name") is a ColRef on the enclosing op's target table.
  ...
  down() {
    ...
    // reverse backfill (safe-concat models the NULL-skipping form authors should copy):
    backfill("users", {
...
**Why `down` is hand-written here:** this migration contains a `backfill` (DML — no general inverse) and a `dropColumn` (data-destroying). Per the down-derivation matrix (§2.1.1), the engine cannot auto-derive a reverse for either, so it would yield `down: None`. Authoring `down()` is the only way to make this migration rollbackable; omitting it is legal but leaves it irreversible. The DSL never silently fabricates an inverse for DML or lossy DDL.

> **Portability for this example (§3.3.1 / §9).** As written with the fluent `c.fn.splitPart` helper and a single-ASCII-space delimiter + positive literal `n ≤ 8`, this call is inside the helper's pinned, proven envelope (§9) and **is portable** across PG and SQLite. A helper call **outside** the envelope (multi-character/empty/non-ASCII delimiter, `n<=0`, `n>8`, non-literal args) is **not silently mis-split — it is a hard `EXPR_NOT_PORTABLE` error on the SQLite leg** (§9). There is no raw fallback: an exotic PG-only split is not expressible at all, so it surfaces as `UNSUPPORTED` (`kind:"expr"`). The boundary is precise: portable via the engine-synthesized helper within its pinned envelope; a hard SQLite error outside it; no SQL-text escape anywhere.

**What the AI loop receives for an out-of-envelope `c.fn.splitPart` (the machine-readable feedback, §8.8 / §3.3.1) — the human-facing rendering leads with `suggested_fix`; `code` is secondary metadata:**
...
The AI loop then either reshapes the call to stay in-envelope or explicitly marks the migration `dialect_scope = PgOnly` (§2.4.1) — it cannot silently ship a migration that no-ops/mis-applies on SQLite, because an AI migration is PG-only-by-default until the both-backends dry-run passes (§8.8). This is why the structural validator + structured-error envelope are PR1, not PR6.

**The module shape is a single `export default { name?, up, down? }` object** (mirroring the platform's `export default { schema, fetch, rpc }` idiom, AGENTS.md). `up` is required, `down` optional, and `name` optional — when omitted it is derived from the filename label the way the *version* already is (§2.1), so the identity is not split across a free-floating top-level `const`. A loose top-level `export function up()` plus a stray `export const name` is no longer the shape; one typed default object is more discoverable and cannot desync.

**`up()`/`down()` are parameterless; the imported op-functions RECORD into an ambient per-migration recorder, they do not execute SQL** (§4). This is the established vitest/jest/Playwright pattern (`import { test } from "vitest"` then call it): the build/dev evaluator **installs a fresh ambient recorder before calling `up()` (and again before `down()`)**, and each imported op-function (`createTable`, `addColumn`, `update`, …) appends a plain-data op onto that active recorder, synchronously. Each migration module is evaluated in **isolation** — the PR4a recorder service spawns a fresh sandbox child per record-invocation (§8.9), so there is exactly one active recorder per evaluation and the ambient context is safe and deterministic; recording is synchronous (no `await`). Calling any op-function **outside an active recorder** (i.e. at module top level, or after `up()` returns) throws a clear structured error (`OP_OUTSIDE_RECORDER`, §8.8) — the op cannot be silently lost. The recorded op list is then rendered/linted and produces plan/`--sql` output (Alembic offline-mode discipline: build the op tree, execute later).
...
### 3.2 The named-export op-functions (abridged; full signatures generated from the IR schema)

**All table/column/name references are plain `string`.** The op-functions are top-level named exports of `@zeroship/migrate`, not methods of an `op` object — there is no `op.` prefix. **There is NO generic `S extends Schema` parameter, and no `TableName<S>` / `keyof RowOf<S,T>` / `RowInsert<S,T>` binding to the live schema.** Structural type-safety is preserved for op argument *shapes*, the `t` column-type lexicon, the fluent-expression node shapes, and insert-row *value* shapes. Name existence is an apply-time check against the real DB (§3.3). **There is no `Raw` type on the surface** — every transform/predicate is a fluent `(c) => Expr` callback, never a `Raw` string (property A).

Three shaping conventions, applied uniformly, remove the inconsistencies an earlier draft carried:

1. **One column-type representation.** Every column-type position takes a `ColumnDef` produced by the fluent `t.*` lexicon. There is no second bare-string `ColType` union on the public surface; the neutral discriminant `t.*` carries internally is never a sibling parameter type.
2. **`(table, spec)` for every constraint/index adder.** A constraint adder takes the table and a single named-field `spec` object; `name` always lives in the spec, never as a transposable trailing positional, and `columns`/`references.columns` are named fields, never a positional pair.
3. **`verb(table, { … })` for every DML op.** `insert`/`update`/`del`/`backfill` all read `verb(table, { … })`, and the predicate keyword is `where` everywhere (no `filter` synonym).

```ts
// All are NAMED imports from "@zeroship/migrate", called directly inside up()/down().
// Each RECORDS into the ambient per-migration recorder (§3.1) and returns void.

// ── DDL: tables ──
// `columns` is a plain object literal — no zero-arg () => Record thunk.
function createTable(name: string, columns: Record<string, ColumnDef>,
                     opts?: { ifNotExists?: boolean }): void;
// Table-scoped constraints/indexes (the rare case) use the builder overload:
// createTable(name, columns, (b) => { b.index(["email"], { unique: true }); … })
function createTable(name: string, columns: Record<string, ColumnDef>,
                     build: (b: TableBuilder) => void): void;
function dropTable(name: string, opts?: { ifExists?: boolean; cascade?: boolean }): void;

// ── DDL: columns ──
// `type` is always a ColumnDef from the t.* lexicon — one column-type type everywhere.
function addColumn(table: string, name: string, type: ColumnDef): void;
function dropColumn(table: string, name: string, opts?: { ifExists?: boolean }): void;
function renameColumn(table: string, from: string, to: string, type: ColumnDef): void; // → online intent
function alterColumn(table: string, name: string, change: AlterColumnChange): void;

// ── DDL: constraints / indexes — every adder is (table, spec); `name` lives in spec ──
function addForeignKey(table: string, spec: {
  columns: string[];
  references: { table: string; columns: string[] };
  onDelete?: FkAction; onUpdate?: FkAction; name?: string;
}): void;
function addUnique(table: string, spec: { columns: string[]; name?: string }): void;
// addCheck's body is the fluent (c) => Expr builder — NOT a Raw string.
function addCheck(table: string, spec: { expr: ExprFn; name?: string }): void;
function dropConstraint(table: string, spec: {
  name: string; type?: "pk"|"fk"|"unique"|"check"; ifExists?: boolean;
}): void;
// A partial-index `where:` is the same fluent (c) => Expr builder.
function createIndex(table: string, spec: {
  columns: string[]; name?: string; unique?: boolean;
  using?: "btree"|"gin"|"gist"|"ivfflat"|"hnsw"|"fts5";
  where?: ExprFn; concurrently?: boolean;
}): void;
function dropIndex(name: string, opts?: { table?: string; ifExists?: boolean; concurrently?: boolean }): void;

// ── DML (identifiers are plain strings; transform/predicate bodies are fluent (c) => Expr, §3.3) ──
...
function insert<R extends Row = Row>(table: string, args: {
  rows: R | R[];
  /** PG-ONLY. Rejected as a hard build error on a SQLite target (§9); see the JSDoc warning on the field. */
  onConflict?: { columns: string[]; doUpdate?: Partial<R> };
}): void;
// `set` values and `where` are fluent (c) => Expr builders, NOT Raw.
function update(table: string, args: { set: Record<string, ExprFn>; where?: ExprFn }): void;
// `delete` is a JS reserved word → the DML delete op is named `del`. `where` is mandatory.
function del(table: string, args: { where: ExprFn; limit?: number }): void;
// `where` (not `filter`) — one predicate keyword across update/del/backfill.
// cursorColumn defaults to the single-column PK; batchSize defaults to the engine's choice.
function backfill(table: string, args: {
  set: Record<string, ExprFn>; where?: ExprFn;
...
// The fluent expression callback — ONE injected builder handle. `c` is a COLUMN
// ACCESSOR FUNCTION (not a property proxy): c("name") -> a ColRef chain; the scalar-
// function namespace hangs off it as `c.fn.*`. Builds the closed AST; see §3.3.1.
type ExprFn = (c: ExprBuilder) => Expr;

// `t` (the fluent column-type lexicon, shared with @zeroship/db) is the named import
// authors use to build a ColumnDef. Every factory returns a CHAINABLE ColumnDef
// builder mirroring the expression chain — nullable by default, .notNull()/.default(x)
// /.ref(...) to opt in:
//   t.id(), t.text(), t.numeric(), t.timestamp(), t.uuid(), t.bytes(),
//   t.boolean(), t.json(), t.ref(targetTable: string), t.vector(n: number),
//   t.geoPoint(), …
// Chainable modifiers (each returns the same builder, order-independent):
//   .notNull(), .default(value | { fn: "now" | "genRandomUuid" | … }),
//   .ref(targetTable: string), .primaryKey(), .unique()
// An options-bag overload is ALSO accepted for callers who prefer it
// (t.text({ notNull: true, default: 0 })), but the fluent chain is the headline
// form so the type side reads the same grammar as the expression side.
```

Note `t.ref(target)` / `t.text().ref(target)` carries a **plain string** target-table name, not `TableName<S>`. The `t` lexicon is the same one `@zeroship/db` exposes (§3.3 / PR5) — there is no `Raw` member.

**Nullable is the default; `notNull` is the opt-in.** `t.text()` is nullable — the safe, common shape for an additive column (you cannot add `NOT NULL` to a populated table without a default/backfill). The author writes `t.text().notNull()` only for the rarer, riskier case. There is no `{ notNull: false }` double-negative boilerplate anywhere on the surface.

**`onConflict` is PG-only and the type cannot fully gate it (single dialect-agnostic API, §3.4).** Because there is one API and no dialect branching, `onConflict` lives on the `insert` arg for all targets but is **rejected as a hard build error on a SQLite target** (§9, surfaced as `dialect_scope = PgOnly`). To keep autocomplete honest the field's JSDoc is an explicit **`@deprecated`-style PG-ONLY** warning (rendered prominently by editors), and the structural validator rejects it on the SQLite leg with the structured envelope — so the "passes on PG, fails on SQLite" surprise surfaces at build with a clear payload, never at runtime. A dialect-scoped `pgInsert` builder was considered and rejected: it would reintroduce the per-dialect author branching property A forbids.

**`batchAlterTable` (SQLite-safe rebuild — Alembic `batch_alter_table` analog).** A named import `batchAlterTable(table, build)` whose `build` callback receives a scoped subset of the column/constraint op-functions (`addColumn`/`dropColumn`/`renameColumn`/`alterColumn`/`addForeignKey`/`addCheck`) pre-bound to the one target table, recording into the same ambient recorder. This is the same `(table, build)` scoped-builder shape `createTable`'s second overload uses, so the two table-scoped builders read alike.
...
- **Op-function argument shapes** — `addColumn(table, name, type)` etc. are typed: you cannot pass a number where a `ColumnDef` is expected, or omit a required argument.
- **The `t` column-type lexicon** — `t.text()`/`t.numeric()` and their chainable modifiers (`.notNull()`/`.default()`/`.ref()`) are typed; an invalid type factory or modifier fails `tsc`.
- **The fluent-expression node shapes** — the `c` builder methods (column accessor + the `c.fn` namespace) are typed: `.eq`/`.concat`/`.gt`/`c.fn.splitPart` etc. have typed arities and return an `Expr`; calling a non-existent operator method fails `tsc`. (The method *names* are the typed builder API — that is not the forbidden string-vs-typed mix, because every *identifier* `c` references is still a plain string.)
- **Insert-row VALUE shapes** — `insert` rows are a loose `Record<string, ScalarValue>` (`Row`) by default; a caller MAY supply a generic `insert<R>(…)` for editor convenience, but `R` is **never auto-derived from the live schema**. The value *kinds* (string/number/bigint/boolean/null/decimal/bytes) are typed; the *column-name → type correspondence against the live schema* is not.

**What is NOT type-checked (validated at apply time instead):**
- **Table and column NAMES** — `table`, `column`, `from`, `to`, `name`, `cursorColumn`, the keys of a `set`, every `where`-referenced column, and every `c("…")` argument — are plain `string`. Their existence is **not** a `tsc`-time check. They are validated at **apply time against the real DB** (and `c("…")`/`ColRef` resolution is an apply/render-time check against the actual target table, §3.3.1). A migration referencing a non-existent column fails when it is applied (with the structured error envelope, §8.8), not when it is type-checked — which is correct, because the column may exist at the migration's historical point even if it does not exist in the current declared schema.
- **Transform expressions** — a `set` value or a `where` body is a fluent `(c) => Expr` callback. TypeScript checks the *shape* of the builder calls but cannot verify the *referenced names* against the schema, nor the transform's dialect-portability. Portability and name resolution are validated by the §3.3.1 structural validator (over the constructed AST) and the guard at render time (PG) / authorizer (SQLite) — not by `tsc`.

So the guarantee is: **op-function shapes, the `t` type lexicon, the fluent-expression node shapes, and value kinds are typed; names (every `c("…")` included) are validated at apply/render time, never `tsc`-bound to the live schema.** The part most likely to be semantically wrong (the transform's referenced names) is the part the type system cannot see — authors test it (the shadow-DB dry-run, §6.2).

#### 3.3.1 The all-strings fluent expression surface (the closed AST)

Every expression position — a DML `set` value, a `where`, an `addCheck` body, a partial-index `where:` — is a callback `(c) => Expr` with a **single injected builder handle** `c`. It is **not** a raw string; it constructs a node of the **closed expression AST** via a fluent, all-strings builder. The same closed AST is the single source of truth for the IR Op schema (schemars) and the structural validator (§3.3.1.1) — property C/D.

**`c` — the single builder handle (both a column-accessor function and the function namespace).** `c("first")` returns a `ColRef("first")` chain; the scalar-function namespace hangs off the *same* handle as `c.fn.*`. The argument to `c(…)` is a **plain string** — every identifier is a string literal, uniform with `createTable("orders", …)`. Collapsing the former two-argument `(c, f)` callback to one handle removes the positional-order memory burden (no "is it `(c,f)` or `(f,c)`?") and the dead second slot a pure-column predicate or pure-function expression used to carry. `c` is **scoped to the enclosing op's target table** (the single-target-table capability boundary, §3.3.1.1): `c("x")` resolves against that one table and nothing else; there is no `c("other_table.col")` and no second-table accessor (cross-table is not expressible — §9). Names stay plain strings (no live-schema typing — §3.3; no rot; existence is checked at apply/render time).

**Chainable operator methods (build the AST + auto-wrap bare JS values to `Literal`).** The value `c("…")` returns, and every expression built from it, carries:
- comparison: `.eq(x)`, `.ne(x)`, `.lt(x)`, `.le(x)`, `.gt(x)`, `.ge(x)`
- boolean: `.and(e)`, `.or(e)`, `.not()`
- arithmetic: `.add(x)`, `.sub(x)`, `.mul(x)`, `.div(x)`
- string/value: `.concat(...parts)` (raw `||`; NULL-propagating), `.concatWs(sep, ...parts)` (NULL-skipping, the safe default for joining names), `.coalesce(...)`
- null/bool tests: `.isNull()`, `.isNotNull()`, `.isTrue()`, `.isFalse()`
- cast: `.cast("integer" | "text" | "real" | "boolean" | "blob")` (the closed portable type set only)

Any bare JS value passed to an operator method (the `0` in `c("total").gt(0)`, the `" "` in `.concat(" ", …)`) is **auto-wrapped to a `Literal`** carried as a native bind (typed scalar — §2.3.2 numeric domain). The author never writes a placeholder or a parameter slot; the builder produces the `Literal` node and the engine binds it via `$n`/`?n` (never interpolated).

**`c.fn` — the scalar-function namespace.** It hangs off the single builder handle (there is **no importable `fn` and no second callback argument** — finding consolidated to one idiom). Members:
- `c.fn.lower(e)`, `c.fn.upper(e)`, `c.fn.trim(e)`, `c.fn.length(e)`, `c.fn.abs(e)`
- `c.fn.coalesce(...)`, `c.fn.nullif(a, b)`
- `c.fn.concatWs(sep, ...parts)` — NULL-skipping concatenation: drops NULL operands instead of propagating NULL (the safe form for joining first+last name). Engine-synthesized to be byte-identical across PG (`concat_ws`) and SQLite (a proven `coalesce`-folded `||` expression), so the common string-join transform is not a footgun. `sep` is a literal; for empty-string join use `c.fn.concatWs("", …)`.
- `c.fn.case([[cond, val], …], elseVal)` — the searched/simple `CASE` form as a first-class node
- `c.fn.splitPart(e, delim, n)` — the engine-synthesized portable split helper (§9), within its pinned single-ASCII-delimiter + positive-literal-`n` envelope
- `c.fn.now()`, `c.fn.genRandomUuid()` — DB-evaluated, apply-time `FnSynth` scalars (render to `now()` / `gen_random_uuid()` per dialect; the structured replacement for a frozen `Date.now()`/UUID literal, §4.3)

**One closed AST.** Every method/namespace member above builds exactly one variant of the same closed AST → IR:

`ColRef | Literal | BinOp | UnaryOp | Case | FnCall(allow-listed) | FnSynth | Cast`.

Method↔AST-node mapping:

| Fluent surface | AST node | Notes |
| --- | --- | --- |
| `c("name")` | `ColRef { name }` | name is a plain string; resolved against the enclosing target table |
| bare JS value (auto-wrapped) | `Literal { value }` | typed scalar (i64 / decimal-string / string / bool / null / bytes); native bind |
| `.eq/.ne/.lt/.le/.gt/.ge(x)` | `BinOp { op: =,≠,<,≤,>,≥, lhs, rhs }` | rhs auto-wrapped if bare |
| `.and/.or(e)` | `BinOp { op: AND/OR, … }` | |
| `.add/.sub/.mul/.div(x)` | `BinOp { op: +,−,×,÷, … }` | |
| `.concat(...parts)` | `BinOp { op: \|\| }` folded over parts | NULL operand ⇒ NULL on both backends (the one place PG/SQLite agree, so `\|\|` is admitted); use `c.fn.concatWs` for NULL-skipping |
| `c.fn.concatWs(sep, ...parts)` | `FnSynth { fn: concatWs, args }` | engine-synthesized NULL-skipping join; lowers to `concat_ws` on PG and a proven `coalesce`-folded `\|\|` on SQLite (byte-identical) |
| `.coalesce(...)` / `c.fn.coalesce(...)` | `FnCall { fn: coalesce, args }` | allow-listed scalar |
| `.not()` | `UnaryOp { op: NOT, operand }` | |
| `.isNull()/.isNotNull()` | `UnaryOp { op: IS NULL / IS NOT NULL }` | |
| `.isTrue()/.isFalse()` | `UnaryOp { op: IS TRUE / IS FALSE }` | |
| `.cast("…")` | `Cast { target: text\|integer\|real\|boolean\|blob }` | closed portable type set only; a non-portable cast target is rejected |
| `c.fn.lower/upper/trim/length/abs(e)` | `FnCall { fn, args }` | the allow-listed provably-identical scalar set |
| `c.fn.nullif(a,b)` | `FnCall { fn: nullif, args: [a,b] }` | |
| `c.fn.case([[cond,val],…], elseVal)` | `Case { branches: [(cond,result)], else? }` | each sub-expression must itself be in the closed AST |
| `c.fn.splitPart(e, delim, n)` | `FnSynth { fn: splitPart, args }` | engine-synthesized; in-envelope-only (§9); lowers to `split_part` on PG and the pinned `instr`/`substr` expression on SQLite |
| `c.fn.now()` / `c.fn.genRandomUuid()` | `FnSynth { fn: now \| genRandomUuid }` | DB-evaluated at apply time; renders to `now()` / `gen_random_uuid()` per dialect (§4.3) |

Only the operator/function **methods** are non-string — they are the builder API, not identifiers. That is **not** the forbidden string-vs-typed mix: every identifier (`c("first")`) is a plain string. The author writes the expression as a fluent chain; the recorder captures the constructed AST as data; the engine owns 100% of per-dialect rendering.

**The IR carries the expression as data, never SQL.** A `set: { full: c => c("first").concat(" ", c("last")) }` records as `{ "concat": [{"col":"first"}, {"lit":" "}, {"col":"last"}] }`; a `set: { first_name: c => c.fn.splitPart(c("name"), " ", 1) }` records as `{ "fn": "splitPart", "args": [{"col":"name"}, {"lit":" "}, {"lit":1}] }`. Both are dialect-neutral, so the one plan checksum is dialect-stable (per-dialect rendering is the engine's lowering, never hashed). The engine owns the per-dialect lowering: `FnSynth(splitPart)` → `split_part(...)` on PG and the engine's pinned, exhibited `instr`/`substr` expression on SQLite (§9), proven byte-identical over a bounded envelope against real SQLite 3.51.2.

**`cast` and `concat` semantics are pinned, not assumed identical.**
- **`.cast(...)`** is restricted to the closed portable set (`text`/`integer`/`real`/`boolean`/`blob`); a non-portable target is rejected (`UNSUPPORTED`, `kind:"expr"`). A cast's bit-exact value may differ across backends for edge inputs — "portable in shape, dialect-defined in numeric edge semantics"; authors needing exact cross-dialect equality test it (shadow-DB dry-run, §6.2).
- **`.concat(...)`** maps to `||`, whose NULL rule is documented: a NULL operand yields NULL on both backends. For the common safe join (first+last name) **use `c.fn.concatWs(sep, …)`**, which is NULL-skipping and proven byte-identical across dialects — the hero example (§3.1 `down`) models this form so authors copy the correct pattern, not the NULL-propagating one.

Net: a transform is portable **iff** it is built from the closed AST and any `FnSynth` is in its §9 envelope; anything else (an exotic PG-only function, a subquery/window, a cross-table reference) is simply **not expressible** — there is no fluent method for it — and surfaces as a hard `UNSUPPORTED` (`kind:"expr"`, or `EXPR_NOT_PORTABLE` for an out-of-envelope helper) on the SQLite leg, never a raw fallback. §9 is the authoritative boundary.

##### 3.3.1.1 The validator is STRUCTURAL — it supersedes the former text-grammar parser (HIGH-1 DISSOLVED)

The closed expression AST is **constructed in JS by the fluent builder and serialized to IR, never parsed from text.** Therefore there is **no lexer, no Pratt/precedence-climbing parser, no Rust-vs-JS parser drift, and no differential fuzzer.** This section SUPERSEDES the former "dialect-neutral fragment parser" design entirely: validation is a purely **structural allow-list check** over the same schemars-single-source IR Op schema as every other op. **This is what eliminates HIGH-1** (the expression-grammar anti-drift problem) — it is dissolved, not mitigated, because there are no two parsers to keep equivalent.

The closed AST is part of the IR Op schema (schemars single-source-of-truth, §2.5). Validation (Rust at lower/render; an optional best-effort JS hint) checks, structurally:

- **(a) Every node is in the allow-listed set** — `ColRef | Literal | BinOp(allow-listed operator) | UnaryOp(allow-listed) | Case | FnCall(allow-listed scalar: coalesce/nullif/lower/upper/trim/length/abs) | FnSynth(splitPart | concatWs | now | genRandomUuid) | Cast(portable type)`. A node outside the set cannot exist in a valid IR (the deserializer rejects it). There is no "function not on the allow-list" parse path because there is no text to parse — an unknown function simply has no builder method and no AST variant, so it cannot be recorded.
- **(b) `c.fn.splitPart` args are in-envelope** — the `delim` is a single ASCII character literal (one byte, code point < 0x80), `n` is a positive integer literal with `1 ≤ n ≤ 8`, and the column arg is a `ColRef` or in-AST sub-expression. Out-of-envelope (multi-char/empty/non-ASCII delim, `n ≤ 0`, `n > 8`, non-literal delim/`n`) is a hard `EXPR_NOT_PORTABLE` on the SQLite leg (§9).
- **(c) `ColRef` resolves to a column on the enclosing target table** — an APPLY/RENDER-TIME check scoped to the one target table of the enclosing DML/DDL op. A reference to a column not on that table is a hard error; a cross-table reference is impossible by construction (`c` is single-table-scoped — §3.3.1) and any synthesized cross-table node is a hard error. This is an injection defense **and** the deliberate capability boundary (cross-table/correlated transforms are an explicit portability exclusion, §9). It is checked at apply/render time, not `tsc`-time (consistent with §3.3).
- **(d) A `Cast` target is a portable type** (`text`/`integer`/`real`/`boolean`/`blob`).

Anything that fails (a)–(d) is a structured error (`UNSUPPORTED` with `kind:"expr"`, or `EXPR_NOT_PORTABLE`, §8.8), never a raw fallback. Because validation is structural over constructed data:
- **There is ONE authoritative gate: the Rust structural validator at lower/render.** A JS-side check, if present, is a best-effort fast hint over the same closed AST schema; it cannot disagree on the *structure* because both sides read the one schemars-emitted schema. There is no unbounded fragment language and therefore no accept-drift to fuzz — the former one-directional differential fuzzer is removed.
- **`Literal` binds are native parameters.** A `Literal` node's typed scalar is bound via `$n` (PG) / `?n` (SQLite), never interpolated — a value containing a quote/semicolon/comment cannot alter statement shape (§2.3.2). There is no interpolation surface because there is no template text; binds arise only from auto-wrapped fluent values.

**Fixtures (PR1):** (1) a constructed AST using each allow-listed node validates and renders on both backends; (2) a `.cast("integer")`, a `.concat(...)` with a NULL operand, and a `c.fn.concatWs(" ", …)` over a NULL operand apply identically on PG and SQLite (concat propagates NULL; concatWs skips it); (3) a synthesized node outside the allow-list (or a deserialized IR carrying an unknown node tag) is rejected at load with `UNSUPPORTED` (`kind:"expr"`); (4) a `ColRef` to a column not on the enclosing target table is rejected at render with the structured payload; (5) an out-of-envelope `FnSynth(splitPart)` is rejected with `EXPR_NOT_PORTABLE`; (6) a `Literal` carrying SQL metacharacters cannot alter statement structure (native bind).

### 3.4 Dialect-agnostic by default

There is **one** API and **no dialect branching at all** — there is no raw escape to branch inside (property A). The engine owns lowering (Postgres native vs SQLite rebuild). A transform that cannot be expressed portably is a hard structured error, not a per-dialect author branch. `using: "gin"|"ivfflat"|"hnsw"|"fts5"` index methods are Postgres-only logical hints; on SQLite the engine maps `fts5` to the FTS5 virtual-table path and rejects the PG-only ANN methods with a clear error (no silent degradation).
...
| --- | --- | --- |
| **Build** (`pnpm build` / `zeroship-migrate generate`) | Node (the vite-plugin / CLI build step) | discover `migrations/*.ts`; for a version that does **not** yet have a committed `.ir.json`, install a fresh ambient recorder, call the module's `up()` (and `down()`) so the named op-functions record into it (§3.1), and serialize the recorded op list to `<version>_<name>.ir.json` (+ source-map hint) for the author to commit; for a version that already has a committed `.ir.json`, do **not** re-evaluate the `.ts` — the committed artifact is the input (the build-once skip, §5.1) |
...
The recording builder is **pure JS**: each named op-function synchronously pushes a plain-data op onto the ambient recorder's array and returns (`void` — no `Promise`, no `await`; §3.1). It performs no I/O, no DB connection, no SQL execution. This is the Alembic "build the op tree, execute later" model. The build evaluator installs a fresh recorder before calling `up()` (and `down()`), so the ambient context is per-evaluation and deterministic.

### 4.2 Why not run it in V8 inside the engine?

Two reasons: (1) the engine is deliberately zero-V8 and zero-tokio; embedding a runtime would violate a key invariant. (2) Security: the DSL is untrusted creator/AI code. Running it at build time in Node and shipping only validated IR data means the engine's apply path only ever sees data, never code (§8). The recording builder does not even need V8 because it runs in the author's own Node build, not the engine.
...
3. **Non-determinism is a false-drift hazard — neutralized structurally by the build-once committed artifact (§5.1), with lint + docs as a pre-commit catch.** Because the `.ir.json` is committed build-once and never regenerated for an already-applied version (§5.1), a later rebuild where `Date.now()` differs cannot change a deployed migration's artifact or checksum. The remaining concern is pre-commit: an author should not bake a non-deterministic value into a not-yet-committed artifact. For that: (a) an ESLint/DSL lint flags `Date.now()`, `Math.random()`, `crypto.randomUUID()`, `new Date()` syntactically appearing inside an op-function argument (best-effort, AST-based), run in CI on changed migrations before commit; and (b) docs: "for values that must be computed at apply time use the DB-evaluated synthesized functions `c.fn.now()` / `c.fn.genRandomUuid()` (structured `FnSynth` nodes the engine renders to `now()` / `gen_random_uuid()` per dialect at apply time)."
   - **A DB-evaluated synthesized function is the default scaffold for any time/uuid-valued seed column.** `generate`/`new` scaffolds, when seeding a column whose `t.*` type is `timestamp`/`date`/`uuid` (or whose name matches a `created_at`/`updated_at`/`id` convention), emit the structured `c.fn.now()` / `c.fn.genRandomUuid()` `FnSynth` node (DB-evaluated at apply time) by default — never a frozen `Date.now()` literal, and (property A) never a raw SQL string.
   - **The AI-path determinism check is SCOPED to the actual risk surface (time/uuid-typed columns), NOT a blanket "reject any non-literal value."** Real AI migrations legitimately compute values (a seed from an imported constant, a `rows.map(...)`-built array); rejecting all of them raises iteration cost for little gain, since the build-once contract already makes post-deploy non-determinism unreachable. So: the strict reject fires **only** for a non-literal value bound to a time/uuid-typed column, flagged with "use the DB-evaluated `c.fn.now()` / `c.fn.genRandomUuid()` for apply-time evaluation." A computed string/number or a `map`-built array on a non-time/uuid column is accepted. The syntactic RNG/clock lint (a) still runs on all columns (it flags a *direct* `Date.now()` call anywhere).
   - **Fixture:** an AI migration building rows via `rows.map(r => ({ name: r.name, slug: slugify(r.name) }))` and seeding a config-derived constant is NOT rejected; a migration binding `Date.now()` to a `created_at: timestamp` column IS flagged with the `c.fn.now()` suggestion.
...
| **`OnlineIntent`→SQLite-rebuild bridge + neutral-type translation** | `ExpandContractAuthor` is PG-only and its `ty` is a raw PG type string (`expand_contract.rs:82`); the SQLite rebuild is a different subsystem; nothing wires them. `IrAuthor` must bridge them and map the IR's dialect-neutral column type → PG type / → SQLite affinity (§2.6). Execution rides PR0's dual-dispatch (§2.6.2) | PR2 (online) |
...
| **Structural expression-AST validator** (§3.3.1.1) | a purely **structural** allow-list check over the closed expression AST (`ColRef\|Literal\|BinOp\|UnaryOp\|Case\|FnCall\|FnSynth\|Cast`) carried in the IR Op schema (schemars single-source). The AST is **constructed in JS and serialized to IR, never parsed from text** — so there is **no lexer, no Pratt/precedence parser, no libpg_query, no Rust-vs-JS parser drift, and no differential fuzzer** (HIGH-1 dissolved). The Rust validator at lower/render is the sole authoritative gate; checks: every node allow-listed, `c.fn.splitPart` args in-envelope, `ColRef` resolves to the enclosing target table, `Cast` target portable (§3.3.1.1). | PR1 (the AI loop's primary feedback signal; pure analysis, no executor dependency; PR6a only wires it into the DML path) |
...
.ir.json
  └─ loader: deserialize MigrationIr (loader.rs, new branch); load_dir -> Vec<AppliedPlan> (§5.2)  ← NEW
       ...
       └─ IrAuthor::lower(ops, target_dialect) -> AppliedPlan     ← NEW; routes per op+target_dialect
            ...
            ├─ insert/update/del → DML assembler → PlanStep::Dml      ← NEW (PR6a; binds carried, §2.3.2)
            │                  (set/where AST validated by the §3.3.1.1 STRUCTURAL validator ← NEW PR1)
            └─ backfill/update{batch}
                            → PG: BackfillSpec (backfill.rs:286)
                               SQLite: NEW backfill executor (§2.3.1)  ← NEW (PR6b, committed)
            (NO raw op — every op renders from structured data; property A)
...
```
migration 0007_split_name: op #3 (createIndex at migrations/0007_split_name.ts:14)
  denied by guard: rendered statement is not permitted under the Confined profile
```
...
- **Test:** for a multi-statement op (`createTable` with an inline index and a comment), assert `applied_up` is byte-identical to the concatenation of the individually-guarded fragments, and that a denied rendered fragment aborts the whole step with the op-index + `.ts` location, with nothing applied.
...
| --- | --- | --- |
| `addColumn(table, name, type: ColumnDef)` | `ColumnSnapshot { name, data_type, nullable, default, encryption_sentinel, comment_sentinel }` — `data_type` from the `ColumnDef`'s neutral type→dialect mapping (`def_to_column_type_for_dialect`); `nullable` from the `ColumnDef` (nullable by default per the `t.*` lexicon — `false` only when `.notNull()` is chained, §3.2); `default` rendered by the shared kernel; the two sentinels built by `zeroship_schema::{query,mask_codec}` for an encrypted/masked column (NEVER re-spelled in `IrAuthor`) | `desired_snapshot` per-field loop (`declarative.rs:1069-1101`); `def_to_column_type_for_dialect`; `system_field_columns` (`:844`); the `mask_codec` kernel |
...
The declarative differ (`generate.rs:112` `DeclarativeAuthor::diff`) currently emits `Migration`s (SQL). It will instead emit a **named-import `.ts` scaffold** (and/or the `.ir.json` directly), because both paths converge on the same op vocabulary. This is **Alembic autogenerate parity**: `zeroship-migrate generate` diffs the `t.*` schema against the live/snapshot schema and writes a DSL migration the author reviews and augments with DML (the differ leaves `// TODO: backfill` markers where a non-null add or a type narrowing needs data). The differ's emitted DDL ops are exactly the portable op set in §2.3.
...
- **The named-import DSL** = "author the change imperatively, including data" (the primary path; covers everything).
- `generate` bridges them: schema diff → named-import scaffold.

### 7.2 The dbmate-style Rust CLI

The public CLI (Track A, Task #55) keeps `apply` / `status` / `rollback` / `load`. Changes:
- `new` scaffolds a named-import `.ts` migration by default (was: `.sql`).
- `generate` emits named-import scaffolds (autogenerate).
- The raw-SQL loader **stays** — the engine still ingests `.sql` (Flyway + dbmate grammars at `loader.rs:19`), because (a) the public dbmate-compatible CLI still accepts hand-written `.sql`, and (b) platform-internal Flyway-mode migrations are SQL. This loader is a **separate, operator/platform-facing path**, never reachable from the structured DSL (which has no raw escape — property A).
- **Retired as default for new authoring**: hand-writing dialect-specific raw SQL. It is still loadable, just not what `new`/`generate` produce or what docs recommend. Per the pre-launch no-back-compat stance, we do not build a "convert my old .sql to the DSL" shim.

---

## 8. Security & threat model

**Threat**: untrusted creator/AI DSL code at build time tries to escalate, reach other tenants, run RCE, forge history, or read files.
...
1. **Build-time op code emits data, not execution.** The recording builder (the ambient per-migration recorder behind the named-import op-functions, §3.1) is pure JS in the author's own Node build. It produces a JSON op list, opens no DB connection, executes no SQL. The engine never runs the author's code — it ingests JSON it deserializes and validates. An attacker controlling the `.ts` controls only the op list, which the engine subjects to the full guard/role/journal pipeline.
2. **Rendering is trusted-Rust; there is NO path from an op to arbitrary SQL.** Every op is rendered by `PgEmitter`/`SqliteEmitter` (DDL) or the structured expression renderer (transforms), which emit only the shapes they know how to emit. Because the DSL has no raw escape (property A), an attacker cannot inject SQL text through any op — the only author-controlled inputs are structured op fields, typed scalar `Literal` binds, and the closed expression AST.
...
3. **Guard coverage is per-dialect, NOT uniform.** The defense applies to every engine-rendered statement; the only difference is the mechanism.
   - **Postgres.** Every rendered `up` passes `SqlGuard::check` in `Confined` (`guard.rs`): the libpg_query parse + deny-list blocks RCE (`LOAD`, `plpython`), privilege escalation (`CREATE ROLE`, `ALTER SYSTEM`, `GRANT`), cross-tenant (foreign schema refs), file (`pg_read_file`) and network (`dblink`) functions, plus deny-by-default. The operator/platform `.sql` loader path (§7) goes through the same deny-list; it is not a DSL-reachable surface.
   - **SQLite.** There is no libpg_query parse guard (`guard.rs:165-172`). In `Confined` the SQLite path accepts only descriptor/IR-generated DDL, and an arbitrary raw SQLite string (from the operator/platform `.sql` loader path) is refused fail-closed. For IR-rendered SQLite DDL, the line of defense is the SQLite runtime authorizer.
   - The structured-op abstraction (rendered DDL/DML) is uniformly defended — PG by deny-list, SQLite by authorizer — and there is no creator-reachable raw escape on either dialect (property A).
...
7. **No creator raw on either dialect, with a consequence for `onConflict`.** Because the DSL has no raw escape (property A), a creator cannot author any raw dialect SQL — there is nothing to refuse, because there is no surface to express it. An operator who needs raw SQLite must use the operator-only `.sql` loader path on the `Trusted`/`Platform` profile, gated by the capability token (`guard.rs` `OperatorCapability`, unreachable from the creator path). **Direct consequence (§9):** any structured op whose only SQLite lowering would have required raw — notably `insert { onConflict }` — has no valid SQLite path and is a **hard authoring error on SQLite** (`UNSUPPORTED` (`kind:"op"`)/`DIALECT_SCOPE_PGONLY`), not a silent route to a rejected raw string.

**Net (per-dialect, honest):** the DSL adds zero new privilege — it is a new way to request operations the same role/journal backstops vet, and it is strictly *less* powerful than raw SQL (it cannot express raw at all). On PG every rendered statement is additionally deny-list-guarded. On SQLite there is no deny-list; the defenses are: only descriptor/IR-generated DDL/DML is accepted, the runtime authorizer screens it, the least-priv role contains it, and there is no creator raw surface to refuse.
...
- **Rejections must be machine-readable.** Every authoring-time rejection the engine/validator can emit carries a structured error code + payload `{ suggested_fix?, code, op_index, ts_location, dialect, reason }`, not only a prose string. **The human-facing rendering leads with `suggested_fix`** (the field that actually unblocks the author/AI loop); `code` is secondary, machine-actionable metadata. This is the canonical taxonomy of authoring-time error codes (new validators add their code here): a guard denial (§6.1.1), an **unsupported op or expression node** — one code `UNSUPPORTED { kind: "op" | "expr" }` (the op-vs-expr boundary is an internal distinction the author need not perceive; it is carried as a `kind` field, not two top-level codes, §3.3.1.1), a non-portable expression (`EXPR_NOT_PORTABLE` — expressible but out-of-envelope, e.g. an out-of-envelope `c.fn.splitPart`, §3.3.1.1/§9; kept distinct from `UNSUPPORTED` only because its `suggested_fix` is "stay in-envelope or accept PgOnly", a different remedy), an out-of-`i64` numeric (`EXPR_INVALID_NUMERIC`, §2.5), an op-function called outside an active recorder (`OP_OUTSIDE_RECORDER`, §3.1), a `dialect_scope = PgOnly` against a SQLite target (`DIALECT_SCOPE_PGONLY`, §2.4.1), a SQLite `onConflict` refusal (`UNSUPPORTED { kind:"op" }`, §9), a pending-contract/dependency block (`TABLE_HAS_PENDING_CONTRACT`/`DEPENDENCY_PENDING_CONTRACT`/`ORPHANED_PENDING_CONTRACT`, §2.0.3), an unknown-owner ownership fail-closed (§8.6), a build-recorder budget overrun (`BUILD_RECORDER_BUDGET_EXCEEDED`, §8.9), a missing `.ts` provenance blob (`PROVENANCE_BLOB_MISSING`, §5.1) or a `.ts`/`.ir.json` checksum divergence at deploy (`PROVENANCE_MISMATCH`, §5.1), and a fresh-rebuild squash skipping non-replayable data steps (`SQUASH_FRESH_REBUILD_DATA_NOT_RECONSTRUCTED`, §5.4 — a status signal, not an authoring reject). The human-readable message is the projection.
- **AI-authored migrations are PG-only-by-default until a portability check passes.** An LLM will confidently reach for a non-portable transform, the "passes on PG, mis-applies on SQLite" trap §9 fights — and there is no human to read the §6.1.1 attribution. So: (1) the structural expression-AST validator (§3.3.1.1) runs on every constructed transform AST and surfaces its structured reject; (2) a migration is treated as `dialect_scope = PgOnly` until it passes the portability check (renders + dry-runs clean on SQLite); (3) **the shadow-DB dry-run (§6.2) on BOTH backends is MANDATORY in the AI path** — an AI migration not dry-run on SQLite is not eligible for a `Both` `dialect_scope`. A human author may accept a PG-only migration knowingly; the AI loop may not. Because there is no raw escape, a non-portable reach simply has no fluent method to call — the boundary is in the API shape, not a free-text string that mis-applies.
...
§8.1–§8.7 reason about the *emitted IR data*. They do not cover the *build-time execution* of the author's `.ts`, a distinct threat surface: the recorder evaluates untrusted creator/AI TypeScript (calling `up()`/`down()`, which record into the ambient recorder) in Node at build time (§4.1/§3.1). The `.ts` is "pure" only by convention — a malicious or buggy `.ts` could `import` a package that opens a network connection, spawns a subprocess, reads the build host's filesystem, or loops/allocates without bound at build time. Resolution:
...
- `del` with a `where` — `DELETE FROM … WHERE …` is identical.
- One-shot `update` whose `set`/`where` use only the **closed fluent expression AST of §3.3.1**: column refs (`c("…")`), literals (auto-wrapped), arithmetic, comparison/boolean operators, `c.fn.case`, the allow-listed provably-identical scalar functions (`c.fn.coalesce`, `c.fn.nullif`, `c.fn.lower`, `c.fn.upper`, `c.fn.trim`, `c.fn.length`, `c.fn.abs`, `.cast("<portable type>")`, `.concat` (`||`)), **and the engine-synthesized `c.fn.splitPart` helper within its pinned envelope** (single-ASCII delimiter + positive literal `1 ≤ n ≤ 8`). `c.fn.replace` is PG-only until `replace` is allow-listed; `c.fn.substr` is **deferred** (portability not yet proven, §9). `substr`/`replace`/`instr`/`split_part` are **not available as author-named functions** (cross-dialect semantics diverge, §3.3.1) — only the engine-synthesized `c.fn.splitPart` helper is. A node outside the closed AST, OR a helper call outside its envelope, is a hard error on SQLite — there is no raw fallback (property A).
...
**Engine-SYNTHESIZED portable transform helpers — pinned, bounded, and PROVEN with an exhibited expression.** The closed-AST scalar set (`c.fn.coalesce`/`c.fn.nullif`/`c.fn.lower`/`c.fn.upper`/`c.fn.trim`/`c.fn.length`/`c.fn.abs`/`.cast`/`.concat`) deliberately omits `split_part`/`substr`/`replace` as author-named functions because their cross-dialect semantics diverge. A small set of high-level, engine-synthesized `FnSynth` helpers recovers portability for the *common* transforms — but only inside an explicitly bounded, in-doc-pinned envelope. We commit to the exact, exhibited SQLite expression, the precise input envelope it is proven over, and a fail-closed error for everything outside it.
...
- **The helper is PROVEN-PORTABLE only for the ASCII-single-character-delimiter case AND a BOUNDED literal `n`, and HARD-ERRORS otherwise.** `c.fn.splitPart(col, delim, n)` is admitted as portable **iff**: `delim` is a literal single ASCII character (one byte, code point < 0x80), `n` is a literal positive integer with `1 ≤ n ≤ 8`, and `col` is a `ColRef` or an in-AST sub-expression. Any other shape — a multi-character delimiter, a non-literal/runtime delimiter, an empty delimiter, `n = 0`, negative `n`, `n > 8`, or a non-ASCII delimiter — is a **hard build/render error on the SQLite leg** (`EXPR_NOT_PORTABLE`, §8.8). **The fallback is unambiguous: mark the migration PG-only (derived).** Because there is no creator raw escape on either dialect (property A), there is **no SQLite tested-expression option** for an out-of-envelope split — the only real resolutions are (i) restructure to stay in-envelope (e.g. split into ≤8 parts), or (ii) accept that the migration is `dialect_scope = PgOnly` (the engine renders `split_part` on PG; the migration is then not authorable on the SQLite dev tier). The structured `suggested_fix` names exactly these two, and does **not** imply a raw escape exists. The value being split *may* contain multibyte content (the proof covers UTF-8 *values*); it is the **delimiter** that is constrained to single-ASCII, because an ASCII byte cannot occur inside a UTF-8 multibyte sequence.
  ...
  - **The `n > 8` cliff is surfaced at authoring time, consistently with the cross-table gap (§9 cross-table marker).** The `n > 8` `EXPR_NOT_PORTABLE` (§8.8) carries a structured `suggested_fix` naming the **two real resolutions — there is no raw option** (property A): `{ restructure: "split into ≤8 parts to stay in-envelope", pg_only: "accept dialect_scope=PgOnly — the engine renders split_part on PG; the migration is not authorable on the SQLite dev tier", note: "there is no creator raw escape on either dialect; an out-of-envelope split is NOT authorable on the SQLite leg" }` — and `generate`/`new` emit the same `// TODO: split with n>8 — exceeds the portable c.fn.splitPart bound (n≤8); restructure to ≤8 parts, or accept dialect_scope=PgOnly (no raw escape)` structured marker when scaffolding a high-n split, exactly mirroring the cross-table `// TODO` marker (§9). So the AI loop is told about the high-n cliff — and that the only SQLite path is restructure-or-PG-only — at authoring time through the same channel as the cross-table gap, not only at render-time rejection.
- **The exact PG and SQLite expressions are EXHIBITED in-doc and PROVEN against real SQLite 3.51.2.** For `c.fn.splitPart(col, d, n)` with a single-ASCII `d` and positive literal `n`, the engine appends a sentinel delimiter (`s' = col || d`) so every token (including the last) is delimiter-terminated, then unrolls the boundary walk to literal depth `n`. The n-th part is the substring between the (n−1)-th and n-th delimiter in `s'`:
  ...
  - **`c.fn.replace` is PG-only until `replace` is reviewed onto the allow-list.** `c.fn.replace` lowers to PG/SQLite `replace`; SQLite `replace` is on the allow-list's candidate set under the same review — until that review lands `c.fn.replace` is PG-only.
  - **`c.fn.substr` is DEFERRED — NOT shipped in PR6b.** Unlike `c.fn.splitPart`, `c.fn.substr` is **not** exhibited with a byte-identical proof corpus against real SQLite 3.51.2 in this spec, and §3.3.1 notes `substr`/`substring` as a silent-divergence trap (negative-start behavior, start-beyond-length, 2-arg-vs-3-arg). Shipping a portability claim for the exact function flagged as a divergence trap without an exhibited proof would be exactly the "passes on PG, mis-applies on SQLite" failure the design fights. So `c.fn.substr` is **deferred to a follow-up PR** whose scope is to exhibit the precise PG (`substring(col from start for len)`) and SQLite (`substr(col, start, len)`) expressions and PROVE them byte-identical over the positive-literal-`start`/`len` envelope — including the start-beyond-length and length-overrun edges — against real SQLite 3.51.2, mirroring the §9 `splitPart` proof corpus. **PR6b ships ONLY `c.fn.splitPart`** (proven below). Until the substr proof lands, `c.fn.substr` is not admitted (it is not in the `c.fn` namespace yet); a positional substring transform is either restructured or `dialect_scope = PgOnly` — never raw (property A).
- **REQUIRED edge-matrix fixtures (PR6b).** The corpus MUST assert `c.fn.splitPart` produces byte-identical results on PG and SQLite for the full envelope AND that every out-of-envelope shape hard-errors identically on both legs:
...
- **Why an engine-synthesized helper, not an author-named function.** There is no author-named `substr`/`split_part`/`instr` at all — the only split surface is the engine-synthesized `c.fn.splitPart` `FnSynth` node, whose per-dialect lowering the engine pins. Adding `instr` to the SQLite *authorizer* allow-list makes it available to the **engine's pinned lowering**, not to any author-written body: a creator has no way to name `instr` (the fluent `c.fn` namespace does not expose it; there is no raw escape). The two lists are distinct — the authorizer gates which builtins the migrator role may execute; the `c.fn` namespace gates which functions an author may build. The helper is safe **only because the engine pins the exact expression and the envelope**, proven with the required fixtures. So the §1.1 hero example written as `c.fn.splitPart(c("name"), " ", 1)` is portable; with a multi-char delimiter it is a hard SQLite error (`EXPR_NOT_PORTABLE`), and an exotic PG-only split is simply not expressible (`UNSUPPORTED` (`kind:"expr"`)).

**NOT portable — NOT expressible in the closed AST (no raw fallback; the migration is `dialect_scope = PgOnly` only if the engine can still render it on PG):**
- **Dialect-only functions** (anything outside the §3.3.1 closed AST): `json_agg`, `array_agg`, `jsonb_*`, `regexp_replace`, SQLite `json_extract` vs PG `->>`, and `split_part`/`substr`/`substring`/`replace` as author-named functions (the only split surface is the engine-synthesized `c.fn.splitPart` helper). For the common split case the `c.fn.splitPart` helper is the portable path within its envelope; `c.fn.replace` is PG-only until `replace` is allow-listed, and `c.fn.substr` is deferred until its byte-identity is exhibited + proven (§9). None of these can be named in the fluent surface — there is no `f.jsonAgg` etc. and no raw escape — so they surface as `UNSUPPORTED` (`kind:"expr"`).
- **Window functions in UPDATE** (`ROW_NUMBER() OVER …`) — not expressible (no window node in the closed AST).
- **Generated columns**, **vendor extension DML** (pgvector ops, PostGIS functions) — Postgres-only; not expressible.

**Upsert / `insert { onConflict }` — no raw fallback; `UNSUPPORTED` (`kind:"op"`) on SQLite.** PG `INSERT … ON CONFLICT … DO UPDATE` and SQLite `INSERT … ON CONFLICT`/`INSERT OR REPLACE` have incompatible clauses:
- **PG**: `insert { onConflict }` renders natively (engine-assembled `ON CONFLICT … DO UPDATE`); the migration is `dialect_scope = PgOnly` (derived, §2.4.1).
- **SQLite**: a **hard authoring error** at build/render (`UNSUPPORTED` (`kind:"op"`)) — "`onConflict` is not supported on SQLite; restructure as separate insert + update, or accept dialect_scope=PgOnly." We do not silently drop the conflict clause and there is no raw route (property A). (A future engine-authored, descriptor-derived SQLite UPSERT is out of scope, explicitly deferred.)

**Portable-DML coverage — the honest sum.** The portable-DML surface is **DDL (all of it) + literal `insert`/`del` + `update`/`backfill` whose `set`/`where` fit the closed AST + the `c.fn.splitPart` helper within its single-ASCII envelope — all scoped to a SINGLE table (the enclosing step's target).** What it does *not* cover, which real apps routinely need: **JSON reshaping** (`json_extract`/`->>`/`jsonb_*`), **date arithmetic**, **regex**, **array ops**, **window functions**, **generated columns**, any vendor extension, **and any CROSS-TABLE / correlated transform**. The closed AST covers most *single-table structural/conditional* transforms (rename-via-copy, `c.fn.case` bucketing, NULL-coalescing, casing/trimming, the common space-delimited split); a substantial fraction of non-trivial real-world data transforms (JSON, dates, regex, OR another table) is simply **not expressible** — `UNSUPPORTED` (`kind:"expr"`/`"op"`), with no raw fallback. (No precise percentage is claimed — unmeasured.) This is the deliberate cost of property A: portability by construction, with an honest unsupported tail rather than a silent mis-apply.
- **Cross-table / correlated transforms are an explicit exclusion — not expressible on EITHER dialect.** The validator's `ColRef` resolution is scoped to the single target table (§3.3.1.1) and `c` is single-table-scoped (§3.3.1): a transform cannot reference another table's columns. A common data migration — derive a column from a JOINed/correlated other table (e.g. backfill `orders.customer_name` from `customers` via `customer_id`) — is not expressible in the closed AST.
  - **On both PG and SQLite:** there is **no raw escape** (property A), so a cross-table transform is a hard `UNSUPPORTED` (`kind:"expr"`) on both legs — it is not a "PG-only via raw" case. (This is a deliberate change from any earlier reading: removing raw means even PG loses the cross-table escape until the structured join facet ships.)
  - **Why not a portable cross-table grammar now.** A controlled `FROM`/join facet is the strongest fix but materially enlarges the closed AST (join resolution, multi-table `ColRef` scoping that re-opens the cross-table-reference surface, a SQLite lowering of `UPDATE … FROM`). It is **deferred to a scoped follow-up** as a structured AST extension (never a raw escape), recorded as a known capability gap. Until it lands, `generate`/`new` scaffold a `// TODO: cross-table backfill — not expressible until the structured join facet ships (no raw escape)` structured marker (§8.8) so the AI loop is told at authoring time.
- **The dev-tier (SQLite) consequence, stated plainly.** A transform that cannot be expressed portably is not authorable on the SQLite dev tier at all — a hard authoring error there. An app whose data migration needs JSON/date/regex/cross-table reshaping cannot run that migration on SQLite dev today; the structured follow-ups (a structured JSON/date facet, the join facet) are the path forward, not a raw escape. The interim guidance is to express what fits the closed AST and defer the rest to the structured-extension roadmap.
- **Considered and DEFERRED: structured AST extensions (NOT a raw path).** The strongest fixes are *more structured AST surface* — a structured JSON-path facet, a structured join/`FROM` facet, a structured date-arithmetic facet — each with an engine-owned per-dialect lowering and proof corpus, exactly as `c.fn.splitPart` is today. They are deferred (each carries its own correctness burden) but are the committed direction; a raw SQLite/PG creator escape is explicitly NOT the path (it would reintroduce the silent-mis-apply and parser-drift surface property A/D eliminated).

**Design stance**: the DSL does not pretend the non-portable cases are portable, and it offers **no raw escape** to paper over them (property A). `insert`/`update`/`del` accept the closed-AST portable subset; `backfill` is portable on both backends once §2.3.1 ships (committed, PR6b); anything beyond the closed AST is a hard structured error (`UNSUPPORTED` (`kind:"expr"`/`"op"`) / `EXPR_NOT_PORTABLE`), surfaced early to the AI loop, and resolved by reshaping or by accepting a derived `dialect_scope = PgOnly` (for the PG-renderable-but-SQLite-unsupported subset). Honesty here prevents the worst failure mode: a migration that passes on PG and corrupts/no-ops/silently-rejects on SQLite.
...
PR0  (the ONE shared apply_plan + AppliedPlan/PlanStep/RenameStep; RE-POINT
  ...
  ├─▶ PR1  (IR JSON contract + loader IR branch + IrAuthor DDL-only + golden-fixture
  │         value-equality gate + the STRUCTURAL expr-AST validator + structured-error
  │         envelope + skeletal JS)
  ...
  │     │     │           └─▶ PR5  (structural type-safety: op shapes + the t.* type lexicon from @zeroship/db
...
EPIC 2 — portable bi-dialect DML (COMMITTED; the §1.1 headline)
  └─▶ PR6b (SQLite BATCHED-backfill executor §2.3.1 on PR6a's shared SQLite-DML module
        + instr allow-list + portable c.fn.splitPart SQLite lowering)
        └─▶ PR7  (CLI rewire + raw-SQL demotion)   GATED ON: PR6b (SQLite DML) AND PR2 (online)
              └─▶ PR8  (docs)
```

Reading the plan: **PR0 builds the single shared `apply_plan` AND re-points the declarative path onto it via the shape-adapter (§6.0) — there is ONE orchestrator from PR0, no fork.** The regression safety net is the existing ~507-test declarative suite staying green through `apply_plan`, plus the frozen-commit golden-trace oracle (captured before the re-point) and the crash-injection property fuzz for net-new topologies. PR1 freezes the IR shape, the golden-fixture value-equality gate, and — because the AI builder is the dominant author — the **structural expression-AST validator + structured-error envelope** (the AI loop's primary feedback signal; pure analysis, no executor dependency; no text parser, no fuzzer — HIGH-1 dissolved). PR2 (online) and PR6a (DML) are siblings off PR1, each consuming `apply_plan`. The JS/build/type-safety chain (PR3→PR4a→PR4→PR5) is a separate branch off PR1, with the **kernel-sandboxed recorder service carved out as its own PR4a** (the riskiest single deliverable — OS-level isolation of untrusted JS — split from the `new`/`generate` CLI ergonomics in PR4, with a faithful kernel-level e2e). **Epic 2's PR6b (the SQLite backfill/DML executor) is committed scope — the bi-dialect "one script, both backends, DDL+DML" headline — not a demand-gated maybe.** PR7 (the raw-SQL demotion) is gated on PR6b + PR2 so the DSL is a credible full bi-dialect replacement before raw SQL is demoted.
...
- **Structured-error envelope + the STRUCTURAL expression-AST validator land HERE, at IR-freeze.** (1) Every authoring-time rejection carries the `{ code, op_index, ts_location, dialect, reason, suggested_fix? }` payload (§8.8); (2) the **structural expression-AST validator** (§3.3.1.1 — an allow-list check over the closed AST in the schemars-emitted IR Op schema; **no lexer, no parser, no libpg_query, no differential fuzzer**) ships in PR1. The Rust validator at lower/render is the sole authoritative gate; an optional JS-side check is a best-effort structural hint over the same schema. There is no text-grammar parser to build and no parser-drift to fuzz (HIGH-1 dissolved). (The DML *executors* remain PR6a/PR6b; only the validator + error envelope move to PR1, since the validator is pure analysis with no executor dependency and the AI loop needs it before any DML PR.)
- **Tests**: golden `.ir.json` fixtures authored by both the skeletal JS builder and read by Rust, value-equal (typed-value checksum) → render → apply on PG + SQLite; a portable migration's checksum is identical across PG and SQLite renders (§2.4); checksum stability; unknown `ir_version` fail-closed; a deserialized IR carrying an unknown expression-AST node tag is rejected at load (`UNSUPPORTED` (`kind:"expr"`)); non-owner op rejected; an op targeting a table with no registry entry rejected (unknown-owner); a `createIndex{concurrently}` auto-derives a non-txn idempotent `DROP INDEX CONCURRENTLY IF EXISTS` down on PG and a txn-safe `DROP INDEX IF EXISTS` down on SQLite (§2.1.1).
  ...
  - **Guard byte-identity**: for a multi-statement `createTable` (inline index + comment), `applied_up` is byte-identical to the concatenation of the individually-guarded fragments; a denied rendered fragment aborts the step with op-index + `.ts` location and applies nothing.
  - **PG-only surfacing**: an `insert { onConflict }` migration loads with `dialect_scope = PgOnly`, and deploying it against a SQLite target is rejected at load.
  - **Structural validator + structured-error envelope**: the validator accepts every allow-listed AST node and rejects an out-of-allow-list node, an out-of-envelope `c.fn.splitPart`, and a `ColRef` to a column not on the enclosing target table (target-table-scoped, an apply/render-time check); a synthesized cross-table reference is rejected; every rejection emits the structured payload (§8.8). The validation is structural over the constructed AST — no text parser, no fuzzer.
...
- **Execution rides PR0's dual-dispatch**: this PR authors the two `RenameStep` variants; PR0's `apply_plan` already routes `PgExpandContract`→`run_online` and `SqliteRebuild`→`rebuild_one` (§2.6.2). This PR wires authoring into the execution PR0 built; its e2e proves both legs from a single `renameColumn`.
- **Neutral-type translation**: the op carries a dialect-neutral column type; `IrAuthor` maps it to a PG type string before building `OnlineIntent` (PG) and to a SQLite affinity type before the rebuild bridge (SQLite). No PG type string is ever passed to the SQLite leg.
- Down = contract→expand reversal for a fully-applied rename; `None` + roll-forward for in-flight (§2.6.1).
- **Tests**: faithful e2e — `renameColumn` applies as online dual-write on PG (5 sub-steps + backfill, correct journaling + crash-resume) and as offline rebuild on SQLite via `rebuild_one` (§2.6.2); mid-sequence crash resumes roll-forward; a `renameColumn` whose new type is a neutral column type renders the correct PG type in `OnlineIntent` AND the correct SQLite affinity in the rebuild; an `renameColumn`'s E1..C2 ids + intra-chain `depends_on` are byte-equal to the equivalent declarative `t.*`-diff rename's ids (reconciling with `expand_contract.rs:777-783` + the set-integrity manifest); the SQLite leg executes through `MigrationBackend::rebuild_one`, not `run_online` (asserted via the journal).

### PR 3 — `@zeroship/migrate` JS op builder package (full surface)
- Flesh out the named-export op-functions + ambient recorder (pure, no I/O) to the full §3.2 surface; emit canonical `.ir.json`. **Named ESM imports, no `op` object/prefix; the `export default { name?, up, down? }` module shape (§3.1); `up()`/`down()` parameterless, recording into a per-evaluation ambient recorder; `del` not `delete`; all identifier args (including every `c("…")`) are `string`** (§3.3). Calling an op-function outside an active recorder throws `OP_OUTSIDE_RECORDER`.
- **The refined primitive surface (§3.2/§3.3.1), exactly:** the **fluent `t.*` column-type lexicon** (`t.text().notNull().default(…)`, nullable-by-default, the options-bag overload accepted); **one column-type representation** (`ColumnDef` everywhere — no bare `ColType` union; `renameColumn` takes `ColumnDef`); **`createTable(name, columns)`** with no thunk (plus the `(b) => …` scoped-builder overload for table-scoped constraints/indexes); **`(table, spec)` constraint/index adders** with `name` always in the spec; **`verb(table, { … })` DML** with one `where` keyword across `update`/`del`/`backfill` and `insert(table, { rows, onConflict? })`; **`backfill` defaults** (`cursorColumn` → single-col PK, `batchSize` → engine default); the **single-handle `(c) => Expr` builder** with `c("name")` + `c.fn.*` (no importable `fn`, no second callback arg); **`c.fn.concatWs`** the NULL-skipping safe-join helper.
...
- The migration module shape (`export default { name?, up, down? }`, §3.1); the all-strings fluent `(c) => Expr` builder (`c("name")` + the operator methods + the `c.fn.*` namespace, §3.3.1) — **no `op` object, no importable `fn`, no raw escape, no `Raw` type**; determinism lint (Date.now/RNG AST flag, §4.3); the `onConflict` field's JSDoc carries the PG-ONLY warning (§3.2).
- **Tests**: builder emits the full fixture corpus value-equal to the Rust-read goldens; determinism (same source → same JSON); lint flags a `Date.now()` in an op argument; **a `t.text()` records `nullable:true` and `t.text().notNull()` records `nullable:false`**; **a `createTable(name, {…})` object-literal and the `(b) => …` overload both record the same table op**; **`c.fn.concatWs(" ", c("a"), c("b"))` with a NULL operand records a `FnSynth(concatWs)` node** that renders byte-identically on PG/SQLite; **an `addForeignKey(t, { columns, references, name })` spec records the same FK op regardless of field order** (named fields, not transposable positionals); a name omitted from the module records the filename-derived label.
...
- `zeroship-migrate new` scaffolds a named-import `.ts` with parameterless `up()`/`down()` (time/uuid seed columns default to the DB-evaluated `c.fn.now()` / `c.fn.genRandomUuid()`, §4.3).
- `generate` emits a named-import `.ts`/`.ir.json` scaffold from the declarative diff (autogenerate parity, §7.1), with `// TODO: backfill` markers (machine-readable open-obligations in the AI path, §8.8).
...
- Wire the shared column-type lexicon (`t.*`, the neutral column-type discriminant it reduces to) from `sdks/db` so the migration DSL and runtime schema share one type lexicon. There is no separate bare-string `ColType` union on the op surface (§3.2). **Do NOT bind table/column NAMES to the live schema** (§3.3): there is no `TableName<S>`/`keyof RowOf<S,T>`/`RowInsert<S,T>` on the op surface. The optional row generic `insert<R>`/`update<R>` is caller-supplied, never auto-derived from the schema.
- `tsc` gate: a migration with a malformed op *shape* or an invalid `t.*` type fails type-check. A migration referencing a non-existent table/column does **not** fail `tsc` — it is validated at apply time against the real DB (§3.3).
- **Tests**: type-level tests (expect-error for a bad op shape / a bad `t.*` type or modifier); a real migration whose names are plain strings type-checks even when those names are not in the current `t.*` schema (proving names are not live-schema-bound); an apply-time test that a non-existent column fails at apply with the structured error, not at `tsc`.

### PR 6a — DML surface (`insert`/`update`/`del`/one-shot + PG batched `backfill`)
- **New creator-DML assembler** for `insert`/`update`/`del`: identifier quoting + **parameterized value binding**, NULL/typed-literal handling, PG `onConflict` rendering, migrator role, guard (PG) / authorizer (SQLite). Dialect-aware (binds via `$n` on PG and `?n` on SQLite), so literal-`insert`/`del`/portable-`update` **one-shot** DML is portable on both backends here; only the *batched backfill* SQLite executor is PR6b.
...
- **Tests**: faithful e2e — seed via `insert`, prune via `del`, transform via one-shot `update` on **both real PG (`:5440`) AND real SQLite** (literal/portable forms — the one-shot SQLite DML path is exercised here, distinct from and ahead of PR6b's batched-backfill SQLite e2e, so PR6a's "portable on both backends for one-shot DML" claim is backed by a real-SQLite apply, not only PG), and via batched `backfill` on **PG** (resumable, crash-safe); **the AUTHORING-PATH e2e for `Dml`** (completing PR0 test (9)): a creator `insert`/`update` flows through the NEW assembler → a `Dml{template,binds}` step → PR0's `apply_plan` → a real DB, asserting the assembler-produced template/binds match what the executor records; assert a non-portable `set` (an unsupported AST node), a SQLite `onConflict`, and a SQLite-targeted batched `backfill` are rejected at build/render (the `onConflict` as `dialect_scope = PgOnly`), not silently mis-applied. **Bind-safety**: an `insert`/`update` whose auto-wrapped `Literal` value contains SQL metacharacters cannot alter statement structure on either backend (native `$n`/`?n` binding, not interpolation). **Structural-validator fixtures**: `(c) => c("x").cast("integer")` and `(c) => c("a").concat(c("b"))` with a NULL operand apply identically on both backends; an attempt to build an unsupported node (a window, a cross-table reference, an out-of-allow-list function) is rejected with `UNSUPPORTED` (`kind:"expr"`). **Column-scoping fixtures**: `(c) => c("threshold").gt(v)` auto-wraps `v` as a native parameter bind; a `ColRef` to a column not on the enclosing target table is rejected; a synthesized cross-table reference (e.g. a node naming `other_table.secret`) is rejected by the target-table-scoped `ColRef` resolution.
...
- **Add `instr` to the SQLite authorizer allow-list — its own reviewed line item (§9).** Append `"instr"` to `FUNCTION_ALLOWLIST` (`backend_sqlite/authorizer.rs:131`), with the doc-comment justification (deterministic, side-effect-free scalar; no extension load / fs / network / tenant escape; required by the engine's pinned `c.fn.splitPart` lowering; kept in lockstep with the emitter function set). This is the *authorizer* allow-list only (what the migrator role may execute) — `instr` is NOT exposed in the `c.fn` namespace, so a creator has no way to name it; the only `instr` in any rendered statement comes from the engine's pinned `c.fn.splitPart` lowering.
- **Portable-helper SQLite lowering** (§9): the pinned, exhibited, authorizer-legal `instr`/`substr` SQLite expression for **`c.fn.splitPart` only** (the `cur₀ = col||d`, `curᵢ = substr(curᵢ₋₁, instr(curᵢ₋₁,d)+1)`, `result = substr(cur_{n-1},1,instr(cur_{n-1},d)-1)` unroll, proven against SQLite 3.51.2) within the single-ASCII-delimiter + positive-literal-`n` envelope, plus the out-of-envelope hard-error path. **`c.fn.substr` is NOT shipped here** — it is deferred to a follow-up PR that must exhibit + prove its PG/SQLite byte-identity (§9); `c.fn.replace` stays PG-only until `replace` is allow-listed.
- **Tests**: faithful e2e — transform via `backfill` (resumable, crash-safe) on **SQLite** (no PG-gated skip hiding the SQLite leg); the §3.1 hero DDL+backfill shape applies on **both** backends from one `.ir.json`. **Portable-helper edge-matrix fixtures (the full §9 envelope, MANDATORY)**: `c.fn.splitPart(x,' ',n)` byte-identical on PG and SQLite across the in-envelope cases (two-token, single-token→`''`, beyond-tokens→`''`, empty input, trailing/leading/consecutive delimiter→`''`, NULL, a multibyte/UTF-8 VALUE split on an ASCII delimiter, a non-default-collation value); every out-of-envelope shape (multi-char/empty/non-ASCII delimiter, `n=0`/negative `n`, `n>8`, non-literal delimiter/`n`) rejected with `EXPR_NOT_PORTABLE` on the SQLite leg on BOTH the Rust validator and the JS lint. A raw `split_part(...)`/`substr(...)`/`instr(...)` string is rejected on SQLite while the in-envelope `f.*` helper is accepted; the SQLite lowering's `instr` is asserted to apply cleanly under the SQLite authorizer (the allow-list addition exercised end-to-end). `c.fn.replace` asserted PG-only until `replace` is allow-listed.

### PR 7 — CLI rewire + raw-SQL demotion — **GATED ON PR6b (SQLite DML) AND PR2 (online)**
**Precondition: PR6b (SQLite backfill executor) and PR2 (online) are merged.** The demotion of raw-`.sql` authoring below the DSL is only honest once the DSL can express data migrations and online rename portably **on both backends**; before PR6b, batched data migrations are PG-only, so raw `.sql` is still the only way to author a portable bi-dialect data migration.
- `new`/`generate` default to the named-import DSL for the full surface (DDL + DML + online); raw `.sql` loader retained (for the dbmate CLI + platform Flyway-mode operator/platform path only) but no longer the recommended authoring path and never reachable from the DSL (property A).
- **Tests**: mixed history (IR + legacy `.sql`) loads in one ordered timeline; `apply`/`status`/`rollback` unchanged; an e2e proves a DSL-authored DDL+backfill migration (the §3.1 hero shape) applies on **both** backends without any raw `.sql` — the concrete evidence the "replaces raw SQL" headline is true.

### PR 8 — Docs
- New `docs/reference/migrate-op-dsl.md`: the named-import op vocabulary, the all-strings fluent expression surface (§3.3.1), the names-are-strings typing stance (§3.3), the DML portability boundary, and the no-raw-SQL/unsupported-tail stance (property A; resolve by reshaping or accept `dialect_scope = PgOnly`).
...
| online rename intent (PG only) | `OnlineIntent::RenameColumn` — `expand_contract.rs:70`; **`ty` is a raw PG type string** (`:82`) — so the IR carries a neutral column type and `IrAuthor` maps it to PG type before building the intent (§2.6); `ExpandContractAuthor::author` `:285` (no dialect param, no SQLite arm); `ExpandContractPlan` `:106` |
...
| --- | --- |
| **Surface (no raw, named imports, all-strings)** | No raw SQL of any kind (no raw statements, no raw expression text — property A). The DSL is consumed as **named ESM imports** from `@zeroship/migrate`; **no `op` object/prefix** (property B); the module is `export default { name?, up, down? }` whose parameterless `up()`/`down()` record into an ambient per-migration recorder (§3.1). Expressions are an **all-strings fluent** `(c) => Expr` builder with a **single injected handle**: `c("name")` is a column-accessor (string arg → `ColRef`) and the scalar-function namespace hangs off it as `c.fn.*` (no importable `fn`, no second callback arg); operator methods build the closed AST, auto-wrapping bare values to `Literal` (property C). `del` not `delete` (reserved word). (§0, §3.1, §3.2, §3.3.1) |
| **Primitive-surface refinements (round of task #77)** | Applied to maximize clean/helpful/elegant/composable within the locked constraints: (1) **one fluent grammar** — the `t.*` column lexicon is chainable (`t.text().notNull().default(…)`, nullable-by-default) to match the expression chain, options-bag kept only as an overload; (2) **`createTable(name, columns)`** drops the zero-arg `() => Record` thunk (object literal, plus a `(b) => …` scoped-builder overload for table-scoped constraints/indexes, parallel to `batchAlterTable`); (3) **no importable `fn`** — the function namespace is `c.fn.*` off the single builder handle, and the `(c, f)` two-arg callback is collapsed to `(c)`; (4) **one column-type representation** — `ColumnDef` everywhere (the bare `ColType` string union is removed from public signatures; `renameColumn` takes `ColumnDef`); (5) **nullable-by-default** — `t.text()` nullable, `.notNull()` to opt in (no `{ notNull:false }`); (6) **`(table, spec)` constraint/index adders** with `name` always in the spec and named `references.{table,columns}` (no transposable positional pairs); (7) **`verb(table, { … })` DML** with `insert(table, { rows, onConflict? })` and one `where` keyword across `update`/`del`/`backfill` (former `filter` renamed); (8) **`backfill` defaults** — `cursorColumn` → single-col PK, `batchSize` → engine default; (9) **`c.fn.concatWs`** added as the NULL-skipping safe-join helper (hero example models it); (10) **module shape** consolidated to `export default { name?, up, down? }` (the stray top-level `name` const removed; name derivable from filename); (11) **error taxonomy** — `OP_UNSUPPORTED`+`EXPR_UNSUPPORTED` merged into one `UNSUPPORTED { kind:"op"\|"expr" }`, `EXPR_NOT_PORTABLE` kept distinct (different remedy), and the human-facing rendering leads with `suggested_fix`; (12) **`onConflict`** stays on the single API (no dialect-scoped `pgInsert`, which would reintroduce per-dialect author branching) but carries a prominent PG-ONLY JSDoc and is rejected structurally on SQLite. None of these regress property A/B/C/D or the names-are-strings stance. (§3.2, §3.3.1, §8.8, PR3) |
| **Typing** | Table/column NAMES are plain strings (every `c("…")` included); never bound to the live schema (`TableName<S>` / `keyof RowOf<S,T>` / `RowInsert<S,T>` are NOT used). Structural type-safety (op-function shapes, `t` lexicon, fluent-expression node shapes, insert-row value kinds) is preserved; name existence is an apply-time check against the real DB, `ColRef` resolution is an apply/render-time check against the enclosing target table. Precedent: Kysely `Kysely<any>`, Alembic string names, Drizzle generated SQL. (§0.1, §3.2, §3.3) |
| **Validator (structural; HIGH-1 dissolved)** | The expression validator is **purely structural** — an allow-list check over the closed AST in the schemars-emitted IR Op schema (property D). The AST is constructed in JS and serialized to IR, never parsed from text, so there is **no lexer, no Pratt/precedence parser, no libpg_query, no Rust-vs-JS parser drift, and no differential fuzzer** — HIGH-1 is dissolved, not mitigated. The Rust validator at lower/render is the sole authoritative gate (every node allow-listed, `c.fn.splitPart` args in-envelope, `ColRef` resolves to the target table, `Cast` target portable). (§3.3.1.1) |
...
| **Fluent-expression `Literal` canonicalization** | An auto-wrapped `Literal` param inside a fluent `set`/`where` expression (the bare value passed to an operator method, e.g. the `0` in `c("total").gt(0)`) folds into `Checksum::of_ir` via the expression-node rule (the typed scalar, same canonicalization as the `insert` `rows` facet), so changing a threshold value is drift. There is no template text and no interpolation — the node tree *is* the canonical form. (§2.4 point 3, §2.3.2) |
...
| **`c.fn.splitPart` SQLite lowering** | `instr`-based, exhibited, CSE-not-required inline form bounded to `n ≤ 8` (the inline unroll is O(2ⁿ); ~17 KB at n=8 against SQLite 3.51.2); a CSE-shared form to raise the bound is deferred. `instr` is added to the SQLite authorizer allow-list (PR6b). PR6b ships ONLY `c.fn.splitPart`; `c.fn.substr` is deferred until its PG/SQLite byte-identity is exhibited + proven (it is the divergence trap §3.3.1 names), and `c.fn.replace` is PG-only until `replace` is allow-listed. (§9) |
...
| **LOCKED model** | no-raw-SQL fully-structured; named-ESM-import surface (no op. prefix); all-strings fluent column accessor c("name"); structural validator (HIGH-1 dissolved). |
| **Determinism AI-path check** | Scoped to time/uuid-typed columns; the DB-evaluated `c.fn.now()` / `c.fn.genRandomUuid()` scaffold default is primary; the blanket "reject any non-literal value argument" is not the rule. (§4.3) |

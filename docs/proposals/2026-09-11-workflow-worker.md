...
creator-side connectivity. A worker's zone is the `zone` claim of the join token
Control verified when it joined, recorded on the instance and frozen there.
Registration carries no zone; the
...
closed, durable, retryable refusal (`pool_exhausted`, `no_signer`,

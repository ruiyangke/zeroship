...
2. **Double-injection collision.** With migrations present, the gen-types emit
   ...
   `crates/zeroship-data-v8/policies/confined.policy.toml`:
...
| --- | --- | --- |
| **Postgres deploy** | the `migrated` service, at deploy, ahead of the worker (imperative replay of migration ops under the confined charter) | **no-op** (`crates/zeroship-data-v8/src/register_model/mod.rs`: `(Some(_pg), _) => Ok(())`) |
...
Mirror the Postgres arm in `crates/zeroship-data-v8/src/register_model/`: on SQLite, **skip
...
5. **Retirement.** With SQLite register a no-op, `run_sqlite_via_engine`,
   ...
   the runtime copy of `crates/zeroship-data-v8/policies/confined.policy.toml` become dead
...
- Postgres register no-op to mirror: `crates/zeroship-data-v8/src/register_model/mod.rs`.

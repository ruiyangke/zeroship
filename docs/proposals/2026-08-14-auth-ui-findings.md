...
1. The initial failed-login error had no explicit announcement semantics. The
   banner at `crates/auth/src/ui/templates/login.html:6` was a plain
...
2. The initial failed-login fields were not programmatically tied to the
   ...
   `crates/auth/src/ui/templates/login.html:17` and
   `crates/auth/src/ui/templates/login.html:21`, both initially reported
...
3. No accessible-name defect was found in the public forms. The wrapped labels
   and button text at `crates/auth/src/ui/templates/login.html:16`,
   `crates/auth/src/ui/templates/signup.html:11`, and
   `crates/auth/src/ui/templates/forgot.html:13` gave every visible form
...
4. No heading-order defect was found. The shared heading at
   `crates/auth/src/ui/templates/base.html:12` was the only `h1`; each route's
   page heading at `crates/auth/src/ui/templates/login.html:4`,
   `crates/auth/src/ui/templates/signup.html:4`, and
   `crates/auth/src/ui/templates/forgot.html:4` was an `h2`. Chromium measured
...
5. No focus-visibility defect was found in Chromium. The controls styled at
   `crates/auth/static/style.css:7`, `crates/auth/static/style.css:8`, and
   `crates/auth/static/style.css:10`, plus the summary at
   `crates/auth/static/style.css:75`, retain the browser focus ring. Keyboard

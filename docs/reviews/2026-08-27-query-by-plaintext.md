...
- `MaskPolicy` is `role -> set of classifications`
  (`crates/zeroship-data-v8/src/protection/mask_policy.rs`).
...
- `check_unmask_authorization` is synchronous, takes
  ...
  (`crates/zeroship-data-v8/src/protection/unmask.rs`). An absent actor
...
- `dispatch_find` applies `sanitize_app_actor` to the app-supplied actor before
  it can reach authorization (`crates/zeroship-data-v8/src/crud/mod.rs`).
...
- The audit write takes a **pool** connection (`unmask.rs:757`, `:774`), not the
  ...
  `crates/zeroship-data-v8/src/exec.rs`). Consequences in section 6.4.
- The data pool is 8 connections (`crates/zeroship-data-v8/src/lib.rs`,
...
- Deterministic encryption **is implemented at the crypto layer and does produce
  ...
  (`crates/zeroship-data-v8/src/encryption/aead.rs`, key material
...
- What has no implementation is the **query** half. The filter path never
  ...
  schema (`crates/zeroship-data-v8/src/crud/bytes_pass.rs` states this
...
1. **The key is a third HKDF leg, not a reuse of `k_siv`.** `derive_key` already
   ...
   (`crates/zeroship-data-v8/src/encryption/keys.rs`); this adds a
...
1. Read-set capture normalises a filter into a predicate the broker evaluates
   per event (`crates/zeroship-data-v8/src/read_set.rs`). A token
...
- Native op: `findByUnmasked` on the `Collection` v8 class, beside
  `unmaskField` (`crates/zeroship-data-v8/src/v8_classes/collection.rs`)

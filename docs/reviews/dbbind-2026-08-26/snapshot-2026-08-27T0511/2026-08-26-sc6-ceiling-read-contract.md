...
**Why this is better than what was recommended above, and not merely different.**
Every option above leaves the design *fail-open*: the plaintext sits in the
column with the natural name, so any code path that forgets to ask "is this
masked?" reads it. That is not a hypothetical - it is exactly how this defect
arose, because `build_where_with_dialect` takes no schema hint while the read
path takes one and calls `column_is_masked(field, schema_hint)`

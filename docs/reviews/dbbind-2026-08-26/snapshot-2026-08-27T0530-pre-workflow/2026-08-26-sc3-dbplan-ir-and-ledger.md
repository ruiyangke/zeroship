...
> **A determinism arm here is probabilistic, not discriminating** (found by the
> round-7 reviewer, 2026-08-27, as its required cannot-fail finding).
>
...
> prepared-statement cache key rests on (see L16 in the parent). A plan that
> renders two different SQL strings for the same logical operation silently
...
The IR makes that cache far more valuable: values are always parameters, so two calls

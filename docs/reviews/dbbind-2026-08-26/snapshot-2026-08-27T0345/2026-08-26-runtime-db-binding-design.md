...
#### A partitioned creator table gets NO runtime metadata, so its protection passes are skipped
...
requests, spending the per-app cache budget this document is separately trying
to bound. That half cannot be fixed in the populate loop, because `LiveSchema`
...
Two things to get right while doing it, since both bear on the bound above:
...
"Until eviction" above assumed a mechanism that does not exist. Measured on the
current tree, the per-app metadata maps on `IsolateDbContext` have **no
eviction at all** - counting `remove`/`clear`/`retain` sites:
...
which at 5 collections per app is ~2,000 apps resident per thread. That is a number an operator can reason about, and it is also the number
...
### The cold-start fix opened a cross-tenant eviction DoS

This is a defect **introduced by the change this document recommended**, and it
...
- per-app sub-maps rather than one flat thread-global map, so eviction is
  scoped to the tenant that caused it (the hierarchical shape argued for above,
  now load-bearing for isolation rather than just for lookup cost);
...
established above - K copies of the expensive query, not K copies of a cheap
one - and it lands squarely on the case the populate-all fix was written for,
since a cold app is exactly the app whose first request arrives as a burst.

This document's own end-state Caches section already mandates a per-thread
...
### The pattern is not confined to plugin-db

Two more per-app stores outside this plugin have the same shape, and they matter
to this design because they sit on the paths it depends on. Neither is
plugin-db's to fix, but a design that claims a bounded per-app footprint while
these are unbounded is claiming something the process does not deliver.

**The worker's env cache retains decrypted secrets per app, indefinitely.**
...
**The meter's drain is a stop-the-world proportional to app history.**
...
**Isolate admission builds the runtime before it can know it will be rejected.**
...
### The same map is a per-transaction CPU cost, not only a memory ceiling
...
1. **The scan is cross-tenant.** `self.schemas` is the thread-global map keyed
   ...
   the unbounded map from the previous section, walked linearly, on a hot path.
   At the stated target this is a million-entry scan per `db.transaction()`.
...
2. **Every matching schema is deep-cloned and immediately dropped.** `v.clone()`
   ...
   the 7x table this section had already retracted two screens above. Keeping
   ...
   *written* and the derived figures were *not re-derived*, so a stale constant
   walked straight into a new claim. Any figure in this document traceable to

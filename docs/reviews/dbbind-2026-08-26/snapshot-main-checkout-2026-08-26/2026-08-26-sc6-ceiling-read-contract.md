...
(`Pool::connect(&url, 8)`, `crates/zeroship-plugin-db/src/lib.rs:872`), and an
...
### Masking is projection-shaped, so the filter path is an unaudited oracle
...
**This is a policy question the design set never asks, and it needs an explicit
answer rather than an implementation detail.** The options are not equivalent:
...
**Recommended: refuse ordered comparison, permit equality, and say so in
`docs/reference/db.md` beside the mask kinds** - a creator who masks a column
should learn the query surface narrows at the point they declare it, not when a
filter starts failing. Equality's residual leak is then a stated, bounded
property rather than an accident.

This one needs an operator decision because it removes capability from a
shipped surface; it is recorded here rather than chosen unilaterally.
...
### The write path, which this contract had left as an outcome with no mechanism
...
- **Where.** A platform-owned table in `__zeroship_admin`, on the same privilege
  terms as the epoch row and with its own privilege arm (below). Not the app
  schema, and not `app_schema_state` - it is separately grantable and separately
  revocable on purpose.
...
Therefore: **each projected column is authorized against the policy of the
collection it came from**, the classification is the one declared on that
collection, and the audit row records the source collection alongside the field.
A test whose parent and child share a policy passes on the broken
implementation, so the arm SC-3 carries requires policies that **differ on the
same classification**.
...
```
(`crates/zeroship-plugin-db/src/crud/mask_policy.rs:102-110`)
...
That is defensible - `auto` is the platform acting on its own behalf, not a
creator role - and it is **not forgeable from app JS**, because
`sanitize_app_actor` strips an app-supplied `kind == "auto"` before it reaches
any of the three authorization entries. But an earlier draft's flat "the ceiling
can only ever narrow" was false as written, and a reader relying on it to reason

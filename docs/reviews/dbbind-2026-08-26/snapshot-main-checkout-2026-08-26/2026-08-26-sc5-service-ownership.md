...
- **Two OS worker threads** resolving the same
  `(authority_domain, app_id, incarnation, epoch)` concurrently produce **one**
  process-wide cache/catalog resolution, while each thread keeps its **own**
  driver resources.
...
- Current and deploy-pinned isolates for one app on one worker thread resolve
  **one** `DbThreadResources` and **one** cache entry for the same
  `(authority_domain, app_id, incarnation, epoch)`, asserted by `Arc::ptr_eq`
  plus cache introspection, with backend factory opens and catalog
  introspections counted **separately**.

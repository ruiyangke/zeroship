...
`DbPlan` is an identifier-validated, backend-neutral operation tree. What it must
carry is fixed by what the 48 builders do; the exact node spelling is
...
- **search**: vector, full-text, spatial - the three that today read declared
  schema **inside** the concrete backends
  (`backend/postgres.rs:494-527`, `:701-724`, `:826-855`, and the SQLite
  equivalents), which is why they cannot be ported as a wrapper;
...
- **`LiteralSet` is NON-EMPTY by construction** and enforces a homogeneous
  ...
  **The lower bound is the part an earlier draft omitted, and without it this
  ...
  class of defect unrepresentable, and it made **one of the two** unrepresentable
  while the other stayed expressible. "The types prevent it" is exactly the kind
  of claim that stops people checking, so it has to be true node by node.
...
The driver already caches prepared statements
(`Client::new_with_statement_cache`, `statement_cache_execution_threshold`;
`libs/compio-postgres/src/{bind.rs:163,prepare.rs:312-356}`), and the IR makes
that cache far more valuable: values are always parameters, so two calls
differing only in their arguments produce **identical SQL text** and reuse one
prepared plan.
...
ordered collections, and the property is worth an arm: the same plan shape
...
option and matches the refuse-rather-than-emulate rule above: a subscription
that never fires is indistinguishable, from the creator's side, from a table
that never changes.
...
- the arm **reports** its ruled-on count and asserts it is non-zero. It does
  **not** pin that count to a literal.

  An earlier draft said the count "must equal 86 today", which is precisely the
  hard-coded census this repository forbids - and which this same session watched
  fail: `PLUGIN_DB_MIN_PASSED=118` in `tests/run_plugin_db_live_suite.sh` now
  exceeds the maximum the tree can produce (108 measured), so that gate cannot go
  green on any code. A number maintained by hand beside the thing it measures
  goes stale exactly this way. The arm compares two sets it derives at run time -
  the ledger's source column and the crate's actual exports - and a mismatch in
  either direction fails. 86 is what that comparison yields today, not what it
  is told to expect;
...
- The ledger exists and **the gate asserts set equality** between its source
  ...
  "Reports a non-zero count" is a degenerate floor - one row clears it - and the
  real property was stated only in the prose above. An arm that a single row
  satisfies cannot distinguish an exhaustive ledger from an abandoned one.
...
- **A relation plan submitted to the live-query path is refused with a typed
  ...
  like.
- **The same plan shape renders byte-identically twice**, so the driver's
  prepared-statement cache can hit. A rendering that iterates an unordered map
...
- An unsupported capability surfaces as a typed error from the concrete backend,
  ...
  **That arm must first assert the boundary file exists.** `backend/api.rs` does
  ...
  shape, and it appeared three times across these documents.

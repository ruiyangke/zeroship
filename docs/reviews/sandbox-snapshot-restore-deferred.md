...
- **Status**: **CLOSED** by `a3cfca10` (escape the 3 backtick pairs inside the heredoc body — rustdoc-style references to `nomad_ch.rs:797`, `bbadbe68`, and `nomad_ch.rs:120`). Verified live on a 3+3 cluster at HEAD `a3cfca10`: `cat /etc/systemd/system/zsbx-ctl.service | grep -E "command not found|No such file"` → empty; `systemctl status zsbx-ctl.service` → `Active: active (running)`; the full `Environment=` block (25 keys) is intact. The heredoc no longer corrupts the unit write.

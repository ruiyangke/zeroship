...
### [STARTUP-HEREDOC-LEAK] `gcp-worker-startup.sh` zsbx-ctl heredoc executes back-ticked tokens at boot — ch driver never installed on workers
- **Status**: NEW (surfaced 2026-05-24 stress-r9 with user-granted budget override). The `cat > /etc/systemd/system/zsbx-ctl.service <<EOF` heredoc beginning at L511 is **unquoted**, so backtick-wrapped tokens inside its body undergo command substitution at boot. Two offending lines inside the heredoc body:
  ...
  Bash tries to execute `crates/sandbox/src/backend/nomad_ch.rs:797` and `bbadbe68` while expanding the heredoc, aborting (or corrupting) the systemd-unit write half-way. **The subsequent ch-driver-install block (L179-201) and `/etc/nomad.d/client.hcl` emission never run.** Workers boot ch-less; `nomad node status -self -json` shows only qemu/raw_exec/exec/java/docker. Every `driver = "ch"` alloc falls back to raw_exec and Code-1s in ~ms.
- **Cluster evidence**: stress-r9 0/400 e2e (`docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r9.md`). All 6 workers across 3 GCE VMs reproduced; `/etc/zeroship/nomad-plugins/` absent, `/etc/nomad.d/client.hcl` absent.
- **Fix**: change L511 `<<EOF` → `<<'EOF'` (quote heredoc terminator → suppresses expansion). Alternate: strip the back-ticked tokens at L597/L599. Quoted terminator is the safer fix — those are not the only back-tick tokens in the unit body (L343 also references `nomad_ch.rs:120`, currently unscathed only because it's not inside the broken heredoc).
- **Pre-flight gate to add**: worker post-boot `nomad node status -self -json | jq '.Drivers.ch.Healthy' = true || abort` in `provision-gcp-cluster.sh` so this regression fails loudly at provision time rather than at run time.
- **Blocks**: T-8b-stress-r9 RETRY-2 (validating r24-A2-S2+S3), and downstream T-8b-cutover. No work on the binding-wedge ladder is testable in cluster until this lands.

### [STRESS-R9-RETRY-2] T-8b-stress-r9 RE-RUN required after STARTUP-HEREDOC-LEAK fix
- **Status**: PENDING the heredoc fix above. First stress-r9 attempt (22:09Z dispatch with budget override) ran end-to-end ($0.50 / 16 min cluster lifetime, full teardown verified zero residuals) but **failed to exercise r24-A2-S2+S3** because the v19 driver was never loaded on any worker. e2e 0/400 with `Failed tasks: ch: Unhealthy because of failed task` is upstream of the binding wedge.
- **r24-A2-S2+S3 efficacy**: **UNKNOWN** — not refuted, not confirmed. The stress-r8 cycle-1-19 EEXIST `Tap zsbx-nm-N already exists` wedge stands neither closed nor re-confirmed by this run.
- **Validates** (unchanged from prior dispatch): r24-A2-S2 sync `ip tuntap del` + ENODEV verify (`cd118547`) + r24-A2-S3 VmIndexAllocator 5s release delay (`c969b94d`) closes the cycle-1-19 EEXIST wedge. Driver v19 + controller v36 + r7-B file exporter for counter visibility.
...
- **Pre-flight additions for retry**: (a) verify heredoc fix landed, (b) confirm `Drivers.ch.Healthy = true` on at least one worker via `nomad node status -self -json` before driving the harness, (c) confirm `/var/lib/zsbx/driver-metrics.prom` populates after first DestroyTask (would smoke-test r7-B exporter wiring).

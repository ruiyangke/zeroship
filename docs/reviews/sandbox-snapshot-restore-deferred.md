...
Last updated: 2026-05-24 T-8b-stress-r9 **RED — INCONCLUSIVE** (user-granted budget override at 22:08 UTC): cluster ran end-to-end (3 servers + 3 workers in asia-northeast3-a, 22:11→22:27Z, full teardown verified zero residuals, ~$0.50 burn) but e2e was **0/400 at CREATE** with 100% `Failed tasks: ch: Unhealthy because of failed task`. **Root cause is upstream of r24-A2** — the v19 ch driver was never loaded on any worker because `gcp-worker-startup.sh` L511 zsbx-ctl.service heredoc is unquoted and L597/L599 backtick tokens (referencing `crates/sandbox/src/backend/nomad_ch.rs:797` and ADR `bbadbe68`) trigger command substitution at boot. The startup-script aborts the unit-write half-way; the subsequent ch-driver-install block (L179-201) and `/etc/nomad.d/client.hcl` emission never run; workers boot with only qemu/raw_exec/exec/java/docker. Nomad falls back to raw_exec for `driver = "ch"` allocs which Code-1 in ms. **r24-A2-S2+S3 efficacy UNKNOWN** — not refuted, not confirmed; the binding-wedge fix's `destroy_task_tap_stuck_total` counter file `/var/lib/zsbx/driver-metrics.prom` never materialized because no DestroyTask ever ran. Controller `sandbox_*` counters all 0 pre==post. Two new CRITICAL deferred entries added: **[STARTUP-HEREDOC-LEAK]** (the heredoc bug) and **[STRESS-R9-RETRY-2]** (re-run after heredoc fix lands). Review file: `docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r9.md` (overwrote the prior BLOCKED stub at `2468ab96`). Prior update (now superseded): 2026-05-24 T-8b-stress-r9 BUDGET-BLOCKED (NO PROVISION): cluster validation for the r24-A2-S2+S3 binding-wedge fix (driver v19 = `nomad-driver-ch.v19` at GCS MD5 `3e4349744f994f8dc58e2d82d6168e59`, size 20259000; sandbox HEAD `086971d2`) ABORTED at pre-flight. The `/tmp/zsbx-cluster-budget-20260524` rate-limit marker contained **24 entries** at agent start, well past the brief's **≥10 entries → ABORT** threshold. No `provision-gcp-cluster.sh` invocation, no GCE instances, no snapshot_stress.py run, **$0 incurred**. All other pre-flight gates PASSED: HEAD matches `086971d2` (v19 pin commit); `gcp-worker-startup.sh` L180/L188/L195 reference `nomad-driver-ch.v19` (not v18); `gsutil stat gs://suger-dev-zsbx-artifacts/nomad-driver-ch.v19` returns `Content-Length: 20259000` + base64 hash `PkNJdE+ZT43Fji2C1haOWQ==` which decodes to exactly the expected MD5 `3e4349744f994f8dc58e2d82d6168e59`. The fix bundle is correctly staged; only the daily-burn rate-limit gate (defending the $30 cap referenced in the brief's MANDATORY TEARDOWN section) blocks the cluster verdict. The 24 budget entries span 00:03Z → 11:13Z UTC and are predominantly smoke-r1..r18 cycles (smoke-r12 → smoke-r18 inclusive at the end of the day). r24-A2-S2+S3 efficacy **UNKNOWN** — not refuted, not confirmed; deferred to next UTC day when `/tmp/zsbx-cluster-budget-20260525` rolls over empty. Cluster review committed at `docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r9.md` (full pre-flight gate matrix + budget marker contents + carry-forward from r1..r8 ladder). **Action**: re-dispatch T-8b-stress-r9 after 2026-05-25 00:00 UTC against the fresh marker; pre-flight gates 1-4 are already PASS and don't need re-verification unless the worktree moves. Prior update: 2026-05-25 r25 / R23-API1+R25-S1+R25-I1+R25-I2 convergent fixer (typed `WakeErrorCode::StagingPathMissing` + wire code `staging_image_missing`): four findings on the same data-flow chain — api-surface-r23 (controller-side `submit_restore_job` preflight free-text errors leaked host paths + locale-dependent `os error 2` via the §10.0 `message` field), security-r25 (the leak elevated to ACTIVE now that AdminRole::ReadOnly bearers can read these strings via `GET /admin/sandboxes/{id}/wake/{wake_id}`), code-quality-r25 R25-I1 (bundle's preflight re-inlined path derivation that has `pub(crate)` helpers `workspace_image_path` + `user_home_image_path` — same 3-strike DRY shape as r21-A1), and code-quality-r25 R25-I2 (bundle's error strings used bare-UUID `sandbox_id` while sibling sites use `sbx_<base62>` typed-id form). **Architectural choice: Option B + Option C-lite hybrid** (per task brief): introduced `pub enum SubmitRestoreError { Preflight { which, path, source }, Other(String) }` as the new return type of `RestoreBackend::submit_restore_job` (was `Result<(), String>`); introduced `RestoreHandlerError::StagingPreflight { which, sandbox_id_typed }` whose `Display` impl is path-free; both sync (`do_restore_inner`) and async (`wake_machine` submit branch) classify `SubmitRestoreError::Preflight` to `RestoreHandlerError::StagingPreflight` and tracing-log the verbatim host path via `SubmitRestoreError::log_detail(typed_id)` at the controller boundary BEFORE the path-free typed map; `classify_failure` adds the `StagingPreflight → WakeErrorCode::StagingPathMissing` arm; the wire code is `staging_image_missing` (operator-facing resource name) while the internal pg `as_str` form is `staging_path_missing` (failure shape) — they differ intentionally (pinned in the wire-code map test). Option A rejected because the variant payload would force every match site (pg row read, classifier, wire-render, admin-handler exhaustive pin test) to handle data with no win; Option C-lite (separate `wake_jobs.error_path` column) rejected because the wake_jobs schema is a wire contract and the routing decision can stay structurally typed without a column migration. Path-derivation helpers reused (R25-I1 closed): `submit_restore_job` calls `workspace_image_path(&host_dir)` + `user_home_image_path(&user_home_dir_root, user_id)` instead of re-inlining the `{root}/{sid}/<file>` join. Typed-id form used (R25-I2 closed): wire-visible `sandbox_id` is `sbx_<base62>` (matches `restore_handler.rs:633-636` / `:731-734`). Migration 0013 extends `wake_jobs.error_code` CHECK domain to admit `staging_path_missing`; `LATEST_MIGRATION_VERSION` bumped 12 → 13; idempotent shape mirrors 0012. Wire-code coverage: all 9 `WakeErrorCode` variants now have `as_str` / `from_str_opt` / `wire_code` symmetry — pinned in `wake_error_code_as_str_round_trip`, `wake_error_code_wire_code_uses_existing_envelope_codes`, `r16_api1_failed_state_renders_every_wake_error_code` (admin-handler render exhaustiveness), and `classify_failure_maps_restore_variants_to_wake_codes`. New test `staging_preflight_display_is_path_free` pins R25-S1 invariant: no `/` in the `Display` form. Two commits: `79871194` (db layer — variant + wire code + migration + pinning tests; 454/454 lib pass) and `022f778a` (producer chain — typed enum at the trait boundary + sync/async classifier mapping + admin handler arm + typed-id form + helper reuse + path-free Display pin; 455/455 lib pass, +1 for path-free pin). Build clean. Source: api-surface-r23 R23-API1, security-r25 R25-S1, code-quality-r25 R25-I1, code-quality-r25 R25-I2 — ALL CLOSED. Prior update: 2026-05-25 r23 / R23-I1 fixer (Path B — WakeMachine terminal-overwrite counter e2e coverage): concurrency-r23 R23-I1 finding noted the bare db noop tests (`update_wake_job_state_after_{ok,failed}_is_noop`) prove only the SQL guard returns `rows_affected == 0` — they do NOT exercise the counter wiring at the WakeMachine call sites (wake_machine.rs:146 / :191 / :528). **Path B chosen** (vs Path A "bump inside `db::update_wake_job_state`"): the R22-I1 architecture (commit `f98611fb`) deliberately placed the bump at each caller so the WARN log can carry a site-specific `attempted_state` field (`Phase::Ok` vs `Phase::Failed` vs the intermediate set_state value). Moving the bump into the db function would erase that context for no benefit — it's a deliberate caller-side responsibility, not a missing abstraction. Path B test: drive the full WakeMachine end-to-end with the wake_jobs row pre-flipped to a terminal state, so every `update_wake_job_state` inside the machine hits the R20-C1 guard. +2 pg-gated tests in `crates/sandbox/tests/sandbox_pg_e2e.rs::wake_machine_e2e`: `wake_machine_terminal_overwrite_failed_to_ok_bumps_counter` (pre-set Failed, drive happy-path → counter strictly increases, row stays Failed) and `wake_machine_terminal_overwrite_ok_to_failed_bumps_counter` (pre-set Ok, drive `fail_livez=true` → counter strictly increases, row stays Ok). Counter delta asserted `>= pre + 1` (not `== pre + 1`): the machine issues 5-6 intermediate set_state calls + one terminal write; each no-ops + bumps when the row is already terminal. Strict monotonic increase is the honest contract — asserting `== 1` would require the machine to short-circuit on first guard fire, which production behavior does not (and should not — intermediate writes are best-effort). No production code touched (wake_machine.rs / db.rs / metrics.rs all unchanged). Lib 433/433 unchanged; pg-gated +2 (verified locally against `postgres://postgres:zeroship@localhost:5440/zeroship` — both PASS). Build clean. Source: concurrency-r23 R23-I1. Prior update: 2026-05-25 r22 / R22-I1 fixer (terminal-overwrite-blocked observability): R20-C1's SQL guard (`AND state NOT IN ('ok','failed')`) was correct on the data plane but invisible to operators — all 3 `update_wake_job_state` callers in `wake_machine.rs` (Phase::Ok ~:128, Phase::Failed ~:161, `set_state` ~:487) discarded `rows_affected`. Each site now captures the return value; `rows_affected == 0` emits `tracing::warn!` at target `sandbox::wake::terminal_overwrite_blocked` + bumps `sandbox_wake_terminal_overwrite_blocked_total` counter via `crate::metrics::inc_wake_terminal_overwrite_blocked()`. Counter + test-only accessor added to `crates/sandbox/src/metrics.rs` (mirrors `inc_vm_index_leak` / `vm_index_leak_value` pattern). +2 new lib tests (`wake_terminal_overwrite_blocked_counter_starts_at_zero`, `inc_wake_terminal_overwrite_blocked_monotonic`). No behavior change; pure observability. Lib 429 → 431; build clean. Prior update: 2026-05-25 r20+r21 / R20-C1 fixer (terminal-overwrite guard): `update_wake_job_state` SQL hardened with `AND state NOT IN ('ok', 'failed')` predicate; stale writes from racing producers (takeover sweep vs state-machine driver) silently no-op once terminal. Rustdoc updated. +2 pg-gated tests (`update_wake_job_state_after_ok_is_noop`, `update_wake_job_state_after_failed_is_noop`). Lib 429 unchanged; build clean. Prior update: 2026-05-25 r20 / R20-I1 fixer (doc extraction): `from_host_fence_timeout` rustdoc extracted to ADR `docs/decisions/2026-05-25-vm-index-retry-policy.md`; doc trimmed from ~116 to 13 lines; r17-Q1/r18-M2/r19-M2/r20-M-NEW doc-inflation carry CLOSED. No behavior change. Prior update: 2026-05-25 r19 / R19-C1 fixer (CRITICAL LANDED): wake_jobs takeover sweep — `claim_orphan_wake_for_recovery` + `spawn_wake_jobs_takeover` + `WakeErrorCode::WakeWorkerAborted` + migration 0012. PR1 at `1d3724fe` (db primitive + variant + migration + 4 pg-gated tests), PR2 at `8d163d58` (sweep loop + `takeover_threshold_secs` config + 5 lib tests + wiring). Co-closes R18-A1 / R17-T4 / R19-A2 — all four findings tracked the same gap (lessee_updated_at written by R17-A1 since `96678eaa`, no reader until now). 414 → 423 lib tests; release build clean. Prior update: 2026-05-25 r19 / smoke-r13 fixer (C-7-LT-2 LANDED): host-fence probe wedge fixed at `40811d8b` (PR1: replace ureq-based `compio::runtime::spawn_blocking(|| ureq::get(...).timeout(500ms).call())` with compio-native `compio::time::timeout(150ms, compio::net::TcpStream::connect(addr))`; +5 new tests including `pr1_probe_unroutable_address_returns_false_within_timeout` which pins the wedge-fix invariant — outer compio timeout MUST cap a stuck SYN within 750 ms vs. the kernel's 30-90 s SYN-retransmit ceiling); leak counter + scoped log target at `bfff5acc` (PR2: `sandbox_vm_index_leaks_total{reason=host_fence_timeout|wait_failed}` counter, `inc_vm_index_leak(reason)` API matching `inc_lost_leadership`'s convention with unknown-label-fold-into-host_fence_timeout fallback, `target: "sandbox::teardown::leak"` on the two `stop_inner` leak-branch WARN macros so operators can `RUST_LOG=sandbox::teardown::leak=warn` scope without the full sandbox chatter; +1 new test). Sandbox lib 413 → 414 (+1; PR1 was test-neutral after repurposing the R16-I2 LEAK-case test as a PR1 regression pin since alternating-answer LEAK is structurally impossible under connect-only probe). **Retrospective**: smoke-r13 (`docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-smoke-r13.md`) surfaced that the entire C-4..C-8c diagnostic chain was reasoning over a faulty premise — "teardown completes at +60 s and releases the slot, but the wake budget gives up earlier". The truth: teardown completes at +60 s but LEAKED the slot every cycle because `wait_for_agent_silent`'s ureq probe fired only ONCE in the 30 s fence budget (`probes=1, consecutive_misses=1, last_status=None`). C-4..C-8c's budget tuning is still correct *as fixes* (the budget knobs matter when the agent does go silent), but the production-only signal those fixes were chasing was always upstream of them. C-7-LT-2-PR1 fixes the upstream wedge; C-7-LT-2-PR2 adds defense-in-depth observability so a residual hang (deadlocked agent, OOM-killed without socket-close) is quantifiable. Smoke-r14 next — re-smoke at controller v29 (binary pin not yet bumped; will follow under separate scripts commit per scope guard). Prior r17 PR2 note (C-7-LT-PR2 LANDED): wake_machine state-machine driver at `17e9f421` (typed_id `new_wake_id` + `wak_` 3-char prefix per R16-API2, `WakeErrorCode::wire_code` for §10.0 wire-parity per R16-API1, deprecation counter `sandbox_wake_sync_uses_total` per R16-API1 #5); dual-mode `POST /admin/sandboxes/{id}/wake` + new `GET /admin/sandboxes/{id}/wake/{wake_id}` polling endpoint at `98032273` (idempotency replay + state-mismatch pre-flight + §10.0 envelope on failed-poll body); wake_jobs GC sweep at `9f006c87` (60 s cadence, T_KEEP=5min); pg-gated wake_machine e2e stub fixtures at `c2f24ede` (6 tests: happy + livez/submit/reserve failures + idempotency + GC eviction — closes test-coverage-r16 EMERGENCY HOLD + R10-T1/T2 + 7-round R15-T1 carry). Sandbox lib 360 → 373 (+13). PR3 (cluster smoke validation under SANDBOX_WAKE_RESPONSE_MODE=async) next. Prior PR1 note (C-7-LT-PR1 LANDED): R14-A1 helper extracted at `3d8acc23`, 5 sites migrated `eab3ec43`/`4fd195ef`/`96fa5f0f`, WakeResponseMode flag default-OFF at `b64d2f39`, 0009_wake_jobs migration at `259f0e50`, WakeJobRow/State/ErrorCode + 5 CRUD methods at `a0888d9e`. R14-A1 CLOSED, R16-I1 CLOSED; C-7-LT moves from DESIGN-READY to PR1-LANDED. Lib tests 347 → 360; pg-gated tests written but skipped (local pg down). Prior r16 r1 note: C-8c surfaced from smoke-r11 (sync wake contract structurally out of knobs); C-7-LT design proposal READY at docs/proposals/c7-lt-async-wake.md (uncommitted); R15-S1 CLOSED at da951dd9; R15-I2 CLOSED at 64af1803; #24 CLOSED at a4c481e1; R16-I1/R16-I2/R16-A1/R15-S2 added. Prior r2 note: deferred-backlog refresh: #24 CLOSED at `a4c481e1` — `ZSBX_SANDBOX_ID` env injection verified in tree at `nomad_ch.rs:2373` + regression tests at L4020-4069; entry was stale paperwork written before the fix landed. Prior r1 note: post critical-fix-sweep cluster smoke at v18 + rootfs v5: Phase 1 c=4 HARD FAIL 0/16 creates, NEW bug #24 — controller `Tasks[].Env` block in `nomad_ch.rs:2224-2264` missing `ZSBX_SANDBOX_ID` so the wrapper's R8-DEPLOY1 guard fires at line 153 on every cold boot, killing alloc in ~50ms with empty ch.stderr. Phase 2 c=20 NOT REACHED. Cluster fully torn down. B19 + B-SLO REMAIN UNVERIFIED at cluster on this branch HEAD. Detail: `docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-r1.md`).
...
### [r29-A1] (CLOSED 2026-05-25 round-42) STARTUP-HEREDOC-LEAK class-level audit + harden across 11 callsites
- **Status**: **CLOSED** by `b8310356`. Architecture-r29 r29-A1 (CRITICAL) flagged the class: `a3cfca10` escaped 3 backticks at one site (`gcp-worker-startup.sh:511`) but the same trap was re-arming across 10 other unquoted-`<<EOF` sites where any future rustdoc backtick lands. Audit results: 11 sites total — 3 switched to `<<'EOF'` (static bodies: `plugin-dir.hcl`, `99-zsbx.conf`, `zsbx-taps.service`); 8 kept unquoted because they require host-time `$var` expansion (two `nomad.hcl` blocks, 2 `*.env` files, `zsbx-taps-up.sh`, `zsbx-ctl.service`, `bake-rootfs.sh::usage()`, `provision-gcp-cluster.sh` summary). All 8 kept-expanding sites scanned for unintended backticks / `$(..)` / `\`; only intentional patterns found (escaped rustdoc refs at L560/L595-L599 from `a3cfca10`, intentional `$(.. && echo ..)` conditionals at L586/L592, escaped `\$var` run-time references in the `zsbx-taps-up.sh` body). Verified `bash -n` clean across all scripts; `lint.sh` exit 0 at `--severity=error` AND `--severity=warning`. Dry-run with fake vars confirmed kept-expanding sites still expand correctly and quoted sites leave `$` literal.
- **Architectural choice**: Option A (audit + selective quoting) over Option B (move heredoc bodies to file-based templates + `cp` at install). Option B would have been ~40-60 LOC vs the 3-line diff; the per-site classification table in the commit message captures the intent durably, and the next reviewer can grep for `<<EOF` (8 sites) to see the kept-expanding shape immediately.

### [STARTUP-HEREDOC-LEAK] (CLOSED 2026-05-24 r9-retry-2) `gcp-worker-startup.sh` zsbx-ctl heredoc executed back-ticked tokens at boot
- **Status**: **CLOSED** by `a3cfca10` (escape the 3 backtick pairs inside the heredoc body — rustdoc-style references to `nomad_ch.rs:797`, `bbadbe68`, and `nomad_ch.rs:120`). Class-level closure tracked at r29-A1 above. Verified live on a 3+3 cluster at HEAD `a3cfca10`: `cat /etc/systemd/system/zsbx-ctl.service | grep -E "command not found|No such file"` → empty; `systemctl status zsbx-ctl.service` → `Active: active (running)`; the full `Environment=` block (25 keys) is intact. The heredoc no longer corrupts the unit write.
- **Cluster evidence**: `docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r9-retry-2.md` Pre-stress diagnosis section. Retry-2 aborted on a SEPARATE issue (missing metadata flag — see STRESS-R9-RETRY-3 below) before the binding-wedge tests could fire, but the heredoc itself is conclusively fixed.
- **Pre-flight gate to still add**: worker post-boot `nomad node status -self -json | jq '.Drivers.ch.Healthy' = true || abort` in `provision-gcp-cluster.sh` so any future driver-load regression fails loudly at provision time rather than at run time. (Out of scope for this run; the retry-2 dispatch was script-touch-prohibited.)
- **Reconciliation note**: the r9 review (`def11cb4`) attributed driver-absence to this heredoc bug. Retry-2 evidence partially refutes that: the heredoc fix DID land cleanly but the driver is STILL absent — because the driver-install block is gated on `INSTALL_CH_PLUGIN_DRIVER=1`, which is opt-in metadata. The r9 RED outcome's true root cause likely involved a different config path (possibly r9's invocation passed `EXTRA_WORKER_METADATA` from shell env we couldn't reproduce, or the failure mode was misattributed). Either way, the heredoc isn't the active blocker.

### [STRESS-R9-RETRY-3] T-8b-stress-r9 re-run still required — retry-2 aborted pre-stress (missing `install-ch-plugin-driver=1` metadata)
- **Status**: PENDING. Retry-2 (HEAD `a3cfca10`, 22:31-22:40Z, ~$0.10) provisioned cleanly and confirmed the STARTUP-HEREDOC-LEAK fix is real, but the driver-list pre-stress check returned `docker, exec, java, qemu, raw_exec` — no `ch` — so per the dispatch directive ("STOP, teardown, document, exit") the harness was never invoked. Teardown verified zero residuals.
- **Root cause of retry-2 abort**: `gcp-worker-startup.sh:97` resolves `INSTALL_CH_PLUGIN_DRIVER` from instance metadata key `install-ch-plugin-driver` and defaults to `0`. The retry-2 dispatch invoked `provision-gcp-cluster.sh` with `SERVER_COUNT=3 WORKER_COUNT=3` only, and `EXTRA_WORKER_METADATA` defaults to empty (`provision-gcp-cluster.sh:62`), so the metadata key was never written. The driver-install block at `gcp-worker-startup.sh:177-223` was skipped entirely, leaving Nomad on the raw_exec/qemu/etc. baseline.
- **r24-A2-S2+S3 efficacy**: still **UNKNOWN** — the binding wedge has now been blocked from execution across r9 + r9-retry-2.
- **Next-dispatch correction (REQUIRED)**:
  ```bash
  ...
  ```
  Alternative (cleaner): flip the default in `provision-gcp-cluster.sh:62` to `EXTRA_WORKER_METADATA=${EXTRA_WORKER_METADATA:-install-ch-plugin-driver=1}` so the T-8b cutover line stops being implicit-off. 1-line script change. Out of scope for the retry-2 run (script-touch was disallowed).
- **Validates** (unchanged): r24-A2-S2 sync `ip tuntap del` + ENODEV verify (`cd118547`) + r24-A2-S3 VmIndexAllocator 5s release delay (`c969b94d`). Driver v19 + controller v36 + r7-B file exporter for counter visibility.
...
- **Pre-flight additions** (carry over): (a) confirm heredoc fix landed [DONE per retry-2], (b) confirm `Drivers.ch.Healthy = true` on worker-1 via `nomad node status -self -json` before driving harness, (c) confirm `/var/lib/zsbx/driver-metrics.prom` populates after first DestroyTask.

### [R29-P1] (CLOSED 2026-05-25) snap-idle-gc serial loop multiplied R29-C1's `release_vm_index_after.await` into vm_index starvation (stress-r9-retry-5 RED)
- **Status**: **CLOSED** by `81b6e689` (parallelize snap-idle-gc stops with bounded concurrency cap=8). Performance-r29 R29-P1 finding from `docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r9-retry-5.md`. The R29-C1 class-fix at `62b083e1` made `stop_inner` block on `release_vm_index_after.await` (was fire-and-forget pre-fix); the design is correct (needed for `detach_isolated` short-lived runtimes per R29-C1) but the *caller-side* snap-idle-gc loop at `registry.rs:828-878` then serialized those 5 s waits N times per tick. At N=12 expired sandboxes per 60 s tick: 12 × 5 s wall = the tick interval itself, plus the 1-2 s of real nomad teardown → backlog grows faster than it clears, vm_index slots stay held, controller boot hits `vm-index allocator exhausted (floor=1, ceil=12)` at idx=0. stress-r9-retry-5 (HEAD `e66d5efb`, 2026-05-25 00:30 UTC) caught it: **0/400 CREATE successful**, all with the allocator-exhausted error.
- **Architectural choice**: Option A (`futures::future::join_all` with cap=8, mirroring `sweep::snapshot_rows_chunked`) over Option B (`compio::runtime::spawn` per stop). The `join_all` shape is the existing crate idiom (sweep.rs already uses it for the idle-eviction snapshotter at the same per-chunk concurrency boundary); reusing it keeps a single uniform pattern across both background loops. Cap=8 was chosen as a conservative bound against Nomad — each stop drives a `/shutdown` request, and unbounded fan-out would trade one starvation for another. Wall-time math: N=12 → ⌈12/8⌉ × 5 s = 10 s (was 60 s); N=24 → 15 s (was 120 s). The backlog drains.
- **Implementation**: private `trait GcStopper` (mirrors `sweep::IdleSnapshotter` shape) + `async fn gc_stop_chunked(ids, stopper, cap)` helper inside `registry.rs`. Production wires `AppStateGcStopper` holding an `Arc<AppState>`; per-id sequence is verbatim from the pre-fix loop (log → `backend.stop(id).await` → `sandboxes.remove(&id)`, both halves still error/panic-wrapped). `nomad_ch.rs` is 0-line diff — `stop_inner` and `release_vm_index_after` are unchanged. `Cargo.toml` is 0-line diff — `futures` was already a dep (added at the M3 detached-cleanup catch_unwind helper). 1 file changed, +192/-21 LOC.
- **Test**: `registry::tests::gc_stop_chunked_is_actually_concurrent` mirrors `sweep::sweep_concurrency_is_actually_concurrent`. SleepingGcStopper records `max_in_flight` + sleeps per_stop=2 s per id; N=10 ids, cap=8 → two chunks of (8, 2) → ~4 s wall (vs 20 s serial). Asserts `elapsed < 5 s` (catches serial regression at ≥18 s with margin) and `max_in_flight == cap` (semantic check). Measured local wall: `elapsed=4.000218719s max_in_flight=8` — matches the ideal exactly. Lib tests 548 → 549.
- **Cluster validation**: stress-r9-retry-5 re-run REQUIRED to confirm the 0/400 regression is gone. Same SERVER_COUNT=3 WORKER_COUNT=3 shape; controller pin needs bumping past v37 (the v37 binary at the head of `sandbox/scripts/.../zeroship-sandbox-controller-pin` carries the R29-C1 class-fix but NOT this R29-P1 parallel-loop fix). Cluster review committed at `docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r9-retry-5.md` (the trigger doc for this finding).

### [R32-P1] (CLOSED 2026-05-25) Cold-boot CREATE serialized two `mkfs.ext4` subprocesses inside one spawn_blocking (~1-2 s win on first-sandbox-per-user)
- **Status**: **CLOSED** (Knob A from `docs/reviews/sandbox-snapshot-restore-performance-2026-05-25-r32.md`). The cold-boot CREATE path at `nomad_ch.rs:1133-1146` staged `workspace.img` (per-sandbox) and `home.img` (per-user) by calling `create_ext4_image_if_missing` twice in sequence inside one `compio::runtime::spawn_blocking`. Each `mkfs.ext4 -q -F` round-trip is ~0.5-2 s on GCE NVMe; serializing them put the second mkfs entirely on the critical path. Perf-r32 traced 8.9 s c=1 CREATE → ~2.5 s in this block; warm (home.img idempotent-skip) was 1.5 s less. Knob A is the controller-side fix; Knob B (`driver_stages_disk_images=true` default flip) is the durable answer and remains Phase-4 gated.
- **Architectural choice**: `std::thread::scope` (std since 1.63) over `compio::spawn_blocking` × 2 + `futures::join!`. The two mkfs calls already run inside a spawn_blocking on the blocking-thread pool — nesting compio spawn_blocking would require Send-bound restructuring and split the error-routing into two future-poll sites. `std::thread::scope` borrows from the enclosing closure (no Arc / `'static` dance), joins both children before the scope exits, and surfaces panics as `Result` via `JoinHandle::join`. Error precedence: if both children fail, workspace.img's error is returned first (it's the per-sandbox cold-boot image — the more diagnostic fault domain).
- **Implementation**: ~28 LOC inside the existing `let staged = compio::runtime::spawn_blocking(...)` body at `nomad_ch.rs:1133`. Two `s.spawn(|| create_ext4_image_if_missing(...))` handles, joined into `(workspace_res, home_res)`; each `join()` panic is mapped to a string `Err` matching the pre-existing `spawn_blocking` panic catch-all on the outer call. The closure return type, mkdir-prelude, and `Ok(workspace_img)` tail are unchanged. `create_ext4_image_if_missing` itself is unchanged (each call still does truncate → mkfs.ext4 → fsync_dir → assert_disk_image_present on its own dirent; the parent fsyncs target disjoint directories — `host_dir/workspace.img` vs `user_home_dir_root/<user_id>/home.img` — so there's no cross-thread coordination needed). 1 file changed (`crates/sandbox/src/backend/nomad_ch.rs`), +33/-2 LOC.
- **Expected p50 improvement**: per perf-r32 §"Where the 8.9 s c=1 CREATE goes" the mkfs-pair was the 2.5 s slice. With parallel mkfs, wall = max(workspace, home) ≈ 1.0-1.5 s, saving ~1.0-1.5 s on cold-boot c=1 CREATE. First-sandbox-per-user is the load shape that pays the full saving; warm boots already skipped home.img (idempotent existence check) so the win is bounded to cold runs.
- **Test**: `cargo build -p zeroship-sandbox --tests` clean; `cargo test -p zeroship-sandbox --lib` 551 passed (unchanged — no test-surface mutation, the change is purely a concurrency reshape of existing per-call semantics). Cluster validation deferred to next perf-pin r32-T1-style trace; lib tests are sufficient for this commit per the brief.
- **Source**: performance-r32 R32-P1 (`docs/reviews/sandbox-snapshot-restore-performance-2026-05-25-r32.md`, Knob A).
...
### [C-7-LT-12 / C-7-LT-12a] (CLOSED at driver `7a85ed7b` + `50fb987d`, controller `7fd661c9`) Restore branch never stages rootfs.img into the new alloc's runDir — CH `--restore` aborts at `DeviceManager(Disk(NotFound))`
- **Source**: T-8b-smoke-r22 cluster review. C-7-LT-11's stderr-on-resume-failure capture surfaced the verbatim CH stderr that smoke-r21's terse `ch-remote resume: HTTP 500: VM is not running` had been hiding: `VM Restore failed: DeviceManager(Disk(NotFound))` against `disks[0].path`. Root cause reclassified from C-7-LT-12 (CH-internal hypothesis: "VM is not running" was the resume-side reflection of a paused-state misuse) to C-7-LT-12a (driver-side: the restore branch never staged the rootfs at the rewritten destination).
- **Symptom**: The snapshot's config.json carries `disks[0].path = /opt/nomad/data/alloc/<OLD>/ch/local/rootfs.img` — an alloc-scoped path on the source side. The driver's path-rewriter (C-7-LT-6 PathFieldDisk slot 3) retargets the field to `<NEW-runDir>/rootfs.img` and validation passes against the task_dir allow-list. But pre-fix the restore branch **never staged a real file at the new destination**. workspace.img + home.img survive the source-alloc GC because they live at stable persistent prefixes (`/var/zeroship/ch/<sbx>/`, `/var/zeroship/ch/users/<usr>/`); rootfs.img is alloc-scoped on the source side (cold-boot's `materializeRootfs` copies it INTO the alloc task_dir) and gets GC'd with the source alloc — its bytes had nowhere to live across the snapshot/restore boundary unless the driver re-staged them from a content-addressed artifact.
- **Fix shape (driver, ~30 LOC + helper, cross-worktree)**:
  - `TaskConfig.RootfsSource string` (codec `rootfs_source`) + hclspec entry — absolute host path of the source rootfs. Optional schema-wide; the restore branch validates non-empty and surfaces `rootfs_source is empty (...; C-7-LT-12a)` on misconfig (pre-fix the operator would have seen only CH's cryptic `Disk(NotFound)`).
  ...
  - `startTaskRestoreBranch`: after the C-7-LT-10 immutable-artifact symlinks, validate `RootfsSource` non-empty + readable, then stage `<RootfsSource>` → `<runDir>/rootfs.img`.
- **Fix shape (controller, 1-line emission per cross-worktree scope guard)**: `build_restore_nomad_job_json` in the ChPlugin branch emits `rootfs_source = cfg.runtime_dir / "rootfs-slim.img"` — the same artifact the cold-boot's `materializeRootfs` reads from (`$ZSBX_ARTIFACT_DIR/rootfs-slim.img` where `ZSBX_ARTIFACT_DIR == runtime_dir`). Wake VMs boot off byte-identical rootfs as fresh VMs; on the same FS the driver's hardlink shares an inode (zero copy cost).
- **Files**:
  - Driver worktree: `ch/task_config.go` (`7a85ed7b`, +29 LOC), `ch/restore_task.go` + `ch/export_test_api.go` + `tests/{helpers_test.go,restore_task_test.go}` (`50fb987d`, +373 LOC).
  - Sandbox worktree: `crates/sandbox/src/restore_handler.rs` (`7fd661c9`, +75 LOC).
- **Tests**:
  - Driver: 121 → 124 PASS (+3): `TestStartTaskRestoreBranch_StagesRootfs` (integration witness — regular file at runDir/rootfs.img, byte-identical contents, same-FS hardlink shares inode via `Stat_t.Ino`); `TestStartTaskRestoreBranch_RootfsSource_MissingErrors` (empty-field negative path — error carries `rootfs_source is empty` + `C-7-LT-12a` tag); `TestStartTaskRestoreBranch_RootfsSource_EXDEV_FallsBackToCopy` (hardlink/copy decision in isolation — same-FS hardlink shared inode, idempotent re-stage, forced-copy path via parallel `CopyRootfsForRestoreTest` helper with distinct inode + byte-identical contents).
  - Controller: 431 → 432 PASS (+1): `ch_plugin_restore_jobspec_populates_restore_from` extended with `config.rootfs_source == /var/lib/zeroship/ch/rootfs-slim.img` assertion under fixture_cfg; new `ch_plugin_restore_jobspec_rootfs_source_matches_cold_boot` pins the cross-side invariant (rootfs_source MUST compose from `ZSBX_ARTIFACT_DIR + /rootfs-slim.img`).
- **Verify in tree**: driver `nix develop -c go vet ./...` clean + `go test ./...` 124 PASS / 0 FAIL; controller `cargo build -p zeroship-sandbox --tests` clean + `cargo test -p zeroship-sandbox --lib` 432/0/1-ignored.
- **Out of scope for this PR**: driver v12 binary upload + controller pin bump v11→v12 — follows under separate scripts commits in the respective worktrees. T-8b-smoke-r23 cluster cycle is the validation gate (prediction: CH `--restore` no longer aborts at `Disk(NotFound)`; the wake path either succeeds or surfaces a NEW failure mode beyond C-7-LT-12).
...
### ~~[T1] Read-only `sandbox_admin_ro` role for admin read endpoints~~ (PARTIAL-CLOSED 2026-05-25 — HTTP-layer half landed; pg-layer half rolls into Phase-5)
- **Source**: `crates/sandbox/TODO.md` P1 (Round-4 IMPORTANT #7 deferred)
- **Original scope**: new migration (5th role + read-only grants); `Database::pool_admin_ro()` helper; per-handler routing decision (`open_app_pool` → `open_admin_ro_pool` for the list/detail/export read paths).
- **2026-05-25 T1 fixer LANDED (HTTP-layer half)**: `AdminRole::{Full, ReadOnly}` enum + `admin_check_required` replace the prior `admin_check`; new `SANDBOX_ADMIN_RO_TOKEN_PATH` env var mirrors `SANDBOX_ADMIN_TOKEN_PATH` (mode 0o400, uid 0, non-empty); `AppState::with_admin_ro_token` builder + `pub(crate) admin_ro_token` field; boot-time `assert_distinct_admin_tokens` guard refuses to boot when both bearer paths resolve to the same secret (constant-time compare). Route classification: READ endpoints (`GET /admin/sandboxes`, `…/{id}`, `…/{user_id}/sandboxes`, `…/{user_id}/shares`, `GET /admin/hosts`, `GET /admin/sandboxes/{id}/wake/{wake_id}`) accept either bearer; WRITE endpoints (`POST snapshot/wake/cold-boot`, `DELETE /admin/users/{id}`, **plus** `GET /admin/users/{id}/export` — reclassified Full because the GDPR-cascade aggregation has write-class blast radius) require Full. RO bearer on a Full endpoint surfaces **403 `insufficient_role`** (distinct from 401 so operator tooling can branch); missing-token-for-required-role surfaces **503 `admin_api_disabled`**. 9 new e2e tests in `crates/sandbox/tests/sandbox_admin_e2e.rs` pin every cell of the matrix (RO-on-write 403, RO-on-read 200/503-post-auth, Full-on-read accepted, Full-on-write accepted, unknown 401, both-None 503, RO-only-deploy write 503, DELETE 403, export 403); 11 new lib unit tests pin the `with_admin_ro_token` builder + `assert_distinct_admin_tokens` truth table.
- **Still deferred (pg-layer half) → folded into Phase-5 JWT migration**: the 5th pg role with `SELECT`-only grants + `Database::pool_admin_ro()` + per-handler routing. The HTTP-layer gate stops every WRITE endpoint from being callable by RO operators (the higher-impact half); the pg role adds defense-in-depth for a future stray write inside a "read" handler. Schema work belongs with Phase-5's broader admin-side restructure (JWT, per-endpoint scopes, audit) per `docs/decisions/2026-05-05-sandbox-admin-shared-bearer.md`.
...
### ~~[T5] Restore handler: signed `/version` fingerprint check~~ — CLOSED 2026-05-24
- **Source**: Phase A subagent report (deferred from initial impl)
- **Status**: **CLOSED 2026-05-24** (T5-fixer cycle). The wake state machine now probes the restored agent's signed `/version` endpoint after `wait_for_livez` returns 200 and BEFORE `clock_resync_post_restore`, comparing the agent-reported `git_commit` against the controller's compile-time `CONTROLLER_GIT_COMMIT` (build.rs-injected, mirrors `crates/sandbox-agent/build.rs`'s `AGENT_GIT_COMMIT`). Per-sandbox signing key is plumbed out of the sealed record via the same `Persistence::unseal` call that `clock_resync_post_restore` already uses — no new key material crosses the wire and no new SDK calls land in the sealed-record schema.
- **Option chosen**: **A (simple)** — expected fingerprint = controller's own `CONTROLLER_GIT_COMMIT`. Both binaries deployed together; the per-sandbox typed fingerprint at snapshot time (Option B) is future work once a real cross-version restore scenario surfaces. Option A catches the partial-rollout case the deferred entry was written for (one Nomad node still runs the previous agent image while the controller upgraded) without the schema work Option B would require.
- **Implementation summary**:
  - `crates/sandbox/build.rs` (new) — injects `CONTROLLER_GIT_COMMIT` short-hex; "unknown" sentinel for non-git builds (vendor tarball).
  - `db.rs` — new `WakeErrorCode::AgentVersionMismatch` variant, full triangle (as_str/from_str_opt/wire_code, both forms = `agent_version_mismatch`). Migration 0014 extends `wake_jobs.error_code` CHECK domain to 10 values; `LATEST_MIGRATION_VERSION` 13 → 14.
  - `restore_handler.rs` — `verify_agent_version_post_restore` + structured `VersionCheckOutcome::{Match, Skipped, Mismatch}`. Reuses `ureq` + `zeroship_sandbox_agent::sig::sign` (no new deps). Sentinel handling: controller `unknown` OR agent `unknown` OR agent omits `git_commit` (legacy pre-T5) OR transport error OR non-200 OR non-JSON body → `Skipped` with a structured reason + WARN log, NOT a wake failure (availability decision: don't roll back on a flaky GET when livez just succeeded).
  - `wake_machine.rs` — wires the call in the `Some(persist)` arm between `unseal` and `clock_resync_post_restore`. `Mismatch` rolls back via `rollback_with(..., AgentVersionMismatch, ...)`; `Skipped` continues with WARN; `Match` continues silently at DEBUG. The `persist=None` test-fixture arm skips the check (matches the existing skip of unseal/resync/register).
- **Test count delta**: sandbox lib 529 → 539 (+10). 8 outcome tests at `restore_handler::real_backend_tests::verify_agent_version_*` covering Match/Mismatch/Skipped(7 reasons including the controller-sentinel short-circuit, pinned via call-counter == 0), 1 const-presence test for the build.rs injection, 1 structural test at `wake_machine::tests::t5_agent_version_mismatch_maps_to_distinct_wire_code` pinning the wire-code non-collision with `livez_timeout`/`restore_backend_failed`/`internal_error`. sandbox-agent lib unchanged (245) — the `/version` endpoint with `git_commit` was already shipped.
- **Why we didn't write a wake_machine::drive() integration test**: full path requires a fake-agent bound to the StubRestoreBackend's `derive_agent_url`, which today returns `127.0.0.1:17777+vm_index` (closed port). Threading a fake-agent through would require a stub-backend rework that's orthogonal to T5. The probe-layer outcome production + wake_machine call-site mapping (which is straightforward — a 3-arm `match` on `VersionCheckOutcome`) are pinned independently.

---

## MINOR (defer to broader cleanup)

### [r32-A3 / r32-Mcc] (CLOSED 2026-05-25) `alloc_first_seen` trace lacks `sandbox_id` correlation field — parallel-CREATE correlation gap
- **Status**: **CLOSED**. Triple-confirmed by arch-r32 r32-A3, concurrency-r32 R32-Mcc, and code-quality-r33: the `submit_done` emit at `nomad_ch.rs:1226-1231` carries `sandbox_id` but its sibling `alloc_first_seen` emit at `nomad_ch.rs:3102-3106` (inside the free fn `wait_for_alloc_running`) did not. On parallel CREATEs the journal-grep workflow has to first map `job_id → sandbox_id` via a second log line before correlating the placement-window milestone with the rest of the per-sandbox emit chain — `job_id` is derivable from `sandbox_id` but not vice-versa.
- **Architectural choice**: thread `sandbox_id: &str` through the free fn rather than emit `job` only. The free fn is called from exactly two sites — `NomadCHBackend::create` (cold-boot, in `nomad_ch.rs`) and one in-module unit test — and the value is already a string-formed UUID `.simple()` at the controller's call site (matches `submit_done`'s emit shape). `sandbox_id` is also now plumbed into the three rate-limited warn macros inside the function (`JSON parse error`, `HTTP non-200`, `HTTP transport error`) so a malformed-poll or unreachable-Nomad blip on a single sandbox is grep-able by sandbox_id rather than job_id alone. No back-compat constraint on the trace shape — these are operator-only log fields.
- **Implementation**: 1 file changed (`crates/sandbox/src/backend/nomad_ch.rs`). Function signature gained `sandbox_id: &str` as the 3rd parameter; production call at L1238 passes `&sandbox_id.simple().to_string()`; test fixture at L5500 passes a sentinel 32-hex zero-string. 4 emit macros gained the `sandbox_id = %sandbox_id` field. +9/-1 LOC.
- **Test**: `cargo build -p zeroship-sandbox --tests` clean; `cargo test -p zeroship-sandbox --lib` 551 passed (unchanged — the test fixture call updated in the same diff). `git grep alloc_first_seen` confirms shape consistency across the codebase (4 production refs in `nomad_ch.rs`, all now share `sandbox_id`/`job`/`elapsed_ms` field set).
- **Source**: architecture-r32 r32-A3 (`docs/reviews/sandbox-snapshot-restore-architecture-2026-05-25-r32.md` §"r32-A3 NEW"), concurrency-r32 R32-M1-renamed-r32-Mcc (`docs/reviews/sandbox-snapshot-restore-concurrency-2026-05-25-r32.md`), code-quality-r33 carry note.

### [R33-M1] (CLOSED 2026-05-25) `vm_index_ceil` rustdoc says "default 155" but env-parse default is `20u16` (R32-M2 carry)
- **Status**: **CLOSED**. Code-quality-r33 R33-M1 (carry from R32-M2): `config.rs:357-358` rustdoc closes with `SANDBOX_NOMAD_CH_VM_INDEX_CEIL` **(default 155)** while the env-parse fallback at `config.rs:935` is `parse_env("SANDBOX_NOMAD_CH_VM_INDEX_CEIL", 20u16)?`. Pure doc rot — the "Must be ≤ 155" hard upper bound text earlier in the rustdoc (line 349 onward) is correct (subnet third-octet ceiling), but the default-value parenthetical is stale from an earlier draft that set the env default equal to the hard ceiling.
- **Implementation**: 1-line edit in `crates/sandbox/src/config.rs` — `(default 155)` → `(default 20)`. No code change; no test change.
- **Test**: `cargo build -p zeroship-sandbox --tests` clean; `cargo test -p zeroship-sandbox --lib` 551 passed.
- **Source**: code-quality-r33 R33-M1 (`docs/reviews/sandbox-snapshot-restore-code-quality-2026-05-25-r33.md` §"R33-M1").

### [R32-M1] (CLOSED 2026-05-25 r32-fixer) Stale "wrapper" rustdoc/inline comments — 25 stragglers in `crates/sandbox/src/`
- **Status**: **CLOSED**. Completes the partial R31-M1 fix at `4d10ba45` (9 surfaces cleaned). R32-M1 from code-quality-r32 flagged 18 stragglers (inline comments, error messages, test names); the full sweep landed 26 edits across 4 files. Rewrites attribute the wrapper's deleted responsibilities to the ch driver (`nomad-driver-ch`, `TaskConfig.{Net,Disks,Cmdline,Disks}` fields) and keep module-level rustdoc references as explicit `(legacy: pre-T-8 ...)` notes so the deletion history is preserved.
- **Classification breakdown**: (a) stale historical comment — 22 edits across `nomad_ch.rs` (module-level docs L50/L104, inline comments along the create/stop/snapshot paths), `restore_handler.rs` (module step 6, do_restore_inner orchestrator comment, rewrite_config_json rustdoc + inline, RealRestoreBackend doc, build_restore_nomad_job_json rationale block, UUID canonical-form comments, image-path test docs), `config.rs` (vm_index_floor/_ceil + host_fence_timeout_secs + subnet_second_octet rustdoc), `backend/mod.rs` (teardown_source_for_snapshot trait rustdoc); (b) error message — 2 edits (`nomad_ch.rs:3972` "stale agent" error + `nomad_ch.rs:7331` B15 assertion message — both now reference the ch driver's `TaskConfig.Disks` failure mode); (c) test name — 2 renames (`derive_mac_matches_wrapper_pattern` → `_matches_pinned_format`, `derive_tap_matches_wrapper_pattern` → `_matches_pinned_format`); (d) generic English "wrapper" — left alone (9 sites: compio spawn `Result` panic-catch wrapper at `nomad_ch.rs:449,4928,4929,4962`, async-signing wrapper rustdoc at `nomad_ch.rs:3728` + `k8s.rs:905`, `db.rs:994` pg driver wrapper, `db.rs:2221` back-compat wrapper, `lib.rs:108/408/455/966/971/1315/3130` builder + AEAD wrappers, `persist.rs:305` AEAD key wrapper, `preview.rs:749` test helper, `snapshot_handler.rs:436/751` ch-remote subprocess wrapper, `snapshot_store_gcs.rs:988` private wrapper, `restore_handler.rs:518/2982/3013/3832/3837/4526` C-4 helper + ClockResyncOutcome wrapper + transport-error wrapper text).
- **Source**: code-quality-r32 R32-M1 (`docs/reviews/sandbox-snapshot-restore-code-quality-2026-05-25-r32.md`).
- **Test result**: `cargo build -p zeroship-sandbox --tests` clean; lib tests 551 passed (unchanged from pre-fix; the two test renames are tracked by cargo as new test fns and the prior fns are removed in the same diff).
...
### [R10-API4] (ACTUALLY CLOSED at `00a00d01`, corrected from premature close at `528c3c44`) `readyz` 503 body aligned to §10.0 envelope — sandbox-agent side (api-surface-r10 + R21-API1)
- **Source**: 2026-05-25 api-surface-r10
- **Premature-close note**: `528c3c44` (r28 fixer) was incorrectly recorded as closing R10-API4. That commit only fixed `crates/sandbox/src/handlers.rs:132-142`, which is R12-API1's site (the sandbox controller). The actual R10-API4 site is `crates/sandbox-agent/src/handlers.rs:498-510` — a separate service with an identically-named endpoint. The two services had drifted wire shapes: controller had the §10.0 envelope; agent still emitted pre-§10.0 bare-status bodies.
- **Symptom (agent side)**: draining path returned `{"status":"draining"}`; reaper-down path returned `{"status":"reaper-down"}`; 200 path returned `{"status":"ready"}` — all bypassing `ErrorEnvelope`.
- **Fix**: draining path now routes through `error_response(SERVICE_UNAVAILABLE, "draining", "agent is draining for shutdown")`; reaper-down path through `error_response(SERVICE_UNAVAILABLE, "reaper_down", "PID 1 reaper is not healthy; agent running without zombie reaping")`; 200 body changed to `{"status":"ok"}`. Wire shapes now in sync with the controller's readyz and consistent with §10.0 contract.
- **Tests added**: `readyz_200_body_is_status_ok` + `readyz_503_draining_body_is_envelope_compliant` + `readyz_503_reaper_down_body_is_envelope_compliant` in `crates/sandbox-agent/src/handlers.rs::tests`. Agent lib: 242 → 245.
- **Verification**: `cargo build -p zeroship-sandbox-agent --tests` clean. `cargo test -p zeroship-sandbox-agent --lib` → 245 passed / 0 failed / 0 ignored. Corrected by r21 R21-API1.
...
### [R11-P1] **CLOSED at `ee702d5f`** (CRITICAL, performance-r11) Every `Database` method opens a fresh pg Pool per call
- **Source**: 2026-05-25 performance-r11 (sustained five-round carry → R26-C1)
...
- **Resolution**: see R26-C1 closure entry at file tail — per-compio-worker `thread_local!<RefCell<Option<(String, Rc<Pool>)>>>` keyed by DSN. Two cells (`POOL_APP_CELL`, `POOL_AUDIT_CELL`); GDPR uncached per § 13.2. All 42 internal call sites work unchanged via `Rc<Pool>::Deref → &Pool`.
...
### [R13-I1] **CLOSED at `ee702d5f`** (IMPORTANT, concurrency-r13) R11-P1 pool churn = correctness risk under c≥10
...
- **Resolution**: see R26-C1 closure entry at file tail — closed bundled with R11-P1, R23-P1, R25-C1 under R26-C1's thread-local Rc<Pool> refactor.
...
## r21-A1 closure (2026-05-25 — emit user_id in restore-path ChPlugin Config)

- [r21-A1] **CLOSED** — `build_restore_nomad_job_json` in `crates/sandbox/src/restore_handler.rs` was missing `user_id` from the ChPlugin `Config` block. C-7-LT-7 added `user_id` emission to the cold-boot builder (`nomad_ch.rs:2451`, commit `03d2f4a8`) but the restore-path builder was not updated. Restore is the only builder invoked during WAKE, so driver v8's per-user-home allow-list received an empty `user_id` and rejected `disks[*].path = /var/zeroship/ch/users/<user_id>/home.img`. Fix: added `"user_id": user_id` to the ChPlugin Config map in `restore_handler.rs` with a C-7-LT-7/r21-A1 comment pointing back to the cold-boot parallel. Also updated the `ch_plugin_restore_jobspec_populates_restore_from` test to assert `config["user_id"] == "usr_alice"` — this assertion was the gap that masked the bug. Field-list parity between cold-boot and restore-path builders is now identical (restore carries `restore_from` as a restore-specific field; both emit empty `pubkey_hex`; all other fields match). Driver v9 NOT required — this is sandbox-controller-side only; v8 driver consumes `user_id` correctly. Debt note: r20-A1 "three-rewriter / no-ownership-rule" surfaces here as a two-emitter problem (cold-boot + restore). A field-list contract test between the two builders would catch future divergences. Sandbox lib tests: 429 passed (assertion added to existing test, no new test function). Source: architecture-r21 r21-A1.

## R20-C1 closure (2026-05-25 — update_wake_job_state terminal-overwrite guard)

- [R20-C1] **CLOSED** — `update_wake_job_state` in `crates/sandbox/src/db.rs` issued `UPDATE sandbox.wake_jobs ... WHERE wake_id = $1` without an `AND state NOT IN ('ok', 'failed')` guard. A stale write from a racing producer (e.g., the wake-machine state-machine driver's `update_wake_job_state(row, "restoring", ...)` arriving after the takeover sweep's `claim_orphan_wake_for_recovery` had already transitioned the row to `failed`) would overwrite the terminal state with a non-terminal value, resurrecting a dead row and potentially spawning a duplicate WakeMachine or silently losing the recorded failure cause. Fix: added `AND state NOT IN ('ok', 'failed')` to the WHERE clause — once a row reaches a terminal state, no further mutations land; stale writes silently no-op (`rows_affected == 0`). The `Result<u64>` return type and callers are unchanged (the existing "0 means 404" semantic covers the "0 means stale-noop" case without modification). Rustdoc on the function updated to document the no-op semantics. Existing test `wake_jobs_update_preserves_fields_on_none` and `wake_jobs_agent_url_check_constraint_enforced` updated to drive COALESCE/CHECK assertions through non-terminal states (the terminal-immutability semantics make re-updates on terminal rows no-ops, so those sub-cases were migrated). +2 new pg-gated tests: `update_wake_job_state_after_ok_is_noop` and `update_wake_job_state_after_failed_is_noop` pin both terminal flavours. Sandbox lib tests: 429 → 429 (lib unchanged; pg-gated +2). Build clean. Source: concurrency-r20/r21 carry R20-C1.

## R21-API1 closure (2026-05-25 — sandbox-agent readyz corrects premature R10-API4 close)

- [R21-API1] **CLOSED** — api-surface r21 finding: R10-API4 was prematurely marked CLOSED at `528c3c44`. That commit fixed `crates/sandbox/src/handlers.rs:132-142` (R12-API1's site, the sandbox *controller*); the actual R10-API4 site at `crates/sandbox-agent/src/handlers.rs:498-510` (the sandbox *agent*) still emitted pre-§10.0 bare-status bodies: `{"status":"draining"}`, `{"status":"reaper-down"}`, `{"status":"ready"}`. Two services with identically-named `/readyz` endpoints had drifted wire shapes — the controller had the §10.0 envelope; the agent did not. Fix: all three response paths in `readyz()` updated to use `crate::error_envelope::error_response`; 503-draining now returns `{error:"draining",message:"agent is draining for shutdown"}`; 503-reaper-down now returns `{error:"reaper_down",message:"PID 1 reaper is not healthy; agent running without zombie reaping"}`; 200 now returns `{status:"ok"}`. Three wire-shape tests added (`readyz_200_body_is_status_ok`, `readyz_503_draining_body_is_envelope_compliant`, `readyz_503_reaper_down_body_is_envelope_compliant`) pinning the corrected shapes and asserting the old `{"status":...}` bare field is absent from error responses. Agent lib: 242 → 245. `cargo build -p zeroship-sandbox-agent --tests` clean; `cargo test -p zeroship-sandbox-agent --lib` → 245 passed / 0 failed / 0 ignored. R10-API4 entry in this file updated to reflect the actual closing commit. Source: api-surface-r21 R21-API1.

## R22-T1 + R21-API2 closure (2026-05-25 — ChPlugin Config field-list parity contract test)

- [R22-T1 + R21-API2] **CLOSED at `b6c55d93`** — Three-lens, two-round consensus (api-surface r21 R21-API2 + architecture r22 r22-A3 + test-coverage r22 R22-T1) converged on the same gap: `build_nomad_job_json_with` (cold-boot) and `build_restore_nomad_job_json` (restore-path) are two independent Config emitters with no structural enforcement of field-list parity. Twice the gap was exploited — r21-A1 (`user_id`) and C-7-LT-12a (`rootfs_source`) both landed in cold-boot but missed restore, causing cluster failures only caught by smoke-rN. Fix: new unit test `ch_plugin_config_field_list_parity` added to `crates/sandbox/src/restore_handler.rs::r12_i1_tests`. The test builds both Config blocks with their respective fixture helpers, extracts the JSON object key sets, and asserts the symmetric difference equals exactly `{"rootfs_source"}` (the sole documented intentional divergence — cold-boot uses `ZSBX_ARTIFACT_DIR` env + driver const; restore-path emits the full path explicitly per C-7-LT-12a). `pubkey_hex` and `restore_from` are present in both paths (with different values — empty string on the non-applicable side) and are therefore NOT in the diff set. Correction to the task spec: the expected diff is `{rootfs_source}` only, not `{pubkey_hex, restore_from}` — those fields ARE symmetric across both emitters. The test is purely preventative; it passes with current HEAD and will fail loudly if any future change adds a field to one emitter without the other. Sandbox lib: 431 + 1 = 432 from this commit (prior R22-I1 was 431; net after both = 433); `cargo build -p zeroship-sandbox --tests` clean; `cargo test -p zeroship-sandbox --lib` 433 passed / 0 failed / 0 ignored. Source: api-surface-r22 R22-API1 / test-coverage-r22 R22-T1 / api-surface-r21 R21-API2.

## R23-I1 closure (2026-05-25 — WakeMachine terminal-overwrite counter e2e coverage, Path B)

- [R23-I1] **CLOSED** — concurrency-r23 R23-I1 finding (`docs/reviews/sandbox-snapshot-restore-concurrency-2026-05-25-r23.md`) flagged that R22-I1's `inc_wake_terminal_overwrite_blocked()` wiring at the three `wake_machine.rs` call sites had no e2e coverage. The bare db noop tests added under R20-C1 (`update_wake_job_state_after_{ok,failed}_is_noop`) only assert that `db::update_wake_job_state` returns `rows_affected == 0` against a terminal row — they bypass the WakeMachine and therefore do NOT verify the counter actually fires from the callers. **Path B chosen explicitly**: the R22-I1 architecture (commit `f98611fb`) places the counter bump at each WakeMachine call site (`wake_machine.rs:146` for `Phase::Ok`, `:191` for `Phase::Failed`, `:528` inside `set_state`) so the paired `tracing::warn!` carries a site-specific `attempted_state` field. Moving the bump into `db::update_wake_job_state` (Path A) would erase that context — caller-site bump is a deliberate observability choice, not a missing abstraction. Path B test design therefore drives the full WakeMachine end-to-end with the `wake_jobs` row pre-flipped to a terminal state via direct `db.update_wake_job_state` (bypassing the state machine for setup only — the R20-C1 guard does not block transitions INTO a terminal). Fix: +2 pg-gated tests in `crates/sandbox/tests/sandbox_pg_e2e.rs::wake_machine_e2e` (~180 LOC total including comments): (1) `wake_machine_terminal_overwrite_failed_to_ok_bumps_counter` — pre-set row to `Failed`, drive happy-path StubRestoreBackend → machine attempts terminal `Ok`, counter strictly increases, DB row stays `Failed` with original `LivezTimeout` error_code preserved, `agent_url` not populated; (2) `wake_machine_terminal_overwrite_ok_to_failed_bumps_counter` — pre-set row to `Ok` with `agent_url=http://10.0.0.99:7000`, drive with `fail_livez=true` → machine attempts terminal `Failed`, counter strictly increases, DB row stays `Ok` with `agent_url` intact and `error_code` remaining `None`. Counter delta asserted `>= pre + 1` (NOT `== pre + 1`) — the machine issues 5-6 intermediate `set_state` calls plus the terminal write; each one no-ops + bumps when the row is already terminal. Asserting strict monotonic increase is the honest contract; `== 1` would require the machine to short-circuit on first guard fire, which it does not (and should not — intermediate writes are intentionally best-effort per `set_state`'s rustdoc). No production code touched: `wake_machine.rs`, `db.rs`, and `metrics.rs` are all unchanged; tests use the existing `StubRestoreBackend` failure knobs (`fail_livez`) and the existing `migrated_db_arc` / `make_machine` / `seed_snapshotted_row` fixture chain. Sandbox lib 433/433 unchanged (lib regression check clean); pg-gated tests +2 (89 → 91 in the `sandbox_pg_e2e` binary). Verified locally against `postgres://postgres:zeroship@localhost:5440/zeroship` (started via `docker compose up -d postgres`): both new tests PASS in 6.2s under `--include-ignored --test-threads=1`. `cargo build -p zeroship-sandbox --tests` clean (pre-existing unused-import warning on `restore.rs:43` and dead-code warning on `sweep.rs:96` unchanged). Source: concurrency-r23 R23-I1.

## R24-I1 closure (2026-05-25 — thread error_code+error_message into terminal-overwrite WARN)

- [R24-I1] **CLOSED** — code-quality-r24 R24-I1 finding (`docs/reviews/sandbox-snapshot-restore-code-quality-2026-05-25-r24.md`) flagged that the `Phase::Failed` arm in `crates/sandbox/src/wake_machine.rs` emits TWO WARN events when the R20-C1 terminal-overwrite guard fires on a `Failed` terminal write: (1) the primary WARN at `:166-172` (`"wake_machine: terminal failed"`, carrying `error_code` + `error_message`); (2) the terminal-overwrite WARN at `:184-198` (`"update_wake_job_state no-op: row already terminal (R20-C1 guard tripped)"`, carrying only `wake_id` + `attempted_state`). Operator triage of a leak event required grep-correlating the two log lines by `wake_id` to recover the error context for the second event. Fix: threaded `error_code = code.as_str()` and `error_message = %message` into the terminal-overwrite WARN at `:184-202` so a single line carries full context. Used the unsanitized `message` (not `sanitized`) to match the primary WARN at `:170` — both events route to the same operator-only journald stream; the R16-S2 sanitization is required only for the pg column (SELECT-able by other roles, retained T_KEEP=5 min) and the WARN log is operator-only per the comment block at `:159-164`. Sibling check: `Phase::Ok` terminal-overwrite WARN at `:140-145` has no error context to thread (success path); `set_state` intermediate WARN at `:521-528` already carries `attempted_state` and has no error info available (the only error source — the DB write failure itself — is covered in the Err arm). No changes there. Build: `cargo build -p zeroship-sandbox --tests` clean (pre-existing unused-import warning on `restore.rs:43` and dead-code warning on `sweep.rs:96` unchanged). Sandbox lib: 439 → 439 (no regression; no new tests added — the log content is operator-debugging output, not a critical contract; adding a `tracing_test` subscriber test would be scope creep for a 5-line WARN field addition). Diff size: +12 / -1 lines in `wake_machine.rs` (1 comment block + 2 new fields on the existing `tracing::warn!` invocation). Source: code-quality-r24 R24-I1.

## R24-T1 closure (2026-05-25 — commit polling-aware T-8b stress harness in-tree)

- [R24-T1] **CLOSED at `61492e54` + `dd2079a9`** — test-coverage r24 CRITICAL finding: `snapshot_stress.py`, the polling-aware harness that gates the T-8b nomad-driver-ch cutover stress runs, lived ONLY at `gs://suger-dev-zsbx-artifacts/stress/snapshot_stress.py`. Workers `gsutil cp`ed it at boot; no in-repo source meant the harness gating T-8b-cutover was un-reviewable, un-diff-able, and trivially replaceable on the GCS side without anyone noticing. **Two-commit fix.** Commit `61492e54`: byte-exact import of the canonical GCS object into `crates/sandbox/scripts/snapshot_stress.py`. SHA256 of the in-repo file matches the pre-existing canonical SHA exactly — `89ba229e2c8544bc648b46f4963e57cf524cd7edfd7af82093a1217afb123d43` (333 lines / 11 KiB, polling-aware CREATE → SNAPSHOT → WAKE-post+poll → STOP harness consuming the C-7-LT two-step async wake contract). Commit `dd2079a9`: (1) extended the docstring with a PROVENANCE + UPDATE PROCEDURE header documenting the in-repo file as canonical and the GCS object as a mirror, plus the three-way lockstep (edit in-repo → bump SHA → re-upload to GCS → bump `SNAPSHOT_STRESS_SHA256` literal in `gcp-worker-startup.sh`); (2) replaced the silent best-effort GCS pull of `snapshot_stress.py` in `crates/sandbox/scripts/gcp-worker-startup.sh:203-212` with a SHA-pinned pull — a mismatch now `exit 1`s with a clear "FATAL: snapshot_stress.py SHA mismatch" message naming the expected/actual SHAs and pointing at the in-repo source + UPDATE PROCEDURE. `stress_one.py` + `typed_id.py` retain the prior best-effort semantics (they are companions, not the canonical T-8b harness). **Why gsutil-with-SHA-verify (not workspace-tarball install)**: the worker VM is bare GCE with no repo checkout — there is NO existing workspace-tarball pattern in the codebase to mirror; every other in-repo asset (including `nomad-vm-wrapper.sh`) is pushed to GCS out-of-band and `gsutil cp`ed at worker boot. Mirroring the established pattern + adding SHA verification gives drift detection without restructuring the worker boot path. **Caveat / next step**: after `dd2079a9` the in-repo file SHA is `c4d2a2d4...` (because of the docstring extension), but the SHA pin in the startup script still tracks the unchanged GCS object SHA `89ba229e...`. The worker continues to boot OK against the currently-deployed GCS object; the GCS mirror is now stale relative to in-repo and needs re-upload + a pin bump at the next stress-run window (push-blocked while user is offline 2026-05-24). The drift-detection behavior — fatal-on-mismatch — is live as of `dd2079a9`. Verification: `python3 -m py_compile` on the harness clean; `bash -n` on the startup script clean; `cargo build -p zeroship-sandbox --tests` clean (pre-existing warnings on `restore.rs:43` + `sweep.rs:96` unchanged). Diff size: +333 LOC (harness import) + +88/-4 LOC (provenance header + SHA-pin install). Source: test-coverage-r24 R24-T1.

## r24-A3 + r25-A4 closure (2026-05-25 — restore-debug-playbook ADR landed)

- [r24-A3 + r25-A4] **CLOSED at `28fa64d1`** — architecture-r24 r24-A3 + architecture-r25 r25-A4 third-escalation finding: r23-A3's restore-debug-playbook ADR did NOT land between r23 and r24, then did NOT land between r24 and r25, despite three force-multiplier evidence points accumulating: (1) smoke-r22 `DeviceManager(Disk(NotFound))` reclassified "CH-internal" → "driver staging" via 5 LOC of CH stderr-tail capture (driver v11 at `6efdc42e` + pin `8b366b6d`); (2) stress-r1 `disk[1] /var/.../workspace.img does not exist (controller must stage before spawn)` reclassified "Failed tasks" rollup → "controller-staging gap" via verbatim TaskEvent observable (defense-in-depth fix at v33 `30960451`); (3) stress-r2 33% retry-win-rate (96/144 alloc submissions) reclassified v33's "staging visibility" hypothesis → "cleanup-vs-retry race" (v34 fix at `d638b10f` + `e82bffd7`). Each case: a verbatim layer-N observable resolved multi-cycle misclassification when captured BEFORE adding defense-in-depth at layer N+1. Fix: shipped `docs/decisions/2026-05-25-restore-debug-playbook.md` (193 lines markdown, no code). Six sections: Context (8 cluster cycles of "patch first, observe later" pattern between smoke-r15 and stress-r2); Decision (triage tier ladder: validator → wire → CH input contract → CH internal → agent boot, 1 diagnostic cycle per tier max, verbatim-observable-before-fix rule); Force-multiplier retros (r22-A2 CH-internal REFUTED by stderr; r23-A2 wrapper-retirement-unblocked REFUTED by stress; r24-A1 fsync-visibility REFUTED by 33% race-win rate); Observability-before-architecture rule (rejected hypothesis costs $0.20 + cluster slot per refutation cycle vs ~5 LOC stderr-capture pattern + one capture cycle); Stress-as-gate rule (smoke validates state-space traversal at N=1, stress validates state-space transitions under N>1 retry + vm_index reuse; cutover-class decisions require stress GREEN, not smoke GREEN); Cross-references (all cited commits + sibling kernel-state-surface-inventory ADR). cargo build clean (no code change). Source: architecture-r23 r23-A3, architecture-r24 r24-A3, architecture-r25 r25-A4.

## R25-T4 + R24-A1 closure (2026-05-24 — host_dir GC sweeper unit-test matrix + grace tighten)

- [R25-T4] **CLOSED at `901dfbf2`** — test-coverage-r25 R25-T4 follow-up to bundle commit `e82bffd7` (T-8b-stress-r2 controller v34 sweeper) which explicitly deferred the eligibility-matrix unit tests to a follow-up. `run_host_dir_gc_once` is destructive (`rm -rf <host_state_dir>/<sandbox_id>`) and gates the action on six checks (FS1 users-subdir, FS2 typed-id name, FS3 is_dir, FS4 mtime grace, DB1 sandbox row state, DB2 pending wake_jobs) — pre-closure these had zero lib-test backstop. Fix: refactor the inner loop to delegate FS gates to a pure classifier `classify_host_dir_entry(...) -> HostDirEntryDecision` and DB gates to `host_dir_eligible_by_db(row, has_pending_wake) -> bool`, then add 12 unit tests pinning each gate plus the grace-boundary inequality and clock-skew (saturating_sub) semantics. The pure helpers sidestep the "`Database` is a 4 KLOC struct holding a real pg pool" mock problem — the pg-gated end-to-end path stays covered by `tests/sandbox_pg_e2e.rs`. Lib test count: 465 → 478 (+13). Source: test-coverage-r25 R25-T4.

## r3-A closure (2026-05-24 — Nomad node-affinity Constraints to fix cross-node placement)

- [r3-A] **CLOSED at `883df7fe` + `9b623f44` + `d71f1a8c`** — T-8b-stress-r3 RED at 1/60 (78% cross-node placement failure at WORKER_COUNT=3). Root cause (verified by prior diagnostic agent): the controller stages `workspace.img` on its LOCAL filesystem then submits a Nomad job with NO Constraints/Affinity; Nomad's scheduler picks any worker; on the 2/3 of allocs that landed on a different worker, the driver's `assert_disk_image_present` stat'd the path on THAT node's local fs and ENOENT'd. Smoke-r* ran WORKER_COUNT=1 (controller+driver collocated) so it stayed GREEN; stress-r* ran WORKER_COUNT=3 so 2/3 of allocs landed cross-node and the file vanished. Wake path had the same shape — the diagnostic report flagged 11/12 wake failures with matching signature. **Fix shape: Option 1 (Constraints block).** Three commits per the task brief: (1) `883df7fe` "sandbox/lib: cache local Nomad node_id at boot (r3-A precursor)" — adds `fetch_local_nomad_node_id` + pure `parse_nomad_agent_self_node_id` helpers in `backend/nomad_ch.rs` (accepts both lowercase Nomad-1.x and PascalCase older-doc shapes), `AppState.local_nomad_node_id: Option<String>` field with boot-time fetch + WARN-and-continue on failure (boot does NOT block on Nomad agent reachability), new `sandbox_nomad_node_id_lookup_failures_total` counter at `metrics.rs` for operator alerting; (2) `9b623f44` "sandbox/nomad-ch: emit Constraints block pinning placement to staging worker (r3-A cold-boot)" — `NomadCHBackend.local_nomad_node_id` field + `with_local_nomad_node_id` builder, `Backend::from_config_full(cfg, persist, node_id)` new entry point (existing `from_config_with_persist` / `from_config` thread None for source-compat), `build_nomad_job_json[_with]` extended with trailing `Option<&str>` argument emitting `{"Constraints": [{"LTarget": "${node.unique.id}", "Operand": "=", "RTarget": <node_id>}]}` at the Job level when Some, omitting entirely when None; (3) `d71f1a8c` "sandbox/restore-handler: emit Constraints block pinning placement to staging worker (r3-A restore-path)" — `RealRestoreBackend.local_nomad_node_id` + `with_local_nomad_node_id` builder, `build_restore_nomad_job_json` extended identically, `AppState::from_config` installs the value alongside the existing `with_wake_response_mode` call. **Cross-emitter parity contract**: new test `node_affinity_constraints_parity_between_cold_boot_and_restore_emitters` (mirrors R22-T1's spirit but for Job-level Constraints) asserts byte-identical Constraints emission on both Some and None inputs — a regression that ships the constraint on one path but not the other would re-open the race on the dropped side. **Strict equality (Operand = "=")** is deliberate: a misconfigured node_id MUST fail Nomad scheduling, not silently fall back to random placement. Lib tests: 459 → 481 (+22 across all three commits — 5 parser pins, 4 metric/counter + builder round-trip, 4 cold-boot Constraints, 3 restore-path Constraints + parity, balance from R25-T4 closure that landed mid-task at `901dfbf2`). Build clean. **Fallback shape preserved**: when boot-time `/v1/agent/self` fetch fails (Nomad unreachable, non-200, parse error, missing `stats.client.node_id`) the boot logs a WARN naming addr+error, bumps the counter, and leaves the field as None — controller continues to operate with pre-r3-A random placement until next restart against a reachable Nomad agent. Source: T-8b-stress-r3 diagnostic report (the task brief).

- [R24-A1] **CLOSED at `34b52cf1`** — performance-r24 R24-A1: perf-r24 (R24-P1) instrumented run found the 1-hour `HOST_DIR_GC_GRACE_SECS` default stranded ~130 GB steady-state at the 10 creates/min mandate target (10/min × 60 min × ~50-200 MB/dir). Fix: tighten default 3600 s → 600 s (10 min). Steady-state math: 10/min × 10 min × ~50-200 MB ≈ 5-20 GB, well under disk-pressure (6× cut). The grace window protects the retry-CREATE-vs-StartTask race; the retry race window is <10 s typical / ~30 s upper bound under back-pressure, so 600 s preserves a 20-60× safety margin. The 60 s floor (`HOST_DIR_GC_GRACE_FLOOR_SECS`) is unchanged; env override `SANDBOX_HOST_DIR_GC_GRACE_SECS` already in place (`read_u64_env` at sweep.rs:1249). Adds `host_dir_gc_grace_default_and_floor_pinned` test to catch silent drift. Source: performance-r24 R24-A1.

## r24-A2 + r25-A2 closure (2026-05-25 — kernel-state surface inventory ADR landed)

## R22-S1 closure (2026-05-24 — sanitize widening Mode A: filesystem paths + typed-IDs)

- [R22-S1] **CLOSED at `7647cd4d`** — security-r26 R22-S1 ELEVATED-to-ACTIVE finding: typed `StagingPathMissing` closure (`022f778a`) addressed the CONTROLLER-preflight fork of the `wake_jobs.error_message` path-leak via `RestoreHandlerError::StagingPreflight`'s path-free `Display` impl, but the v34 `d638b10f` / `3d431eb8` verbatim driver-msg propagation reopened the leak on the OTHER fork: `TaskEvent.DisplayMessage` strings (driver-side preflight failures, e.g. `disk[1] /var/zeroship/ch/<sid>/workspace.img does not exist`) extract through `extract_failed_task_event_msgs` → `Err(composed_string)` from `wait_for_alloc_running_blocking` → `SubmitRestoreError::Other(s)` → `RestoreHandlerError::Backend(s)` → `rollback_and_classify` → `Phase::Failed { message: err.to_string() }` → `sanitize_error_message` (pre-fix: IPv4/IPv6 only, no path/typed-id stripping) → `wake_jobs.error_message`. With T1's `AdminRole::ReadOnly` bearer landed (`97fcbcda` + `7b5d84f5`), RO admins could read `/var/zeroship/ch/<sid>/workspace.img` paths and `sbx_<base62>` / `wak_<base62>` typed-IDs via `GET /admin/sandboxes/{id}/wake/{wake_id}`. **Mode A fix**: extend `sanitize_error_message` with two new passes in the existing hand-rolled byte-scan pipeline (matches `match_rfc1918_at` / `strip_ipv6_link_local` style; deliberately no `regex` crate dep — avoids T-8b-stress-sprint churn): (1) `strip_filesystem_paths` — whitelist-by-prefix on the three roots the controller + driver write to with tenant/sandbox topology: `/var/zeroship/` (workspace.img, home.img, snapshot artifacts), `/opt/nomad/` (alloc dirs, task_dir, secrets), `/etc/zeroship/` (keys, certs, TLS material). Non-secret absolute paths (`/usr/local/bin/...`, `/tmp/...`) deliberately NOT redacted so binary-path errors remain operator-readable. Path body consumed up to whitespace / quote / comma / semicolon / closing-bracket so trailing prose stays intact. Replaces matched span with `<redacted-path>`. (2) `strip_typed_ids` — matches the canonical `^[a-z]{3}_[A-Za-z0-9]{22}$` shape from `crates/core/src/typed_id.rs` (`sbx_`, `wak_`, `usr_`, `app_`, `ses_` prefixes); length range 18-32 is drift-tolerance against future prefix-encoding-width changes; word-boundary semantics on both ends so substring matches inside larger identifiers are excluded. Replaces matched span with `<redacted-typed-id>`. **Composition order**: URL → RFC1918 → IPv6-LL → fs-paths → typed-IDs. Filesystem paths intentionally before typed-IDs so a path like `/var/zeroship/ch/<sbx-typed-id>/workspace.img` redacts as one `<redacted-path>` token, not two adjacent redactions (the typed-id pass would otherwise match the embedded base62 segment). **Why Mode A (not B or C)**: Mode A is the highest-leverage single patch — closes BOTH the previously-closed R25-S1 controller-preflight axis AND the newly-active driver-msg axis at the SAME write site (`update_wake_job_state` already wraps the input in `sanitize_error_message` at `wake_machine.rs:165`). Mode B (split tracing-verbatim + column-stripped sinks) and Mode C (route differently per sink) require structural rework; Mode A is ~30 LOC of helpers in the existing pipeline. **Path-prefix whitelist** (what gets masked): `/var/zeroship/` + `/opt/nomad/` + `/etc/zeroship/`. **Typed-ID regex pattern**: `[a-z]{3}_[A-Za-z0-9]{18,32}` with word-boundary on both ends; canonical width is exactly 22 per `typed_id::roundtrip_typed_id` / `typed_id::wake_prefix_is_three_chars`. **Tests added**: +7 in `wake_machine::tests`: `sanitize_strips_var_zeroship_path` (the driver-msg verbatim path), `sanitize_strips_opt_nomad_alloc_dir` (Nomad alloc-dir leak), `sanitize_strips_etc_zeroship_config` (key-material path), `sanitize_strips_typed_ids` (covers all five canonical prefixes via a `for` loop), `sanitize_preserves_non_secret_paths` (whitelist-by-prefix guarantee — `/usr/local/bin/...` + `/tmp/...` remain unredacted), `sanitize_strips_combination` (path + typed-id + IP in one msg, all three pass-orders pin), `sanitize_idempotent` (`sanitize(sanitize(x)) == sanitize(x)` across four input shapes — the redaction tokens themselves must not match any pattern). Sandbox lib tests: 481 → 488 (+7). Build clean (no new warnings). Source: security-r26 R22-S1 (ELEVATED via d638b10f/3d431eb8 driver-msg propagation; R25-S1's StagingPreflight closure does NOT cover this fork).

## R26-I1 closure (2026-05-25 — collapse SnapshotRowMeta / WakeSnapshotMeta duplication)

- [R26-I1] **CLOSED at `6c475c30` + `b5ec01a1`** — code-quality r26 IMP, longest-running open code-quality carry (first surfaced in r21-A1-era, persisted through r22-A4 + r24-A4 + r25 → r26-I1). `read_snapshot_row` had two structurally near-identical implementations: `crates/sandbox/src/restore_handler.rs:748-806` (`SnapshotRowMeta` with `{artifact_path, sha256, vm_index, user_id}`) and `crates/sandbox/src/wake_machine.rs:727-769` (`WakeSnapshotMeta` with `{sha256, vm_index, user_id}` — strict subset of the restore_handler shape). Three rounds of "phase 5 will fix this" never landed; the proposal § 7 deferred deletion of the sync path was reasoning the wake-machine clone could be retired only after the sync path went, but the bump-visibility-and-share path collapses the duplication earlier without coupling the two lifecycles. **Two-commit mechanical fix.** Commit `6c475c30`: `SnapshotRowMeta` struct + four fields (`artifact_path`, `sha256`, `vm_index`, `user_id`) + `read_snapshot_row` function bumped from `pub(super)` / module-private to `pub(crate)` in `restore_handler.rs`; rustdoc on both items extended to point at the R26-I1 closure rationale (the `artifact_path` field the wake-path does not consume costs one `String` per wake read — negligible vs the DRY win). No call-site changes in this precursor. Commit `b5ec01a1`: wake_machine.rs:267 call site swapped to `crate::restore_handler::read_snapshot_row(...)`, mapping `RestoreHandlerError`'s `thiserror::Error` Display impl into the existing `Phase::Failed { code: WakeErrorCode::Internal, message: format!("read_snapshot_row: {e}") }` shape (wire-identical to the prior bespoke `Result<_, String>` shape); the local `WakeSnapshotMeta` struct + 36-line `read_snapshot_row` mirror at :727-769 deleted; replaced by a 9-line provenance comment naming the precursor commit + the `artifact_path` extra-field cost. **Field-level divergence handling**: the two structs were strict subset/superset, not divergent — superset shape wins; wake-path silently ignores the extra `artifact_path`. This matches the architecture r24-A4 note flagging the drift had reached one column. `sandbox_id_typed` helper retained (still used at 4+ call sites upstream; the deleted reader was one of five consumers, not the only one). **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only the pre-existing `restore.rs:43` unused-import + `sweep.rs:96` dead-code warnings, neither touched here). `cargo test -p zeroship-sandbox --lib` 488 passed / 0 failed / 1 ignored — identical to baseline at HEAD `d0d7abd6` (R26-I1 is pure refactor, no new tests required). `grep -rn 'WakeSnapshotMeta\|SnapshotRowMeta' crates/sandbox/` confirms exactly one struct definition (`pub(crate) struct SnapshotRowMeta` in restore_handler.rs) + one `read_snapshot_row` function remain across the sandbox crate; remaining matches are rustdoc references in both files. **Net diff**: -56 / +21 in wake_machine.rs (35-line DRY win) + 23 inserts / 6 deletes in restore_handler.rs (visibility-bump rustdoc only) = workspace-wide net -18 LOC. Source: code-quality-r26 R26-I1 (carries r21-A1-era / r22-A4 / r24-A4 / r25).

## r27-S1 + r27-M1 + r27-M2 closure (2026-05-24 — security r27 bundle)

- [r27-S1] **CLOSED at `069dd277`** — security-r27 Guard A: r3-A's Job-level Constraints emit (`9b623f44` + `d71f1a8c`) pins each alloc to the node whose Nomad agent reports it via `/v1/agent/self`, but a tampered local Nomad agent could return any node_id (DoS the cluster) and a misconfigured `NOMAD_ADDR` pointing at a remote Nomad would emit Constraints keyed off a foreign-cluster node_id (allocs land cross-cluster, ENOENT on driver-stat). Both attack vectors closed by gating boot on `nomad_addr.host` resolving to one of `{localhost, 127.0.0.0/8, ::1}`. Matches r3-A's strict-equality Constraints choice: fail-CLOSED, never fall back to random placement. Hand-rolled host parser (no `url` crate dep — sandbox crate already validates the scheme prefix). 10 unit tests pin the host parser: 127.0.0.1 / 127.0.0.2 (RFC 1122 /8) / [::1] bracketed v6 / localhost case-insensitive / no-port / with-path accepted; remote DNS / 10.0.0.5 / 2001:db8::1 rejected with an r27-S1 marker in the error message. Lib test count: 488 → 498 (+10).

- [r27-M1] **CLOSED at `4f0f2259`** — security-r27 MINOR: R22-S1's sanitize widening (`7647cd4d`) covered `/var/zeroship/`, `/opt/nomad/`, `/etc/zeroship/` but missed three leak vectors: (1) `/var/lib/zeroship/` — the default `NomadCHConfig::runtime_dir` (vmlinuz / rootfs source / kernel blobs); a driver "open /var/lib/zeroship/ch/vmlinuz" error would have leaked the runtime layout. (2) `/run/zeroship/` — per-VM CH api-socket location; a CH connect-refused error leaks both the socket path AND the embedded sandbox-id (doubly bad: enumerable + correlatable with a wake_job). (3) Hyphenated UUID (RFC 4122 `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` 8-4-4-4-12) — Nomad quotes `alloc_id` verbatim into errors; an RO admin reading `wake_jobs.error_message` could enumerate alloc IDs across tenants. Fix: extend `strip_filesystem_paths` ROOTS table with the two new path prefixes (longest-prefix-first discipline preserved) and add a `match_hyphenated_uuid_at` helper to `strip_typed_ids` enforcing the strict 8-4-4-4-12 group widths; both share the existing `<redacted-typed-id>` / `<redacted-path>` tokens so the idempotency guarantee carries over. **Judgment call**: hyphenated UUIDs extend the existing typed-id pass (same word-boundary handling, same redaction token, identical privacy rationale) rather than a separate strip pass — net 35 LOC added vs ~80 if duplicated. **Known limitation**: an operator-overridden `SANDBOX_NOMAD_CH_HOST_STATE_DIR` outside the whitelist roots still leaks. Closing that requires plumbing runtime config into the sanitizer; documented in the `strip_filesystem_paths` rustdoc. 5 new tests: `sanitize_strips_var_lib_zeroship_path`, `sanitize_strips_run_zeroship_path`, `sanitize_strips_hyphenated_uuid_id` (lowercase + uppercase + mixed), `sanitize_preserves_uuid_substring_inside_longer_identifier` (word-boundary on trailing alnum), `sanitize_preserves_uuid_with_wrong_group_widths` (strict 8-4-4-4-12). Lib test count: 498 → 503 (+5).

- [r27-M2] **CLOSED at `dfccd049`** — security-r27 MINOR: `extract_failed_task_event_msgs` in `backend/nomad_ch.rs:2896` truncated DisplayMessages >2 KiB via `&trimmed[..PER_TASK_CAP]` — byte-indexed slice with NO char-boundary check. A driver-emitted error containing a multi-byte UTF-8 codepoint straddling byte 2048 would panic at the slice constructor. Fix: mirror the canonical `is_char_boundary` decrement loop from `sanitize_error_message` (`wake_machine.rs:811`) — walk down byte-by-byte until the boundary lands on a valid char, then slice. Cost: a handful of byte comparisons per truncation event (the existing `extract_failed_task_event_msgs_caps_oversized_message` test is the only fixture that exercises this path today; truncations are rare). Regression test `extract_failed_task_event_msgs_truncate_handles_multibyte_at_boundary` builds a body with `€` (`E2 82 AC`) straddling byte 2048 — 2047 X's + € + 1000 Y's — asserts upfront the byte is mid-codepoint (so a silent regression can't pass), then confirms post-fix that no panic occurs and the body slice ended just before `€` (decrement found byte 2047 as the nearest valid boundary). Lib test count: 503 → 504 (+1).

## r24-A2 + r25-A2 closure (2026-05-25 — kernel-state surface inventory ADR landed)

- [r24-A2 + r25-A2] **CLOSED at `3e853cc6`** — architecture-r24 r24-A2 + architecture-r25 r25-A2 pre-cutover finding: T-8b-cutover (wrapper retirement + `SANDBOX_TASK_DRIVER=ch_plugin` default flip) blocked because driver-owned kernel-state surfaces beyond tap (cgroup, mount-ns, vsock CID, jailer chroot, PID files, ...) were unaudited. r25-A2 escalated: v34's `run_host_dir_gc_once` (`e82bffd7`) and v14's `tapsOrphanedTotal` (`177ff165`) close 2 of N surfaces via the right architectural shape (per-surface sweeper), but the bundle made the shape choice without an ADR codifying it — future surfaces inherit the per-surface explosion path by mimicry. Fix: shipped `docs/decisions/2026-05-25-kernel-state-surface-inventory.md` (184 lines markdown, no code). Six sections: Context (stress-r1 / stress-r2 surfaced two leaks the wrapper handled implicitly — host_dir cleanup-vs-retry + tap residue); Decision (per-surface sweeper pattern NOT unified reaper — rationale: each surface has different reap mechanism + grace policy; unified reaper would force cross-surface ordering dependencies that change failure semantics under partial-failure); Kernel-state surface inventory table (2 CLOSED: tap, host_dir; 5 OPEN: cgroup, mount-ns, vsock CID, jailer chroot, PID files; 4 N/A: systemd transient units, iptables/nft, loop devices, network namespaces — with re-audit triggers per N/A row); Per-surface closure roadmap (four-part deliverable per OPEN surface: EEXIST-safe re-entry test mirroring `TestDestroyTask_DefensiveTapCleanup_*` + `{surface}_orphaned_total` counter + sweeper task mirroring `spawn_host_dir_gc` + grace policy decision); Cutover gate (T-8b-cutover BLOCKED until ≥4 of 5 OPEN surfaces CLOSED with the four-part deliverable; closure order by risk: cgroup → PID files → mount-ns → jailer chroot → vsock CID); r19-A1 leak ledger demotion folded in (REMOVE `sandbox_vm_index_leaks_total` per 11/15-cycle dormancy, ADD `host_dir_leaked_total` + `host_dir_reaped_total` counter pair on v34 sweeper, ADD `{surface}_orphaned_total` per OPEN surface as those surfaces close — per r25-A5 / r25-A6 carry); Cross-references (all cited commits + sibling restore-debug-playbook ADR). cargo build clean (no code change). Source: architecture-r24 r24-A2, architecture-r25 r25-A2, architecture-r25 r25-A5 (leak-ledger demotion), architecture-r25 r25-A6 (host_dir counter symmetry).

## r27-A1 + r27-A4 + r26-A2 closure (2026-05-25 — staging-locality ADR landed)

- [r27-A1 + r27-A4 + r26-A2] **CLOSED at `bbadbe68`** — architecture-r27 r27-A1 CRITICAL + architecture-r27 r27-A4 + architecture-r26 r26-A2 third-escalation finding: five rounds of T-8b cluster stress (r1=2/60, r2=2/60, r3=1/60, r4=3/60, r5=3/60) each peeled exactly one defect layer beneath the prior cycle's fix; ~290 LOC of layer-peel landed across `3d03cb90` (driver v13 tap pre-delete), `d638b10f` + `e82bffd7` (controller v34 host_dir leak + sweeper), `177ff165` (driver v14 defensive tap cleanup), `883df7fe` + `9b623f44` + `d71f1a8c` (r3-A node-affinity Constraints), `05440498` (driver v15 netdev release poll), `e7ce7f1f` + `9af429c7` + `1e8fa7e8` (driver v16 r4-A reap-wait + counter). r27-A1 made the diagnostic explicit: every fix defends the same architectural assumption — the controller stages disk images on its LOCAL filesystem and the driver consumes from the SAME LOCAL filesystem — and the cumulative shape names a structural problem the patches do not solve. r27-A4 carried the break-even argument: ~230 LOC of structural rewrite (Option C — driver-side staging) replaces ~290 LOC of layer-peel AND eliminates four candidate-future-layers (SNAPSHOT-side reap race, workspace.img lock-mode collision, cross-sandbox SNAPSHOT/STOP race, +1 unknown) in one pass; the r24-A1 typed staging schema half (`022f778a`) was the prerequisite and HAS landed, so the original "biggest rewrite, depends on the schema half" framing from r3-A no longer applies. r26-A2 (node-affinity-trade ADR) folded in per r27 cross-lens consensus — the trade is a CONSEQUENCE of choosing Option A, not a standalone decision. Fix: shipped `docs/decisions/2026-05-25-staging-locality.md` (408 lines markdown, no code). Sections: References (five-cycle commit ledger cited verbatim), Status (ACCEPTED — Option C), Context (the five-layer ladder table with verbatim layer-N observable + fix-commit per row), Decision (three options A/B/C with explicit pros/cons + per-failure-class collapse rationale + the "why C, not A or B" argument), Migration plan (five phases: this ADR / driver-side `stageDiskImages` observability / controller cutover with `RealRestoreBackend::try_create` → no-op + sweeper rustdoc rewrite / cluster smoke + stress falsification gate / patch-deprecation pass with per-patch keep-or-remove disposition table), Trade-offs (six named consequences: fail-fast-at-start-time instead of submit-time, wake/restore stages from snapshot artifacts via `TieredSnapshotStore`, host_dir sweeper retains role with rustdoc-only update, r3-A Constraints become defense-in-depth, structured-error sanitizer unchanged, snapshot manifest gains `rootfs_source` content-address field), Risks (HIGH new bug class post-cutover, MEDIUM ~230 LOC underestimate, MEDIUM snapshot-side latency on L1 miss, LOW driver-fs quota under cross-tenant contention, LOW staging-vs-Nomad-task-start-timeout race), Cross-references (all sibling ADRs + the five cluster-stress reviews + the prerequisite schema commit). **Judgment call — bundled migration plan, not deferred to a separate ADR**: the migration phases 2-5 sit inside this ADR rather than a separate `staging-locality-migration.md` because the phase ordering and the per-patch keep/remove disposition table ARE part of the architectural decision (Option C without phase 5's deprecation pass leaves the layer-peel defense-in-depth as dead code; Option C without phase 4's stress-gate is back to "structurally unblocked on smoke" — the very category error r23-A2's retro warned against in the playbook ADR). Splitting them would re-introduce the structural-equivalence gap. **Judgment call — Option C over B**: explicitly named in the Decision section. B (shared filesystem) trades the architectural rewrite for an infrastructure rewrite (NFS server SPOF, mount lifecycle, cross-worker sweeper coordination) and hides failure-mode discovery behind NFS-class failure modes that don't compose with the existing structured-error pipeline. C collapses four failure classes via the same per-surface sweeper pattern the kernel-state-inventory ADR codified — it's a smaller surface-of-novelty than B. cargo build clean (no code change). Source: architecture-r26 r26-A2, architecture-r27 r27-A1, architecture-r27 r27-A4.

## R26-I2 + R25-I1 + R23-A2 closure (2026-05-24 — spawn_blocking try_create sync-IO body)

- [R26-I2 + R25-I1 + R23-A2] **CLOSED at `73725aa3`** — performance r23+r25+r26 sustained carry (perf-r26 `docs/reviews/sandbox-snapshot-restore-performance-2026-05-25-r26.md:50`): `try_create`'s step-3 sync-IO bundle (`std::fs::create_dir_all` ×2 + `create_ext4_image_if_missing` ×2 — the latter internally spawns `truncate`+`mkfs.ext4` subprocesses and runs `fsync_dir` on the parent) ran inline on the calling ntex worker thread at `crates/sandbox/src/backend/nomad_ch.rs:670`. Cold-boot first-sandbox-per-user pegged a ntex worker for ~3-5s on mkfs metadata writes; under c=20 stress (post r3-A node-pin, where all CREATEs land on one controller's pool of 4-8 ntex workers) the pool saturated after 4-8 concurrent CREATEs and remaining requests queued behind the per-cold-boot block — p99 sibling-request add ~9-15s. **Fix shape (~40 LOC)**: wrap the entire sync-IO bundle in `compio::runtime::spawn_blocking` so the ntex worker is freed during the mkfs subprocess wall. Closure returns `Result<PathBuf, String>` (the `workspace_img` path the post-IO step 5 job builder needs). JoinError flattened with the existing `unwrap_or_else(|p| Err(format!("spawn_blocking panic: {p:?}")))` pattern (matches `snapshot_handler.rs:366` / `wake_machine.rs:454`). **CreateGuard boundary choice (Option 1 from the task brief)**: only owned `PathBuf` clones of `host_dir` and `user_home_img` cross the spawn_blocking boundary; the `&mut CreateGuard` reference stays on the calling thread; `guard.host_dir_created = true` flips AFTER spawn_blocking returns Ok. This preserves the existing rollback contract (IO failure → guard armed with `host_dir_created=false`, the v34 sweeper-owned invariant leaks the dir deliberately if any partial state exists; mkfs success but later step fails → guard's existing arming + Drop path tears down via `detach_isolated`). Threading `CreateGuard` ownership into the closure was rejected: the guard holds `Arc<Mutex<VmIndexAllocator>>` (Send-clean) + a `&mut` reference (not Send), and the post-IO half of try_create still needs the guard for `job_submitted = true` after `submit_nomad_job` succeeds — round-tripping ownership in and out of the closure would force splitting try_create into pre-IO + IO + post-IO halves for no gain. **Per-CREATE wall change**: zero (same subprocess walls, just on a spawn_blocking pool thread); net effect is unblocking sibling ntex workers, not faster CREATEs. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only the pre-existing `restore.rs:43` unused-import + `sweep.rs:96` dead-code warnings; neither touched). `cargo test -p zeroship-sandbox --lib` 504 passed / 0 failed / 1 ignored — identical to baseline. No new tests added: the behavior contract is preserved (same call shape, same error mapping, same guard lifecycle); an "off-thread invariant" pin test would require a slow-mkfs fixture (the function spawns real `mkfs.ext4` subprocesses, not mockable without a test-only seam) and would only re-prove the documented spawn_blocking shape. **Net diff**: +39 / -13 LOC in `crates/sandbox/src/backend/nomad_ch.rs`. Source: performance-r26 R26-I2 / performance-r25 R25-I1 / architecture-r23 R23-A2 (sustained 3-round carry).

## R26-API2 closure (2026-05-25 — controller `/metrics` Prometheus exporter landed)

## R27-M2 closure (2026-05-24 — bytes-as-char Latin-1 cast in six sanitize-strip sites)

- [R27-M2] **CLOSED at `821cc9bd`** — code-quality-r27 LATENT (`docs/reviews/sandbox-snapshot-restore-code-quality-2026-05-25-r27.md`): each of the six error-message strip passes in `crates/sandbox/src/wake_machine.rs` — `strip_agent_urls` ×2 fallthrough branches (`:932`, `:938`), `strip_rfc1918` (`:982`), `strip_ipv6_link_local` (`:1039`), `strip_filesystem_paths` (`:1112`), `strip_typed_ids` (`:1221`) — advanced its non-match branch via `out.push(bytes[i] as char); i += 1`. The cast `bytes[i] as char` for any `b > 0x7F` yields the matching Latin-1 codepoint rather than the actual UTF-8 character; a 3-byte glyph like `€` (`E2 82 AC`) would round-trip as three separate Latin-1 codepoints `â‚¬`. **Latent today**: Nomad bodies are JSON / UTF-8 passthrough and observed driver-emitted msgs are 100% ASCII, so the corruption never triggers in production traffic. **Why fix now**: r27-M2 (the prior r27 round, Aug 2026) added `is_char_boundary`-aware truncation at the tail of `sanitize_error_message`; the strip passes in the same file had not gotten the same treatment, and code-quality-r27 flagged the asymmetry between sibling functions. **Fix shape (Pattern B — minimal change)**: introduce a free helper `utf8_char_len_at(bytes: &[u8], i: usize) -> usize` that derives the codepoint byte-width from the leading-byte high bits (1/2/3/4 for `b < 0x80` / `< 0xC0` continuation defensive / `< 0xE0` / `< 0xF0` / else); replace each `out.push(bytes[i] as char); i += 1` with `let c_len = utf8_char_len_at(bytes, i); out.push_str(&msg[i..i + c_len]); i += c_len`. Matcher signatures (which key on ASCII byte-prefixes like `b"http://"`, `b"/var/zeroship/"`, `[a-z]{3}_`) are unchanged — only the fallthrough branches advance by codepoint instead of by byte. **Pattern A rejected** (rewrite outer loop as `char_indices()`): every matcher would need re-shaping to accept char-aware indices, with no functional benefit since all match patterns are pure ASCII anyway. Pattern B keeps the existing structure and is the smaller diff. **Defensive helper branch (continuation byte → return 1)**: cannot fire given the precondition that callers only invoke the helper on the fallthrough branch where `i` was advanced by either a matched ASCII prefix length or by the previous codepoint width; included so the outer loop terminates even if a future caller violates the precondition. **§10.0 envelope**: unchanged — this is internal sanitize correctness, no wire-format / DDL / config-schema change. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only pre-existing `restore.rs:43` unused-import + `sweep.rs:96` dead-code warnings; neither touched). `cargo test -p zeroship-sandbox --lib` 518 passed / 0 failed / 1 ignored (512 baseline + 6 new) — all 27 existing `sanitize_*` cases green. **New tests** (six `sanitize_*_preserves_unicode_*` cases, one per site): `sanitize_strips_path_preserves_unicode_suffix` (site 5 — filesystem-path fallthrough), `sanitize_strips_typed_id_preserves_unicode_suffix` (site 6 — typed-id fallthrough), `sanitize_strips_uuid_preserves_unicode` (site 6 — hyphenated-UUID branch), `sanitize_strips_rfc1918_preserves_unicode` (site 3), `sanitize_strips_ipv6_ll_preserves_unicode` (site 4), `sanitize_idempotent_with_unicode` (sites 1+2 in `strip_agent_urls` + full-composition round-trip across all passes; asserts `€` survives byte-identical and `sanitize(x) == sanitize(sanitize(x))`). Each test uses `€` (the canonical 3-byte UTF-8 test glyph — all three bytes are `> 0x7F`, so a single Latin-1 byte-cast would corrupt every one). **Net diff**: +151 / -12 LOC in `crates/sandbox/src/wake_machine.rs` (helper + 6 site rewrites + 6 tests). No clippy run (clippy not available in this Nix toolchain — `cargo --version` 1.94.0 stable, no clippy component); the verification stack is build + lib tests + grep gate (`grep -n "as char" wake_machine.rs` → 0 matches outside the helper rustdoc reference). Source: code-quality-r27 R27-M2.

## R27-I1 closure (2026-05-24 — BackendBuilder replaces from_config* telescoping cascade)

- [R27-I1] **CLOSED at `df06d172`** — code-quality-r27 IMPORTANT (sustained 2-round carry: R26-I2 → R27-I1, `docs/reviews/sandbox-snapshot-restore-code-quality-2026-05-25-r27.md:16`): `Backend::from_config` → `from_config_with_persist` → `from_config_full` was a 3-level telescoping constructor with ~17 LOC of pure delegation across `crates/sandbox/src/backend/mod.rs:188-234`. The cascade had not grown in r27 but was structurally unchanged; arch-r27 r27-A1 (staging-locality, `driver_materializes_disks: bool`) + r26 VFIO-handoff + r26 tap-leak each name a plausible-near-term 4th orthogonal field that would force a 4-level constructor. Code-quality-r27's break-even argument: ~25-30 LOC builder absorbs every future orthogonal field as a `.with_*()` setter without re-shaping any call site. **Fix shape (per AGENTS.md "pre-launch no back-compat" — old constructors deleted in the same PR, no `#[deprecated]` aliases)**: new `BackendBuilder<'a> { cfg, persist: Option<Arc<Persistence>>, local_nomad_node_id: Option<String> }` with `Backend::builder(&cfg)` entry point + `.with_persist(p: Arc<Persistence>)` / `.with_local_nomad_node_id(id: String)` setters that take `T` (not `Option<T>`) so callers only invoke them when they have a value; `.build() -> Result<Backend, String>` materialises the variant based on `cfg.backend`. Missing optional fields fall through as `None` — same semantics as before. 12 call sites migrated: production boot path (`crates/sandbox/src/lib.rs::AppState::from_config` — the `from_config_full(&cfg, persist, node_id)` site, now an if-let chain over the two Options into the builder); 4 in-lib test fixtures (admin_token_setter_tests, persist_setter_tests, with_config_replaces_existing both branches); 7 e2e tests (sandbox_pg_e2e ×3, sandbox_typed_id_e2e, sandbox_preview_e2e, sandbox_preview_ws_e2e, sandbox_preview_share_e2e — the only `from_config_with_persist` site, threaded as `if let Some(p) = persist { b = b.with_persist(p); }`, sandbox_admin_e2e ×2); 2 examples (stress_e2e, lifecycle_e2e). Rustdoc references to the deleted names updated: `config.rs` "network probe runs in `Backend::from_config`" → "runs in `Backend::probe`"; `nomad_ch.rs::local_nomad_node_id`'s wiring note now references the builder chain; 3 in-`lib.rs` `min_cfg()` docstrings retitled "Minimal config that satisfies `Backend::builder(&cfg).build()`". **Judgment call — builder over `Default`-able extras struct**: the prompt offered both shapes; `BackendBuilder` chosen because (a) explicit setter names self-document which knob a call site exercises (`.with_local_nomad_node_id(id)` vs. an opaque `ExtrasConfig { local_nomad_node_id: Some(id), ..Default::default() }`); (b) the `#[must_use]` lint catches unbuilt builders at compile time, whereas a config-struct + final `Backend::with_config(cfg, extras)` doesn't; (c) the `'a` lifetime on `BackendBuilder` keeps `cfg` by reference (it's cloned only at variant-construct time inside `build`), avoiding a per-builder `SandboxConfig` clone the extras-struct shape would need. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only pre-existing `restore.rs:43` unused-import + `sweep.rs:96` dead-code warnings — both unrelated, untouched). `cargo test -p zeroship-sandbox --lib` 512 passed / 0 failed / 1 ignored — identical to post-R26-API2 baseline. `cargo test -p zeroship-sandbox --test sandbox_admin_e2e` 22 passed / 0 failed / 10 ignored — identical to baseline. `grep -rn 'from_config_with_persist\|from_config_full' crates/sandbox/` → 0 matches. **Net diff**: +126 / -63 LOC across 12 files (+63 net); `mod.rs` itself is +91 / -49 (+42 net — the new builder carries verbose rustdoc on the "why a builder, not Option-fields" design choice + lists the future orthogonal fields r27-A1 / VFIO / tap-leak it absorbs). The investment buys absorption of every plausible-near-term extension field as a 1-line `.with_*()` setter without re-shaping the 12 existing call sites. No clippy run (clippy not available in this env); the verification stack is build + lib tests + admin e2e + grep gate. Source: code-quality-r27 R27-I1 / code-quality-r26 R26-I2 (sustained 2-round carry).

## R26-C1 + R25-C1 + R23-P1 + R11-P1 closure (2026-05-24 — thread-local Rc<Pool> connection reuse)

- [R26-C1 + R25-C1 + R23-P1 + R11-P1] **CLOSED at `ee702d5f`** — performance-r26 CRITICAL (sustained five-round carry: r11-P1 → r13-I1 → r23-P1 → r25-C1 → r26-C1): `crates/sandbox/src/db.rs:543-549::open_pool` constructed a fresh `Pool::connect_with_config` per call across 34 call sites (~14 opens per wake path; 2 opens per `host_dir` GC scan tick). At c=20 burst × ~14 conns × ~8ms = ~280 conns/sec sustained on the pinned controller's pg — below 1× safety against default `max_connections=100/200`. r4-A's wake-path unmask combined with stress-r6's c=20 sustained-wake state was reproducing pool exhaustion. Architecture-r11 / r13 had sketched the fix five rounds prior; "R11-P1 thread-local feasibility CONFIRMED" (this file, post-r12-Q1) verified `compio_postgres::Pool` is `!Send + !Sync` (uses `Rc<TcpStream>` + `RefCell` internally per `crates/compio-postgres/src/pool.rs:14-17,228`), so the cache cannot live as `Arc`/`OnceLock` on `Database` — a per-compio-worker `thread_local!<RefCell<Option<Rc<Pool>>>>` is the only viable shape. **Fix shape**: thread-local cache keyed by DSN — two cells (`POOL_APP_CELL` for the `sandbox_app` role, `POOL_AUDIT_CELL` for `sandbox_audit`); GDPR pool remains uncached per § 13.2 (privileged role; per-request lifetime). First call on a thread pays the connect cost; subsequent calls return the cached `Rc<Pool>`. Signature change: `Database::{open_pool,pool_app,pool_audit,pool_gdpr}` now return `Result<Rc<Pool>>` instead of `Result<Pool>`. All 42 internal call sites in `db.rs` work unchanged — the pattern is `let pool = ...await?; pool.get().await` and `Rc<Pool>` derefs to `&Pool` so `.get()` resolves transparently. One signature change in `admin_handlers.rs::open_app_pool` (`Result<Pool, HttpResponse>` → `Result<Rc<Pool>, HttpResponse>`). **Thundering-herd**: compio is single-threaded per worker so concurrent tasks on the same thread cannot truly race, but two tasks can interleave around the `Pool::connect_with_config` await; the post-await re-check in `install_pool` (borrow_mut + DSN comparison) keeps the first inserted pool, the second task's locally-built pool drops on function exit. **DSN-keyed (not unkeyed)**: `set_role_dsns_for_test` exists for the role-permission integration tests; keying by DSN ensures a rotated fixture's stale pool gets evicted on next call rather than returning auth-mismatched connections. **GDPR uncached rationale**: § 13.2's design call is privileged-role-per-request lifetime; caching would mean a long-lived `Rc<Pool>` authenticated as `sandbox_gdpr` could be observed via heap inspection across unrelated requests. The `Rc<Pool>` return type matches the other accessors for call-site uniformity; the `Rc` count is 1, dropped at end-of-handler. **Why not a separate per-AppState Arc<Database> field**: rejected — `Pool` is `!Send + !Sync`, so an `Arc<Pool>` field on `Database` would not compile, and a `Mutex<Pool>` would serialise all DB access against `compio-postgres`'s single-threaded design. The thread-local pattern is the only one that respects both the !Send invariant AND the cross-worker shape (`Arc<Database>` plumbs through `AppState` and ntex's worker-factory bounds unchanged). **Per-CREATE wall change**: zero (connect cost amortised, query path unchanged); net effect is removing ~280 conns/sec of churn and unblocking c=20 stress runs against default-tuned pg. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only the pre-existing `restore.rs:43` unused-import + `sweep.rs:96` dead-code warnings; neither touched). `cargo test -p zeroship-sandbox --lib` 512 passed / 0 failed / 1 ignored — identical to baseline at HEAD `e3a95a28`. No new tests added: the cache shape is internal to `Database`, observable only via runtime conn-count metrics; an "n>1 calls produce 1 connect" pin test would require a connect-counting fixture (real `Pool::connect_with_config` is not mockable without a test-only seam) and would only re-prove the documented thread_local shape; the architectural correctness comes from the !Send invariant the compio-postgres crate enforces at the type level. PG-gated subset not run (worker pg not reachable in this run; lib-test parity to baseline is the gate). **Net diff**: +126 / -38 LOC in `crates/sandbox/src/db.rs` (cache infra + 4 accessor rewrites) + 1 LOC in `crates/sandbox/src/admin_handlers.rs` (signature). Source: performance-r26 R26-C1 (sustained five-round carry r11-P1 → r13-I1 → r23-P1 → r25-C1 → r26-C1).

## R26-API2 closure (2026-05-25 — controller `/metrics` Prometheus exporter landed)

- [R26-API2] **CLOSED at `3ec2762d` + `05224bd1`** — api-surface-r26 IMPORTANT (`docs/reviews/sandbox-snapshot-restore-api-surface-2026-05-25-r26.md:217`): controller exposed NO `/metrics` route; the `metrics.rs` Phase-3 TODO ("Phase 3 wires a `/metrics` exporter") had carried for multiple rounds while 14 counters + 1 gauge accumulated as bump-only with no operator wire surface. Post-stress-r5 operators could not `rate(sandbox_vm_index_leaks_total{reason}[5m])`, could not alert on `sandbox_wake_terminal_overwrite_blocked_total`, could not gate C-7-LT Phase-5 on zero `sandbox_wake_sync_uses_total` — every playbook ADR's MEASUREMENT-driven gate had no surface to measure from. **Two-commit fix**. Commit `3ec2762d` (precursor): new `crates/sandbox/src/metrics_export.rs` (446 LOC) — pure-function `render() -> String` over the existing `*_value` accessors, emits Prometheus text-exposition v0.0.4 with one `# HELP` + one `# TYPE` per metric, alphabetically-sorted blocks, deterministic output. Three production accessors resurrected (`takeover_unreachable_value`, `takeover_corrupt_value`, `sandbox_corrupt_id_value`) — these had been deleted at `370fdbba` as `#[doc(hidden)]` orphans on the rationale that they had no callers; the orphan status was itself the symptom of the missing exporter. New `lost_leadership_snapshot_by_op() -> Vec<(&'static str, u64)>` production accessor on the dynamic `LOST_LEADERSHIP_BY_OP` map; returns empty Vec on poisoned mutex (observability is best-effort, never load-bearing). 8 unit tests pin: every counter name appears, exactly-one HELP+TYPE per metric, counter-vs-gauge tagging, labelled-series labels, dynamic per-op label appears post-bump, trailing newline, NaN gauge renders as Prometheus sentinel, label-value `\\`/`"`/`\n` escape. Commit `05224bd1` (route + auth): `admin_handlers::metrics_endpoint` thin handler gated on `AdminRole::ReadOnly` (either bearer accepted; symmetric with the rest of the operator API rather than the open-by-default Prometheus convention — the bearer-leak threat model stays uniform). Sets `Content-Type: text/plain; version=0.0.4` + `Cache-Control: no-store`. `main.rs` mounts `web::resource("/metrics")` at the root (NOT under `/admin/*`) so scrapers using their default `/metrics` path work without per-host config. +6 e2e tests in `tests/sandbox_admin_e2e.rs`: RO bearer 200, Full bearer 200, unknown bearer 401, no bearer 401, body-contains-all-14-counters, Prometheus-format pin (HELP + TYPE + labelled value line + Cache-Control + trailing newline). **Posture call**: AdminRole::ReadOnly over unauthenticated open. Reviewer recommendation was "unauthenticated, same posture as `/livez` / `/readyz`"; brief overrode to AdminRole::ReadOnly. Rationale: counter values surface fleet topology + per-call-site lost-leadership labels — both are operator-internal, never end-user-visible; gating them keeps the operator API surface uniform (one bearer-leak threat model, not two). **Counter count discrepancy noted**: reviewer's r26 artifact said "27 counters"; actual static-counter count in `metrics.rs` is 16 (14 distinct counter atoms + 1 gauge atom + 1 dynamic per-op map). The aspirational names in the reviewer's list (`sandbox_taps_orphaned_total`, `sandbox_host_dir_gc_*_total`) do not exist as `static AtomicU64`s today — they are tracked in the kernel-state-surface inventory ADR (`2026-05-25-kernel-state-surface-inventory.md`) as the closure deliverable per OPEN surface. As those surfaces close their counters will be added to `metrics.rs` and surface automatically through the exporter (the four-part deliverable's `{surface}_orphaned_total` counter only needs a new `write_counter` call in `metrics_export::render`). **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only the pre-existing `restore.rs:43` + `sweep.rs:96` warnings; neither touched). `cargo test -p zeroship-sandbox --lib` 504 → 512 (+8 metrics_export unit tests). `cargo test -p zeroship-sandbox --test sandbox_admin_e2e` 16 → 22 (+6 /metrics e2e). Source: api-surface-r26 R26-API2.

## composite-r1 MINOR-cleanup bundle closure (2026-05-24 — four r1 MINOR items shipped as one tidy-up)

- [composite-r1 #1 + #2 + #3 + #4] **CLOSED at `a26adadd` + `d2abef0b` + `de5a3eff` + `826d3abd`** — composite-validation r1 (`docs/reviews/sandbox-snapshot-restore-composite-validation-2026-05-25-r1.md`): four MINOR items the r1 review recommended shipping as a single bundled cleanup, all non-blocking and non-architectural. **#1 (`a26adadd`)**: `crates/sandbox/src/metrics.rs` carried `#[doc(hidden)]` on twelve read-side accessors (`takeover_lease_expiration_value`, `lost_leadership_value`, `lost_leadership_value_for_op`, `takeover_orphan_value`, `takeover_mismatched_value`, `vm_index_leak_value`, `wake_sync_deprecated_value`, `wake_terminal_overwrite_blocked_value`, `nomad_node_id_lookup_failures_value`, `dead_hosts_observed_value`, `clock_rewind_value`, `heartbeat_lag_value`) from when they were test-only orphans. R26-API2 wired the Prometheus exporter that consumes them in production; the annotation was stale. Dropped on all twelve; each "Test-only accessor for …" rustdoc replaced with one noting the production consumer ("Used by `metrics_export::render()` and by tests."). `lost_leadership_value_for_op` is still test-only in the strict sense (production reads `lost_leadership_snapshot_by_op`), but kept de-hidden for accessor-family uniformity — its rustdoc now spells out the test-vs-exporter split. **#2 (`d2abef0b`)**: `crates/sandbox/src/lib.rs:21` was `pub mod metrics_export`. Sole production consumer is `admin_handlers::metrics_endpoint` (same crate, `crate::metrics_export::render`); sole test consumer is the module's own unit tests; no integration test references `zeroship_sandbox::metrics_export::*` (`grep -rn 'zeroship_sandbox::metrics_export' crates/sandbox/` → 0 matches). Tightened to `pub(crate)`; in-line comment notes the "called only from admin_handlers" architectural intent at the module declaration. **#3 (`de5a3eff`)**: the existing `/metrics` test set covered 401-no-bearer, 401-unknown-bearer, 200-RO, 200-Full, body-shape — but not the 503 path. Added `metrics_503_when_no_admin_tokens_configured` to `crates/sandbox/tests/sandbox_admin_e2e.rs` mirroring the sibling 503 pattern at `admin_503_with_correct_bearer_when_pg_disabled`. Builds AppState via `make_state_with_admin_tokens(None, None, None)`, fires `GET /metrics`, asserts (a) 503, (b) §10.0 envelope `error: "admin_api_disabled"`, (c) `message` field's presence. **#4 (`826d3abd`)**: the `sandbox_corrupt_id_total` HELP text — "restore encountered a pg row whose sandbox_id doesn't parse as a typed-id" — narrowed the counter's scope to the restore path, but `inc_sandbox_corrupt_id` is bumped from any typed-id-parse-failing read site. Broadened to "decode of a stored sandbox_id as a typed-id failed at any read site". Not a wire-format change (Prometheus consumers parse name + value, not HELP body). **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only the pre-existing `restore.rs:43` unused-import + `sweep.rs:96` dead-code warnings; neither touched by this bundle). `cargo test -p zeroship-sandbox --lib` 512 passed / 0 failed / 1 ignored — identical to baseline. `cargo test -p zeroship-sandbox --test sandbox_admin_e2e` 22 → 23 (+1 new 503 test). `grep -n '#\[doc(hidden)\]' crates/sandbox/src/metrics.rs` 12 → 0 (only a historical reference in a rustdoc comment at `:351` survives, mentioning the prior orphan deletion at `370fdbba`). `grep -n 'pub mod metrics_export' crates/sandbox/src/lib.rs` 1 → 0 (now `pub(crate)`). No integration test broken by the `pub(crate)` tightening — all 23 admin e2e tests green. **Net diff**: +75 / -32 LOC across four files (`metrics.rs` +28/-28, `lib.rs` +5/-1, `sandbox_admin_e2e.rs` +37/-0, `metrics_export.rs` +1/-1). Source: composite-validation-r1.

## T-8b-stress-r7-A closure (2026-05-25 — bump pg `max_connections` 100→500 to absorb R26-C1 thread-local pool footprint)

- [r7-A] **CLOSED at `<this commit>`** — cluster-validation-T-8b-stress-r7 (`docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r7.md`): r7 RED 0/60 e2e on a NEW wedge — PostgreSQL connection-pool exhaustion (`FATAL: sorry, too many clients already`) on the single server-1 pg instance, observable as the "every 8th cycle succeeds" cadence (3 successes per worker across all 3 workers = 9 of 60 snapshot cycles). Even an interactive `psql` from server-1 itself was rejected. **r7-C diagnostic** (no v36 code-path regression): the v35→v36 source delta is `2226de7a` (config flag scaffold) + `6e928a25` (220-line `nomad_ch.rs` diff + 48-line `restore_handler.rs` diff). Reading every line of both diffs: (a) the spawn_blocking truncate+mkfs.ext4 bypass at `crates/sandbox/src/backend/nomad_ch.rs:790-820` adds zero pg queries — it changes whether the controller stages workspace.img locally or whether the driver does, nothing touches `Database::open_pool` / `pool.get()`; (b) the new wire fields `Task.Config.stage_disk_images` and `Job.Meta.zsbx_stage_disks` are pure JSON-emit additions on the cold-boot `build_nomad_job_json_with` branch — no pg involvement; (c) `CreateGuard::drop` at `nomad_ch.rs:2104-2255` issues ZERO pg queries on any branch (the drop sequence is Nomad purge HTTP + in-memory `vm_index` release + intentional `host_dir` leak — the `host_dir_created=false` change under flag-on means the diagnostic log line at `:2244` doesn't fire, but the pg behaviour is identical); (d) the restore-emitter's `stage_disk_images: false` literal addition is purely outbound JSON, no pg. **The actual driver of per-cycle pg pressure is R26-C1** (commit `ee702d5f`, "sandbox/db: thread-local Rc<Pool> for connection reuse"), which shipped in the v35→v36 pin window: the thread-local `POOL_APP_CELL` / `POOL_AUDIT_CELL` caches each warmed compio-worker thread's pool indefinitely, with `PoolConfig::min_idle=2` floor and growth to `pool_max=16` per role under load. `start_housekeeper()` is NEVER called from the sandbox crate (`grep -rn start_housekeeper crates/sandbox/` → 0 matches), so per the `pool.rs` warning at `:222`, "connections grow unbounded up to max_size and are never reaped early." Steady-state floor per worker process: 32 ntex worker threads (n2-standard-32) × 2 cached pools × 2 min_idle = 128 conns; bursty growth to 32 × 2 × 16 = 1024 worst case. **Three workers × 128 floor = 384 steady-state conns, far above pg default `max_connections=100`.** Why r1-r6 (no R26-C1) escaped saturation: pre-R26-C1 each `open_pool()` constructed a fresh `Pool::connect_with_config` that was dropped at scope end — short-lived, 2-conn bursts (`min_idle=2`), released on Pool drop. Steady-state idle was near zero; R26-C1's commit message even cited "stress-r6 was hitting this exhaustion in sustained-wake state" but the fix shape (cache-and-retain) overshot the cap in the OPPOSITE direction once stress-r7 ran with it under c=20 × 3 workers. **Verdict**: NOT a v36 regression at the source level; r7's wedge is the architectural side-effect of R26-C1's retention semantics meeting the default pg cap. **r7-A fix shape** (config bump, zero controller code touched per mandate constraint "DO NOT touch nomad_ch.rs production code"): `crates/sandbox/scripts/gcp-server-startup.sh` — `max_connections=500` (vs default 100, sized for the 384-floor worst case plus headroom for `psql` diagnostic / Nomad client / housekeeping), `shared_buffers=1GB` (vs default 128 MB, ~6 % of the n2-standard-4 PG host's 16 GB RAM, well under the PG docs rule-of-thumb 25 % ceiling), `work_mem=8MB` (vs default 4 MB, 500 × 8 MB sort-scratch worst case = 4 GB, within the 16 GB envelope). `max_prepared_transactions` NOT bumped (no 2PC usage in sandbox crate — `grep -ncE "max_prepared_transactions|PREPARE TRANSACTION" crates/sandbox/src/**/*.rs` → 0). Sed lines use `#\?max_connections.*` so they overwrite both the commented-out default and any previously-applied value (idempotent across re-runs of `provision-gcp-cluster.sh`). **What r7-A explicitly does NOT do**: it does not fix the R26-C1 retention semantics (would require either calling `start_housekeeper()` on each thread-local pool insert, OR lowering `pool_max` from 16 to a value sized for the actual per-thread workload — both are controller code changes that would push this sprint over the >30 LOC threshold the mandate flagged as "regression sprint, not a config bump"). Those are tracked as a follow-up in [r7-C-followup] below. **What r7-A is sufficient for**: surviving stress-r8 at 3+3×20 with the current R26-C1 shape unchanged. The Phase-2/4 Option-C-Phase-2 architectural validation that r7 was supposed to perform stays DEFERRED — it will be exercised once the upstream pg wedge is gone. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (no code touched in this commit; existing `restore.rs:43` + `sweep.rs:96` warnings unchanged). `cargo test -p zeroship-sandbox --lib` 538 passed / 0 failed / 1 ignored (baseline unchanged; this is a config-only change). `bash crates/sandbox/scripts/lint.sh` → "OK — 7 script(s) clean at --severity=error". **Net diff**: +59 / -2 LOC in `crates/sandbox/scripts/gcp-server-startup.sh` (3 sed lines + ~50 lines of rationale comment); 0 LOC in `crates/sandbox/src/`. Stress-r8 retry gated on r7-A landing + driver counter surfacing (r7-B, separate sprint).

## NEW r28 ROUND FINDINGS (added by pilot cycle 2026-05-25 — T-8b-stress-r7-A diagnostic byproduct)

- [r7-C-followup] **CLOSED at `8c0b361e`** — pool-lifecycle: R26-C1 thread-local `Rc<Pool>` cache (`ee702d5f`) did not start `pool.start_housekeeper()`, so under stress each warmed compio worker thread held its pool's idle conns indefinitely (no idle reap, no max-lifetime rotation). Steady-state floor per worker: 32 threads × 2 roles × min_idle(2) = 128; theoretical max 32 × 2 × 16 = 1024. The pg-cap bump to `max_connections=500` (r7-A above) absorbed the floor but not bursty saturation. **Picked Option A from the parent entry** (smallest controller change): `pool.start_housekeeper()` is now called on the locally-built `Rc<Pool>` immediately after `Pool::connect_with_config` in both `open_pool` (sandbox_app, `db.rs:617-643`) and `pool_audit` (sandbox_audit, `db.rs:651-674`), before the `install_pool` cache write. The housekeeper is detached fire-and-forget — it holds `Weak<Pool>` and self-terminates when the last strong `Rc` drops (`compio-postgres/src/pool.rs:341-353`), so even the race-loser pool (when two tasks on the same compio worker concurrently miss the cache and both build) gets cleaned up cleanly without leaking the spawn. No handle threading needed — `start_housekeeper`'s `&Rc<Self>` receiver is exactly the shape the cache already maintains. **Option B** (lower `pool_max` default from 16) NOT picked: the housekeeper's idle-timeout + max-lifetime semantics solve the unbounded-growth wedge directly; capping `pool_max` would just trade one knob for another while still leaving warmed-but-idle conns pinned. **What this enables**: conns DO release back to pg over time (10 min idle timeout, 30 min max-lifetime rotation — `PoolConfig` defaults), so a sustained c=20 × 3 workers stress that warms most threads simultaneously can no longer accumulate idle conns past pg's cap. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only pre-existing `restore.rs:43` + `sweep.rs:96` warnings — neither touched). `cargo test -p zeroship-sandbox --lib` 538 passed / 0 failed / 1 ignored (baseline unchanged; structural fix verified by code inspection — `start_housekeeper` has no observable test surface from the consumer side since it spawns a detached task on a 30s timer). Grep gate: `grep -n start_housekeeper crates/sandbox/src/db.rs` 0 → 2 (one per pool role). **Net diff**: +14 / -0 LOC in `crates/sandbox/src/db.rs`. Source: r7-C diagnostic (parent closure r7-A above).

## T-8b-stress-r8 substrate closure (2026-05-25 — r24-A2-S2 driver tap-verify + r24-A2-S3 controller delayed-release + r7-B driver-metrics exporter)

- [r24-A2-S2 + r24-A2-S3 + r7-B] **CLOSED** — cluster-validation-T-8b-stress-r8 (`docs/reviews/sandbox-snapshot-restore-cluster-2026-05-25-T8b-stress-r8.md`): stress-r8 RED 3/60 surfaced cycles 1-19 per worker failing identically with `LockingError(AlreadyLocked) on rootfs.img + Tap zsbx-nm-N already exists`, and r5-A `lock_held_total` monotonically incrementing 1→21 meant the 5 s OFD probe budget was exhausted every non-first cycle but the driver proceeded anyway. Per the r8 review, three substrates of r24-A2 (kernel-state surface inventory ADR) became CRITICAL PATH: S1 (destroyTask sync wait for CH reap — already in via r4-A `waitForReap`); S2 (destroyTask sync `ip tuntap del <tap>` + verify ENODEV before returning); S3 (VM-index pool gates on tap-deletion confirmation before re-issuing an index). Plus r7-B observability: driver counters `nomad_driver_ch_*` not visible via Nomad `/v1/metrics`. **Three-commit bundle** across two worktrees. **Driver commits** (`nomad-driver-ch` worktree, branch `feat/nomad-driver-ch`):
  1. `a5268e88` (`nomad-driver-ch/metrics: add destroy_task_tap_stuck_total counter`): r24-A2-S2 observability — atomic.Int64 mirroring the existing destroy_task_unreaped_total / destroy_task_lock_held_total shape; inc / read / reset; no external deps.
  2. `cd118547` (`nomad-driver-ch/stop_task: sync ip tuntap del + ENODEV verify in DestroyTask`): r24-A2-S2 wiring — new `deleteTapAndVerifyAbsent` helper in `ch/stop_task.go` re-issues `removeTapFn` (idempotent against ENODEV) then polls a new `tapLookupFn` seam (default `realTapLookup` → `ip link show <tap>` classifies ENODEV via existing `isNoSuchDevice`) for the kernel to evict the netdev. Budget: 50 attempts × 100 ms = 5 s wall — same ceiling as r4-A/r5-A, finer cadence since `ip link show` is a single netlink RPC. Wired into `DestroyTask` AFTER the existing cleanup tail (preserves orphan-counter semantics) via a new `uniqueTapsForVerify` helper that enumerates h.tap + the VMIndex-derived defensive name dedup'd. On budget exhaustion: bump `destroy_task_tap_stuck_total`, WARN-log, proceed (mirrors r4-A/r5-A "never block Nomad destroy" tolerance). Three new test seams (`SetDestroyTapDeletePollForTest` / `SetSleepForTapDeletePollForTest` / `SetTapLookupForTest`). +3 tests in `tests/stop_task_test.go`: `TestDestroyTask_TapDeletedAndVerifiedAbsent` (happy path; no sleeps, counter at baseline), `TestDestroyTask_TolerantOfTapDeleteENODEV` (3 Busy then Absent; exactly 3 sleeps per tap, counter at baseline), `TestDestroyTask_PollExhaustionBumpsCounter` (always Busy; counter bumps +2 for two unique taps, DestroyTask returns nil). Existing `TestDestroyTask_DefensiveTapCleanup_{SkipsWhenHandleTapMatches, BothFireWhenNamesDiffer}` updated to expect the gate's extra removeTapFn calls (idempotent re-issue before polling). The fixture installs a no-op tapLookupFn default so existing tests don't shell to real `ip link show`. Driver test count: 144 → 147 (+3 top-level top-of-file tests).
  3. `d04711a1` (`nomad-driver-ch/metrics: driver-metrics.prom file exporter`): r7-B / R26-API1 — new `ch/metrics_exporter.go` (283 LOC): pure-function `renderDriverMetricsProm` over the 6 existing counters (destroy_task_lock_held_total + destroy_task_tap_stuck_total + destroy_task_unreaped_total + start_task_stage_failures_total + start_task_stage_total + taps_orphaned_total), atomic tmp+rename `writeDriverMetricsSnapshot`, and goroutine `runDriverMetricsExporter` ticking every 5 s + writing one final snapshot on ctx cancellation. Path: `/var/lib/zsbx/driver-metrics.prom` (configurable via test seam); format: Prometheus exposition v0.0.4 (HELP/TYPE/sample triples, `_total` suffix, counter type). Operators wire node_exporter's textfile collector at the parent dir. Wired into `NewPlugin` via `go runDriverMetricsExporter(ctx, logger)` bound to the plugin's signalShutdown ctx, gated by `driverMetricsExportEnabled` so TestMain can disable it (the 30+ NewPlugin calls would otherwise each leak a goroutine trying to MkdirAll /var/lib/zsbx under the unprivileged test user). +5 tests in `tests/metrics_exporter_test.go` + `tests/main_test.go` (TestMain disabling exporter by default). Driver test count: 147 → 152 (+5).
- **Sandbox commit** (`sandbox-snapshot-restore` worktree, branch `feat/sandbox-snapshot-restore`):
  4. `c969b94d` (`sandbox/nomad-ch: delay VmIndexAllocator::release by N seconds after stop`): r24-A2-S3 — new `VmIndexAllocator::spawn_delayed_release(allocator, i, delay, reason, sandbox_id)` (detached compio task that sleeps then releases; `delay == ZERO` skips the sleep). Two call sites converted: `stop_inner` post-fence-passed (`reason="stop-fence-passed"`) and `CreateGuard::drop` post-purge-confirmed (`reason="create-failure-cleanup"`). `CreateGuard` carries the configured `release_delay: Duration` through to its detached cleanup task. New config field `vm_index_release_delay_secs` on `NomadCHConfig`, defaulted to 5 s in `SandboxConfig::default` and `::from_env` (matches the driver's tap-deletion-verify budget ceiling so the controller doesn't release the index until the driver has finished its synchronous netdev cleanup). Env var: `SANDBOX_NOMAD_CH_VM_INDEX_RELEASE_DELAY_SECS`. All test fixture configs set the delay to 0 so test wall-time is unaffected (lib.rs, docker.rs, restore_handler.rs, config.rs test helpers, nomad_ch.rs test cfg). +2 tests: `spawn_delayed_release_with_zero_delay_releases_immediately` (pins the ZERO-delay branch — no sleep call, still async), `spawn_delayed_release_honors_configured_delay` (pins the non-zero-delay branch — slot NOT in freed mid-delay; IS in freed after delay elapses). Updated `restored_sandbox_is_stoppable_and_releases_vm_index` (B19 regression test) to poll the freed set non-destructively before consuming via alloc() — direct alloc() would race against `next` and return slot 5 before the delayed release populates the freed set with slot 4. New helper `VmIndexAllocator::freed_for_test()` exposes the freed BTreeSet for non-destructive observation (test-only, gated on `#[cfg(test)]`). Sandbox lib test count: 538 → 540 (+2).
- **Architectural shape**: r24-A2-S2 closes the worker-side tuntap-add race; r24-A2-S3 closes the controller-side index-reuse race; together they close the cycle. Stress-r8 RED 3/60 should resolve to GREEN under these two layers + the existing r4-A reap-wait + r5-A OFD-lock-probe; the next stress retry (driver v19 + controller v37, separate scripts/* pin-bump sprint per the mandate's scope guard) will exercise the bundle end-to-end. r7-B closes the operator-side observability gap that stress-r8 review flagged (review couldn't verify counter deltas without scraping). **Verification across both worktrees**: driver `nix develop -c go vet ./...` clean; `nix develop -c go test ./tests/...` 152 top-level tests pass (was 144 baseline; +3 r24-A2-S2 + +5 r7-B). Sandbox `cargo build -p zeroship-sandbox --tests` clean (only pre-existing `restore.rs:43` + `sweep.rs:96` warnings); `cargo test -p zeroship-sandbox --lib` 540 passed / 0 failed / 1 ignored (was 538 baseline + 2 new). Source: cluster-validation-T-8b-stress-r8 r24-A2-S2 + r24-A2-S3 + r7-B / R26-API1.

## Round-39 closures

- [R27-API2] **CLOSED at `c2e07b2f`** — api-surface-r27 IMPORTANT (`docs/reviews/sandbox-snapshot-restore-api-surface-2026-05-25-r27.md:157` + `:215-244`): `NomadCHBackend::_test_inject_sandbox` at `crates/sandbox/src/backend/nomad_ch.rs:1699` was unconditionally `pub` despite being test-only scaffolding that accepts a raw `SigningKey` + arbitrary `agent_url` (bypasses the `derive_agent_url` rule for `127.0.0.1:<ephemeral>` fixtures in the preview-proxy e2e tests). The author had flagged the fix shape in the inline doc-comment at `:1682-1687` ("the `#[cfg(any(test, feature = \"test-support\"))]` gate would be cleaner") but deferred to "Phase 1"; that phase landed and this carry never closed. Asymmetry with the `freed_for_test` precedent at `:399` (`#[cfg(test)] pub fn`) was the anomaly flagged by r27 — same shape, same rationale, two different disclosure postures. **Fix shape (Option A per r27 `:230-241` — pure subtractive)**: gate the function with `#[cfg(any(test, feature = "test-support"))]`; add a `test-support = []` empty feature to `crates/sandbox/Cargo.toml` with `default = []`; add a self dev-dep `zeroship-sandbox = { path = ".", features = ["test-support"] }` so `cargo test -p zeroship-sandbox` auto-enables the feature for integration tests in `tests/` (which link the library as an external crate where `cfg(test)` does NOT apply — the `cfg(test)` half of the gate only covers lib unit tests). Call sites enumerated: 7 in `tests/sandbox_{pg,typed_id,preview,preview_ws,preview_share}_e2e.rs` (all integration tests; feature auto-enabled via dev-dep) + 1 in `src/backend/nomad_ch.rs` definition site. Zero production call sites. **Verification**: `cargo build -p zeroship-sandbox` (no tests, no feature) compiles clean; `nm target/debug/libzeroship_sandbox.rlib | grep _test_inject_sandbox` → 0 matches (symbol stripped from production rlib, proving the gate works). `cargo build -p zeroship-sandbox --tests` compiles clean (feature auto-enabled). `cargo test -p zeroship-sandbox --lib` 540 passed / 0 failed / 1 ignored — identical to baseline. **Net diff**: +13 / -7 LOC across `crates/sandbox/src/backend/nomad_ch.rs` (rustdoc rewrite + 1-line cfg attribute) + `crates/sandbox/Cargo.toml` (+12 LOC: feature declaration + self dev-dep block) + 1 LOC in `Cargo.lock` (self-dep reflection). Mirrors `freed_for_test` precedent exactly, minimum disclosure principle satisfied. Source: api-surface-r27 R27-API2 IMPORTANT.

- [r1-DISC-3] **CLOSED at `871752c7`** — test-discipline-r28-r1 IMPORTANT (`docs/reviews/sandbox-snapshot-restore-test-discipline-audit-2026-05-25-r1.md:498-506` + summary-table row at `:518`): R26-C1 thread-local `Rc<Pool>` cache (`crates/sandbox/src/db.rs:62-105` + `:617-672`) carried **ZERO** predicate tests; the R26-C1 closure entry above (`ee702d5f`) acknowledged this gap explicitly ("No new tests added: the cache shape is internal to `Database`, observable only via runtime conn-count metrics; an 'n>1 calls produce 1 connect' pin test would require a connect-counting fixture and would only re-prove the documented thread_local shape"), and that absence was the proximate cause of the stress-r7 wedge → r7-A `max_connections=100→500` bump → r7-C-followup `start_housekeeper` patch sequence — a ~$2 stress cycle + ~8h investigation that a predicate oracle would have caught earlier. **Mandate from `:518`**: pure structural argument from `!Send` was deemed an R28-DISCIPLINE violation (uncategorised); behavioral predicate needed to ship. **Fix shape**: three pg-gated tests in a new `r26_c1_pool_cache` sub-module at the tail of `crates/sandbox/tests/sandbox_pg_e2e.rs` (~254 LOC including module-level rationale comment + helpers; tighter than the audit's ~80 LOC estimate per test once shared helpers + barriered-coordination overhead are accounted for). **(1) `pool_cache_returns_same_rc_within_thread`**: same DSN + same compio worker thread → `Rc::ptr_eq` on the two `pool_app()` returns must hold; defense-in-depth `Rc::strong_count(&p1) == 3` (cache cell + p1 + p2) catches an exotic regression where a second cell would hold a phantom strong ref. The DSN is tagged `?application_name=zs_disc3_t1` so prior tests' default-DSN cache entries do NOT contaminate (the cache key is the verbatim DSN string). **(2) `pool_cache_per_thread_isolated`** — production-state oracle via `pg_stat_activity`: two `std::thread::spawn` workers each build their own `compio::runtime::Runtime`, each calls `pool_app()` twice (the same-thread `Rc::ptr_eq` invariant holds within each worker independently), then both arrive at `Barrier(3)` so an observer pool (built on the untagged default DSN, neutral application_name so its own backend is excluded via `pid <> pg_backend_pid()`) can `SELECT count(*) FROM pg_stat_activity WHERE application_name = 'zs_disc3_t2'`. With `PoolConfig::default()::min_idle = 2`, two workers warm 4 conns floor; asserts `>= 2` (loose floor against PoolConfig drift; one-conn-per-worker would already disprove a shared-cache regression). Both pools held alive across the count via a second `Barrier` so pg's view is consistent with the in-memory state. **(3) `pool_cache_dsn_tiebreaker_evicts_on_mismatch`**: two `Database` instances with DSNs that differ only in `application_name` query param (both connect to the same db, but the cache key is the verbatim DSN string); same thread; asserts `!Rc::ptr_eq(&pa, &pb)` — the cache evicts the prior entry on DSN mismatch and `install_pool` builds afresh. Second-order check: `Rc::ptr_eq(&pb, &pb2)` confirms the newly-installed pool itself caches on a same-DSN repeat call. Third-order check: returning to `dsn_a` builds a FRESH pool (`!Rc::ptr_eq(&pa, &pa2)`) — eviction does NOT resurrect the original entry, validating the `set_role_dsns_for_test` guard called out in `db.rs:53-55`. **Test 4 (housekeeper reaps idle conns)**: DEFERRED with explicit rationale at the module header. `PoolConfig` defaults are `idle_timeout=600s` and `max_lifetime=1800s`; neither is adjustable from the `Database` boundary (the `pool_max.max(2)` knob is the only public lever) and a 10+ minute CI test is not viable. Wiring correctness is verified by code inspection (`db.rs:640,670` — `start_housekeeper()` is called immediately after `Pool::connect_with_config` in both `open_pool` and `pool_audit`, before `install_pool`) plus the structural argument that the housekeeper holds `Weak<Pool>` (`compio-postgres/src/pool.rs:341-353`) so it self-terminates when the last strong `Rc` drops — race-loser pools, dropped on function exit, get a clean self-terminating housekeeper. **Why no `_clear_pool_cache_for_test()` seam was added**: prior tests' cached pools have different DSN keys (untagged default DSN vs the test-specific `application_name` tags), so `cached_pool` returns `None` on first call and `install_pool` evicts cleanly — no cross-test contamination. Adding a `#[cfg(test)]` clear-cache fn would be dead code. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (pre-existing `restore.rs:43` + `sweep.rs:96` warnings only). `cargo test -p zeroship-sandbox --lib` 538 passed / 0 failed / 1 ignored (baseline unchanged; new tests are pg-gated in `tests/`, not `src/lib`). `cargo test -p zeroship-sandbox --test sandbox_pg_e2e r26_c1_pool_cache:: -- --ignored --test-threads=1` → 3 passed / 0 failed (verified locally against `docker compose up -d postgres`). pg-gated test count delta: 91 → 94. **Net diff**: +254 / -0 LOC in `crates/sandbox/tests/sandbox_pg_e2e.rs` (new `r26_c1_pool_cache` sub-module with 3 tests + 2 helpers `dsn_with_app_name` / `count_conns_with_app_name`); zero production-code changes. **Predicate-discipline lesson banked**: structural `!Send` arguments are necessary but not sufficient under R28-DISCIPLINE — the next change to cache semantics (a DSN-rotation race fix, a per-role `pool_max` differentiation, a min_idle bump) now has three behavioral oracles to regress against. Source: r1-DISC-3 (test-discipline-audit-2026-05-25-r1 IMPORTANT).

- [R29-C1] + [r29-A2] + [R29-I1] **CLOSED at `62b083e1`** — concurrency-r29 CRITICAL + architecture-r29 P0 class-fix + concurrency-r29 IMPORTANT (`docs/reviews/sandbox-snapshot-restore-concurrency-2026-05-25-r29.md` R29-C1 + R29-I1 / `docs/reviews/sandbox-snapshot-restore-architecture-2026-05-25-r29.md` r29-A2): R28-C1 (`9e1f6276`) closed the CreateGuard::drop call site inline but left `VmIndexAllocator::spawn_delayed_release` as a `pub` footgun. R29-C1 surfaced a second leak via the same trap: `admin_handlers.rs:1491-1506` dispatches `crate::detach::detach_isolated("snap-teardown-<tail>", …)` → `state.backend.teardown_source_for_snapshot(sandbox_id)` → `stop_preserving_state` → `stop_inner` (`crates/sandbox/src/backend/nomad_ch.rs:1324`), and `stop_inner` invoked `VmIndexAllocator::spawn_delayed_release` against the CURRENT runtime — which on this call path is the short-lived private compio runtime minted by `detach_isolated`. The runtime drops as soon as the cleanup future returns Ready; `Scheduler::clear` discards the pending 5 s timer; vm_index leaks on every successful admin snapshot teardown until the next controller-boot orphan prune. The other reachable callers (`sweep.rs:780` `snap-idle-evict`, `registry.rs:836` `snap-idle-gc`, `admin_handlers.rs:1851` `wake-<tail>`) are clean because their runtimes outlive the 5 s delay; only the admin-snapshot tail was the second occurrence. **Fix shape — r29-A2 class-fix per "best not simplest" preference**: replace the single fire-and-forget `spawn_delayed_release` with TWO typed helpers that surface the runtime-lifetime decision in the type system. (1) `pub async fn release_vm_index_after(allocator, i, delay, reason, sandbox_id)` — inline-await variant: sleeps, releases, emits the operator log line. Safe everywhere because the timer is bound to the caller's task via `.await`, not detached. Now used by BOTH `stop_inner:1324` (`stop_inner.await`s the release — adds the configured 5 s delay to wall, negligible vs host_fence up to 30 s and Nomad purge already on path) AND `CreateGuard::drop` (uniform replacement of R28-C1's inline sleep + release + log, -22 LOC of inline rationale now captured once in the helper rustdoc). (2) `pub fn spawn_delayed_release_in_worker(...) -> compio::runtime::Task<Result<(), Box<dyn Any+Send>>>` — typed escape hatch returning a JoinHandle (compio's `spawn` wraps the spawned future's output in the panic-catch `Result`; we surface that rather than hide it). Caller MUST `.await` or `.detach()`; `.detach()` is only sound when the current runtime is guaranteed to outlive the delay. `#[allow(dead_code)]` because no production call site uses this today — the helper exists for any future background-task path that needs fire-and-forget delayed release on a long-lived runtime without blocking the caller. Adding the typed return makes the pre-r29 `spawn_delayed_release` mistake (detaching onto whatever runtime happens to be current) impossible to make by accident — callers must declare intent. **Deleted**: original `pub fn spawn_delayed_release(...)` at `nomad_ch.rs:363-387`. `grep -n spawn_delayed_release crates/sandbox/src/` only matches rustdoc/comment historical mentions; the function is gone. **Tests (full set)**: (a) `release_vm_index_after_with_zero_delay_releases_immediately` — zero-delay path collapses to a synchronous release inside the calling task; (b) `release_vm_index_after_honors_configured_delay` — non-zero delay variant; uses `spawn_delayed_release_in_worker` for the joinable shape so the main task can observe the "halfway" state mid-delay (slot NOT in freed at delay/2; IS in freed after `task.await`); (c) NEW `spawn_delayed_release_in_worker_returns_joinable_task` — pins the typed `Task<Result<(), Box<dyn Any+Send>>>` return contract via shadowing (compiler catches a future refactor that changes the shape before the rustdoc drifts) + post-`.await` release observation; (d) NEW R29-C1 regression `release_vm_index_after_survives_short_lived_runtime` — `#[test]` (NOT `#[compio::test]`) on plain `std::thread`, dispatches the release inside a `crate::detach::detach_isolated("test-r29-c1", ...)` body with `delay = 100 ms` against a single-element pool, polls up to 5 s for the index to reappear. This is the precise R29-C1 leak shape: a detached helper would discard the timer when the private runtime drops; the inline-await fix keeps the timer bound to the caller's task so `block_on` cannot return until the release completes. Existing R28-C1 regression test `create_guard_drop_releases_vm_index_under_isolated_runtime` continues to pass on the new code path (route now goes through `release_vm_index_after` instead of inline sleep+release). Added `"test-r29-c1"` (11 B) to `detach.rs`'s `STATIC_NAMES` kernel-limit list per the R17-I1 thread-name discipline. **Wall-time semantics change (documented)**: `stop_inner` now awaits the release before returning. ntex callers' HTTP response is 5 s later in the worst case. Acceptable because: (1) stop_inner's wall was already dominated by host_fence (up to 30 s) + Nomad purge (~10 s) + bookkeeping; 5 s on top is <20% added wall; (2) the vm_index MUST NOT be reused until the kernel netdev cleanup + fcntl drain completes, so returning early without the delay is just lying about when the slot is safe to reuse; (3) all test fixtures set `vm_index_release_delay_secs = 0` so test wall is unaffected. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only pre-existing `restore.rs:43` + `sweep.rs:96` warnings; neither touched). `cargo test -p zeroship-sandbox --lib` 546 → 548 passed / 0 failed / 1 ignored (2 renamed/adapted + 4 new = net +2). All `stop_inner`-touching tests (`stop_preserving_state_does_not_remove_host_dir`, `stop_for_real_leaks_host_dir_for_sweeper`, `restored_sandbox_is_stoppable_and_releases_vm_index`, `stop_preserving_state_does_not_delete_sealed_record`) still pass — the polling in `restored_sandbox_is_stoppable` (which was guarding against the previous detached-task race) is now over-cautious but correct. **Net diff**: +267 / -116 LOC across two files (`crates/sandbox/src/backend/nomad_ch.rs` + `crates/sandbox/src/detach.rs`); ~85 LOC of net helper code + ~70 LOC of net test code + comment rewrites at call sites. Source: concurrency-r29 R29-C1 CRITICAL + R29-I1 IMPORTANT, architecture-r29 r29-A2 P0 class-fix.

- [R28-API2] **CLOSED at `9ac5b850`** — api-surface-r28 IMPORTANT (`docs/reviews/sandbox-snapshot-restore-api-surface-2026-05-25-r28.md`): 5 test-scaffolding `pub` items remained reachable from production rlibs after the R27-API2 close (`c2e07b2f`) introduced the `test-support` feature + self dev-dep but only used it on `_test_inject_sandbox`. Mechanical sweep applied the same `#[cfg(any(test, feature = "test-support"))]` gate to the remaining four items — (1) `Database::from_test_config` (`crates/sandbox/src/db.rs:518`), (2) `restore_handler::StubRestoreBackend` (`crates/sandbox/src/restore_handler.rs:1302`) plus its `impl StubRestoreBackend` and `impl RestoreBackend for StubRestoreBackend` blocks, (3) `snapshot_handler::StubSourceVmOps` (`crates/sandbox/src/snapshot_handler.rs:691`) plus its `impl StubSourceVmOps` and `impl SourceVmOps for StubSourceVmOps` blocks, (4) `sweep::RecordingIdleSnapshotter` (`crates/sandbox/src/sweep.rs:492`) plus its `impl IdleSnapshotter for RecordingIdleSnapshotter` block. The 5th item — `Database::set_role_dsns_for_test` — was **DELETED outright** instead of gated: `grep set_role_dsns_for_test crates/sandbox/` returned 0 real callers (only stale rationale comments in `tests/sandbox_pg_e2e.rs:5739,5937,5939` and the cache docstring at `db.rs:55`, all updated in this commit). Per the pre-launch "no back-compat" rule, dead audit/GDPR DSN-override scaffolding loses the symbol entirely rather than carrying a `#[cfg]` gate forward. **Cargo.toml unchanged**: the `test-support = []` feature declaration and the self dev-dep `zeroship-sandbox = { path = ".", features = ["test-support"] }` were both added at the R27-API2 close (`c2e07b2f`), so integration tests in `crates/sandbox/tests/` auto-enable the feature with no churn. **Call-site verification** before the gate landed: `grep -rn` across the crate confirmed every caller of the four gated items lives in `crates/sandbox/tests/` (integration tests, feature auto-enabled) or inside a `#[cfg(test)] mod tests` block (inline unit tests, `cfg(test)` half of the gate covers them — restore_handler.rs:1525/1565/1598 inside `#[cfg(test)]` at :1439; snapshot_handler.rs inside :751; sweep.rs inside :1299; lib.rs:2543 inside `#[cfg(test)] mod field_setter_tests` at :2537). Zero production call sites. **Verification**: `cargo build -p zeroship-sandbox` (no `--tests`, no `--features`) compiles clean — confirms the production rlib drops these symbols. `cargo build -p zeroship-sandbox --tests` compiles clean (feature auto-enabled via dev-dep). `cargo test -p zeroship-sandbox --lib` 548 passed / 0 failed / 1 ignored — identical to the R29-C1+r29-A2+R29-I1 baseline (`62b083e1`). **Symbol-strip evidence (verbatim)**: `nm target/debug/deps/libzeroship_sandbox-c9ce1cd4691cc5ef.rlib | grep <sym>` returned 0 occurrences for each of `from_test_config`, `set_role_dsns_for_test`, `StubRestoreBackend`, `StubSourceVmOps`, `RecordingIdleSnapshotter` — all five symbols absent from the production rlib, proving the gates / deletion strip cleanly. **Net diff**: +26 / -11 LOC across four files (`crates/sandbox/src/db.rs` +6/-10 = delete of `set_role_dsns_for_test` + gate on `from_test_config` + cache-docstring cleanup; `crates/sandbox/src/restore_handler.rs` +7/-0; `crates/sandbox/src/snapshot_handler.rs` +7/-0; `crates/sandbox/src/sweep.rs` +6/-0). No Cargo.toml or Cargo.lock changes — the R27-API2 close already paid that cost. Mirror of the R27-API2 mechanical sweep precedent. Source: api-surface-r28 R28-API2 IMPORTANT.

- [R28-C1] **CLOSED at `9e1f6276`** — concurrency-r28 CRITICAL (`docs/reviews/sandbox-snapshot-restore-concurrency-2026-05-25-r28.md`): `CreateGuard::drop` (`crates/sandbox/src/backend/nomad_ch.rs:2266-2280`) called `VmIndexAllocator::spawn_delayed_release(allocator, i, 5s, "create-failure-cleanup", …)` inside the cleanup future dispatched by `crate::detach::detach_isolated("create-rollbk", …)`. `detach_isolated` mints a fresh `compio::runtime::Runtime` on a dedicated OS thread, runs `block_on(cleanup_future)`, then drops the runtime — so the `compio::runtime::spawn(sleep(5s).then(release)).detach()` task that `spawn_delayed_release` plants lands on THIS short-lived runtime. The cleanup future returns Ready inside ~10s (Nomad purge HTTP + bookkeeping); `block_on` does one final `self.run()` cycle where the 5 s timer registers but returns Pending; `block_on` returns; `Runtime::Drop` runs `Scheduler::clear()` (compio 0.11 `runtime/mod.rs:389-399` + `scheduler/mod.rs:233-251`) which DROPS the pending future; the vm_index release never fires; the slot leaks until the next controller-boot orphan prune. The sibling `stop_inner` call site at `:1324` is NOT affected because it runs on the long-lived ntex-worker compio runtime, which outlives the 5 s delay. Existing CreateGuard tests (`create_guard_releases_vm_index_when_no_job_submitted` / `create_guard_drop_runs_without_ambient_compio_runtime`) all used `Duration::ZERO`, which collapses spawn-and-sleep to near-synchronous and fits inside the post-Ready `self.run()` cycle — so they passed despite the bug. **Fix shape — Option A (preferred per the task brief; minimal)**: at the `CreateGuard::drop` call site only, replace `VmIndexAllocator::spawn_delayed_release(…)` with an inline `if !release_delay.is_zero() { compio::time::sleep(release_delay).await; } vm_index_allocator.lock().release(i); tracing::info!(…)` INSIDE the cleanup future. The short-lived runtime stays alive until the cleanup future completes, so an inline sleep + release runs to completion before `block_on` returns. `spawn_delayed_release` itself is UNCHANGED (the helper is correct for long-lived-runtime callers — `stop_inner` at `:1324` continues to use it). Option B (inline flag on the helper) rejected as brittle; Option C (long-lived global queue) rejected as over-engineered. **Inline rustdoc** at the fixed call site documents why this site differs from `stop_inner` and explicitly warns "Do NOT unify the two call sites by changing `spawn_delayed_release` — the bug is the runtime lifetime mismatch, not the helper itself." **Test**: new `create_guard_drop_releases_vm_index_under_isolated_runtime` (`#[test]`, NOT `#[compio::test]`) drops a `CreateGuard` with `release_delay = Duration::from_millis(100)`, `job_submitted = false`, `host_dir_created = false` against a single-element pool (floor=ceil=99); polls up to 5 s for the index to reappear via `pool.alloc()`. The 100 ms delay is the load-bearing detail: a zero delay would not catch the bug (matches the existing test gap). **Confirmed regression-catching**: temporarily reverting just the fix (test kept) makes the new test FAIL with the precise panic message documented in the assert text — the index never returns to the pool within the 5 s deadline. **Verification**: `cargo build -p zeroship-sandbox --tests` clean (only pre-existing `restore.rs:43` + `sweep.rs:96` warnings; neither touched). `cargo test -p zeroship-sandbox --lib` 540 → 541 passed / 0 failed / 1 ignored. **Net diff**: +128 / -6 LOC in `crates/sandbox/src/backend/nomad_ch.rs` (the +128 is mostly rustdoc — ~30 LOC of inline rationale on why this call site is inlined + ~95 LOC of test body + rustdoc; the actual behavioural fix is the ~10 LOC swap from the helper call to inline-sleep + release + tracing::info!). Single file touched per the task's HARD constraint. Source: concurrency-r28 R28-C1 CRITICAL.

## Round-50 reviewer findings (2026-05-25)

- [r30-A1] **OPEN (IMPORTANT, architecture-r30)** `crates/sandbox/src/backend/nomad_ch.rs` — `build_nomad_job_json_with` + `build_restore_nomad_job_json` still emit the `ZSBX_*` env block (12 keys) in `Tasks[].Env` post-T-8 cutover; self-described comment marks it "debugging parity only — the Go driver ignores Env". Same instance-vs-class shape r29-A1/A2 flagged: manual sync between typed `Config` and Env-block keys. Blocker: none (mechanical delete). Source: architecture-r30. Last considered: 2026-05-25.
...
- [R33-I1] **CLOSED** (this commit) — per-user `home.img` mkfs fence landed on `AppState` as `user_home_mkfs_locks: Arc<Mutex<HashMap<UserId, Arc<Mutex<()>>>>>` (DashMap-equivalent shape; dashmap not in the workspace, std-only impl preserves the lazy per-user insert + Arc::clone semantics). Installed on the inner `NomadCHBackend` via `OnceLock` mirroring the `nomad_stop_permits` precedent. The `home_h` spawned thread inside `crates/sandbox/src/backend/nomad_ch.rs::try_create`'s `std::thread::scope` block acquires the per-user inner `Mutex` across the `create_ext4_image_if_missing(&user_home_img, ...)` call; the `workspace_h` thread is unchanged (workspace.img is per-sandbox, no race). 3 unit tests pin install/lookup/serialization invariants — including the load-bearing "home_h serialised, workspace_h stays parallel" assertion. Source: concurrency-r33 IMPORTANT.

- [R33-T1] **OPEN (MINOR, test-coverage-r33)** R32-P1's `std::thread::scope` block at `nomad_ch.rs:1155-1174` has no test coverage; the inner helper `create_ext4_image_if_missing` is covered (3 tests) but the parallel-threading shape isn't. ~30 LOC test. Blocker: none. **Partial coverage added in R33-I1 commit**: `user_home_mkfs_locks_serialises_home_but_not_workspace` exercises a synthetic c=2 same-user playback of the home_h serialisation + workspace_h parallelism invariants. Still open: a test that drives the full `std::thread::scope` shape (both `s.spawn`s + the panic-payload `unwrap_or_else` paths) end-to-end via a mocked `create_ext4_image_if_missing`.

...
- [r7-C-followup] **OPEN** — pool-lifecycle: R26-C1 thread-local `Rc<Pool>` cache (`ee702d5f`) does not start `pool.start_housekeeper()` and does not constrain `pool_max=16`, so under stress each warmed compio worker thread holds 2-16 conns per cached role indefinitely (no idle reap, no max-lifetime rotation). Steady-state floor per worker: 32 threads × 2 roles × min_idle(2) = 128; theoretical max 32 × 2 × 16 = 1024. The pg-cap bump to `max_connections=500` (r7-A above) absorbs the floor but not bursty saturation — a sustained c=20 × 3 workers stress that warms most threads simultaneously could still cross 500. Two follow-up options: **Option A** (smallest controller change): call `pool.start_housekeeper()` immediately after each thread-local install at `crates/sandbox/src/db.rs:629` + `:657` — adds idle-timeout reaping (10 min default) + max_lifetime rotation (30 min default) so conns DO release back to pg over time. **Option B** (sizing knob): expose `SANDBOX_PG_POOL_MAX` via a per-thread cap suitable for the warmed-thread count, e.g. drop default from 16 to 4. Both options together would also work. **Why not in this sprint**: mandate explicitly capped controller code touches at "<30 LOC controller-side" and routed regression fixes via a separate sprint. r7-A solves the immediate wedge; this follow-up is the structural fix. **Verification needed when picked up**: cluster stress-r8 retry with the chosen option engaged, plus a unit test pinning that `pool.start_housekeeper()` is called from `install_pool` (or that `pool_max` defaults to the new value). Source: r7-C diagnostic (this entry's parent closure).

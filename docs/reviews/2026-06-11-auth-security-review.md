...
- **File:** `crates/zeroship-gateway/src/compiled.rs:449-525` (`resolve_effective_policy`) + `:555-586` (`build_inheritance_chain`); enabling shape `crates/zeroship-bundle/src/manifest.rs:304-313` (`Manifest::passthrough`).
...
- **File:** `crates/zeroship-runtime/src/transport/ssrf.rs:56-65` (`is_blocked_ip` IPv6 arm).

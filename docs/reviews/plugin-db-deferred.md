...
### ~~[I3] Migration advisory lock has no RAII guard (security DoS)~~ — CLOSED drain-cycle 2026-05-22 (post-18:17)
- **Closed by**: this cycle's commit (see git log; PR landed as `plugin-db/backend: introduce OwnedLockGuard for owned-client lock RAII (security [I3])`). Implements arch r13 §I-R13-2 option (b) — sibling `OwnedLockGuard<'b, B: LockManager<Client = compio_postgres::Client>>` wrapping `Option<compio_postgres::Client>` + `LockScope` + `&backend`. 5 hardening rounds of `LockGuard`'s pattern (extract, defer-released-flag, must_use, lifted-warn, log-on-fn-err) ported to the owned-client case. Three migration call sites (`migrations.rs:312, 552, 777` at pre-PR HEAD) collapse from open-coded `backend.try_acquire` / `backend.release` pairs to typed-RAII via `OwnedLockGuard::{try_acquire, assume_held, release, into_held}`. The three Err rails the deferred entry called out by name (`exec_commit_batch` BEGIN failure, audit-lock failure, dry-run ROLLBACK / real-run COMMIT failure) now release via guard drop — the owned client terminates its PG session on drop, PG auto-releases the session-scoped advisory lock as a *guaranteed* cleanup, and a `tracing::warn` with the F1 family shape `{app_id, name, transition}` surfaces the missed-explicit-release for operators. The F1 warn-shape contract `{app_id, name, error}` on unlock-SQL failure is preserved verbatim (i6 snapshot test still pinned). 5 new `OwnedLockGuard` lifecycle/structural tests land alongside; 373 lib / 397 hardening tests pass (was 368 / 392). Caller audit: zero remaining raw `LockManager::{try_acquire, release}` call sites in production code — all routed through either `LockGuard` (pooled client / register-model) or `OwnedLockGuard` (owned client / migrations).
- **Pickable this cycle**: yes — closed.

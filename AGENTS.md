...
- **The platform schema has one migration source and one authoring package.** Creator and platform migrations both import the single `@zeroship/migrate`. A second implementation records into another ambient singleton and lets the host drain empty, so there is no alias and no second SDK package. History: `docs/reviews/2026-08-28-migrate-dsl-fork-divergence.md`.

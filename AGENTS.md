...
| **Auth** (OIDC IdP + login UI + RPs) | `docs/reference/auth.md` · `crates/zeroship-auth/` · `crates/zeroship-gateway/src/oidc_rp.rs` · gates: `cargo xtask test auth` (owned PostgreSQL and service fixtures) + the `auth_ui` browser case in `crates/zeroship-auth/tests/` (real Chromium against the auth router, ignored by default) |
...
source scanners as their suites are migrated. There is no central script-count
...
assertions. Other repository gates remain shell scripts under `tests/`.

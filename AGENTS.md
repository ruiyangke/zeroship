...
# git submodule at tests/wpt (no vendored mirror under crates/). The
# `crates/runtime/tests/wpt_*.rs` runners `include_str!` upstream files
# verbatim (test files stay pristine — any shims/skips/sentinels live
# in the Rust runner code). After cloning the repo:
git submodule update --init --depth=1
./tests/setup-wpt.sh                                # ~12MB sparse checkout

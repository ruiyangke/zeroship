...
# crates/zeroship-data-orm/src/live_tests/integration.rs skipped-and-counted-as-passed,
...
if command -v docker >/dev/null 2>&1; then
  ...
  if [ -n "$pg_cid" ]; then
    ...
    if [ "$wal" = "logical" ]; then
      ...
      echo "       crates/zeroship-data-orm/src/live_tests/integration.rs will FAIL" >&2

...
# Per-arm anti-vacuity accounting, wired on the --range path only (see below).
# --self-test rules on hand-written fixtures, not real commits, and would
# refuse for declaring zero arms if it called gate_arms_init/gate_arms_finish
# at all - the arm contract is about a REAL enumeration collapsing, and
# --self-test has no enumeration to collapse.
...
main() {
  ...
  case "$1" in
    ...
    --range)
      ...
      #
      # gate_arm's floor reuses this check's own existing threshold (examined
      # >= 1) rather than a "well under today's count" number: unlike the other
      # three gates in this pass, this arm's subject is a CALLER-SUPPLIED range,
      # not a roughly fixed corpus - CI commonly calls this over a 2-5 commit PR
      # range, so a floor derived from today's measurement (115 commits on
      # origin/main..HEAD in this worktree) would refuse a normal small range.
      # What this arm can honestly guard against is the range enumerating to
      # NOTHING, which is exactly what already made this a hand-rolled check.
      if ! gate_arm commits_linted "$n" 1; then
        printf 'commit-msg gate: range %s matched 0 commits; nothing was checked.\n' "$2"
        return 1
      fi
      printf 'commit-msg gate: checked %d commits, %d rejected.\n' "$n" "$fail_count"
      gate_arms_finish || fail_count=$((fail_count + 1))
      [ "$fail_count" -eq 0 ]
      return
      ;;
    --self-test)

...
ZEROSHIP_CONTROL_PORT=9178; ZEROSHIP_WORKER_PORT=8078; ZEROSHIP_GATEWAY_PORT=8068; PG_PORT=5478; RP_PORT=19178
...
GW_OK=0; for i in $(seq 1 $N_REQ); do for _ in 1 2 3; do [ "$(curl -s -o /dev/null -w '%{http_code}' -H 'Host: mm-probe.localhost' -H 'content-type: application/json' --data "$BODY" "http://localhost:$ZEROSHIP_GATEWAY_PORT/probe/$i")" = "200" ] && { GW_OK=$((GW_OK+1)); break; }; sleep 0.2; done; done

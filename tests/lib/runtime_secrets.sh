...
# `PLATFORM_CLI_ISSUABLE_SCOPES` (read it in
# crates/zeroship-core/src/device_grant.rs; the enumeration that used to stand
# here had already gone stale), which carries no env:*, no secrets:write and no billing
# scope -- all of which these harnesses ask for.
...
#
# One ed25519 private key per platform service that holds one, plus ONE peer
# document naming every public half. Every harness that boots more than one
# binary needs both, because the internal edges between them carry a
...
e2e_export_service_keys() {
  ...
  for svc in gateway control auth; do
  ...
  # and without them the auth binary refuses to start rather than running with a
  # preflight that cannot be authenticated.
  ZEROSHIP_AUTH_SERVICE_KEY_FILE="${ZEROSHIP_AUTH_SERVICE_KEY_FILE:-$secret_dir/svc-auth.pem}"
...
_e2e_write_service_assertion_js() {
  local path="$1"
  cat > "$path" <<'SVC_ASSERTION_JS'
// Generated by tests/lib/runtime_secrets.sh.
...
// thumbprint DERIVED from the key (never written into the peer document), `sub`
// equals `iss`, `aud` is the ONE callee's issuer identifier as a string, and
// `jti` is required because single use is what the replay store claims.
import { readFileSync } from "node:fs";
import { createHash, randomBytes } from "node:crypto";

const [keyPath, joseUrl, issuer, audience] = process.argv.slice(2);

const jose = await import(joseUrl);
const key = await jose.importPKCS8(readFileSync(keyPath, "utf8"), "EdDSA", {
  extractable: true,
});
const x = (await jose.exportJWK(key)).x;
const kid = createHash("sha256")
...
const now = Math.floor(Date.now() / 1000);
// Well inside MAX_ASSERTION_LIFETIME. The callee caps the lifetime, so a longer
// one here would be refused rather than honoured.
const jwt = await new jose.SignJWT({ jti: randomBytes(16).toString("base64url") })
  .setProtectedHeader({ alg: "EdDSA", typ: "svc-assertion+jwt", kid })
  .setIssuer(issuer)
  .setSubject(issuer)
...
# e2e_mint_service_assertion <private-key.pem> <issuer-service> <audience-service>
#
# Echoes ONE service assertion on stdout, e.g.
#
#   e2e_mint_service_assertion "$WORK/svc-gateway.pem" svc/gateway svc/control
#
# The service names are the ones `e2e_export_service_keys` publishes, and the
# trust domain is prefixed here so a caller cannot spell it two ways.
#
# WHY A HARNESS MAY MINT AT ALL. The comment on E2E_STALE_WORKER_BEARER above
# says these scripts cannot mint a signed credential, and that was true of the
# SHARED-BEARER era it describes: there was no per-service key to sign with. It
# is not true now - `e2e_export_service_keys` writes each service's private key
# into the workspace, so a harness that wants to drive one internal edge holds
# exactly what that service holds. It buys nothing a caller could not get by
# starting the service itself; what it buys is driving ONE edge without one.
e2e_mint_service_assertion() {
  local key="$1" iss="$2" aud="$3" js
  [ -s "$key" ] || {
    echo "e2e_mint_service_assertion: no private key at ${key:-<unset>}" >&2
    return 1
  }
  [ -n "$iss" ] && [ -n "$aud" ] || {
    echo "e2e_mint_service_assertion: an issuer and an audience service are required" >&2
    return 1
  }
  [ -f "${E2E_JOSE_JS:-}" ] || {
    echo "e2e_mint_service_assertion: missing jose at ${E2E_JOSE_JS:-<unset>} (run pnpm install)" >&2
    return 1
  }
  command -v node >/dev/null 2>&1 || {
    echo "e2e_mint_service_assertion: node is required" >&2
    return 1
  }
  js="$(dirname "$key")/service-assertion.mjs"
  [ -s "$js" ] || _e2e_write_service_assertion_js "$js" || return 1
  node "$js" "$key" "file://$E2E_JOSE_JS" \
    "spiffe://zeroship.ai/$iss" "spiffe://zeroship.ai/$aud"
}

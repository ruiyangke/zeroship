...
tier_of_file() {
  case "$1" in
    ...
    ./backend/postgres.rs|./backend/pg_row_json.rs|./backend/pg_session_sql.rs|./backend/pg_autocommit.rs|./backend/pg_error.rs) echo PG ;;
...
tier_of_target() {
  case "$1" in
    backend::sqlite)                                     echo SQLITE ;;
    backend::postgres|backend::pg_row_json|backend::pg_session_sql|backend::pg_autocommit|backend::pg_error) echo PG ;;

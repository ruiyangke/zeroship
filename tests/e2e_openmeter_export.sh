...
ZEROSHIP_CONTROL_PORT=9173; ZEROSHIP_WORKER_PORT=8073; ZEROSHIP_GATEWAY_PORT=8063; ZEROSHIP_MIGRATE_SERVER_PORT=9073; PG_PORT=5473; RP_PORT=19173
...
for i in $(seq 1 $N_REQ); do
  for _ in 1 2 3; do
    RESPONSE="$(curl -s -w '\n%{http_code}' -H 'Host: om-probe.localhost' -H 'content-type: application/json' --data "$BODY" "http://localhost:$ZEROSHIP_GATEWAY_PORT/probe/$i")"

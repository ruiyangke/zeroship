...
ZEROSHIP_CONTROL_PORT=9176; ZEROSHIP_WORKER_PORT=8076; ZEROSHIP_GATEWAY_PORT=8066; ZEROSHIP_MIGRATE_SERVER_PORT=9076; PG_PORT=5476; RP_PORT=19176
...
probe_req(){ local out; out="$(curl -s -D - -o /dev/null -H 'Host: spend-probe.localhost' "http://localhost:$ZEROSHIP_GATEWAY_PORT/probe/$1" 2>/dev/null)"; local code warn; code="$(printf '%s' "$out" | awk 'NR==1{print $2}')"; warn=0; printf '%s' "$out" | grep -qiE '^x-zs-spend-warn:' && warn=1; echo "${code:-000} $warn"; }
...
READY=0; for _ in $(seq 1 30); do [ "$(curl -s -o /dev/null -w '%{http_code}' -H 'Host: spend-probe.localhost' -H 'content-type: application/json' --data "$BODY" "http://localhost:$ZEROSHIP_GATEWAY_PORT/probe/ready")" = "200" ] && { READY=1; break; }; sleep 1; done
[ "$READY" = "1" ] && pass "app reachable" || { fail "app never reachable"; tail -15 "$WORK/gate.log"; exit 1; }
GW_OK=0; LAST_BODY=""
for i in $(seq 1 $N_REQ); do
  for _ in 1 2 3; do
    RESPONSE="$(curl -s -w '\n%{http_code}' -H 'Host: spend-probe.localhost' -H 'content-type: application/json' --data "$BODY" "http://localhost:$ZEROSHIP_GATEWAY_PORT/probe/$i")"
...
C=0
for _ in $(seq 1 30); do C="$(curl -s "$CONTROL_URL/api/apps/$APP/projected-charge" -H "Authorization: Bearer $ADMIN_TOKEN" | jget '.projected_charge_cents')"; [ -n "$C" ] && [ "$C" -ge 20 ] 2>/dev/null && break; sleep 2; done
...
# probe per band adds.
read_charge(){ curl -s "$CONTROL_URL/api/apps/$APP/projected-charge" -H "Authorization: Bearer $ADMIN_TOKEN" | jget '.projected_charge_cents'; }
assert_band(){ # $1=label $2=target_pct $3=expect_state $4=expect_gw_code $5=expect_warn(0|1)
  ...
  set_limit "$APP" "$lim"
  st=""; for _ in $(seq 1 15); do st="$(spend_state "$APP")"; [ "$st" = "$want_state" ] && break; curl -s -o /dev/null -X POST -H "Authorization: Bearer $ZEROSHIP_CONTROL_KEY" "$CONTROL_URL/internal/spend/reconcile"; sleep 1; done
...
# 30 assertions; migrate-server health and app migration apply add two mandatory
# rows, so this version expects 32. The floor counts PASS+FAIL because a mutation moves

...
assert_band(){ # $1=label $2=target_pct $3=expect_state $4=expect_gw_code $5=expect_warn(0|1)
  ...
  set_limit "$APP" "$lim"
  st=""; for _ in $(seq 1 15); do st="$(spend_state "$APP")"; [ "$st" = "$want_state" ] && break; curl -s -o /dev/null -X POST -H "Authorization: Bearer $CONTROL_KEY" "$CONTROL_URL/internal/spend/reconcile"; sleep 1; done

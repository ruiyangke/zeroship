module github.com/zeroship/nomad-driver-ch

go 1.25.8

// Forked from github.com/hashicorp/nomad-driver-virt on 2026-05-25 for
// Cloud Hypervisor support. libvirt-specific dependencies (libvirt.org/go,
// diskfs, gopacket, ceph, go-iptables) are intentionally NOT carried over —
// the CH driver talks to ch-remote over a Unix-domain HTTP API instead.
//
// Pin to the same Nomad release the upstream driver tracks; bump when we
// upgrade the cluster Nomad version (see DESIGN.md § "Open questions").
require (

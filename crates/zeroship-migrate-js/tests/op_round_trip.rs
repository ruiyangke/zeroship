...
//!    `UPDATE_CORPUS=1 cargo test -p zeroship-migrate-js --test op_round_trip` to
...
//!
//! These tests carry V8 (via the recorder) and so live in `zeroship-migrate-js`,
//! the only migrate-family crate with a JS engine.
...
fn every_op_variant_has_a_fixture() {
    ...
    assert_eq!(
        expected.len(),
        47,
        "the closed Op set has 47 variants (28 portable/core enum/domain/sequence/trigger/view/comment variants + 19 @zeroship/migrate/pg vendor)"
    );

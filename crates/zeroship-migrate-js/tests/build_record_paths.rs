...
//! child must be built (`cargo build -p zeroship-migrate-js --bins`); the test
...
fn local_and_hosted_paths_yield_same_typed_value_checksum() {
    ...
    impl RecorderClient for LocalChildHostedClient {
        fn record(
            ...
            use zeroship_migrate_js::recorder_service::{
                spawn_sandboxed_record, RecordRequest,
            };
            use zeroship_migrate_js::{ResourceBudget, SandboxPosture};

...
//! to a real `.ts` MODULE ([`zeroship_migrate_js::ENV_DTS_FILE`] = `env.db.ts`).
//!
...
use zeroship_migrate_js::{eval_schema_to_ir, render_artifacts};
...
/// (`<root>/crates/zeroship-migrate-js`).
fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(Path::parent)
        .expect("crate is <root>/crates/zeroship-migrate-js")
        .to_path_buf()
}

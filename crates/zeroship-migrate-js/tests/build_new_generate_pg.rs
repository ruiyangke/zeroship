...
//!
//! Requires `zeroship_migrate_test` on :5440 (the existing `*_pg.rs` convention:
...
fn assert_child_built() {
    ...
    assert!(
        bin.exists(),
        "recorder child binary missing at {} — run `cargo build -p zeroship-migrate-js --bins` first",
        bin.display()
    );
}

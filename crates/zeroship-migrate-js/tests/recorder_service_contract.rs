...
use zeroship_migrate_js::recorder_service::{Authorizer, ConcurrencyLimits};
...
fn record_succeeds_for_owned_app() {
    let (s, calls) = svc();
    let res = s
        .record("pat_alice", "app_alice", MIGRATION, "m1")
...
fn record_fails_closed_for_unowned_app() {
    ...
    let err = s
        .record("pat_alice", "app_someone_else", MIGRATION, "m1")
...
fn record_fails_closed_for_unknown_token() {
    let (s, _) = svc();
    let err = s
        .record("pat_unknown", "app_alice", MIGRATION, "m1")
...
fn per_token_concurrency_limit_rejects_a_burst() {
    ...
    let h = std::thread::spawn(move || {
        ...
        let _ = s1.record("pat_alice", "app_alice", &slow_owned, "slow");
    });
    ...
    let second = s.record("pat_alice", "app_alice", MIGRATION, "m2");
    match second {

...
pub enum GenTypesError {
    ...
    Fold(zeroship_migrate::FoldError),
...
fn collection_needs_schema_builder(meta: &RuntimeCollectionMetadata) -> bool {
    meta.options.soft_delete
        ...
        || !matches!(meta.options.strictness, zeroship_migrate::TableStrictness::Strict)
...
fn render_collection_chains(meta: &RuntimeCollectionMetadata) -> String {
    ...
    match meta.options.strictness {
        zeroship_migrate::TableStrictness::Strict => {}
        zeroship_migrate::TableStrictness::Lenient => out.push_str(".strictness(\"lenient\")"),
        zeroship_migrate::TableStrictness::Off => out.push_str(".strictness(\"off\")"),
    }

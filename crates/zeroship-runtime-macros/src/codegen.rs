                ...
                ::zeroship_runtime::macro_runtime::state::OpErrorKind::CodedError {
                    ...
                    if let Ok(__obj) = v8::Local::<v8::Object>::try_from(__exc) {
                        ...
                        if let Some(&__s) = __status.as_ref() {
                            let __sk = v8::String::new(#scope_expr, "status").unwrap();
                            let __sv = v8::Integer::new_from_unsigned(
                                #scope_expr,
                                u32::from(__s),
                            );
                            __obj.set(#scope_expr, __sk.into(), __sv.into());
                        }
                    }
                    __exc
                }

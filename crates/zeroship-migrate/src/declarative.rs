...
fn null_clause(c: &ColumnSnapshot, dialect: SqlDialect, inline_pk: bool) -> &'static str {
    if c.nullable || (matches!(dialect, SqlDialect::Sqlite) && sqlite_identity_pk(c, inline_pk)) {
        ""
    } else {
        " NOT NULL"
    }
...
pub struct FieldDescriptor {
    ...
    #[serde(default)]
    pub generated: Option<crate::ir::GeneratedCol>,
    /// A SQL identity column facet. Mirrors `identity` on the migrate FieldDef
    /// bridge.
    #[serde(default)]
    pub identity: Option<crate::ir::IdentityCol>,
}
...
fn generated_column_snapshot(
    ...
    let expr = crate::dml::render_expr_inline(&generated.expr, dialect).map_err(|e| {

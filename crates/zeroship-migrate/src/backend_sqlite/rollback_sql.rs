...
//! is P3b). We do NOT half-implement a rebuild here. The classifier
...
//!
//! # Confinement (P2 invariants, unchanged)
//!
...
use crate::executor::RollbackError;
...
///   rebuild (P3b) — refused before any statement runs; nothing changes.
...
pub(crate) async fn rollback_one_transactional(
    ...
    // P5 GATE: refuse a rebuild-needing `down` UP FRONT (before BEGIN), so nothing
    // is half-done. This is the additive-only boundary; the rebuild is P3b.
...
/// (the same H1 autocommit-probe discipline the apply path uses).
async fn run_rollback_txn(
    ...
    match result {
        ...
        Err(e) => {
            ...
            // Same H1 discipline as apply: the AUTOCOMMIT state — not the ROLLBACK
...
/// would require the 12-step table REBUILD (P3b), rather than an operation SQLite

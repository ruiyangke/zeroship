...
//! |---------------|---------------------------|--------------------------------|
//! | identity      | int / bigInt              | lowers, `USING` cast emitted   |
...
// ---------------------------------------------------------------------------
// GAP 1 - an IDENTITY column may only become integer / bigint.
// ---------------------------------------------------------------------------
...
fn a_retype_of_an_identity_column_to_a_non_integer_type_is_refused_on_both_routes() {
    for to_type in [
        ...
        for (route, verdict) in both_routes(IDENTITY_COL, to_type) {
            ...
            assert!(
                error.contains("int") && error.contains("bigInt"),
...
fn a_retype_of_an_identity_column_to_an_integer_type_still_lowers_on_both_routes() {
    ...
    // with `attidentity` intact - so refusing them would deny a migration the
    // database honours.
    for to_type in [r#""bigInt""#, r#""int""#] {
        for (route, verdict) in both_routes(IDENTITY_COL, to_type) {

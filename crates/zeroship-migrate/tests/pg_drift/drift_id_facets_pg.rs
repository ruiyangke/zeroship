...
async fn live_postgres_introspects_identity_default_format_and_reference_drift() {
    ...
    let result: Result<(), String> = async {
        ...
        let migrations = IrAuthor::new(zeroship_migrate::shipping_vendors(),
            &schema,
            OWNER,
            &zeroship_migrate_postgres::DIALECT,
        ...
        let mut shadow_clean_drift = diff_snapshots(zeroship_migrate::shipping_vendors(), &expected, &shadow_clean);
        ...
        for (mutation, expected_actual) in single_mutations {
            let actual_snapshot =
                snapshot_after_mutation(&session, &schema, &mutation).await?;
            let drift = diff_snapshots(zeroship_migrate::shipping_vendors(), &expected, &actual_snapshot);

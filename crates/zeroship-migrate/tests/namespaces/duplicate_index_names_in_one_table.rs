...
fn two_create_index_ops_sharing_a_name_are_refused() {
    ...
    let refusal = verdict_envelope(&ops, &zeroship_migrate_postgres::DIALECT).expect_err(
...
fn reusing_an_index_name_after_dropping_it_is_still_allowed() {
    ...
    verdict_envelope(&ops, &zeroship_migrate_postgres::DIALECT)
...
fn an_index_op_colliding_with_an_inline_index_is_refused() {
    ...
    let refusal = verdict_envelope(ops, &zeroship_migrate_postgres::DIALECT)

...
use zeroship_migrate::frontend::recorder_http::{handle_record, RecordHttpOutcome, RecordHttpRequest};
use zeroship_migrate::frontend::recorder_service::Authorizer;
use zeroship_migrate::frontend::RecorderService;
...
fn oversized_app_id_is_rejected_before_spawn() {
    ...
    let huge_id = "a".repeat(zeroship_migrate::frontend::recorder_http::MAX_APP_ID_BYTES + 1);
    let req = RecordHttpRequest {
        ts_source: MIGRATION.to_string(),
        app_id: huge_id,
        schema_types_blob: None,
...
fn oversized_schema_types_blob_is_rejected_before_spawn() {
    ...
    let huge_blob = "a".repeat(zeroship_migrate::frontend::recorder_http::MAX_SCHEMA_TYPES_BYTES + 1);
    let req = RecordHttpRequest {
        ts_source: MIGRATION.to_string(),
        app_id: "app_x".into(),
...
fn oversized_ts_source_is_rejected_before_spawn() {
    ...
    let huge = "a".repeat(zeroship_migrate::frontend::recorder_http::MAX_TS_SOURCE_BYTES + 1);
    let req = RecordHttpRequest {
        ts_source: huge,
        app_id: "app_x".into(),
        schema_types_blob: None,

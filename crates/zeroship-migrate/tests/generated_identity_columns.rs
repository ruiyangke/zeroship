...
fn pk(columns: &[&str]) -> IrConstraint {
    IrConstraint {
        name: None,
...
fn auto_increment_identity_by_default_renders_per_dialect() {
    ...
    let pg = lower_create(SqlDialect::Postgres, create_table(vec![id.clone()], vec![pk_id()])).unwrap();
    assert!(
    ...
    let up = lower_create(SqlDialect::Sqlite, create_table(vec![id.clone()], vec![pk_id()])).unwrap();
    assert_eq!(
        up,
        format!(
    ...
    let mysql = lower_create(SqlDialect::Mysql, create_table(vec![id], vec![pk_id()])).unwrap();
    assert!(
...
fn identity_always_is_postgres_only() {
    ...
    id.nullable = Some(false);
    id.identity = Some(IdentityCol { always: true });
    let op = create_table(vec![id], vec![pk_id()]);
...
fn auto_increment_requires_single_column_primary_key_on_sqlite_and_mysql() {
    ...
    validate_platform(
        &ir_platform(create_table(vec![seq.clone()], Vec::new())),
        ValidatorDialect::Postgres,
    )
    ...
    for dialect in [ValidatorDialect::Sqlite, ValidatorDialect::Mysql] {
        let err = validate_platform(
            &ir_platform(create_table(vec![id.clone(), tenant.clone()], vec![pk(&["id", "tenant_id"])])),
            dialect,
        )

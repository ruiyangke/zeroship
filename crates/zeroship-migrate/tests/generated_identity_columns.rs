use zeroship_migrate::{
    ...
    IrDefault, IrFlagsOverride, IrLowerError, IrScalar, IrAuthor,
...
fn pg_generated_stored_column_renders_exact_create_table_ddl() {
    ...
    assert_eq!(
        up,
        r#"CREATE TABLE "app"."line_items" ("created_at" timestamptz NOT NULL DEFAULT now(), "created_by" text, "deleted_at" timestamptz, "id" text PRIMARY KEY NOT NULL, "qty" integer, "total_cents" integer GENERATED ALWAYS AS (("qty" * "unit_cents")) STORED NOT NULL, "unit_cents" integer, "updated_at" timestamptz NOT NULL DEFAULT now(), "updated_by" text, "version" integer NOT NULL DEFAULT 1)"#,
    );
}
...
fn sqlite_generated_stored_and_virtual_columns_render_exact_create_table_ddl() {
    ...
    assert_eq!(
        stored,
        format!(
            r#"CREATE TABLE "line_items" ("created_at" TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, "created_by" TEXT, "deleted_at" TEXT, "id" TEXT PRIMARY KEY NOT NULL, "qty" INTEGER, "total_cents" INTEGER GENERATED ALWAYS AS (("qty" * "unit_cents")) STORED NOT NULL, "unit_cents" INTEGER, "updated_at" TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, "updated_by" TEXT, "version" INTEGER NOT NULL DEFAULT 1){SQLITE_SYSTEM_INDEXES}"#
        ),
    );
    ...
    assert_eq!(
        virtual_col,
        format!(
            r#"CREATE TABLE "line_items" ("created_at" TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, "created_by" TEXT, "deleted_at" TEXT, "id" TEXT PRIMARY KEY NOT NULL, "qty" INTEGER, "total_cents" INTEGER GENERATED ALWAYS AS (("qty" * "unit_cents")) VIRTUAL NOT NULL, "unit_cents" INTEGER, "updated_at" TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, "updated_by" TEXT, "version" INTEGER NOT NULL DEFAULT 1){SQLITE_SYSTEM_INDEXES}"#
        ),
    );
}
...
fn generated_column_cannot_also_have_default() {
    ...
    let err = validate_ir(
        &ir(create_table(vec![col("qty", ColType::Int), col("unit_cents", ColType::Int), total], None)),
...
fn pg_identity_always_and_by_default_render_exact_create_table_ddl() {
    ...
    assert_eq!(
        always,
        r#"CREATE TABLE "app"."line_items" ("created_at" timestamptz NOT NULL DEFAULT now(), "created_by" text, "deleted_at" timestamptz, "id" bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL, "updated_at" timestamptz NOT NULL DEFAULT now(), "updated_by" text, "version" integer NOT NULL DEFAULT 1)"#,
    );
    ...
    assert_eq!(
        by_default,
        r#"CREATE TABLE "app"."line_items" ("created_at" timestamptz NOT NULL DEFAULT now(), "created_by" text, "deleted_at" timestamptz, "id" bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY NOT NULL, "updated_at" timestamptz NOT NULL DEFAULT now(), "updated_by" text, "version" integer NOT NULL DEFAULT 1)"#,
    );
}
...
fn auto_increment_identity_by_default_renders_per_dialect() {
    ...
    let pg = lower_create(SqlDialect::Postgres, create_table(vec![id.clone()], pk_id())).unwrap();
    assert!(
    ...
    assert_eq!(
        up,
        format!(
            r#"CREATE TABLE "line_items" ("created_at" TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, "created_by" TEXT, "deleted_at" TEXT, "id" INTEGER PRIMARY KEY AUTOINCREMENT, "updated_at" TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, "updated_by" TEXT, "version" INTEGER NOT NULL DEFAULT 1){SQLITE_SYSTEM_INDEXES}"#
        ),
    );
    assert!(
...
fn auto_increment_requires_single_column_primary_key_on_sqlite_and_mysql() {
    ...
    for dialect in [ValidatorDialect::Sqlite, ValidatorDialect::Mysql] {
        let err = validate_platform(
            &ir_platform(create_table(vec![id.clone(), tenant.clone()], pk(&["id", "tenant_id"]))),
            dialect,
        )

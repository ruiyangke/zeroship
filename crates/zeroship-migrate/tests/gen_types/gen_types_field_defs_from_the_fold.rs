...
///   cargo test -p zeroship-migrate --test gen_types -- --ignored update_field_defs_goldens
...
#[ignore = "regenerates tests/goldens/field_defs_artifacts.txt; run explicitly with \
            `cargo test -p zeroship-migrate --test gen_types -- --ignored \
...
fn update_field_defs_goldens() {
    ...
    let measured: std::collections::BTreeSet<String> =
        measure_corpus().into_iter().collect();

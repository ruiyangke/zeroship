...
async fn apply_additive(
    ...
    desired: &zeroship_migrate::DesiredSchema,
    live: &SchemaSnapshot,
    author: &zeroship_migrate::DeclarativeAuthor,
    cfg: &ExecutorConfig,
    conn: &Client,
) {
    let plan = engine
...
fn contract_plan(engine: &MigrationEngine, contract: &[Migration], cfg: &ExecutorConfig) -> zeroship_migrate::MigrationPlan {
...
async fn p0_1_declarative_rename_two_deploy_zero_data_loss() {
    ...
    let author = zeroship_migrate::DeclarativeAuthor::new(cfg.project_schema.clone(), "app_acme");
...
async fn p0_4_security_full_stack_guard_dry_run_and_role_backstop() {
    ...
    let guard = zeroship_migrate::SqlGuard::new(guard_cfg(&cfg));

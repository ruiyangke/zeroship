...
//!   `&zeroship_migrate::DialectId`, turning it into a backend exactly where it crosses in — survives
...
//! took `dialect: &zeroship_migrate::DialectId`, and the resolution happens once.
//!
...
//!
//! * A core emitter that still takes `dialect: &zeroship_migrate::DialectId` and looks a backend up.
...
/// silently while a broken scanner and a carrier reverted to `dialect: &zeroship_migrate::DialectId`
/// both go red.
///
...
fn a_schema_emitter_holding_a_backend_never_resolves_another() {
    ...
    assert!(
        ...
        "found {} backend-carrying emitter(s) in schema/query.rs, expected at least \
         ...
         went back to taking a `dialect: &zeroship_migrate::DialectId`, which is the regression this \

...
fn create_table_render_is_byte_identical_pg() {
    ...
    assert!(decl
        .iter()
        .any(|(up, _)| up.contains("CREATE TABLE \"app\".\"widgets\"")));
    assert!(decl
        .iter()
        .any(|(up, _)| up.contains("CREATE UNIQUE INDEX")));
}
...
fn create_table_with_encrypted_column_render_is_byte_identical_pg() {
    ...
    let desc = CollectionDescriptor {
        ...
        fields: vec![FieldDescriptor {
            ...
            ty: "string".into(),
            encrypted: Some(true),
            // Mirror the SDK's `.encrypted()` normalized shape: encrypted columns
    ...
    let ops = vec![Op::CreateTable {
        ...
        name: "vault".into(),
        columns: vec![IrColumn {
            // The IR carries an encrypted column whose plaintext type is a string.
            name: "secret".into(),
            ty: ColType::Text,
            nullable: None,
            default: None,
    ...
    assert!(
        decl.iter()
            .any(|(up, _)| up.contains("zero-migrate:mask:kind=full,classification=pii")),
...
fn create_table_with_explicit_masked_column_render_is_byte_identical_pg() {
    ...
    assert!(
        decl.iter()
            .any(|(up, _)| up.contains("zero-migrate:mask:kind=last4,classification=spi")),
...
fn add_column_render_is_byte_identical_pg() {
    ...
    let desired = test_desired_snapshot(SCHEMA, &[desc]).expect("desired snapshot");
    ...
    assert_eq!(
        decl, ir,
        "addColumn render must be byte-identical across paths"
    );
...
fn create_index_render_is_byte_identical_pg() {
    ...
    let desired = test_desired_snapshot(SCHEMA, std::slice::from_ref(&desc)).expect("desired");
    ...
    assert_eq!(
        decl, ir,
        "createIndex render must be byte-identical across paths"
    );
...
fn alter_column_type_render_is_byte_identical_pg() {
    ...
    let desired = test_desired_snapshot(SCHEMA, &[desired_desc]).expect("desired");
    ...
    assert_eq!(
        decl, ir,
        "setColumnType render must be byte-identical across paths"
    );
    assert!(decl
        .iter()
        .any(|(up, _)| up.contains("ALTER COLUMN \"qty\" TYPE")));
}
...
fn add_constraint_fk_render_is_byte_identical_pg() {
    ...
    let decl: Vec<_> = sql_pairs(&plan.migrations)
        .into_iter()
        .filter(|(up, _)| {
            up.contains("ADD CONSTRAINT \"authors_pinned_fkey\"") && up.contains("FOREIGN KEY")
        })
        .collect();
    assert!(
        !decl.is_empty(),
        "the differ must defer the authors.pinned FK to a stand-alone ADD CONSTRAINT"
    );
    ...
    assert_eq!(
        decl, ir,
        "addConstraint(fk) render must be byte-identical to the differ's deferred FK"
    );
...
fn add_constraint_unique_and_pk_and_drop_constraint_render_pg() {
    ...
    assert_eq!(
        uniq,
        vec![(
            "ALTER TABLE \"app\".\"widgets\" ADD CONSTRAINT \"widgets_slug_key\" UNIQUE (slug)"
                .to_string(),
            Some(
...
#[test]
fn create_table_with_authored_index_is_byte_identical_sqlite() {
    let desc = CollectionDescriptor {
        name: "events".into(),
        owner_app: OWNER.into(),
        fields: vec![FieldDescriptor {
    ...
    let ops = vec![Op::CreateTable {
        ...
        constraints: vec![],
        indexes: vec![IrIndex {
            name: Some("events_kind_idx".into()),
            columns: vec![idx_col("kind")],
            unique: None,
            using: None,
            r#where: None,
            include: Vec::new(),
    ...
    assert!(ir
        .iter()
        .any(|(up, _)| up.contains("CREATE INDEX IF NOT EXISTS \"events_kind_idx\"")));
}
...
fn create_table_with_live_fk_render_is_byte_identical_sqlite() {
    ...
    assert_eq!(
        decl_posts, ir_posts,
        "SQLite FK createTable must be byte-identical across policy-resolved paths"
    );
    assert!(
        ir_posts
            .iter()
            .any(|(up, _)| up.contains("FOREIGN KEY") || up.contains("REFERENCES")),
...
fn add_column_render_is_byte_identical_sqlite() {
    ...
    assert_eq!(
        decl, ir,
        "addColumn render must be byte-identical across paths on SQLite"
    );
    assert!(decl.iter().any(|(up, _)| up.contains("ADD COLUMN")));
}
...
fn create_index_render_is_byte_identical_sqlite() {
    ...
    assert_eq!(
        decl, ir,
        "createIndex render must be byte-identical across paths on SQLite"
    );
    assert!(decl.iter().any(|(up, _)| up.contains("CREATE INDEX")));
}

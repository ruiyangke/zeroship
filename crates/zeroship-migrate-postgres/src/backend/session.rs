...
pub(crate) async fn apply_transactional<D: SqlSession>(
    ...
    if let Some(probe) = &m.existence_guard {
        let live = match super::drift_sql::snapshot_schema_for(conn, probe.schema()).await {
            Ok(s) => s,
            Err(e) => {
                let _ = conn.batch("ROLLBACK").await;
...
pub(crate) async fn rollback_one_transactional<D: SqlSession>(
    ...
    crate::guard::guard(&cfg.guard_config_for(&super::DIALECT))
        .check(down)
...
mod pg_confinement_shape_tests {
    ...
    fn pg_confinement_bracket_is_emitted_from_the_pg_block() {
        ...
        // The migrator role bracket comes from crate::confinement::of(cfg).migrator_role.
    ...
    fn sqlite_shaped_config_carries_no_pg_role_confinement() {
        ...
        assert!(
            crate::confinement::of(&cfg).migrator_role.is_none(),
...
pub(crate) fn search_path_clause(
    ...
    for ext in &crate::confinement::of(cfg).extension_schemas {

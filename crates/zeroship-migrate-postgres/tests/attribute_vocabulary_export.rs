...
//! # Regenerating
//!
//! `cargo test -p zeroship-migrate-postgres --test attribute_vocabulary_export -- --ignored
...
fn emit_attribute_vocabulary() {
    ...
    let on_disk = std::fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "attribute-vocabulary.json missing or unreadable at {}: {e}. Run \
             `cargo test -p zeroship-migrate-postgres --test attribute_vocabulary_export \
    ...
    assert_eq!(
        ...
        "attribute-vocabulary.json is stale. Regenerate with `cargo test -p \
         zeroship-migrate-postgres --test attribute_vocabulary_export -- --ignored \

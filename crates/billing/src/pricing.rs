...
impl PricingTable {
    ...
    pub fn cloudflare_comparable() -> Self {
        ...
        // $12.50/million CPU-ms = 12500 millicents / 1B us (1M ms = 1B us)
        table.add_flat("cpu_us", 12500, 1_000_000_000);
...
mod tests {
    ...
    fn multi_dimensional() {
        ...
        let mut usage = HashMap::new();
        usage.insert("requests".to_string(), 3_500_000);
        usage.insert("cpu_us".to_string(), 17_500_000_000); // 17.5M ms = 17.5B us
        usage.insert("egress_bytes".to_string(), 7_000_000_000); // 7 GB
        let cost = table.compute_cost(&usage);
        // requests: 3.5M * 300 / 1M = 1050
        // cpu: 17.5B us * 12500 / 1B = 218750
        // egress: 7B * 90 / 1B = 630

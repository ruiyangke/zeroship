...
use zeroship_data_sql::render::postgres::{self, RenderError};
...
fn all_three_writes(returning: &Returning) -> Vec<zeroship_data_sql::DbPlan> {
    vec![
        zeroship_data_sql::DbPlan::Insert(insert_one(returning.clone())),
        zeroship_data_sql::DbPlan::Update(update_one(returning.clone())),
        zeroship_data_sql::DbPlan::Delete(delete_one(returning.clone())),
    ]
}
...
fn a_returning_list_carries_exactly_the_fields_it_was_given() {
    let sql = sql_of(&zeroship_data_sql::DbPlan::Insert(insert_one(
        returning_name(),
    )));
    let mut ruled_on = 0_usize;

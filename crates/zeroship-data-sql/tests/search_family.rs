//! The search family: what the nodes refuse, what canonicalisation collapses,
//! and where the two backends are allowed to disagree.
//!
//! # What these arms rule on, and what they cannot
//!
//! Every arm here rules on the **IR and its `PostgreSQL` lowering** - the plan
//! shapes, the refusals, and the SQL text a plan produces. That is the whole
//! subject of this crate, which has no database and no dependencies.
//!
//! Two things they deliberately do NOT establish, stated here rather than left
...
//!    `crates/zeroship-data-orm/src/live_tests/search_ir_live.rs`, which executes these
...
use zeroship_data_sql::render::postgres::{RenderError, render, render_search};
...
/// `crates/zeroship-data-orm/src/live_tests/search_ir_live.rs`, because
...
fn a_tiebreak_pins_the_order_of_equal_distances_and_spells_its_null_placement() {
    ...
    .tiebreak(vec![OrderKey {
        path: zeroship_data_sql::FieldPath::column(column("created_at")),
...
fn the_ranking_key_cannot_be_named_by_a_caller() {
    ...
    .tiebreak(vec![OrderKey {
        path: zeroship_data_sql::FieldPath::column(smuggled),
...
fn a_ranking_scalar_is_unreachable_outside_a_search() {
    ...
    .expect("buildable");
    let select =
        zeroship_data_sql::Select::builder(collection("docs"), search.projection().clone())
            .build()
            .expect("a Select accepts the projection value; the refusal is at render");
    match zeroship_data_sql::render::postgres::render_select(&select) {
...
fn an_aggregate_projection_is_refused_by_a_search() {
    let aggregate = Projection::aggregate(vec![ProjectedField::aggregate(
        zeroship_data_sql::AggregateRef::count_rows(),
...
fn an_aggregate_operand_in_a_search_filter_is_refused() {
    let filter = Predicate::Compare {
        lhs: Operand::Aggregate(zeroship_data_sql::AggregateRef::count_rows()),
        op: CompareOp::Gt,
...
fn tiebreak_keys_dedupe_but_do_not_reorder() {
    let key = |name: &str, direction| OrderKey {
        path: zeroship_data_sql::FieldPath::column(column(name)),
        direction,
        nulls: NullOrder::Last,
    };
    let plan = Search::builder(

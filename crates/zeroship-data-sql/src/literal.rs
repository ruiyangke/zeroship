...
/// `zeroship-data-sql`; when SC-3's port lands, `query.rs`'s copy is the one that
...
/// (`crates/zeroship-data-orm/src/backend/mod.rs:1023`) and `vec0` reads a
...
/// (`crates/zeroship-data-sql/src/compile.rs:4980-5013`), while the `SQLite` arm
/// encodes the same values as a raw little-endian `f32` buffer and binds a BLOB
/// (`crates/zeroship-data-orm/src/backend/sqlite/vector.rs:127`).
///

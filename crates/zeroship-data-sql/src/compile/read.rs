...
use super::*;
use crate::{AggregateRef, FieldPath, Ident, Operand, ProjectionSource, Select};
...
pub fn build_select(
    ...
    for join in plan.joins() {
        sql.push_str(&format!(
            " {} JOIN {}.{} AS {} ON {}",
            match join.kind {
                crate::JoinKind::Inner => "INNER",
                crate::JoinKind::Left => "LEFT",
            },
            namespace.quoted(),
    ...
    if plan.filter() != &crate::Predicate::always() {
        sql.push_str(" WHERE ");
    ...
    if plan.having() != &crate::Predicate::always() {
        sql.push_str(" HAVING ");
        sql.push_str(&render_filter(
            plan.having(),
            &mut params,
            ...
            &plan
                ...
                .map(|key| {
                    Ok(format!(
                        ...
                        match key.direction {
                            crate::Direction::Ascending => "ASC",
                            crate::Direction::Descending => "DESC",
                        },
                        match key.nulls {
                            crate::NullOrder::First => "FIRST",
                            crate::NullOrder::Last => "LAST",
                        }
                    ))
                })
                .collect::<Result<Vec<_>, QueryError>>()?

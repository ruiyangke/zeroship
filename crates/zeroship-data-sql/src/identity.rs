...
pub fn reserve_sqlite_writer(
    ...
    let table = format!("{}.{}", namespace.quoted(), quote_ident(collection));
...
pub fn allocation(
    ...
    match dialect {
        ...
        SqlDialect::Sqlite => Ok(Allocation::RowId {
            ...
            has_sequence: BuiltQuery { sql:format!("SELECT name FROM {}.sqlite_schema WHERE type = 'table' AND name = 'sqlite_sequence'", namespace.quoted()), params:vec![] },
            sequence: BuiltQuery { sql:format!("SELECT seq AS id FROM {}.sqlite_sequence WHERE name = $1", namespace.quoted()), params:vec![Value::from(collection)] },
        }),

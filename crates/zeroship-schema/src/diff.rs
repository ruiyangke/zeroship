...
//!
//! The engine intentionally does not run any DDL on its own. It returns a
//! `Vec<DiffOp>` for a migration host to sequence with its locking, audit, and
//! validation policy.
//!
...
fn desired_physical_columns(schema: &Value) -> std::collections::HashSet<String> {
    ...
    for (field, def) in obj {
        ...
        if mask_meta_from_schema_def(def).is_some() {
            columns.insert(crate::query::raw_column_name(field));
        }
    }
...
pub fn compute_diff(
    ...
    if let Some(obj) = schema.as_object() {
        for (field, def) in obj {
            ...
            // is briefly stale.
            let sql = crate::query::build_add_column(schema_name, collection, field, def)
                .ok()
                .map(|s| s.to_string());
            ops.push(DiffOp {
                collection: collection.to_string(),
        ...
        for (field, def) in obj {
            ...
            if !column_exists {
                ...
                if live_cols.is_some() {
                    ...
                    let sql = crate::query::build_add_foreign_key(schema_name, collection, field, def)
                        .ok()
                        .map(|s| s.to_string());
                    ops.push(DiffOp {
                        collection: collection.to_string(),
            ...
            if live_fk.is_none() {
                ...
                let declared_on_delete =
                    crate::query::normalize_fk_action(def.get("onDelete").and_then(|v| v.as_str()));
                let declared_on_update =
                    crate::query::normalize_fk_action(def.get("onUpdate").and_then(|v| v.as_str()));
                    ...
                    || declared_target != fk.target_table
                {
                    ops.push(DiffOp {
                        ...
                        class: ChangeClass::Compatible,
                        sql: crate::query::build_drop_foreign_key(
                            schema_name,
                            collection,
                            &fk.constraint_name,
                        )
                    ...
                    ops.push(DiffOp {
                        ...
                        change_kind: ChangeKind::AddForeignKey,
                        class: ChangeClass::Compatible,
                        sql: crate::query::build_add_foreign_key(schema_name, collection, field, def)
                            .ok(),
                        details: serde_json::json!({
                            "kind": "add_foreign_key",
                            "field": field,
...
mod tests {
    ...
    /// Wrap a fixture app id as the physical schema `compute_diff` now takes.
    fn s(name: &str) -> crate::SchemaName {
        crate::SchemaName::new(name).expect("fixture schema name")
    }
    ...
    fn drop_column_is_destructive() {
        ...
        let ops = compute_diff(&live, &s("app1"), "posts", &declared, "", &[]);
    ...
    fn string_to_encrypted_transition_emits_rewrite_op() {
        ...
        let ops = compute_diff(&live, &s("app1"), "users", &declared, "", &[]);
    ...
    fn create_table_emitted_when_live_empty() {
        ...
        let declared = json!({"name": {"type": "string"}});
        let ops = compute_diff(&live, &s("app1"), "fresh", &declared, "CREATE TABLE ...", &[]);
    ...
    // additive-by-construction. Re-applying migrations does not amend existing
    // CHECK constraints. This is a known follow-up.
    // -----------------------------------------------------------------
    ...
    fn c2_new_variant_field_classifies_as_additive() {
        ...
        let ops = compute_diff(&live, &s("app1"), "events", &declared, "", &[]);
    ...
    fn a_mask_added_to_an_existing_plaintext_column_uses_structured_executor() {
        ...
        assert_eq!(
            backfill_ops[0].class,
            ChangeClass::Destructive,
            "a Destructive op routes to the approval workflow instead of applying",
        );
        ...
        // column-additions branch. Without this arm the assertions above would
        // pass on an implementation that refused masking outright.
    ...
    /// The raw column is kept out of the DropColumn loop: a
    /// `__zs_raw__<col>` column that exists in the live schema MUST NOT
    /// generate a spurious DropColumn just because the creator-declared schema
    /// does not list it. It is platform-managed, and dropping it would destroy
    /// the real value while leaving the mask behind.
    #[test]
    fn the_raw_column_is_not_dropped_when_the_field_is_still_masked() {
        let mut live = LiveSchema::default();
        let mut cols = std::collections::HashMap::new();
        cols.insert(
            "id".to_string(),
        ...
        cols.insert(
            ...
            ColumnInfo {
                ...
                mask: Some(MaskMeta {
                    kind: MaskKind::Last4,
                    classification: Classification::Spi,
                    sibling_column: crate::query::raw_column_name("ssn"),
                }),
                ..Default::default()
            },
        );
        ...
        assert!(
            !ops.iter()
                .any(|o| matches!(o.change_kind, ChangeKind::DropColumn)
                    && o.field.as_deref() == Some(crate::query::raw_column_name("ssn").as_str())),
            "the platform-owned raw column must not be dropped: {ops:?}"
        );
    }

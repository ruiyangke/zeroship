...
use zero_migrate::{provision_migrator, record_started};
...
async fn apply(
    ...
    engine
        ...
        .map(|_| ())
        .map_err(|e| match e {
            zero_migrate::DeclarativeApplyError::Plain(inner) => inner,
            zero_migrate::DeclarativeApplyError::Expand(inner) => {
...
async fn create_index_ifnotexists_invalid_concurrent_residue_recovers_not_noops() {
    ...
    let (version, name, checksum) = match &mut steps[0] {
        PlanStep::Ddl(m) => {
            ...
            m.checksum =
                zero_migrate::Checksum::of(&zero_migrate::ChecksumInput::from_migration(m));
            assert!(

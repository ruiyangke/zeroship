...
use zero_migrate::model::validate::{UnsupportedKind, CODE_UNSUPPORTED};
use zero_migrate::{
...
async fn platform_composite_primary_key_lowers_and_applies_on_pg() {
    ...
    let create_sql = plan
        ...
        .find_map(|step| match step {
            zero_migrate::PlanStep::Ddl(m) if m.up.contains("CREATE TABLE") => Some(&m.up),
            _ => None,
        })
...
async fn table_level_checks_render_and_apply_on_pg() {
    ...
    let rendered = plan
        ...
        .filter_map(|step| match step {
            zero_migrate::PlanStep::Ddl(m) => Some(m.up.as_str()),
            _ => None,
        })
        .collect::<Vec<_>>()
        .join("\n");

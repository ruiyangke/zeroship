...
use zero_migrate::apply::baseline::BaselineOutcome;
use zero_migrate::{
...
async fn baseline(
    ...
    m: &Migration,
    applied_by: &str,
) -> Result<BaselineOutcome, zero_migrate::apply::baseline::BaselineError> {
...
async fn applied_versions(conn: &Client, cfg: &ExecutorConfig) -> Vec<String> {
    let mut v: Vec<String> = zero_migrate::applied(conn, cfg)
        .await
        .expect("read journal")
        .into_iter()
...
async fn l6_repeatable_versioned_interleaved_across_deploys_and_tamper_anchor() {
    ...
    let err = zero_migrate::apply(&conn, &cfg, &set4, Approval::None, "attacker")
        .await
...
async fn l7_h6_expand_with_unmet_skip_precondition_blocks_later_contract() {
    ...
    assert!(
        matches!(
            expand_res,
            Err(zero_migrate::OnlineError::Apply(ApplyError::MissingDependency { ref version, ref missing }))
    ...
    let contract_err = zero_migrate::apply(&conn, &cfg, &plan.contract, Approval::Approved, "app_acme")
        .await
...
async fn l10_multi_app_union_ownership_and_fail_closed_drop_lifecycle() {
    use zero_migrate::DeclarativeAuthor;

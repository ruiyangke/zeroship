...
use zero_migrate::command::runner::{run_migrate, RunConfig, RunProfile};
use zero_migrate::test_support::acquire_global_platform_resource_lock;
use zero_migrate::{Approval, ExecutorConfig, GuardConfig, PolicyProfile, PostgresBackend};

const DEFAULT_DSN: &str =
    "host=localhost port=5440 user=postgres password=zeroship dbname=zero_migrate_ir_test";
const TEST_DB_NAME: &str = "zero_migrate_ir_test";
...
async fn pg() -> Client {
    let (client, conn) = compio_postgres::connect(&dsn(), compio_postgres::NoTls)
        .await
        .expect("connect to zero_migrate_ir_test on :5440");
    compio::runtime::spawn(async move {
        let _ = conn.run().await;
    })
    .detach();
...
async fn confined_ir_still_denies_role_and_grant_vendor_ops() {
    ...
    let role_err = zero_migrate::apply_bundle_ir_postgres(
        &backend,
        "zeroship",
        "app_confined",
        role_dir.path(),
    ...
    let grant_err = zero_migrate::apply_bundle_ir_postgres(
        &backend,
        "zeroship",
        "app_confined",
        grant_dir.path(),

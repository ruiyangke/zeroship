...
fn relower_plan_version(
    ...
    ty: ColType,
    live: &LiveSchema,
) -> zero_migrate::model::migration::MigrationId {
    let author = IrAuthor::new(cfg.project_schema.clone(), "app_test", SqlDialect::Postgres);
    let ir = rename_ir(table, from, to, ty);
...
async fn create_members(engine: &MigrationEngine, be: &PostgresBackend<'_>, cfg: &ExecutorConfig) {
    ...
    let m = zero_migrate::model::migration::Migration {
...
async fn expand_writes_durable_outstanding_obligation_read_back_fresh() {
    ...
    let outstanding = zero_migrate::outstanding_pending_contracts(&conn, &cfg)
        .await
...
async fn deploy_touching_pending_table_is_refused_with_executable_payload() {
    ...
    match err {
        zero_migrate::DeclarativeApplyError::Plain(
            zero_migrate::EngineError::PendingContract(payload),
        ) => {
            assert_eq!(payload.code, zero_migrate::CODE_TABLE_HAS_PENDING_CONTRACT);
...
async fn second_rename_on_pending_table_is_refused() {
    ...
    assert!(
        matches!(
            err,
            zero_migrate::DeclarativeApplyError::Plain(
                zero_migrate::EngineError::PendingContract(_)
            )
        ),
...
async fn orphaned_pending_contract_is_surfaced_by_status_as_distinct_state() {
    ...
    let st = zero_migrate::status(&conn, &cfg, &[])
        .await
    ...
    let payload =
        zero_migrate::OrphanedPendingContract::new(pcs.table.clone(), pcs.pending_version.clone());
    assert_eq!(payload.code, zero_migrate::CODE_ORPHANED_PENDING_CONTRACT);
    ...
    let present = zero_migrate::model::migration::Migration {
    ...
    let st2 = zero_migrate::status(&conn, &cfg, std::slice::from_ref(&present))
        .await
...
async fn dependent_plan_blocks_on_a_pending_contract_dependency() {
    ...
    let b = zero_migrate::model::migration::Migration {
        version: zero_migrate::model::migration::MigrationId::generate(),
        name: "plan_b".into(),
        up: "SELECT 1".into(),
        down: None,
    ...
    let st = zero_migrate::status(&conn, &cfg, std::slice::from_ref(&b))
        .await
    ...
    // pending version.
    let payload = zero_migrate::DependencyPendingContract::new(
        blk.blocked.as_str(),
        blk.dependency.as_str(),
        blk.pending_version.clone(),
    );
    assert_eq!(payload.code, zero_migrate::CODE_DEPENDENCY_PENDING_CONTRACT);
    ...
    let st2 = zero_migrate::status(&conn, &cfg, std::slice::from_ref(&b))
        .await
...
async fn dependent_plan_on_different_table_is_refused_at_apply() {
    ...
    match err {
        zero_migrate::DeclarativeApplyError::Plain(
            zero_migrate::EngineError::DependencyPendingContract(payload),
        ) => {
            assert_eq!(payload.code, zero_migrate::CODE_DEPENDENCY_PENDING_CONTRACT);
...
async fn bare_name_dropindex_on_pending_table_is_refused() {
    use zero_migrate::IndexSnapshot;
...
async fn pr9e_obligation_and_marker_commit_atomically() {
    ...
    setup(&conn, &cfg).await;
    let be = PostgresBackend::new(&conn);
    zero_migrate::ensure_journal(&conn, &cfg)
        .await
        .expect("bootstrap journal");

    let contract_versions = vec!["c1".to_string(), "c2".to_string()];
    let scope = zero_migrate::apply::journal::DeployRecoveryScope { deploy_id: "dep_pr9e_1" };
...
async fn pr9e_combined_write_failure_rolls_back_both_rows() {
    ...
    let scope = zero_migrate::apply::journal::DeployRecoveryScope { deploy_id: "dep_pr9e_2" };
    let err = be
        .pending_contracts()
        .expect("Postgres pending-contract capability")
        .record_pending_contract_with_recovery(

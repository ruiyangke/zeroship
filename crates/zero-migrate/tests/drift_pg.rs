...
//! Requires `zero_migrate_test` on :5440. Each test uses its own meta +
...
use zero_migrate::model::ir::{SelectAst, SelectItem, TableRef, ViewQuery};
use zero_migrate::{
...
async fn apply_aborts_on_checksum_drift_using_the_shared_check() {
    ...
    assert!(
        matches!(
            err,
            zero_migrate::apply::executor::ApplyError::ChecksumDrift { .. }
        ),
        "got {err:?}"
    );

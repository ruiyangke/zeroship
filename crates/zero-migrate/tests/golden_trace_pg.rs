...
fn plan_decl(
    ...
    live: &SchemaSnapshot,
    cfg: &ExecutorConfig,
) -> zero_migrate::DeclarativeDeployPlan {
    engine
...
async fn oracle_apply(
    engine: &MigrationEngine,
    plan: &zero_migrate::DeclarativeDeployPlan,
    approval: Approval,
    conn: &Client,
    cfg: &ExecutorConfig,
...
async fn live_apply(
    ...
    engine
        .apply_declarative(plan, approval, &zero_migrate::PostgresBackend::new(conn), cfg, "app_test")
        .await
}

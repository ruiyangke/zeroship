...
//!
//!   zero-migrate migrate --dir <db/migrations-ts> --database-url <DSN> --profile platform
//!
...
//! `CARGO_BIN_EXE_zero-migrate`, i.e. the freshly-built `target/<profile>/
//! zero-migrate`.
//!
//! To avoid clobbering the real `zeroship` schema (the ported set hardcodes the
//! `zeroship` schema + the global `zeroship_*` roles), the test
//! creates a FRESH, dedicated DATABASE `zsmig_bin_e2e_<token>` on the
//! `zero_migrate_test` cluster, runs the bin against THAT db, asserts the full
...
/// but always targets `zero_migrate_test` as the maintenance db.
fn admin_dsn() -> String {
...
/// Run the BUILT `zero-migrate` binary as a subprocess. Returns
/// `(success, stdout, stderr)`. `CARGO_BIN_EXE_zero-migrate` is injected by
/// cargo for integration tests and points at the freshly-built binary.
fn run_bin(args: &[&str]) -> (bool, String, String) {
    // Extraction Phase A: the recorder-child bin MOVED to `zero-migrate-runtime`,
    // so `CARGO_BIN_EXE_zero-migrate-recorder-child` is not injected for THIS
    // (engine) test crate. It builds into the SAME target dir as the standalone bin,
    // so resolve it as a sibling of `CARGO_BIN_EXE_zero-migrate`.
    let standalone = std::path::PathBuf::from(env!("CARGO_BIN_EXE_zero-migrate"));
    let recorder_child = standalone
        .parent()
        .expect("standalone bin has a parent dir")
        .join("zero-migrate-recorder-child");
...
async fn binary_migrate_applies_whole_set_idempotently_and_materializes_schema() {
    ...
    // 2c. The five platform service roles (cluster-wide).
    for role in [
        "zeroship_auth",
        "zeroship_control",
        "zeroship_gateway",

...
//!
//! Requires a dedicated database `zero_migrate_test` on :5440 (recreated by
...
async fn apply_aborts_on_checksum_drift() {
    ...
    tampered.checksum = Checksum::of(&zero_migrate::ChecksumInput::from_migration(&tampered));
...
async fn apply_honors_depends_on_over_version_order() {
    ...
    m_earlier.checksum = Checksum::of(&zero_migrate::ChecksumInput::from_migration(&m_earlier));
    let m_later = mig(
        later.clone(),
        "create_parent",
...
async fn apply_rejects_dependency_cycle() {
    ...
    ma.checksum = Checksum::of(&zero_migrate::ChecksumInput::from_migration(&ma));
    mb.depends_on = vec![a.clone()];
    mb.checksum = Checksum::of(&zero_migrate::ChecksumInput::from_migration(&mb));
...
async fn h3_per_migration_timeout_override_lets_long_migration_complete() {
    ...
    slow.checksum = Checksum::of(&zero_migrate::ChecksumInput::from_migration(&slow));
...
async fn executor_rollback_refuses_without_approval() {
    use zero_migrate::apply::executor::{rollback, RollbackError, RollbackRequest, RollbackTarget};

...
use zero_migrate::test_support::acquire_fault_injection_test_lock;
...
async fn ir_renamecolumn_applies_as_pg_online_dual_write_through_apply_plan() {
    ...
    engine
        .apply_plan(
            &[PlanStep::Ddl(zero_migrate::model::migration::Migration {
...
fn ir_renamecolumn_pg_e1_to_c2_shape_byte_equal_to_declarative_rename() {
    ...
    let decl_ec = zero_migrate::ExpandContractAuthor::new(&cfg.project_schema, "app_test")
    ...
    let shape = |m: &zero_migrate::model::migration::Migration| {
    ...
    let edges = |ec: &ExpandContractPlan| {
        let all: Vec<&zero_migrate::model::migration::Migration> =
            ec.expand.iter().chain(ec.contract.iter()).collect();
        let pos = |v: &zero_migrate::model::migration::MigrationId| {

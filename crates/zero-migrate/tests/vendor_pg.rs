...
use zero_migrate::model::expr::{CastTarget, Expr, ScalarFn};
...
fn render_create_policy_with_closed_predicate() {
    ...
    let predicate = Expr::BinOp {
        op: zero_migrate::model::expr::BinaryOp::Eq,
...
fn pg_body_trigger_lower_refuses_with_trigger_body_kind() {
    ...
    match err {
        zero_migrate::render::lower::IrLowerError::TriggerUnsupported { kind, dialect } => {
...
fn render_create_function_embeds_body_verbatim() {
    let op = Op::CreateFunction {
        ...
        args: Some(vec![zero_migrate::model::ir::FuncArg {
...
fn grant_revoke_table_target_schema_is_checked_at_validate_time() {
    ...
    assert_eq!(err.code, zero_migrate::model::validate::CODE_CROSS_SCHEMA, "got {err:?}");

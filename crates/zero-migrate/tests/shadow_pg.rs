...
//!
//! The dry-run runs the UNMODIFIED [`zero_migrate::apply`] path against a
...
async fn dry_run_concurrently_index_runs_in_shadow() {
    ...
    idx.checksum = Checksum::of(&zero_migrate::ChecksumInput::from_migration(&idx));
...
async fn dry_run_teardown_survives_a_body_panic() {
    ...
    zero_migrate::arm_panic_after_provision(true);
    let caught = std::panic::AssertUnwindSafe(dry_run(&admin, &[m], &cfg, &scfg, "actor"))
        .catch_unwind()
        .await;
    zero_migrate::arm_panic_after_provision(false);

...
fn scaffold_is_deterministic_by_construction_and_records_zero_warnings() {
    ...
    let outcome = build_migrations(&zero_migrate_host::ZeroMigrateHost, dir.path(), APP, &via()).expect("build the scaffold");
...
async fn new_edit_build_apply_on_sqlite() {
    ...
    let outcome = build_migrations(&zero_migrate_host::ZeroMigrateHost, mig_dir.path(), APP, &via()).expect("build records transient IR");
...
async fn build_records_deterministic_transient_ir_without_writing_artifact() {
    ...
    let first = build_migrations(&zero_migrate_host::ZeroMigrateHost, mig_dir.path(), APP, &via()).expect("first build");
    ...
    let second = build_migrations(&zero_migrate_host::ZeroMigrateHost, mig_dir.path(), APP, &via()).expect("second build");
    assert_eq!(

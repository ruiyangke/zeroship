...
use zero_migrate::model::validate::{validate_ir, Dialect, UnsupportedKind, CODE_UNSUPPORTED};
use zero_migrate::{
...
use zero_migrate::{ExecutorConfig, SchemaScope};
...
fn pg_cfg(tok: &str) -> ExecutorConfig {
    ...
    let role = zero_migrate::migrator_role_name(&cfg.project_id).unwrap();
            ...
            query: ViewQuery::Structured {
                select: SelectAst {
                    ...
                    r#where: Some(Expr::UnaryOp {
                        op: zero_migrate::UnaryOp::IsNull,

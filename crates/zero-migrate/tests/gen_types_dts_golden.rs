...
use zero_migrate::frontend::render_artifacts;
...
fn runtime_descriptor_generated_index_names_match_lowered_capped_names() {
    ...
    let expected = zero_migrate::plan::author::cap_ident_name(&natural);
    let expected_unique = zero_migrate::plan::author::cap_ident_name(&unique_natural);

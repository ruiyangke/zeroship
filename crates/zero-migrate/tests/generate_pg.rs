...
async fn generate_emits_versioned_migration_with_goodie_ddl() {
    ...
    let outcome = zero_migrate::frontend::generate_migration(
        SAMPLE,
        &dsn(),
        &schema,
        "app_test",
    ...
    // `render_create_table`, built via the SHARED `zero_migrate_schema` codec). The
...
async fn generate_against_matching_live_schema_is_noop() {
    ...
    let first = zero_migrate::frontend::generate_migration(
    ...
    let second = zero_migrate::frontend::generate_migration(

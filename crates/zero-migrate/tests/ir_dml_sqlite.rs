...
#[cfg(feature = "zsv8")]
use zero_migrate::frontend::record_migration_to_ir_unsandboxed;
...
async fn lower_and_apply(
    ...
    let plan = zero_migrate::AppliedPlan {
        version: zero_migrate::model::migration::MigrationId::generate(),
        name: "dml".into(),
        steps: migrations.into_iter().map(zero_migrate::PlanStep::Ddl).collect(),
        checksum: zero_migrate::model::migration::Checksum::of(&zero_migrate::model::migration::ChecksumInput {
            up: "x",
            down: None,
        ...
        flags: zero_migrate::model::migration::MigrationFlags::default(),
        dialect_scope: zero_migrate::DialectScope::Both,
        rollbackable: false,
        owner_app: APP.into(),
        depends_on: vec![],
        supersedes: vec![],
...
async fn lower_plan_and_apply(
    ...
    reg: &BTreeMap<String, String>,
    approval: Approval,
) -> zero_migrate::engine::DeclarativeDeployOutcome {
    let ir = resolved_ir_json(ir);
    let author = IrAuthor::new(PROJECT, APP, SqlDialect::Sqlite);
...
async fn recorded_fnsynth_symbol_insert_applies_db_evaluated_values_on_sqlite() {
    ...
    let symbol_ir = record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, symbol_src, APP, "sqlite_fnsynth_symbol")
        .expect("record symbol-form fnSynth insert");
    let explicit_ir = record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, explicit_src, APP, "sqlite_fnsynth_explicit")
...
async fn on_conflict_rejected_on_sqlite() {
    ...
    let err = zero_migrate::model::load::load_ir_document(
        ir,
        APP,
        zero_migrate::model::validate::Dialect::Sqlite,
        &registry(&[("codes", APP)]),
    ...
    let zero_migrate::model::load::IrLoadError::Validate(err) = err else {
        panic!("expected validate error, got {err:?}");
    };
    assert_eq!(err.code, zero_migrate::CODE_UNSUPPORTED);
    assert_eq!(err.kind, Some(zero_migrate::UnsupportedKind::Op));
    assert!(
...
async fn unresolved_colref_rejected_at_apply_seam_on_sqlite() {
    ...
    match err {
        IrLowerError::DmlValidate(e) => {
            assert_eq!(e.code, zero_migrate::CODE_UNSUPPORTED, "structured (c) reject");
            assert_eq!(
                e.kind,
                Some(zero_migrate::UnsupportedKind::Expr),
...
async fn malformed_set_identifier_rejected_on_sqlite() {
    ...
    assert!(
        matches!(err, IrLowerError::DmlAssemble(zero_migrate::render::dml::DmlError::InvalidIdentifier { .. })),

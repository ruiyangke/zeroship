...
//!
//! These tests exercise the REAL path — the actual `zero-migrate-recorder-child`
...
use zero_migrate::frontend::recorder_protocol::{ChildOperation, ChildRequest};
...
fn child_request(src: &str, hosted: bool, netns: bool, rlimit: bool) -> String {
    let req = ChildRequest {
        ...
        parent_netns_inode: zero_migrate::frontend::recorder_service::read_netns_inode()
            .unwrap_or(0),
    };
...
fn hosted_refuses_to_run_with_neither_seccomp_nor_netns() {
    ...
    child.wait().ok();
    let resp: zero_migrate::frontend::recorder_protocol::ChildResponse =
    ...
    match resp.error {
        Some(zero_migrate::frontend::recorder_protocol::ChildError::SandboxRefused(_)) => {}
...
fn hosted_refuses_when_landlock_forced_unavailable_even_with_seccomp() {
    ...
    unsafe {
        cmd.pre_exec(|| {
            zero_migrate::frontend::sandbox::pre_exec_rlimits(ResourceBudget::default())
        });
    }
    ...
    match resp.error {
        Some(zero_migrate::frontend::recorder_protocol::ChildError::SandboxRefused(
            reason,
        )) => {
...
fn netns_alone_contains_network_with_seccomp_disabled() {
    ...
    unsafe {
        cmd.pre_exec(|| {
            ...
            zero_migrate::frontend::sandbox::pre_exec_netns()
        });
    }
...
fn recorder_refuses_to_run_if_not_single_threaded() {
    ...
    match resp.error {
        Some(zero_migrate::frontend::recorder_protocol::ChildError::SandboxRefused(reason)) => {
            assert!(

...
fn cfg_for(tok: &str) -> ExecutorConfig {
    ...
    let role = zero_migrate::migrator_role_name(&c.project_id).unwrap();
...
async fn recorded_fnsynth_symbol_insert_applies_db_evaluated_values_on_pg() {
    ...
    let symbol_ir = record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, symbol_src, APP, "pg_fnsynth_symbol")
        .expect("record symbol-form fnSynth insert");
    let explicit_ir = record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, explicit_src, APP, "pg_fnsynth_explicit")
...
async fn ir_unresolved_colref_rejected_at_apply_seam_on_pg() {
    ...
    let backend = zero_migrate::PostgresBackend::new(&conn);
    ...
    match err {
        zero_migrate::IrLowerError::DmlValidate(e) => {

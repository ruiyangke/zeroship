...
//! child must be built (`cargo build -p zero-migrate --bins`); the test
//! asserts the binary exists with a clear message.

use std::fs;
use std::path::Path;

use zero_migrate::frontend::recorder_protocol::MAX_TS_SOURCE_BYTES;
use zero_migrate::frontend::recorder_http::StructuredError;
...
fn hosted_unreachable_falls_back_to_local_not_a_build_failure() {
    ...
    let outcome = build_migrations(&zero_migrate_host::ZeroMigrateHost, dir.path(), OWNER, &via)
...
fn confined_build_records_resolved_system_shape_before_checksum() {
    ...
    let outcome = build_migrations(&zero_migrate_host::ZeroMigrateHost, dir.path(), OWNER, &RecordVia::local()).expect("build");
...
fn hosted_non_retryable_reject_is_a_build_error_no_silent_fallback() {
    ...
    let err = build_migrations(&zero_migrate_host::ZeroMigrateHost, dir.path(), OWNER, &via)
...
fn local_build_rejects_oversized_source_before_recording() {
    ...
    let err = build_migrations(&zero_migrate_host::ZeroMigrateHost, dir.path(), OWNER, &RecordVia::local())
...
fn build_allows_date_now_call_with_soft_warning() {
    ...
    let outcome = build_migrations(&zero_migrate_host::ZeroMigrateHost, dir.path(), OWNER, &RecordVia::local())
...
fn local_and_hosted_paths_yield_same_typed_value_checksum() {
    ...
    let local = build_migrations(&zero_migrate_host::ZeroMigrateHost, 
        local_dir.path(),
        OWNER,
        &RecordVia::Local {
    ...
    impl RecorderClient for LocalChildHostedClient {
        fn record(
            ...
            use zero_migrate::frontend::recorder_service::{
                spawn_sandboxed_record, RecordRequest,
            };
            use zero_migrate::frontend::{ResourceBudget, SandboxPosture};
    ...
    let hosted = build_migrations(&zero_migrate_host::ZeroMigrateHost, 
        hosted_dir.path(),
        OWNER,
        &RecordVia::Hosted {
            client: &client,

...
use zero_migrate::model::ir::{CanonicalOpList, ColType, Op};
...
use zero_migrate::frontend::{build_migrations, generate_ops, RecordVia, ResourceBudget};
...
fn generate_all_column_types_records_to_direct_ir_checksum() {
    ...
    let built = build_migrations(&zero_migrate_host::ZeroMigrateHost, dir.path(), APP, &via()).expect("record the emitted .ts");

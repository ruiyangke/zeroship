...
//! to a real `.ts` MODULE ([`zero_migrate::frontend::ENV_DTS_FILE`] = `env.db.ts`).
//!
...
use zero_migrate::frontend::{eval_schema_to_ir, render_artifacts};
...
/// (`<root>/crates/zero-migrate`).
fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(Path::parent)
        .expect("crate is <root>/crates/zero-migrate")
        .to_path_buf()
}

...
async fn executor_reruns_guard_even_on_a_hand_built_clean_plan() {
    use zero_migrate::{MigrationPlan, PlannedMigration};
    use zero_migrate::GuardOutcome;
    ...
    let evil = { let mut __mig = Migration {
        ...
        down: None,
        checksum: zero_migrate::Checksum::of(&zero_migrate::ChecksumInput { up: "", down: None, flags: &zero_migrate::MigrationFlags::default(), owner_app: "", depends_on: &[], supersedes: &[], preconditions: &[] }),
        flags: zero_migrate::MigrationFlags::default(),

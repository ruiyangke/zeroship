...
fn shared_infra_crates_do_not_enable_standalone_cli() {
    ...
    for relative in [
        ...
        for section in ["dependencies", "dev-dependencies", "build-dependencies"] {
            let features = dep_features(&manifest, section, "zero-migrate");
            assert!(
                !features.iter().any(|f| f == "standalone-cli"),
                "{} enables zero-migrate/standalone-cli in {section}: {features:?}",
                path.display()
            );
        }
    }
...
fn root_workspace_uses_resolver_3_for_feature_isolation() {
    ...
    let features = manifest
        ...
        .and_then(|deps| deps.get("zero-migrate"))
        .and_then(|entry| entry.get("features"))
        .and_then(toml::Value::as_array)
        .into_iter()
        .flatten()
    ...
    assert!(
        !features.iter().any(|f| *f == "standalone-cli"),
        "workspace zero-migrate dependency must not enable standalone-cli: {features:?}"
    );
}

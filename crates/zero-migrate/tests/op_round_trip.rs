...
//!    `UPDATE_CORPUS=1 cargo test -p zero-migrate --test op_round_trip` to
...
//! `zero-migrate` frontend test suite.
...
fn corpus_is_byte_stable_and_value_equal() {
    ...
    for stem in fixture_stems() {
        ...
        let raw = record_migration_to_json_unsandboxed(&zero_migrate_host::ZeroMigrateHost, &mig_src, OWNER, &stem)
        ...
        let golden = std::fs::read_to_string(&golden_path).unwrap_or_else(|e| {
            panic!(
                "{stem}{GOLDEN_SUFFIX} missing or unreadable ({e}). Run \
                 `UPDATE_CORPUS=1 cargo test -p zero-migrate --test op_round_trip` \
...
fn every_op_variant_has_a_fixture() {
    ...
    // `zero-migrate`'s own `op_ir_schema::op_variant_names_from_schema` does.
    let schema_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../zero-migrate/op-ir.schema.json");
    ...
    for stem in fixture_stems() {
        let mig_src = std::fs::read_to_string(fixtures_dir().join(format!("{stem}.mig.js")))
            .unwrap_or_else(|e| panic!("read {stem}.mig.js: {e}"));
        let recorded = record_migration_to_json_unsandboxed(&zero_migrate_host::ZeroMigrateHost, &mig_src, OWNER, &stem)
            .unwrap_or_else(|e| panic!("record {stem}: {e}"));

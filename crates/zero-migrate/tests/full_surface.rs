...
use zero_migrate::frontend::{lint_migration_determinism, record_migration_to_ir_unsandboxed};
use zero_migrate::{CanonicalOpList, Checksum, MigrationFlags};
...
fn record(src: &str, name: &str) -> Value {
    let ir = record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, name)
        .unwrap_or_else(|e| panic!("record {name}: {e}"));
    serde_json::to_value(&ir).expect("ir -> value")
}

fn record_err(src: &str, name: &str) -> String {
    record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, name)
        .err()
        .map(|e| e.to_string())
...
fn date_now_symbol_is_deterministic_across_records() {
    ...
    "#;
    let a = record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "date_now_symbol_a").unwrap();
    let b = record_migration_to_ir_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "date_now_symbol_a").unwrap();
    let checksum = |ir: &zero_migrate::MigrationIr| {
        Checksum::of_ir(
...
fn date_now_call_just_evaluates_no_error() {
    use zero_migrate::frontend::record_migration_to_ir_with_warnings_unsandboxed;
    ...
    "#;
    let outcome = record_migration_to_ir_with_warnings_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "date_now_call")
...
fn determinism_lint_flags_date_now_in_op_arg() {
    ...
    "#;
    let findings = lint_migration_determinism(&zero_migrate_host::ZeroMigrateHost, dirty).expect("lint runs");
    assert!(
        !findings.is_empty(),
    ...
    "#;
    assert!(
        lint_migration_determinism(&zero_migrate_host::ZeroMigrateHost, clean).expect("lint runs").is_empty(),
...
fn determinism_lint_flags_rng_and_clock_constructor() {
    for (src_frag, needle) in [
        ...
        let findings = lint_migration_determinism(&zero_migrate_host::ZeroMigrateHost, &src).expect("lint runs");
        assert!(
...
fn same_source_records_same_json() {
    ...
    "#;
    let a =
        zero_migrate::frontend::record_migration_to_json_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "det").unwrap();
    let b =
        zero_migrate::frontend::record_migration_to_json_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "det").unwrap();
...
fn record_path_allows_date_now_call_with_soft_warning() {
    ...
    "#;
    let outcome = record_migration_to_ir_with_warnings_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "date_call")
        .expect("Date.now() call evaluates and records");
    assert_eq!(outcome.ir.ops.len(), 1);
    let created_at = match &outcome.ir.ops[0] {
        zero_migrate::Op::Insert { rows, .. } => &rows[0][0],
        other => panic!("expected insert, got {other:?}"),
    };
    assert!(
        matches!(created_at, zero_migrate::IrValue::Scalar(zero_migrate::IrScalar::Int(v)) if *v > 0),
    ...
    "#;
    let clean_outcome = record_migration_to_ir_with_warnings_unsandboxed(&zero_migrate_host::ZeroMigrateHost, clean, OWNER, "clean")
...
fn record_path_allows_math_random_calls_with_soft_warning() {
    ...
    "#;
    let outcome = record_migration_to_ir_with_warnings_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "random_difference")
    ...
    assert!(
        matches!(sample, zero_migrate::IrValue::Scalar(zero_migrate::IrScalar::Decimal(_))),
...
fn record_path_allows_argless_new_date_call_with_soft_warning() {
    ...
    "#;
    let outcome = record_migration_to_ir_with_warnings_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "argless_date")
    ...
    assert!(
        matches!(year, zero_migrate::IrValue::Scalar(zero_migrate::IrScalar::Int(v)) if *v >= 2026),
...
fn record_path_allows_explicit_new_date_argument() {
    ...
    "#;
    let outcome = record_migration_to_ir_with_warnings_unsandboxed(&zero_migrate_host::ZeroMigrateHost, clean, OWNER, "explicit_date")
...
fn record_path_allows_date_now_inside_comment_or_string() {
    ...
    "#;
    let outcome = record_migration_to_ir_with_warnings_unsandboxed(&zero_migrate_host::ZeroMigrateHost, clean, OWNER, "comment_string")

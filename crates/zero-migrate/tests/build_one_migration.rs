...
use zero_migrate::frontend::{build_one_migration, BuildError, RecordVia, ResourceBudget};
...
fn record_one_file_does_not_record_siblings() {
    ...
    let outcome = build_one_migration(&zero_migrate_host::ZeroMigrateHost, &target_path, APP, &via()).expect("build one");
...
fn record_one_file_is_deterministic_across_transient_runs() {
    ...
    let first = build_one_migration(&zero_migrate_host::ZeroMigrateHost, &path, APP, &via()).expect("first");
    assert_eq!(first.migrations[0].record_path, zero_migrate::frontend::RecordPath::Local);
    let bytes = first.migrations[0].committed_bytes.clone();

    let second = build_one_migration(&zero_migrate_host::ZeroMigrateHost, &path, APP, &via()).expect("second");
    assert_eq!(
        second.migrations[0].record_path,
        zero_migrate::frontend::RecordPath::Local
    );
...
fn record_non_ts_path_is_invalid_name() {
    let err = build_one_migration(&zero_migrate_host::ZeroMigrateHost, std::path::Path::new("/tmp/nota.json"), APP, &via())

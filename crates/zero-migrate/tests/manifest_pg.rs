...
async fn content_edited_migration_is_refused_before_apply() {
    ...
    edited.checksum =
        zero_migrate::Checksum::of(&zero_migrate::ChecksumInput::from_migration(&edited));
...
fn denied_migration(cfg: &ExecutorConfig) -> Migration {
    ...
    let flags = zero_migrate::MigrationFlags::default();
    let checksum = zero_migrate::Checksum::of(&zero_migrate::ChecksumInput {
        up: &up,
        down: None,
        flags: &flags,
        owner_app: "app_test",
    ...
    Migration {
        version: zero_migrate::MigrationId::generate(),
...
async fn declarative_verified_content_flipped_plain_migration_is_refused() {
    ...
    item.migration.checksum = zero_migrate::Checksum::of(
        &zero_migrate::ChecksumInput::from_migration(&item.migration),
    );
...
async fn declarative_verified_tampered_rename_expand_is_refused_before_apply() {
    ...
    exp.up = format!("{}\n-- tampered", exp.up);
    exp.checksum =
        zero_migrate::Checksum::of(&zero_migrate::ChecksumInput::from_migration(exp));
...
async fn declarative_verified_tampered_rename_contract_is_refused_before_apply() {
    ...
    con.checksum =
        zero_migrate::Checksum::of(&zero_migrate::ChecksumInput::from_migration(con));

...
fn sqlstate(err: &(dyn std::error::Error + 'static)) -> Option<String> {
    ...
    while let Some(e) = cur {
        if let Some(seam) = e.downcast_ref::<zero_migrate::SeamError>() {
        ...
        if let Some(backend) = e.downcast_ref::<zero_migrate::BackendError>() {
            if let Some(seam) = backend.downcast_ref::<zero_migrate::SeamError>() {
                if let Some(code) = &seam.code {
                    return Some(code.clone());
                }
            }
...
// `zero_migrate_schema::query::def_to_column_type_for_dialect`). pgvector installs
...
async fn backstop_runtime_constructed_cross_schema_is_denied_at_execution() {
    use zero_migrate::{GuardConfig, SqlGuard};
...
async fn backstop_through_apply_flow_is_denied() {
    ...
    match &err {
        zero_migrate::ApplyError::MigrationFailed { source, .. } => {
...
async fn migration_up_cannot_forge_journal_via_unqualified_insert() {
    ...
    let zero_migrate::ApplyError::MigrationFailed { source, .. } = &err else {
...
async fn migration_up_cannot_forge_journal_via_qualified_insert() {
    ...
    assert!(
        matches!(
            err,
            zero_migrate::ApplyError::Guard { .. }
                | zero_migrate::ApplyError::MigrationFailed { .. }
        ),

...
use zero_migrate::frontend::record_migration_to_json_unsandboxed;

const OWNER: &str = "app_lint";

fn record(src: &str) -> Result<String, String> {
    record_migration_to_json_unsandboxed(&zero_migrate_host::ZeroMigrateHost, src, OWNER, "lint").map_err(|e| e.to_string())
}

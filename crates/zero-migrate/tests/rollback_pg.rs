...
//!
//! Requires the dedicated database `zero_migrate_test` on :5440 with a
...
use zero_migrate::apply::role::deprovision_migrator;
use zero_migrate::{
...
fn sqlstate_of(backend: &zero_migrate::BackendError) -> Option<String> {
...
async fn engine_rollback_requires_approval() {
    ...
    let err = engine
        ...
        .unwrap_err();
    assert!(
        matches!(err, zero_migrate::RollbackEngineError::ApprovalRequired),
        "got {err:?}"
    );

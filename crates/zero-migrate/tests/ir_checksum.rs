...
fn checksum_of_ir_byte_stable_golden() {
    ...
    let deps = [dep()];
    let sups = [sup()];
    // Hard-coded expected — re-captured when the `of_ir` checksum
    // domain-separator tag was set to `zero-migrate/of_ir/v1` (`IR_DOMAIN_TAG` in
    // `model/migration.rs`): the tag is folded into `Checksum::of_ir`, so this
    // golden moved by construction. A DELIBERATE, documented wire change with

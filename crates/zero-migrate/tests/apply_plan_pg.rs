...
fn ddl(version: u64, name: &str, schema: &str, up: &str, down: Option<&str>) -> Migration {
    ...
    let _ = schema;
    Migration {
        version: zero_migrate::migration_id_for_version(version),
        name: name.to_string(),
        up: up.to_string(),
...
async fn standalone_dml_step_applies_and_is_idempotent() {
    ...
    let dml_version = dml_version.unwrap_or_else(|| zero_migrate::migration_id_for_version(900));
    ...
    let evil = PlanStep::Dml {
        version: zero_migrate::migration_id_for_version(901),
...
fn generated_flyway_dirs_always_lower_to_single_step_plans() {
    ...
    for seed in 0u64..40 {
        ...
        let plans = zero_migrate::load_dir(dir.path())
...
async fn golden_trace_declarative_refusal_and_idempotent_reapply() {
    let _fault_lock = acquire_fault_injection_test_lock();
    use zero_migrate::plan::author::{AuthorRequest, Column, DeterministicAuthor, MigrationAuthor};
...
fn gen_fuzz_plan(seed: u64, cfg: &ExecutorConfig) -> Vec<PlanStep> {
    ...
    steps.push(PlanStep::Dml {
        version: zero_migrate::migration_id_for_version(10),
...
async fn crash_fuzz_resume_from_any_subistep_boundary_converges() {
    ...
    for seed in 0u64..3 {
        ...
        for &boundary in fault_boundaries() {
            ...
            zero_migrate::fault::arm(boundary, 0);
...
async fn destructive_dml_is_refused_without_approval_and_applies_nothing() {
    ...
    let del_version = zero_migrate::migration_id_for_version(950);
...
async fn run_dml_step_seam_refuses_destructive_dml_without_approval() {
    let _fault_lock = acquire_fault_injection_test_lock();
    use zero_migrate::{ApplyError, MigrationBackend};
    ...
    let del_version = zero_migrate::migration_id_for_version(960);
    ...
    let refused = backend
        .run_dml_step(
            ...
            "app_test",
            Approval::None,
            &zero_migrate::ApprovalScope::All,
            "app_test",
            zero_migrate::apply::executor::LockMode::AlreadyHeld,
        )
        .await;
    assert!(
...
async fn run_dml_step_seam_refuses_destructive_dml_outside_version_scope() {
    ...
    let backend = zero_migrate::PostgresBackend::new(&conn);
    let del_version = zero_migrate::migration_id_for_version(961);
...
async fn dml_first_plan_holds_the_project_lock_for_the_whole_deploy() {
    ...
    let dml = PlanStep::Dml {
        version: zero_migrate::migration_id_for_version(951),
    ...
    let handle = compio::runtime::spawn(async move {
        ...
        engine
            .apply_plan(
                &[dml],
                Approval::None,
                &zero_migrate::PostgresBackend::new(&c3),
                &cfg_owned,
                "app_test",
                zero_migrate::apply::executor::LockMode::Acquire,
            )
...
async fn dml_journal_checksum_binds_the_declaring_owner_app() {
    ...
    let va = zero_migrate::migration_id_for_version(7001);
    let vb = zero_migrate::migration_id_for_version(7002);
...
async fn dml_first_plan_against_fresh_db_bootstraps_the_journal() {
    ...
    let migrator = zero_migrate::migrator_role_name(&cfg.project_id).unwrap();
    conn.batch_execute(&format!(
    ...
    let dml_version = zero_migrate::migration_id_for_version(700);
    let dml = PlanStep::Dml {
        version: dml_version.clone(),
        name: "seed_kv".to_string(),
        template: format!("INSERT INTO {s}.kv (k, v) VALUES ($1, $2)"),

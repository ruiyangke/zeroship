...
//! the V8 host runtime behind three engine-owned traits — [`AuthoringHost`],
...
//! the V8 host runtime / `v8` type.** Every symbol these signatures mention is
//! engine-owned or `std`. That is what makes `cargo tree -p zero-migrate`
//! carry no the V8 host runtime / `v8` node — the traits compile *unconditionally,
//! with zero runtime dep*, and the in-Rust-V8 impls live only in the monorepo
//! adapter crate `zero-migrate-runtime`.
//!
...
// Phase B — `crate::net_policy`, a byte-identical copy of `the upstream core::
// net_policy`), so the engine carries no the upstream core normal-graph dep. The
// engine's `NetPolicy` construction + TLS-pin validation stay engine-owned; the
// adapter maps `NetPolicy` → the host runtime NetPolicy at `open_js_driver`.
...
/// trait signatures carry no the host runtime ModuleEntry. The adapter maps
/// `migrate::ModuleEntry` ↔ the host runtime ModuleEntry (identical fields).
#[derive(Debug, Clone)]
...
///
/// This is the portable peer of the host runtime NetPolicy — it names only
...
/// the host runtime NetPolicy at `open_js_driver`. Only the subset migrate

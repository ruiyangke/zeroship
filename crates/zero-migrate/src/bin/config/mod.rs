//! DX-parity config resolution for the `zero-migrate` CLI: a project config
...
//!
//! Discovery: a `zero-migrate.toml` is searched from the CWD walking UP to the
...
pub struct DiscoveredConfig {
    /// The path the config was loaded from (the discovered `zero-migrate.toml`).
    pub path: PathBuf,
    /// The parsed config.
...
/// - `Ok(None)` if no `zero-migrate.toml` exists anywhere up the tree (fine);

//! `zero-migrate` — the migration CLI (design §9, Phase 3; Track A, Phase A3).
//!
//! A dbmate-like, generic-by-default migration tool. `main` is a THIN arg-parser:
//! it parses flags with `clap`, builds a
//! [`RunConfig`](zero_migrate::command::runner::RunConfig)-equivalent,
...
#[derive(Debug, Parser)]
#[command(name = "zero-migrate", version, about, long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Command,
    ...
    /// `DATABASE_URL` env, then `database_url` in `zero-migrate.toml`. We do NOT
    ...
    let database_url = cli
        ...
        .ok_or_else(|| {
            "missing database URL (pass --database-url, set DATABASE_URL, or set \
             `database_url` in zero-migrate.toml)"
                .to_string()
        })?;
...
struct LintFinding {
    migration: String,
    rule: String,
    severity: zero_migrate::Severity,
    message: String,
...
/// unchanged in `main`.
fn print_lint(pairs: &[(String, Vec<zero_migrate::Advisory>)]) {
    ...
    if warnings == 0 && notices == 0 {
        ...
        for (migration, advisories) in pairs {
            ...
            println!("  {migration}:");
            for severity in [
                zero_migrate::Severity::Warning,
                zero_migrate::Severity::Notice,
            ] {
...
fn print_lint_json(pairs: &[(String, Vec<zero_migrate::Advisory>)]) {
...
fn lint_findings(pairs: &[(String, Vec<zero_migrate::Advisory>)]) -> Vec<LintFinding> {
...
fn lint_counts(pairs: &[(String, Vec<zero_migrate::Advisory>)]) -> (usize, usize) {
    let warnings = pairs
        .iter()
        .flat_map(|(_, advisories)| advisories)
        .filter(|a| a.severity == zero_migrate::Severity::Warning)
        .count();
    let notices = pairs
        .iter()
        .flat_map(|(_, advisories)| advisories)
        .filter(|a| a.severity == zero_migrate::Severity::Notice)
        .count();
    (warnings, notices)
}

const fn severity_label(severity: zero_migrate::Severity) -> &'static str {
    match severity {
        zero_migrate::Severity::Warning => "Warning",
        zero_migrate::Severity::Notice => "Notice",
    }
}
...
fn preview_opts() -> zero_migrate::PreviewOpts {
    zero_migrate::PreviewOpts {
...
/// [`zero_migrate::render::sql_preview`] surfacing layer, and prints it to stdout. DB-
...
fn run_plan_preview(
    ...
    use zero_migrate::render::sql_preview::{render_ir_json_sql, render_set_sql};
    ...
    if artifacts.has_sql {
        match zero_migrate::plan::loader::load_dir(dir) {
            Ok(plans) => {
                output.push_str(&render_set_sql(&plans, dialect, &opts));
            }
            Err(e) => {
                eprintln!("zero-migrate: plan: load `.sql` artifacts: {e}");
                return ExitCode::FAILURE;
            }
        }
    }
    ...
    for path in &artifacts.ir_files {
        let bytes = match std::fs::read_to_string(path) {
            Ok(b) => b,
            Err(e) => {
                eprintln!("zero-migrate: plan: read {}: {e}", path.display());
                return ExitCode::FAILURE;
            }
        };
        match render_ir_json_sql(&bytes, dialect, &opts) {
            ...
            Err(e) => {
                eprintln!("zero-migrate: plan: {}: {e}", path.display());
                return ExitCode::FAILURE;
            }
        }
    }
...
fn lint_offline_artifacts(
    ...
    if artifacts.has_sql {
        let plans = zero_migrate::plan::loader::load_dir(dir)
        ...
        for plan in &plans {
            ...
            pairs.push((
                ...
                zero_migrate::analyze_migration(migration),
            ));
        }
    }
    ...
    for path in &artifacts.ir_files {
        ...
        let (name, statements) =
            zero_migrate::render_ir_json_sql_statements(&bytes, dialect, opts)
...
fn analyze_rendered_statement(statement: &str) -> Vec<zero_migrate::Advisory> {
    zero_migrate::RawSqlAuthor::new(DEFAULT_GENERIC_SCHEMA, "app_preview")
        .wrap("lint_rendered_statement", statement, None)
        .map_or_else(
            |_| zero_migrate::analyze(statement),
            |migration| zero_migrate::analyze_migration(&migration),
        )
}

fn lint_denied(
    pairs: &[(String, Vec<zero_migrate::Advisory>)],
    deny_warnings: bool,
    deny: Option<&str>,
) -> bool {
    let deny_rules = deny
    ...
    pairs.iter().flat_map(|(_, advisories)| advisories).any(|a| {
        (deny_warnings && a.severity == zero_migrate::Severity::Warning)
...
async fn main() -> ExitCode {
    ...
    // ancestor-dir `zero-migrate.toml` (possibly carrying a `database_url`) is
    ...
    if let Command::Load = &cli.command {
        ...
        let content = match std::fs::read_to_string(&path) {
            ...
            Err(e) => {
                eprintln!(
                    "zero-migrate: load: read schema file {}: {e}",
                    path.display()
                );
                return ExitCode::FAILURE;
            }
        ...
        Ok(report) => {
            ...
            if cli.lint && matches!(cli.command, Command::Migrate | Command::Up | Command::Validate)
            {
                match command_runner::lint_advisories(&cfg) {
                    ...
                    Err(e) => eprintln!("zero-migrate: lint: {e}"),
                }
            }
                ...
                && effective_auto_dump(&cli, &layer)
            {
                ...
                if let Err(e) = run_dump(&cfg, &out).await {
                    eprintln!(
                        "zero-migrate: warning: auto-dump of {} after a successful \
...
mod tests {
    ...
    fn profile_defaults_to_trusted() {
        ...
        let cli = parse(&["zero-migrate", "migrate", "--database-url", "x"]);
    ...
    fn profile_precedence_flag_over_env_over_config() {
        ...
        let cli_noflag = parse(&["zero-migrate", "migrate", "--database-url", "x"]);
        assert_eq!(
    ...
    fn new_command_parses_name() {
        let cli = parse(&["zero-migrate", "new", "create_users"]);
        match cli.command {
    ...
    fn dump_parses_schema_file_default_and_override() {
        ...
        let cli = parse(&["zero-migrate", "dump", "--database-url", "x"]);
    ...
    fn load_is_a_command_with_setup_alias_and_global_schema_file() {
        ...
        let cli = parse(&["zero-migrate", "setup", "--database-url", "x"]);
        assert!(matches!(cli.command, Command::Load));
    }
    ...
    fn run_config_falls_back_to_config_database_url() {
        ...
        let cli = parse(&["zero-migrate", "migrate"]);

//! JS/TS schema **front-end** for the `zero-migrate` engine.
//!
//! Atlas's real shape is *many schema front-ends (HCL / SQL / ORM providers)
//! → one internal representation → one diff/migrate engine.* `zero-migrate`
...
//! the V8 host runtime, and the public authoring surface lives here under

...
//! seed default is the DB-evaluated `now()` / `genRandomUuid()` synth
...
/// pattern: a uuid seed defaults to `genRandomUuid()`, a timestamp seed to
/// `now()` — the DB-evaluated synth scalars, never a host clock / RNG.
///
/// # Errors
...
// synth scalar — use the now() / genRandomUuid() synth defaults (shown
...
export function up() {{
  ...
  //   table("{name}").column("token").add({{ type: t.uuid().notNull().default(genRandomUuid()) }});
  //   table("{name}").column("expires_at").add({{ type: t.timestamp().notNull().default(now()) }});
}}
...
fn render_ts(name: &str, synth: &[SynthOp]) -> String {
    ...
    let mut imports = vec!["table", "t"];
    if synth.iter().any(|s| op_needs_nextval_import(&s.op)) {
        imports.push("nextval");
    }
    if synth.iter().any(|s| op_needs_now_import(&s.op)) {
        imports.push("now");
    }
    if synth.iter().any(|s| op_needs_gen_random_uuid_import(&s.op)) {
        imports.push("genRandomUuid");
    }
...
// Determinism (§4.3): all time/uuid defaults are DB-evaluated synth scalars.
import {{ {imports} }} from "@zeroship/migrate";
...
fn op_needs_nextval_import(op: &Op) -> bool {
    match op {
        Op::CreateTable { columns, .. } => columns
            .iter()
            .any(|c| matches!(c.default, Some(IrDefault::Nextval { .. }))),
        Op::AddColumn { default, .. } => matches!(default, Some(IrDefault::Nextval { .. })),
        Op::SetColumnDefault { value, .. } => matches!(value, IrDefault::Nextval { .. }),
        Op::CreateDomain { default, .. } => matches!(default, Some(IrDefault::Nextval { .. })),
        _ => false,
    }
}

fn default_is_synth(default: &IrDefault, synth_fn: SynthFn) -> bool {
    matches!(
        default,
        IrDefault::Expr {
            expr: Expr::FnSynth { r#fn, args },
        } if *r#fn == synth_fn && args.is_empty()
    )
}

fn op_needs_synth_default_import(op: &Op, synth_fn: SynthFn) -> bool {
    match op {
        Op::CreateTable { columns, .. } => columns
            .iter()
            .any(|c| c.default.as_ref().is_some_and(|d| default_is_synth(d, synth_fn))),
        Op::AddColumn { default, .. } => default
            .as_ref()
            .is_some_and(|d| default_is_synth(d, synth_fn)),
        Op::SetColumnDefault { value, .. } => default_is_synth(value, synth_fn),
        Op::CreateDomain { default, .. } => default
            .as_ref()
            .is_some_and(|d| default_is_synth(d, synth_fn)),
        _ => false,
    }
...
fn op_needs_now_import(op: &Op) -> bool {
    op_needs_synth_default_import(op, SynthFn::Now)
}

fn op_needs_gen_random_uuid_import(op: &Op) -> bool {
    op_needs_synth_default_import(op, SynthFn::GenRandomUuid)
}
...
/// to DB-evaluated top-level constructors (deterministic by construction).
    ...
    match d {
        IrDefault::Expr { expr } => match expr {
            ...
            Expr::FnSynth { r#fn: SynthFn::GenRandomUuid, args } if args.is_empty() => {
                ".default(genRandomUuid())".to_string()
            }
            _ => {
        ...
        IrDefault::Nextval { sequence } => {
            let args = match sequence.schema.as_deref() {
                ...
                None => js_str(&sequence.name),
            };
            format!(".default(nextval({args}))")
        }
    }
}

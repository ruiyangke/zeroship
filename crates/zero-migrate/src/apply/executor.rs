...
async fn rollback_with_backend<B: MigrationBackend>(
    ...
    if let Some(snap) = &snapshot {
        if let Err(e) = backend.restore_session(snap).await {
            tracing::warn!(error = %e, "zero-migrate: failed to restore session GUCs after rollback");
        }
    }
    let unlock = backend
        ...
        match step {
            RollbackStep::SkipIrreversible(m) => {
                tracing::warn!(
                    ...
                    project = %cfg.project_id,
                    "zero-migrate: force-skipping irreversible migration during rollback (down: None)"
                );
                outcome

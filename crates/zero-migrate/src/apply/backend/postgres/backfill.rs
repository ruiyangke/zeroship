...
async fn run_one_batch<D: PgSession>(
    ...
    let row = match query_result {
        ...
        Err(e) => {
            if let Err(rb) = conn.batch_execute("ROLLBACK").await {
                tracing::warn!(error = %rb, "zero-migrate: ROLLBACK failed after a backfill batch error");
            }
    ...
    if n > 0 {
        ...
        {
            if let Err(rb) = conn.batch_execute("ROLLBACK").await {
                tracing::warn!(error = %rb, "zero-migrate: ROLLBACK failed after a backfill progress error");
            }

...
async fn teardown_shadow(
    ...
    if let Err(e) = admin_conn.batch_execute(&drop_db).await {
        tracing::warn!(
            error = %e,
            shadow = %shadow_db,
            "zero-migrate: failed to drop shadow database (will be reaped by sweep_leaked_shadows)"
        );
    ...
    if shadow_exec_cfg.pg.migrator_role.is_some() {
        if let Err(e) = deprovision_migrator(admin_conn, shadow_exec_cfg).await {
            tracing::warn!(
                ...
                "zero-migrate: failed to drop shadow migrator role (cluster-global leak)"
            );
            ...
            match admin_conn.batch_execute(&sql).await {
                ...
                Err(e) => {
                    tracing::warn!(
                        error = %e,
                        shadow = %name,
                        "zero-migrate: sweep failed to drop a leaked shadow database (continuing)"
                    );
                }
            }
        }
...
mod tests {
    ...
    fn shadow_dsn_keyword_form_swaps_dbname() {
        let admin = "host=localhost port=5440 user=postgres password=zeroship dbname=zero_migrate_test";
        let s = shadow_dsn(admin, "zsmig_shadow_abc");
        assert!(s.contains("dbname=zsmig_shadow_abc"));
        assert!(!s.contains("dbname=zero_migrate_test"));
    ...
    fn shadow_dsn_url_form_swaps_path() {
        let admin = "postgres://postgres:zeroship@localhost:5440/zero_migrate_test";

...
pub async fn submit_migration(
    ...
    if let Err(e) = executor::release_project_lock_outer(conn, &cfg.project_id).await {
        tracing::warn!(
            ...
            "zero-migrate: failed to release project lock after submit (HIGH-1)"
        );
    }
...
async fn submit_locked(
    ...
    if !report.teardown_ok {
        tracing::warn!(
            ...
            "zero-migrate: shadow DB/role teardown failed during submit — a shadow may \
        ...
        if let Some(row) = rows.first() {
            ...
            if journaled_name != submitted_name {
                tracing::debug!(
                    ...
                    "zero-migrate: submit NoOp fired on a checksum whose journaled \

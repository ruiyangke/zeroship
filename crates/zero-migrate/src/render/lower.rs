...
/// THE PER-FACET VERDICT for `Op::SetColumnType`: what a change of base type does
/// to every OTHER facet the column carries.
///
...
/// cross-references this table; the refusals it raises are listed below because
/// they are part of the same verdict.
///
/// # Re-derived from the target type
///
/// The type TOKEN is not the whole type. `ColType::String { length }`,
/// `Char { length }` and `Vector { vector }` carry their parameter in a SIBLING
/// descriptor field, so a replay that assigns only the token is wrong in BOTH
/// directions: it keeps `maxLength: 24` on a column the database widened to
/// `varchar(40)`, and it emits a bare `{"type":"char"}` — which is not a type —
/// for a retype INTO `char(8)`.
///
/// | facet                          | verdict           |
/// |--------------------------------|-------------------|
/// | `ty`                           | from `to_type`    |
/// | `max_length`, `char_len`, `vector_dims`, `unbounded_text` | from `to_type` |
/// | `encrypted`                    | from `to_type` (always absent — see the refusal) |
///
/// # Cleared: type-bound facets `setColumnType` has no slot to re-declare
///
/// | facet            | reason |
/// |------------------|--------|
/// | `case_sensitive` | On PostgreSQL case-insensitivity IS the `citext` TYPE, so changing the type destroys it. Measured on live PostgreSQL 18.4: `citext → character varying(40)` leaves a plain case-SENSITIVE column, and because `case_sensitive` is DRIFT-COMPARED, keeping it reported `case_sensitive expected "false" actual ""` on a schema that was exactly what had been deployed. |
/// | `vector_metric`  | `t.vector(n, { metric })` is a DECLARED-ONLY opclass selector with no catalog trace. Meaningless off a `vector` type, and `Op::SetColumnType` carries no slot to re-declare it. |
/// | `id_prefix`      | The legacy `<prefix>_<22 base62 UUIDv7>` brand is text-shaped, and — like `value_format` — its writer owns the storage type. `declare_logical_column` already clears it. |
/// | `min`, `max`, `enum_values`, `literal_value` | Value contracts over the OLD type that the declarative renderer turns into CHECKs; a `min: 0` on a column that is now text emits a comparison the type cannot answer. Unreachable through this replay today ([`ir_column_to_field`] never sets them), cleared so it stays that way. |
///
/// # Kept: orthogonal to storage, and the server carries them across
///
/// Measured on live PostgreSQL 18.4, one `ALTER TABLE … ALTER COLUMN … TYPE` per row:
///
/// | facet | measurement |
/// |-------|-------------|
/// | `required` | `attnotnull` still `t`. |
/// | `unique` | the unique index is REBUILT and survives. |
/// | `default` | PostgreSQL re-casts it and REFUSES the whole ALTER when it cannot (`default for column "v" cannot be cast automatically to type integer` — for `'x'` AND for `'7'`), so a default that reaches a fold is one the server kept. |
/// | `references` + `on_delete`/`on_update`/`deferrable` | a separate constraint, not a storage facet. PostgreSQL refuses the ALTER when the result is FK-incompatible (`foreign key constraint "q_v_fkey" cannot be implemented`) and applies it when it is not. |
/// | `generated` | `attgenerated` survives (`int → bigint` keeps `s` and `(c0 + 1)`). |
/// | `identity` | `attidentity` survives (`int → bigint` keeps `d`); PostgreSQL refuses a non-integer target outright (`identity column type must be smallint, integer, or bigint`). |
/// | `fts`, `fts_language` | the collection's `__fts` GENERATED tsvector column and its GIN index are separate objects the retype does not touch — and PostgreSQL refuses to alter a column a generated column reads at all (`cannot alter type of a column used by a generated column`), for every target. Unreachable through this replay today. |
/// | `mask` | see the refusal below. |
///
/// # Refused (raised by the snapshot fold, which every replay runs first)
///
/// `value_format` and the encryption/mask sentinels are NOT cleared and NOT kept:
/// [`crate::fold_ops`]'s `Op::SetColumnType` arm fails closed on them, for the one
/// reason both share — the apply path emits ONLY `ALTER COLUMN … TYPE`, so the
...
pub(crate) fn retype_field_descriptor(field: &mut FieldDescriptor, to_type: &ColType) {
    ...
    field.vector_metric = None;
    field.id_prefix = None;
    field.min = None;
    field.max = None;
    field.enum_values = None;
    field.literal_value = None;

    // Everything else is KEPT, deliberately and by omission of an assignment:

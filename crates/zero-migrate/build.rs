...
//!
//! The GENERIC Postgres apply path — the `seam::SqlSession` trait, the generic
...
//! names NO concrete driver type (the seam is `seam::{Bind, Row, DbError}`).
...
fn main() {
    ...
    // engine does its own introspection over the `seam::SqlSession` seam) and are

...
///
/// Created once in `IsolatePool::new()`. Owns the pipe and watchdog thread.
...
impl CpuTimerSystem {
    ...
    pub fn new() -> Self {
        ...
        unsafe {
            ...
            sa.sa_sigaction = cpu_timeout_handler as usize;
...
impl Drop for CpuTimerSystem {
    #[allow(unsafe_code)]
    fn drop(&mut self) {
        // Close the pipe — watchdog thread will see read() return 0 and exit.
        unsafe {
...
impl CpuTimer {
    ...
    pub fn new(app_id: u64) -> Result<Self, String> {
        ...
        // The nix crate doesn't expose this directly, so we use the libc field.

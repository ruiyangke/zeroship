...
fn storage_and_its_tests_do_not_reach_v8_or_metering() {
    let packages = dependencies("zeroship-storage", "normal,build,dev");
    for required in ["compio-s3", "compio"] {
        assert!(
...
fn binding_connects_storage_to_the_runtime() {
    let packages = dependencies("zeroship-storage-v8", "normal,build");
    for required in ["zeroship-storage", "zeroship-runtime", "v8"] {
        assert!(
            packages.contains(required),
...
fn boundary_recognizes_adapters_and_allows_storage() {
    ...
    for allowed in ["zeroship-storage", "compio-s3", "compio", "testcontainers"] {
        assert!(

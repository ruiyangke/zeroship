...
pub async fn query(
    ...
    sql: &str,
    values: &[Value],
) -> Result<Vec<compio_postgres::Row>, zeroship_data_core::error::DbError> {
...
mod tests {
    ...
    fn vector_parameters_require_finite_numeric_components() {
        ...
        let value = zeroship_data_query_builder::value!([1.0, 0.5]);
        let mut bytes = BytesMut::new();
        Parameter(&value).to_sql(&vector, &mut bytes).unwrap();
        assert_eq!(bytes.as_ref(), b"[1,0.5]");
        for value in [
            zeroship_data_query_builder::value!(["1"]),
    ...
    async fn native_values_round_trip_through_postgres() {
        ...
        let values = [
            ...
            Value::Null,
            Value::from("text"),
            zeroship_data_query_builder::value!({"nested":[1, false]}),
        ...
        let native = crate::pg_row_json::rows_to_values(&rows);
        for (field, value) in [
    ...
    async fn json_filter_literals_are_not_encoded_as_strings() {
        use zeroship_data_query_builder::{compile, value};
        let (client, connection) = compio_postgres::connect(
            &zeroship_core::config::test_database_url(),
            compio_postgres::NoTls,
        )
        ...
        .unwrap();
        let sql = format!("SELECT payload FROM (VALUES ('{{\"key\":\"value\"}}'::jsonb)) AS data(payload) WHERE {filter}");
        let rows = query(&client, &sql, &params).await.unwrap();
        assert_eq!(rows.len(), 1);
        assert_eq!(
            crate::pg_row_json::rows_to_values(&rows)[0]["payload"],
            value!({"key":"value"})
        );
    }
}

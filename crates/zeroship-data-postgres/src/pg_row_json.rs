...
fn column_to_value(row: &compio_postgres::Row, idx: usize, oid: u32) -> Value {
    ...
    match oid {
        ...
        21 => match row.try_get::<_, i16>(idx) {
            Ok(v) => Value::Number(zeroship_data_query_builder::value::Number::from(v)),
            Err(_) => Value::Null,
        },
        // INT4 = 23
        ...
        700 => match row.try_get::<_, f32>(idx) {
            Ok(v) => zeroship_data_query_builder::value::Number::from_f64(f64::from(v))
                .map_or(Value::Null, Value::Number),
            Err(_) => Value::Null,
        },
        // FLOAT8 = 701
        701 => match row.try_get::<_, f64>(idx) {
            Ok(v) => zeroship_data_query_builder::value::Number::from_f64(v)
                .map_or(Value::Null, Value::Number),
            Err(_) => Value::Null,
        },
        // UUID = 2950

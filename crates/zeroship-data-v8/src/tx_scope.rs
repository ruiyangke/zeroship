...
pub(crate) fn current_tx_scope(scope: &mut v8::PinScope<'_, '_>) -> Option<TransactionScope> {
    ...
    Some(TransactionScope::observed(app, generation.u64_value().0, frame.u64_value().0))
}
...
mod tests {
    ...
    fn dispatch_inside_the_callback_routes_to_the_transaction() {
        in_scope!(let scope);
        let prev = super::enter(scope, &super::TransactionScope::observed("app_a".to_owned(), 1, 1));
    ...
    fn a_co_resident_apps_transaction_scope_does_not_capture_this_app() {
        in_scope!(let scope);
        let prev = super::enter(scope, &super::TransactionScope::observed("app_other".to_owned(), 2, 1));
        assert!(
            !super::capture_route(scope, &app_a_binding()).in_tx(),

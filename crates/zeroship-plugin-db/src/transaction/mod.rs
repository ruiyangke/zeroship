...
impl TxAdmission {
    ...
    pub(crate) async fn acquire(app_id: String) -> Self {
    ...
    pub(crate) fn handed_to_reducer(mut self) {
        self.armed = false;
    }
}
...
pub(crate) async fn exec_settle(
    app_id: &str,
    success: bool,
    frame: Option<reducer::frames::FrameId>,
) -> SettleOutcome {
...
pub(crate) fn commit_failed_indeterminate(error: DbError) -> DbError {
    DbError::Coded {
...
mod tests {
    ...
    // The only V8-shaped name this file still shows, and it is test-only: two
    // arms below drive the ADAPTER's `build_settle_resolve_value` to prove the
    // engine's outcome mapping survives lowering. Production code in this
    // module names no runtime type - see the module doc.
    ...
    fn a_settles_own_code_reaches_the_creator_unwrapped() {
        for outcome in [
            ...
            match crate::v8_classes::transaction::build_settle_resolve_value(outcome, true, None) {
    ...
    fn build_settle_resolve_value_ok_resolve_undefined_when_no_body() {
        let rv = crate::v8_classes::transaction::build_settle_resolve_value(SettleOutcome::Ok, true, None);

...
//! # Coupling direction
//!
//! The core is neutral, and that is the hard limit. Nothing in `zero-migrate` was
//! widened to `pub` to make this compile, and core re-exports nothing from here: a
//! `pub use zeroship_migrate_sqlite::SqliteBackend` in core would be core naming a
//! vendor CRATE outside the registry - trading one coupling for another. Consumers
...
//!
//! The dialect ids live in the vendor crates; the neutral vocabulary crate declares
//! none. This crate exports its `DIALECT` identity and its modules read that value.
//!
...
//! literal lives in `zeroship-migrate-backend`; `zeroship_migrate_backend::dml`'s
//! header documents the hazard. The identifier seam

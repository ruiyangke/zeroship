...
impl Harness {
    ...
    fn admit() -> Self {
        let now = Instant::now();
        let (reducer, actions) =
            TxReducer::admit(expected(), TxBudgets::default(), now, MAX_DEPTH);
        Self {
            reducer,
            now,
            emitted: actions,
...
fn quiescing_latches_exactly_one_intent() {
    ...
    assert!(same.is_empty(), "the same attempt joins the waiter already there");
...
/// shipped behaviour (`crates/zeroship-plugin-db/tests/native_transaction.rs:671-705`), so it cannot be
...
fn a_committed_result_against_a_rollback_intent_publishes_nothing() {
    ...
    assert_eq!(harness.reducer.outcome(), Some(TerminalOutcome::ResultMismatch));
    assert!(!harness
        .all()
        .iter()
        .any(|action| matches!(action, Action::PublishEffects(_))));
...
fn a_deadline_firing_after_a_commit_succeeded_does_not_cancel_it() {
    ...
    assert!(actions.is_empty(), "a late cancel joins and changes nothing");
    assert_eq!(
        harness.reducer.outcome(),
...
fn a_force_cannot_claim_settling() {
    ...
    assert!(actions.is_empty(), "it is a late cancel: it joins the terminal waiters");
    assert_eq!(harness.state(), TxState::Settling);

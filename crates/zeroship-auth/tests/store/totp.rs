//! TOTP storage and crypto lifecycle in owned `PostgreSQL` under the auth role.
...
async fn enroll_stores_encrypted_and_unconfirmed() {
    Database::run(async |database| {
        ...
        // The stored ciphertext must not contain the raw secret.
        let stored = db
            .query_one(
...
async fn stored_secret_verifies_at_a_known_time() {
    Database::run(async |database| {
        ...
        assert!(!totp::verify_code_at(&recovered, "000001", 59));
    })
    .await;
}
...
async fn backup_code_works_once_then_is_rejected() {
    Database::run(async |database| {
        ...
        assert!(
            ...
            "the used code is gone from unused"
        );
    })
    .await;
}

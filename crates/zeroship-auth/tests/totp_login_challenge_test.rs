...
async fn only_confirmed_credential_gates_login() {
    ...
    let user = users::create(
        &db,
        &format!("totp-gate-{tag}@zeroship.test"),
        "Gate",
        Some("phc"),
    )
    .await
...
async fn challenge_stash_binds_user_and_credential_version() {
    ...
    let user = users::create(
        &db,
        &format!("totp-stash-{tag}@zeroship.test"),
        "Stash",
        Some("phc"),
    )
    .await
    ...
    let stash = TotpChallenge::new(user.id.clone(), cv, "lc-xyz".into(), FirstFactor::Password);
    ...
    let fresh = users::find_by_id(&db, &user.id).await.unwrap().unwrap();
    // Simulate a password change which bumps credential_version.
    users::update_password_hash(&db, &user.id, "phc2")
        .await
        .unwrap();
...
async fn second_factor_accepts_totp_then_backup_code_once() {
    ...
    let user = users::create(
        &db,
        &format!("totp-2f-{tag}@zeroship.test"),
        "TwoF",
        Some("phc"),
    )
    .await
    ...
    assert!(
        totp::verify_code(&recovered, &code),
        "valid TOTP code accepted"
    );
    assert!(
        !totp::verify_code(&recovered, "111111"),
        "an arbitrary code rejected"
    );

    // --- Backup-code path (post_2fa step 5b): find match, mark used, single-use ---
    let target = &plain[0];
    let codes = totp_store::unused_backup_codes(&db, &user.id)
        .await
        .unwrap();
    let matched = codes
        .iter()
    ...
    assert!(
        totp_store::mark_backup_code_used(&db, matched.id)
            .await
            .unwrap(),
        "first use ok"
    );

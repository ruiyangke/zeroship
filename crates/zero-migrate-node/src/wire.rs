...
//!    back (design §B.2). Plain owned data (`String`/`i64`/`Vec`/`bool`), all
//!    `Send + 'static`, so they ride a `ThreadsafeFunction` call + a `done` callback
//!    without ever carrying an engine `!Send` handle (§B.3). These compile WITHOUT
...
//!    RETURNS. This replaces the old stringly-typed boundary (`*_json` params +
//!    `Promise<string>`): the IR `ops` AST and a lowered `Migration` cross as REAL JS
...
//!
//! ## The exact-integer story (napi6, step 5a)
//! - DB *cell values* keep the [`JsCell`] `int`/`intStr` split — that is the §D.2
...
//! - A driver's affected-/returned-row *count* is an honest [`i64`] (was an
//!   `f64` — the incoherence step 5a removes): node-pg/mysql2 report `rowCount` as a
//!   JS `number`; `i64` reads it losslessly with no float games.
...
pub struct JsReply {
    ...
    /// `None` when the driver reports no count (e.g. `batch`). (Was `f64` pre-step-5a
    /// — a count is an integer, not a float.)
...
/// The typed request for the host-authoring `applyIr` verb (§D.1).
///
/// Replaces the old `owner_app`/`project_schema`/`dialect`/`registry_json`/
/// `envelope_json` positional-string soup. The `envelope` (`{ ir_version, name,
/// ops }`) crosses as a REAL JS value ([`JsonValue`]) — the recorder builds a JS
/// object, no JSON string round-trip. The envelope MUST NOT carry `owner_app` (it is
/// stamped from `owner_app` here — provenance, §D.1).
#[cfg(feature = "napi")]
#[napi(object)]
#[derive(Debug, Clone)]
pub struct ApplyRequest {
    ...
    pub dialect: String,
    /// The project's `{ table: owner_app }` ownership registry (§8.6). Empty on a
...
/// The typed reply for the sync, DB-free `loadVerify` gate (§C.5) — replaces the
/// JSON-string return. Never throws for a malformed document; `ok=false` + a message.
///

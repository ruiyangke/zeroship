//! Compose schema DDL from declared fields and resolved table-injection policy.
...
pub enum QueryError {
    ...
    InvalidIdent(String),
    /// A declaration collides with a column injected by the active policy.
    InjectedColumnCollision(String),
    /// A typed-ID prefix is reserved for platform identities.
...
impl std::fmt::Display for QueryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InjectedColumnCollision(msg) => write!(f, "injected column collision: {msg}"),
            Self::ReservedIdPrefix(msg) => write!(f, "reserved ID prefix: {msg}"),
        }
    }
}
...
/// Validate a declaration and reject collisions with the active injection.
pub fn validate_field_name_for_declaration(
    ...
    if inject.contains_column(name) {
        let active_names = inject
            ...
            .collect::<Vec<_>>()
            .join(", ");
        return Err(QueryError::InjectedColumnCollision(format!(
...
pub fn build_create_table_with_fks_for_dialect_scoped_statements(
    ...
    if let Some(obj) = schema.as_object() {
        for (field, def) in obj {
            ...
            // A typed-ID prefix declaration refines the injected primary-key column.
            if let Some(prefix) = def.get("idPrefix").and_then(|p| p.as_str()) {
                validate_id_prefix(prefix)?;
            }
            if inject.owns_primary_key_column(field)
            ...
            // mask outputs is string-shaped - and carries NONE of the declared
            // constraints, because a `t.number().mask(...)` column would refuse
            // `'***'` under `DOUBLE PRECISION` and an enum CHECK would refuse
            // it outright. The constraints belong to the value, and the value
            // moved.
            //
            ...
            if raw_column_for_field(field, def).is_some() {
                ...
                // classification=... */` inline comment to the MASKED
                // column's DDL so the SQLite introspector can recover the mask
    ...
    let injected_index_stmts =
        build_injected_indexes(app_id, collection, unqualified, &inject, backend)?;

    let mut statements: Vec<String> = vec![create_table];
    statements.extend(injected_index_stmts);

    statements.extend(backend.column_comment_statements(app_id, collection, schema));
...
fn build_fk_clause(
    ...
    let target_qualified = backend.foreign_key_target(app_id, target);
    let target_column = def
        .get("refColumn")
        .and_then(|value| value.as_str())
        .ok_or_else(|| {
            QueryError::InvalidFilter(format!(
                "reference '{collection}.{field}' requires an explicit refColumn"
            ))
        })?;
    validate_field_name(vendors, target_column)?;
    let rendered_target_column = backend.quote_ident(target_column);
...
pub fn build_add_column(
    ...
    // When the field carries a `.mask({...})` declaration, also emit a nullable
    // unbounded-text ADD COLUMN op for the MASKED column - the one with the
    // field's own name - and its sentinel attachment, in the same
    // multi-statement payload. It is NULL here (versus the CREATE TABLE shape)
    // because existing rows would refuse the ALTER if it were NOT NULL; the
    // mask backfill populates it after every row has its mask computed.
    //
    // Note: this branch is taken ONLY when the diff classifier emits
    // an `AddColumn` for a fresh top-level field declared with
    // `.mask({...})`. The separate `MaskBackfill`-paired op the classifier
    // emits for the backfill passes a synthetic def carrying no mask block, so
    // `raw_column_for_field` returns `None` there and we do not double-emit.
    if raw.is_some() {
        let sentinel = mask_sentinel_for_field(def);
        let masked_statements = backend
            .add_column_if_not_exists_statements(AddColumnIfNotExistsRequest {
                schema: app_id,
                table: collection,
                column: field,
            ...
            .map_err(|reason| QueryError::InvalidFilter(reason.to_string()))?;
        statements.extend(masked_statements);
    }
...
pub fn build_create_indexes(
    ...
    for (field, def) in obj {
        ...
        // they land on the column that holds it. Putting a `.unique()` on the
        // masked column would enforce uniqueness over MASKS, where many rows
        // legitimately share `***-**-1234` - and for `kind: "full"` every mask
        // is `***`, so the table would cap at one row.
        let raw = raw_column_for_field(field, def);
        ...
        // filter touches the masked column, so this is the index those queries
        // use; the one above serves the raw column, which only the unmask path
        // reads. Naming: `<coll>__<col>_mask_idx` (double-underscore separator,
        // matching `named_index_name`'s collision-avoidance convention). Never
        // UNIQUE - see above.
        if wants_index || wants_unique {
            if raw.is_some() {
                let idx_name = format!("{collection}__{field}_mask_idx");
                let sql = create_index_if_not_exists(
                    backend,
                    app_id,
                    collection,
...
fn field_to_column_for_dialect(
    ...
    // The declared type and every constraint go on the column that holds the
    // REAL value: `__zs_raw__<field>` when masked, the field's own name
    // otherwise. The LOGICAL name is what got validated above - the physical
    // one is the platform's, and the validator would refuse it (it starts
            ...
            if field.len() > MAX_MASKED_FIELD_NAME_BYTES {
                return Err(QueryError::InvalidIdent(format!(
                    "masked field name exceeds {MAX_MASKED_FIELD_NAME_BYTES} bytes: {field} \
                     ...
                     the identifier limit is 63 bytes)"
                )));
            }
            raw
        }
...
fn def_to_constraints_for_dialect(
    ...
    if let Some(default) = def.get("default") {
        match def.get("type").and_then(|t| t.as_str()) {
            ...
            Some("json") | Some("object") => parts.push(backend.json_object_default()),
            Some("array") => parts.push(backend.json_array_default()),
            _ => {}
        }
    } else {
...
mod tests {
    ...
    #[test]
    fn reference_target_is_required_for_inline_and_deferred_emission() {
        let schema = json!({ "owner": { "type": "string", "refTarget": "people" } });
        let existing = std::collections::HashSet::new();
        for emission in [FkEmission::Inline, FkEmission::Deferred(&existing)] {
            for dialect in [&POSTGRES, &SQLITE, &MYSQL] {
                let error = build_create_table_with_fks_for_dialect(
                    "app1", "entries", &schema, &emission, dialect,
                )
                .expect_err("a missing target must not silently omit the constraint");
    ...
    #[test]
    fn malformed_reference_target_is_not_silently_omitted() {
        for target in ["", "bad.target"] {
            let schema =
                json!({ "owner": { "type": "string", "refTarget": target, "refColumn": "key" } });
            assert!(
                build_create_table_with_fks("app1", "entries", &schema, &FkEmission::Inline)
                    .is_err()
            );
        }
    }
...
"#,
    ...
    fn p7_id_prefix_decl_with_reserved_usr_is_rejected() {
        ...
        assert!(
            matches!(err, QueryError::ReservedIdPrefix(_)),
    ...
    fn no_inject_policy_still_rejects_reserved_id_prefix() {
        ...
        assert!(matches!(err, QueryError::ReservedIdPrefix(_)));
    }
    ...
    fn p7_id_with_non_id_type_still_rejected() {
        ...
        let err =
            build_create_table_with_fks("app1", "posts", &schema, &FkEmission::Inline).unwrap_err();
        assert!(
            matches!(err, QueryError::InjectedColumnCollision(_)),
    ...
    fn b2_explicit_fk_constraint_name_renders_on_every_dialect() {
        let schema = json!({
            "accountId": {
                "type": "ref",
                "refTarget": "accounts", "refColumn": "id",
                "refName": "fk_custom",
            },
        });
    ...
    fn b2_deferred_emission_inlines_self_ref() {
        ...
        // target.
        let schema = json!({
            "managerId": {"type": "ref", "refTarget": "employees", "refColumn": "id"},
        });
        let existing: std::collections::HashSet<String> = std::collections::HashSet::new();
    ...
    fn fk_ref_field_emits_text_column_type_pg() {
        let def = json!({"type": "ref", "refTarget": "users", "refColumn": "id"});
    ...
    /// `injected_column_names_allowed_in_filter_path`).
    #[test]
    fn injected_column_names_refused_at_declaration() {
        for name in confined_injected_names("posts") {
            let err = validate_field_name_for_declaration(&name).unwrap_err();
            match err {
                QueryError::InjectedColumnCollision(msg) => {
                    assert!(
                        msg.contains(&name) && msg.contains("reserved"),
                        "expected reserved-injected-column message naming {name:?}, got: {msg}"
                    );
                }
                other => panic!("expected InjectedColumnCollision for {name:?}, got {other:?}"),
            }
        }
    }
    ...
    /// is declaration-only.
    #[test]
    fn injected_column_names_allowed_in_filter_path() {
        for name in confined_injected_names("posts") {
            assert!(
                validate_field_name(crate::test_fixtures::VENDORS, &name).is_ok(),
                "injected column {name:?} must be accepted by the filter-time validator"
            );
        }
    }

    /// Non-injected-column names continue to be accepted by the
    /// declaration-time validator (regression fence for the
    /// `validate_field_name_for_declaration` wrapper).
    #[test]
    fn non_injected_column_names_accepted_at_declaration() {
        for name in &["title", "content", "user_id", "createdAt", "first_name"] {
            assert!(
                validate_field_name_for_declaration(name).is_ok(),
                "non-injected column {name:?} must be accepted at declaration"
            );
        }
    }
    ...
    /// the injected-column reservation. End-to-end check that the
    ...
    #[test]
    fn build_create_table_refuses_creator_declared_injected_column() {
        for name in confined_injected_names("posts") {
            ...
            match err {
                QueryError::InjectedColumnCollision(msg) => {
                    assert!(
                        msg.contains(&name),
                        "CREATE TABLE must refuse injected-column {name:?}; got: {msg}"
                    );
                }
                other => panic!("expected InjectedColumnCollision for {name:?}, got {other:?}"),
            }
    ...
    // through this module's PG-defaulting test wrapper. The injected-column prefix and
    ...
    fn create_table_prepends_confined_injected_fields_pg() {
        ...
        for name in &names {
            assert!(
                ...
                "missing injected column {name:?} in PG DDL: {sql}"
            );
        }
        ...
        for w in positions.windows(2) {
            assert!(w[0] < w[1], "injected columns out of order: {sql}");
        }
        let title_pos = sql.find("\"title\"").expect("title column present");
        let last_injected_pos = *positions.last().unwrap();
        assert!(
            last_injected_pos < title_pos,
            "injected columns must precede user fields: {sql}"
        );
    }

    /// SQLite mirrors PG for the injected-column prefix; only timestamp
    ...
    fn create_table_prepends_confined_injected_fields_sqlite() {
        ...
        for name in &names {
            assert!(
                sql.contains(name),
                "missing injected column {name:?} in SQLite DDL: {sql}"
            );
        }
        let positions: Vec<usize> = names
            .iter()
    ...
    /// The fixture policy declares a bounded string key across dialects.
    #[test]
    ...
    #[test]
    fn create_table_appends_user_fields_after_injected_columns() {
        let schema = serde_json::json!({
            "title": { "type": "string", "required": true },
            "body":  { "type": "string" },
        });
        ...
        .expect("build ok");
        let last_injected = sql.find("deleted_at").expect("deleted_at present");
        let first_user = sql.find("\"title\"").expect("title present");
        assert!(
            last_injected < first_user,
            "user fields must follow injected columns: {sql}"
        );
    }

    /// FK emission on a user-declared `ref` field continues to work
    /// alongside the injected-column prefix. Pins the structural invariant
    ...
    /// The injected-column index names go through the existing
    ...
    /// declarations end up referencing the same injected-column name in
    /// the column list. The declaration-time validator catches creator-declared
    /// injected columns before this point - so this test exercises the
    ...
    #[test]
    fn debug_assert_panics_when_user_schema_collides_with_injected_column() {
        // The validator raises `InjectedColumnCollision` before
        ...
        for name in confined_injected_names("posts") {
            ...
            .expect_err("validator must reject injected-column declaration");
            assert!(
                matches!(err, QueryError::InjectedColumnCollision(_)),
                "validator must raise InjectedColumnCollision for {name:?}, got {err:?}"
            );
        }
    }

    /// PG and SQLite emit equivalent column COUNT and ORDER for the
    /// injected-column prefix; only the types differ. Snapshot-style
    /// comparison: any drift in the count or the order of injected
    /// fields between dialects fails here.
    #[test]
    fn pg_and_sqlite_emit_equivalent_create_table_for_injected_columns() {
        let names = confined_injected_names("posts");
        let schema = serde_json::json!({
            "title": { "type": "string", "required": true },
        });
        ...
        // Same injected-column NAMES in the same ORDER on both arms.
...
"#,
    ...
    // -----------------------------------------------------------------
    // Path B raw-column DDL emission
    // -----------------------------------------------------------------

    /// `raw_column_for_field` returns `Some(raw_column_name(field))` - after the
    /// storage flip the field's own column holds the mask, and the RAW column
    /// (the one this function names) is where the real value lives.
    #[test]
    fn raw_column_for_field_returns_raw_column_for_masked() {
    ...
    fn build_create_table_emits_raw_column_for_masked_field() {
        ...
        assert!(
            sql.contains(&format!("\"{raw}\" TEXT")),
            "expected raw column with the declared type: {sql}"
        );
        assert!(
            !sql.contains(&format!("\"{raw}\" TEXT NOT NULL")),
            "an unrequired field's raw column carries no NOT NULL: {sql}"
        );
        assert!(
            sql.contains("\"ssn\" TEXT"),
            "the field's own column now holds the mask, bare TEXT: {sql}"
        );
        assert!(
            !sql.contains(&raw_column_name("name")),
            "non-masked column must NOT emit a raw column: {sql}"
        );
    }

    /// Masked column CREATE TABLE emits `COMMENT ON
    /// COLUMN` for the field's OWN column (the one now holding the mask) so PG
    /// introspection round-trips the mask metadata via `pg_description`.
    #[test]
    fn build_create_table_emits_comment_on_column_sentinel_for_masked_column() {
        ...
        assert!(
            ...
            "expected COMMENT ON COLUMN on the field's own (mask) column: {sql}"
        );
        assert!(
    ...
    /// Inline `/* zero-migrate:mask:... */` comment on the field's own (mask)
    /// column, for SQLite-arm introspection (PG ignores SQL comments; SQLite
    ...
    fn build_create_table_emits_inline_mask_sentinel_comment() {
        ...
        assert!(
            ...
            "expected inline /* zero-migrate:mask:... */ comment on the mask column: {sql}"
        );
    }
    ...
    /// `.mask({...})` declaration emits BOTH the RAW column ADD (declared type)
    /// + the mask column ADD (the field's own name, nullable TEXT) + the
    /// `COMMENT ON COLUMN` sentinel on the mask column, in one multi-statement
    /// payload.
    #[test]
    fn build_add_column_emits_raw_column_and_sentinel_when_masked() {
        let def = serde_json::json!({
            "type": "string",
            "mask": { "kind": "last4", "classification": "spi" }
        });
        ...
        let raw = raw_column_name("ssn");
        assert!(
            sql.contains(&format!("ADD COLUMN IF NOT EXISTS \"{raw}\" TEXT")),
            "raw column: {sql}"
        );
        assert!(
            sql.contains("ADD COLUMN IF NOT EXISTS \"ssn\" TEXT NULL"),
            "mask column keeps the field's own name, bare nullable TEXT: {sql}"
        );
        assert!(
            sql.contains("COMMENT ON COLUMN \"app1\".\"users\".\"ssn\""),
            "comment on the mask column: {sql}"
        );
        assert!(
            sql.contains("'zero-migrate:mask:kind=last4,classification=spi'"),
            "sentinel: {sql}"
    ...
    /// only the single column ADD; no raw column, no comment.
    #[test]
    fn build_add_column_no_raw_column_when_unmasked() {
        let def = serde_json::json!({ "type": "string" });
        let sql = build_add_column(crate::test_fixtures::VENDORS, "app1", "users", "name", &def)
            .expect("build_add_column ok");
        assert!(
            !sql.contains(RAW_COLUMN_PREFIX),
            "no raw column for unmasked: {sql}"
        );
    ...
    fn build_create_table_emits_raw_column_for_encrypted_with_default_mask() {
        ...
        assert!(
            ...
            "the ciphertext moved to the raw column, still BYTEA: {sql}"
        );
        assert!(
            !sql.contains(&format!("\"{raw}\" BYTEA NOT NULL")),
            "an unrequired field's raw column carries no NOT NULL: {sql}"
        );
        assert!(
            sql.contains("\"ssn\" TEXT"),
        ...
        assert!(
            ...
            "the mask column must be nullable / omittable: {sql}"
        );
    }

    /// **DDL shape** - `kind: "none"` explicit opt-out -> no raw column at all.
    /// The field's own column behaves like a plain encrypted column.
    #[test]
    fn build_create_table_no_raw_column_when_mask_kind_none() {
        let schema = serde_json::json!({
            "ssn": {
                "type": "string",
                "encrypted": true,
        ...
        assert!(
            !sql.contains(RAW_COLUMN_PREFIX),
            "kind=none must NOT emit a raw column: {sql}"
        );
        assert!(
    ...
    // SELECT-shape gates
    //
    // The masked-column READ shape belongs to the data-plane SELECT builder, not
    // this crate - that builder and the tests that pin it live elsewhere, so
    // nothing here proves the read shape; do not read this heading as though
    // something did. Before the storage flip a default read aliased
    // `"<col>_masked" AS "<col>"`; after it, the field's own column already IS
    // `"<col>"` and already holds the mask, so an ordinary read needs no
    // aliasing at all - only a read that wants the raw value would need to name
    // `__zs_raw__<col>`, and nothing on the creator-facing read surface can.
    //
    // What this layer still owns: `__zs_raw__` falls under the generic `_`
    // prefix reservation, so `validate_field_name` refuses a creator field
    // starting with it wherever a field name is validated. A `__zs_raw__<col>`
    // column can therefore only ever be one the builders in this module
    // emitted.
    // -----------------------------------------------------------------
    ...
    /// (no `"<app_id>".` prefix) and unqualified injected indexes - so the DDL
    ...
    fn sqlite_main_unqualified_drops_app_id_qualifier() {
        ...
        // The injected indexes are unqualified too (no schema on the index name).
        assert!(
            sql.contains(r#"CREATE INDEX IF NOT EXISTS "posts_deleted_at_idx" ON "posts""#),
            "injected index must be unqualified, got: {sql}"
        );
    }
    ...
    /// `MainUnqualified` SQLite carries the goodies: the paired raw column, the
    /// inline `zero-migrate:mask:` mask sentinel on the field's own (now
    /// mask-holding) column, the inline `zero-migrate:enc:` encryption sentinel
    /// on the BLOB column, and an unqualified FK clause - so all four survive
    /// into `sqlite_master.sql` for the drift snapshot to recover.
    #[test]
    fn sqlite_main_unqualified_carries_mask_enc_and_fk() {
        ...
        // The raw column for `ssn` carries the declared type.
        assert!(
            sql.contains(&format!("\"{}\" TEXT", raw_column_name("ssn"))),
            "raw column for ssn must be present: {sql}"
        );
        // Mask sentinel rides inline on the field's own column now, not a
        // `_masked`-suffixed sibling.
        assert!(
            sql.contains(r#""ssn" TEXT /* zero-migrate:mask:"#),
            "mask sentinel must ride inline on the field's own column: {sql}"
        );

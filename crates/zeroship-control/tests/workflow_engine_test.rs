...
use zeroship_control::cron::{workflow_blob_gc, workflow_retention, workflow_signal_fanout};
...
async fn seed_additional_deploy(fx: &Fixture, app_id: &AppId, _label: &str) -> String {
    let deploy_id = zeroship_core::typed_id::generate("dep");
    fx.pg
        .execute(
            "INSERT INTO zeroship.app_deploys (id, app_id, deploy_hash, manifest_json, activated_at) \
...
#[compio::test]
#[serial]
async fn retention_sweep_visits_multiple_app_journals() {
...
/// run by `run_orphan_sweep`, which does not take its app list from
/// `journalled_fleet`. It does not prove the LOGS say anything.

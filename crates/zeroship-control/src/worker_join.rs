...
/// and this decides which apps that instance may see.
///
/// The instance's zone is on the instance row: it is the zone claim of the
/// join token Control verified, resolved to an id and frozen there. Nothing a
/// worker sends reaches it, and no join can move it.
///
/// It applies the same liveness `active_instance_public_key` does - an `active`
/// row whose lease has not run out - for the same reason: a crashed or
/// forgotten worker's row must stop admitting reads on its own.
///
/// # Errors
///
/// Returns the driver's error when the registry cannot be read. A caller must
        ...
        .query(
            "SELECT 1 FROM zeroship.worker_instances instance \
               JOIN zeroship.apps app \
                 ON app.execution_zone_id = instance.execution_zone_id \
              WHERE instance.id = $1 AND app.id = $2 \
                AND instance.status = $3 AND instance.expires_at > now()",

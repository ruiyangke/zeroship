...
/// Load the entire global weight table over a CALLER-SUPPLIED connection.
///
/// [`PricingStore::weights`] is this function plus a connection from the
/// registry. The split exists so a caller already holding a connection - or a
/// TRANSACTION - reads the cost model on it rather than opening a second
/// session; see [`crate::plan_catalog::get_on`] for the same argument about the
/// catalog. One spelling of this SELECT, two ways to reach it.
///
/// A row with `per_units <= 0` (the CHECK forbids it, but be defensive) is
/// skipped so it can never panic the divisor downstream.
///
/// # Errors
/// [`RegistryError`] on a driver failure.
pub async fn weights_on<C: compio_postgres::GenericClient + Sync>(
    conn: &C,
) -> Result<MetricWeights, RegistryError> {
    let rows = conn
        .query(
...
/// The global default FX (pico-cents per CU) over a CALLER-SUPPLIED connection —
/// [`PricingStore::default_fx_pico_cents_per_unit`]'s body, split for the same
/// reason [`weights_on`] is.
///
...
/// # Errors
/// [`RegistryError`] on a driver failure.
pub async fn default_fx_pico_cents_per_unit_on<C: compio_postgres::GenericClient + Sync>(
    conn: &C,
...
impl PricingStore {
    ...
    pub async fn default_fx_pico_cents_per_unit(&self) -> Result<Option<u64>, RegistryError> {
        let conn = self.registry.conn().await?;
        default_fx_pico_cents_per_unit_on(&conn).await
    }

...
//! `#[test]` itself fails iff any non-skipped test FAILs.
//!
//! v1 coverage (this dispatch — value-path only):
//!   - constructor.any.js
//!   - general.any.js (subset — async tests using delay/setTimeout
//!     run as best-effort against our microtask drain)
//!   - default-reader.any.js
//!
//! Skipped: anything requiring byte streams, async iterator, pipeTo,
//! tee, or any feature deferred per the dispatch brief.
...
(function () {
  ...
  // Minimal step_timeout/setTimeout/queueMicrotask for the WPT
  // resources/test-utils.js `delay()` helper. We don't have a real
  // timer — but for "delay(0)"-style flushAsyncEvents calls, falling
  // back to an immediate microtask is close enough that the spec's
  // microtask-ordering tests still work. Tests using genuine timeouts
  // (delay(5+ ms)) are best-effort.
  ...
  // queueMicrotask is provided by V8.
  ...
  function makeTestObj() {
    const t = {
      ...
      // step_func — wrap so any throw propagates via t._failed.
      step_func(cb) {
        const that = this;
        return function () {
      ...
      // step_func_done — same as step_func, with implicit done() at end.
      step_func_done(cb) { return this.step_func(cb); },
      // unreached_func(msg) — returns a fn that always fails.
      unreached_func(msg) {
        const that = this;
        return function () {
          that._failed = true;
  ...
  // Async tests — promise_test / async_test. We install the result
  // synchronously so the microtask drain at end-of-script captures
  // settled promises.
  globalThis.promise_test = function (fn, name) {
    ...
    const t = makeTestObj();
    try {
      // Some promise_test bodies use arrow functions and access `t`
      // via the first argument rather than `this`. Pass `t` both
      // ways to support both styles (testharness.js does the same).
      const p = fn.call(t, t);
  ...
  globalThis.async_test = function (fn, name) {
    // We don't drive a real async runtime; classify as skip so the
    // numbers reflect coverage honestly.
    __wpt_results.push({
  ...
  // Promise.prototype.catch convenience used by some tests.
...
fn run_wpt(label: &str, sources: &[&str]) -> Vec<TestResult> {
    ...
    // Multiple microtask drains so promise chains in promise_test()
    // settle (including chained then handlers).
    for _ in 0..32 {
...
#[test]
fn wpt_streams_readable_compliance() {
    ...
    eprintln!("\n=== WPT streams/readable-streams/ results ===");
    ...
    if !failures.is_empty() {
        ...
        panic!("{} WPT streams/readable-streams tests failed", failures.len());
    }
}

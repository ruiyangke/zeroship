...
//! `#[test]` itself succeeds if at least one subtest passes (watermark
//! test); summary output enumerates pass/fail/skip per file.
...
(function () {
  ...
  // URLSearchParams shim (see fetch_body.rs / wpt_fetch_request.rs
  // for the rationale).
  ...
  // promise_rejects_js: returns a promise that resolves if the input
  // promise rejects with an instance of `ctor` (or matching .name).
...
// Selection: covers the v1 Response surface (init/error/redirect/json
// statics + clone). We include response-clone but expect a chunk of
// failures around structuredClone-of-buffer-chunks in tee — that's a
// streams crate concern, not fetch. (See `wpt_streams_tee.rs`.)
...
#[test]
fn wpt_fetch_response_compliance() {
    ...
    eprintln!("\n=== WPT fetch/api/response/ results ===");
    ...
    if !failures.is_empty() {
        ...
        eprintln!(
            "Note: {} subtests failed (mostly Blob/File-using tests; \
    ...
    assert!(
        totals.pass > 0,
        "no WPT response subtests passed — runner is broken"
    );
}

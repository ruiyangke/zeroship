//! WPT runner for `streams/readable-byte-streams/*.any.js`.
...
#[test]
fn wpt_streams_byob_compliance() {
    ...
    eprintln!("\n=== WPT streams/readable-byte-streams/*.any.js results ===");
    ...
    if attempted == 0 {
        panic!("no WPT byte-stream subtests ran");
    }
    ...
    // BYOB has the highest spec-corner density. Set a generous floor so
    // we don't gate landing on every spec corner; record the gap in the
    // proposal status doc.
    if pass_rate < 0.40 {
        panic!(
            "WPT byte-stream pass rate {:.1}% < 40% threshold — large regression",
            pass_rate * 100.0,
        );
    }
}

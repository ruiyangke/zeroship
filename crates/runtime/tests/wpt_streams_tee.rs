//! WPT runner for `streams/readable-streams/tee.any.js`.
            ...
            .map(|r| {
                ...
                let outcome = match r.status.as_str() {
                    ...
                    "fail" => {
                        ...
                        if byte_tee_dependent(&name) {
                            Outcome::Skip(format!("byte-tee deferred: {err}"))
                        } else {
                            Outcome::Fail(err)
                        }
                    }
...
fn byte_tee_dependent(name: &str) -> bool {
    // tee.any.js mixes default-tee and byte-tee tests; the latter need
    // BYOB byte streams which land in the next dispatch.
    name.contains("byte stream")
        || name.contains("BYOB")
...
fn wpt_streams_tee_compliance() {
    ...
    eprintln!("\n=== WPT streams/readable-streams/tee.any.js results ===");
    ...
    if !unexpected.is_empty() && pass_rate < 0.85 {
        panic!(
            "WPT tee pass rate {:.1}% < 85% threshold — {} unexpected failures",
            pass_rate * 100.0,
            unexpected.len()
        );
    }

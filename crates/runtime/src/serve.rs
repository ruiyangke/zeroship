...
/// Auto-generated client HTML from compiler (for single-file JSX apps).
static CLIENT_HTML: std::sync::Mutex<Option<String>> = std::sync::Mutex::new(None);
...
///
/// If the modules list contains a `__client.html` entry, it is extracted and
/// served on GET requests to `/`. This is used for single-file JSX apps where
/// the compiler generates client HTML automatically.
pub fn start_server(mut modules: Vec<ModuleEntry>, options: ServerOptions) -> ! {
    init_v8();

    // Extract client HTML if present (generated by compiler for JSX single-file apps).
    // Store globally so worker threads can access it.
    if let Some(i) = modules.iter().position(|m| m.specifier == "__client.html") {
        let html = modules.remove(i).source;
        CLIENT_HTML.lock().unwrap().replace(html);
    }
    ...
    loop {
        ...
        loop {
            ...
            match (method, path) {
                ...
                ("GET", "/" | "/index.html") if CLIENT_HTML.lock().unwrap().is_some() => {
                    let html = CLIENT_HTML.lock().unwrap().clone().unwrap();
                    let response = format!(
                        "HTTP/1.1 200 OK\r\nContent-Type: text/html; charset=utf-8\r\nContent-Length: {}\r\n\r\n{}",
                        html.len(),
                        html,
                    );
                    let BufResult(write_result, _) = stream.write_all(response.into_bytes()).await;
                    if write_result.is_err() { return; }
                }
                ("POST", "/rpc") => {

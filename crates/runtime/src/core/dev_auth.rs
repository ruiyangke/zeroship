...
//! ## Why this exists
//!
//! In production the **gateway** is the BFF: it owns the `__Host-zs_app_session`
...
//! `/__zs/auth/{authorize,popup-callback,session,signout}` and mints a LOCAL
//! signed session cookie `__zs_dev_session`. That cookie is the dev equivalent
//! of the gateway's `__Host-zs_app_session` cookie — but the JS layer runs
...
//! dispatch. It reads the `__zs_dev_session` cookie, verifies the dev HMAC
...
//! production `__Host-zs_app_session` cookie, carries a dev-only HMAC, and is
...
/// `__Host-zs_app_session` cookie so the two can never be confused, and so a
/// stray prod cookie is ignored by the dev decoder (and vice-versa).
pub const DEV_SESSION_COOKIE: &str = "__zs_dev_session";
...
/// to sign the `__zs_dev_session` cookie. Generated fresh by the Vite plugin
...
///   3. a `__zs_dev_session` cookie is present and its dev HMAC verifies.
///
...
mod tests {
    ...
    fn extract_cookie_picks_named() {
        let header = "other=1; __zs_dev_session=abc.def ; foo=bar";
        assert_eq!(

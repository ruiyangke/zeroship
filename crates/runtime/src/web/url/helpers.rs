...
//!
//! `USVString` (the lone surrogate scrubber required by §3.2.21) lives in
//! `crate::webidl::usv_string` since it's a shared boundary type used by
//! many Web APIs, not just URL. Re-exported here for back-compat.

pub use crate::webidl::usv_string::{USVString, read_usv_string, read_usv_string_or_throw};

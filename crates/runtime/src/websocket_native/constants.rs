...
/// Default `WebSocketInit.maxMessageSize` per D-28: 4 MiB. Tungstenite's
/// 64 MiB default would let a malicious server OOM the worker.
pub const DEFAULT_MAX_MESSAGE_SIZE: u32 = 4 * 1024 * 1024;

/// Default `WebSocketInit.maxFrameSize` per D-28: 1 MiB.
pub const DEFAULT_MAX_FRAME_SIZE: u32 = 1024 * 1024;

/// Default `WebSocketInit.pingIntervalMs` per D-28: 30s — matches
/// undici and workerd defaults.
...
/// Per-isolate `[[full]]` cap per RFC 6455 §6.1 / D-6: 16 MiB. When
/// `bufferedAmount` would exceed this, the [[full]] flag is set and
/// subsequent sends are dropped on the floor (matches fetch's body-too-
/// large path; addresses critic MAJOR #11).

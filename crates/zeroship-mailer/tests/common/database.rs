//! Isolated platform databases for mailer behavior tests.
//!
...
impl Database {
    pub async fn run(test: impl AsyncFnOnce(&Self)) {
        ...
        let postgres = image()
            ...
            .start()
            .expect("mailer tests require Docker and PostgreSQL");
        let database = Self {
        ...
        let outcome = AssertUnwindSafe(async {
            compio::time::timeout(Duration::from_secs(90), Box::pin(test(&database)))
                .await
                .expect("mailer database case timed out");
        })
        .catch_unwind()
        .await;
...
impl Seed {
    fn build() -> Self {
        eprintln!("mailer fixture: preparing the platform database seed");
        let postgres = image()
            .with_env_var("POSTGRES_DB", "mailer_tests")
            .start()
            .expect("prepare the migrated mailer database seed");
        apply_migrations(database_url(&postgres).as_str());
        eprintln!("mailer fixture: platform migrations applied; capturing the seed");
        let roles = dump(
            &postgres,
...
fn database_url(postgres: &Container<GenericImage>) -> url::Url {
    let mut url = url::Url::parse("postgresql://postgres:fixture@localhost/mailer_tests").unwrap();
    url.set_host(Some(
...
fn root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("mailer crate lives under crates/")
        .to_owned()
}
...
fn apply_migrations(url: &str) {
    ...
    let mut child = OwnedChild(
        command
            ...
            .spawn()
            .expect("mailer database fixtures require Node; run through nix develop"),
    );
...
async fn a_failed_case_releases_its_server_and_cannot_change_the_next_database() {
    ...
    Database::run(async |database| {
        *successful_id.borrow_mut() = database.postgres.id().to_owned();
        ...
        let auth = database.connect_as("zeroship_auth").await;
        ...
        auth.query("SELECT email::text FROM zeroship.email_suppressions", &[])
            .await
            .expect("restored auth role can read migrated suppressions");
    })
    .await;
    assert!(

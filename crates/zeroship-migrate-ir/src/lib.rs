...
//! The engine re-exports every type here (`pub use zeroship_migrate_ir as ir;` plus
//! the flattened re-exports at the engine root), so downstream code keeps naming
//! `zeroship_migrate::MigrationIr`, `zeroship_migrate::Op`, etc. unchanged.
//!
//! # What is NOT here (and why)
//!
//! - **Dialect-support / vendor-capability computation** for an [`ir::Op`] reads
//!   the engine-owned portability tables and lives in
//!   `zeroship_migrate::model::op_support` (free functions over `&Op`).
...
//!   and schema scope and stays engine-side in `zeroship_migrate::model::validate`.

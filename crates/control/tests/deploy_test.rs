//! Integration tests for the `.zsdeploy` ingest path.
//!
...
fn tmpdir() -> PathBuf {
    ...
    p.push(format!("zsdeploy-test-{}", Uuid::new_v4().simple()));
...
async fn deploy_round_trip() {
    ...
    let body = build_zsdeploy(&manifest_bytes, &blobs, true);
...
async fn deploy_rejects_missing_blob() {
    ...
    let manifest_bytes = serde_json::to_vec(&manifest).unwrap();
    let body = build_zsdeploy(&manifest_bytes, &[], true);
...
async fn deploy_rejects_manifest_not_first() {
    ...
    let body = build_zsdeploy(&manifest_bytes, &blobs, false);

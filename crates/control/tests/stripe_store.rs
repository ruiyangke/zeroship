...
async fn link_account_roundtrip() {
    ...
    store.link_account(creator, "acct_testCreator1").await.unwrap();

    let acc = store.get_account(creator).await.unwrap().expect("linked");
    assert_eq!(acc.creator_id, creator);
    assert_eq!(acc.stripe_account_id, "acct_testCreator1");

    // Relink overwrites.
    store.link_account(creator, "acct_testCreator2").await.unwrap();
    assert_eq!(
...
async fn reject_bad_account_id_shape() {
    ...
    match err {
        StripeError::Db(m) => assert!(m.contains("acct_")),
...
async fn record_payout_idempotent() {
    ...
    store.link_account(creator, "acct_idempotent").await.unwrap();
...
async fn total_earnings_aggregates_correctly() {
    ...
    store.link_account(creator, "acct_agg").await.unwrap();
...
async fn recent_payouts_newest_first_with_limit() {
    ...
    store.link_account(creator, "acct_recent").await.unwrap();
...
async fn per_creator_isolation() {
    ...
    store.link_account(a, "acct_A").await.unwrap();
    store.link_account(b, "acct_B").await.unwrap();
...
async fn unlink_cascades_payouts() {
    ...
    store.link_account(creator, "acct_cascade").await.unwrap();

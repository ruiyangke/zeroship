...
// `Domain=`, and `Secure`. RFC 6265bis §4.1.3.2 makes `Secure` non-
// optional for `__Host-` — compliant clients silently reject the
// cookie when Secure is missing. Dev mode (HTTP localhost) therefore
// drops `Secure` AND the prefix together so the cookie can complete
// the round-trip; the rest of the attributes never move.

/// Production console session cookie name (`__Host-` → Secure required).
pub const CONSOLE_SESSION_COOKIE_PROD: &str = "__Host-zs_console_session";
/// Dev console session cookie name (no `__Host-` prefix).
pub const CONSOLE_SESSION_COOKIE_DEV: &str = "zs_console_session";

/// 12-hour absolute lifetime for the console session cookie.
pub const CONSOLE_SESSION_MAX_AGE_SECS: i64 = 12 * 3600;

/// Resolve the console session cookie name for the current environment.
#[must_use]
pub fn console_session_cookie_name(insecure_dev: bool) -> &'static str {
    if insecure_dev { CONSOLE_SESSION_COOKIE_DEV } else { CONSOLE_SESSION_COOKIE_PROD }
}
...
pub fn set_console_session_cookie(session_id: &uuid::Uuid, insecure_dev: bool) -> String {
    let name = console_session_cookie_name(insecure_dev);
    let secure = if insecure_dev { "" } else { "; Secure" };
    format!(
        "{name}={session_id}; Path=/; HttpOnly; SameSite=Lax{secure}; Max-Age={CONSOLE_SESSION_MAX_AGE_SECS}"
    )
}
...
#[must_use]
pub fn parse_console_session_cookie(
    cookie_header: &str,
    insecure_dev: bool,
) -> Option<uuid::Uuid> {
    let name = console_session_cookie_name(insecure_dev);
...
/// Production stash cookie name (`__Host-` → Secure required).
pub const CONSOLE_STASH_COOKIE_PROD: &str = "__Host-zs_console_stash";
/// Dev stash cookie name (no `__Host-` prefix).
pub const CONSOLE_STASH_COOKIE_DEV: &str = "zs_console_stash";

/// 10-minute window for the OIDC dance to complete.
pub const CONSOLE_STASH_MAX_AGE_SECS: i64 = 600;

/// Resolve the console stash cookie name for the current environment.
#[must_use]
pub fn console_stash_cookie_name(insecure_dev: bool) -> &'static str {
    if insecure_dev { CONSOLE_STASH_COOKIE_DEV } else { CONSOLE_STASH_COOKIE_PROD }
}

/// Build the `Set-Cookie` header value for the OIDC stash.
#[must_use]
pub fn set_console_stash_cookie(value: &str, insecure_dev: bool) -> String {
    let name = console_stash_cookie_name(insecure_dev);
    let secure = if insecure_dev { "" } else { "; Secure" };
    format!(
        "{name}={value}; Path=/; HttpOnly; SameSite=Lax{secure}; Max-Age={CONSOLE_STASH_MAX_AGE_SECS}"
    )
}
...
#[must_use]
pub fn parse_console_stash_cookie(cookie_header: &str, insecure_dev: bool) -> Option<String> {
    let name = console_stash_cookie_name(insecure_dev);
    let prefix = format!("{name}=");
    for part in cookie_header.split(';') {
        let part = part.trim();
...
mod tests {
    ...
    #[test]
    fn console_session_set_cookie_drops_secure_and_host_prefix_in_dev() {
        ...
        assert!(c.starts_with("zs_console_session="), "dev cookie name: {c}");
        assert!(
            !c.contains("Secure"),
    ...
    fn console_session_parse_cookie_roundtrips() {
        ...
        assert_eq!(parse_console_session_cookie(&header, false), Some(id));
        assert_eq!(parse_console_session_cookie("nothing-here", false), None);
        assert_eq!(
            parse_console_session_cookie("__Host-zs_console_session=not-a-uuid", false),
            None
        );

        // Dev mode reads the bare-name cookie.
        let dev_header = format!("zs_console_session={id}");
        assert_eq!(parse_console_session_cookie(&dev_header, true), Some(id));
        assert_eq!(parse_console_session_cookie(&header, true), None);
    }
    ...
    #[test]
    fn console_stash_set_cookie_drops_secure_and_host_prefix_in_dev() {
        let c = set_console_stash_cookie("v", true);
        assert!(!c.starts_with("__Host-"), "dev cookie must NOT use __Host- prefix: {c}");
        assert!(c.starts_with("zs_console_stash=v"));
        assert!(!c.contains("Secure"));
    }
    ...
    fn console_stash_parse_cookie_roundtrips() {
        ...
        assert_eq!(parse_console_stash_cookie(header, false), Some("abc.def".into()));
        assert_eq!(parse_console_stash_cookie("nothing", false), None);

        let dev_header = "zs_console_stash=abc.def";
        assert_eq!(parse_console_stash_cookie(dev_header, true), Some("abc.def".into()));
        assert_eq!(parse_console_stash_cookie(header, true), None);
    }
}

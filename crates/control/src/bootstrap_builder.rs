...
pub async fn bootstrap_builder_oauth_client(
    ...
    let lock_conn = open_dedicated_auth_pg(&cfg.auth_db_url).await?;
    with_advisory_lock(&lock_conn, BOOTSTRAP_CLIENTS_LOCK, || async {
        Ok(bootstrap_builder_oauth_client_locked(&lock_conn, cfg).await)
    })
    .await
    .map_err(|err| BuilderClientBootstrapError::Db(format!("builder bootstrap lock: {err}")))?
}

async fn bootstrap_builder_oauth_client_locked(
    pg: &Client,
    cfg: &BuilderClientBootstrapConfig,
) -> Result<BuilderClientBootstrapResult, BuilderClientBootstrapError> {
    if oauth_client_exists(pg).await? {
        match read_existing_client_secret(&cfg.client_secret_path) {
            Ok(_) => {
                tracing::info!(
            ...
            Err(BuilderClientBootstrapError::Io(_)) if !cfg.client_secret_path.exists() => {
                let client_secret = ensure_client_secret(&cfg.client_secret_path)?;
                reconcile_hydra_client(cfg, &client_secret).await?;
                upsert_oauth_client(pg, cfg).await?;
                tracing::warn!(
                    client_id = BUILDER_CLIENT_ID,
                    secret_path = %cfg.client_secret_path.display(),
                    "control: recreated missing builder OAuth client secret and reconciled hydra"
                );
                return Ok(BuilderClientBootstrapResult {
                    status: BuilderClientBootstrapStatus::AlreadyPresent,
                    client_secret_path: cfg.client_secret_path.clone(),

...
fn a_null_shaped_string_is_a_parameter_and_a_null_node_is_a_keyword() {
    ...
    assert!(as_string.sql().ends_with("VALUES ($1)"), "{}", as_string.sql());
    assert_eq!(
        as_string.params(),
    ...
    assert!(as_node.sql().ends_with("VALUES (NULL)"), "{}", as_node.sql());
    assert!(
...
fn every_placeholder_has_exactly_one_parameter_on_a_write() {
    ...
    let returning =
        Returning::rows(Projection::rows(vec![ProjectedField::column(column("name"))
            .expect("projectable")])
        .expect("row projection"))
        .expect("returning");

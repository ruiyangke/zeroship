...
fn a_vector_search_binds_the_query_once_and_orders_by_the_expression() {
    ...
    assert!(
        sql.sql().contains(r#""embedding" <=> $1::vector AS "_distance""#),
...
fn a_geo_search_bounds_by_st_dwithin_and_ranks_by_st_distance() {
    ...
    assert!(
        sql.sql()
            .contains(r#"ST_Distance("location", ST_MakePoint($1, $2)::geography) AS "_distance_m""#),
        "{}",
        sql.sql()
    );
    assert!(
...
fn all_three_metrics_are_representable_and_postgres_renders_each_distinctly() {
    ...
    for (metric, operator) in expected {
        ...
        .build()
        .expect("every metric must be BUILDABLE; a metric one backend cannot serve \
...
fn a_tiebreak_pins_the_order_of_equal_distances_and_spells_its_null_placement() {
    ...
    assert!(
        sql.sql().contains(
            r#"ORDER BY "embedding" <=> $1::vector ASC, "created_at" DESC NULLS LAST"#
        ),
...
fn a_geo_point_and_radius_refuse_what_is_not_on_the_globe() {
    ...
    assert!(GeoPoint::new(90.0, 180.0).is_ok(), "the poles are on the globe");

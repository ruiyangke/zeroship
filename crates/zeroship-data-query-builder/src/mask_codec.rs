...
/// (`zeroship_data_postgres::pg_introspect` on the `pg_description` body,
/// `zeroship_data_sqlite` on the `/* ... */` marker in `sqlite_master.sql`), and
...
mod cross_codec_parity {
    ...
    /// `zeroship_data_postgres::pg_introspect` decides which sentinel family a
    ...
    /// `zeroship_data_sqlite` walks `sqlite_master.sql` for a `/* <PREFIX>`

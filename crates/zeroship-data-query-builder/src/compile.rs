    ...
    fn creator_timestamp_numeric_filter_bind_is_lowered_per_dialect() {
        ...
        let sqlite =
            build_where_with_dialect(&filter, &mut sqlite_params, &schema, SqlDialect::Sqlite)
                .unwrap();
        assert!(sqlite
            .contains(r#""occurred_at" > strftime('%Y-%m-%dT%H:%M:%fZ', $1/1000.0, 'unixepoch')"#));
        ...
        for name in names {
            ...
            if engine_accepts != schema_accepts || engine_accepts != plan_accepts {
                mismatches.push(format!(
                    "{name:?}: engine={engine_accepts}, runtime-compiler={schema_accepts}, zeroship-data-query-builder={plan_accepts}"
                ));
            }
        }

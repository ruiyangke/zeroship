...
use zeroship_data_orm::sql::{
    ...
    CompareOp, Delete, FieldPath, Ident, IdentRole, Insert, JsonKey, Literal, MAX_INSERT_ROWS,
    MAX_ROW_LIMIT, Operand, PlanError, Predicate, ProjectedField, Projection, Returning, RowLimit,
    Update, WriteError, WriteValue,
};
...
fn sql_of(plan: &zeroship_data_orm::sql::DbPlan) -> String {
    postgres::render(plan).expect("renders").sql().to_string()
}

fn all_three_writes(returning: &Returning) -> Vec<zeroship_data_orm::sql::DbPlan> {
    vec![
        zeroship_data_orm::sql::DbPlan::Insert(insert_one(returning.clone())),
        zeroship_data_orm::sql::DbPlan::Update(update_one(returning.clone())),
        zeroship_data_orm::sql::DbPlan::Delete(delete_one(returning.clone())),
    ]
}
...
fn no_write_renders_a_returning_star() {
    ...
    assert_eq!(
        ruled_on, 3,
        "insert, update and delete must all be ruled on"
    );
...
fn a_returning_list_carries_exactly_the_fields_it_was_given() {
    let sql = sql_of(&zeroship_data_orm::sql::DbPlan::Insert(insert_one(
        returning_name(),
    )));
    let mut ruled_on = 0_usize;

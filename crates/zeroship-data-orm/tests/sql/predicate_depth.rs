...
use zeroship_data_orm::sql::{
    Assignment, ColumnAssignment, CompareOp, Delete, Ident, IdentRole, Literal,
    MAX_PREDICATE_DEPTH, Operand, PlanError, Predicate, PredicateError, ProjectedField, Projection,
    Returning, RowLimit, Select, Update, WriteError,
};
...
fn a_plan_at_the_limit_renders_with_its_full_depth() {
    ...
    let plan = Select::builder(collection(), rows())
        ...
        .build()
        .expect("at the limit");
    let rendered = zeroship_data_orm::sql::render::postgres::render_select(&plan).expect("renders");
...
fn the_ir_bound_matches_the_translator_it_replaces() {
    assert_eq!(
        ...
        "MAX_FILTER_NESTING_DEPTH is 16 at crates/zeroship-data-orm/src/sql/compile.rs; a \

...
/// reason [`crate::sql::LiteralSet`]'s are: the invariant has to travel with the
...
/// [`crate::sql::ProjectionSource::Stored`].
///
...
/// [`crate::sql::render`] rests on and the reason [`crate::sql::Predicate::range`] is a
...
///
/// [`RowLimit`] is required, as it is on [`crate::sql::Select`]. Keeping the bound
...
///
/// See [`crate::sql::render::postgres::render_update`]: the bound is expressed as a
...
/// executable - `crate::sql::plan::SelectBuilder::build` is the other - and the check
...
mod tests {
    ...
    /// lives in `crate::sql::render::postgres`.
    #[test]

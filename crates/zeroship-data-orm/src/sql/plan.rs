//! In-process read, write, and search plans. Plans have no serialization contract;
//! compiler tests enforce that boundary. Migration DDL lives in the migration engine.

use crate::sql::ident::Ident;
use crate::sql::path::FieldPath;
use crate::sql::predicate::{Predicate, MAX_PREDICATE_DEPTH};
use crate::sql::projection::{Projection, ProjectionKind, ProjectionSource};
use crate::sql::Join;
use core::fmt;
...
/// One `ORDER BY` key.
///
/// The operand is a [`FieldPath`], not an [`crate::sql::Operand`], so an aggregate
...
impl SelectBuilder {
    ...
    ///   [`crate::sql::render::postgres::RenderError`] rather than an empty result.
    ///
...
/// [`crate::sql::Predicate::range`] a constructor rather than a node.
///
/// **`search` and `near` likewise share one variant.** They differ in what they
/// rank by, which is a [`crate::sql::SearchCriterion`], not in the statement's
...
/// (`crates/zeroship-data-orm/src/sql/compile.rs` and `:5040`) that had drifted
/// into ordering by two different things - the distance *expression* on one and
/// the output *alias* on the other.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DbPlan {
    Select(Select),
    /// One or more rows, one statement. See [`crate::sql::write::Insert`].
    Insert(crate::sql::write::Insert),
    Update(crate::sql::write::Update),
    Delete(crate::sql::write::Delete),
    /// A ranked search. See [`crate::sql::search::Search`].
    Search(crate::sql::search::Search),
}

...
use crate::sql::literal::{Finite, LiteralError, QueryVector};
use crate::sql::plan::{OrderKey, PlanError, RowLimit};
use crate::sql::predicate::{MAX_PREDICATE_DEPTH, Predicate};
use crate::sql::projection::{Projection, ProjectionError, ProjectionKind, SearchScalarKind};
use core::fmt;
...
/// (`crates/zeroship-data-orm/src/sql/descriptors.rs`).
...
/// (`crates/zeroship-data-orm/src/sql/compile.rs`). A transposed pair inside
...
/// [`crate::sql::ProjectionSource::SearchScalar`], and the ordering is **not a
...
/// (`crates/zeroship-data-orm/src/sql/compile.rs`) while the geo arm orders by the
...
/// (`crates/zeroship-data-orm/src/backend/sqlite/mod.rs` sorts in
...
///
/// A tiebreak carries its own [`crate::sql::NullOrder`], for the reason
/// [`crate::sql::OrderKey`] does: the two backends' defaults differ.

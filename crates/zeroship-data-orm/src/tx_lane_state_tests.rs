...
fn frame_emit_marks_never_discard_an_enclosing_frames_events() {
    ...
    assert_eq!(
        lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_t")),
        None,
    ...
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c1"));
    lanes.discard_frame_effects(&crate::tests::fixtures::harness_route("app_t"));
    assert_eq!(
        lanes
            .by_route()
            .get(&crate::tests::fixtures::harness_route("app_t"))
    ...
    lanes.push_frame_emit_mark(&crate::tests::fixtures::harness_route("app_t"));
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c2"));
    lanes.discard_frame_effects(&crate::tests::fixtures::harness_route("app_t"));
    let kept = lanes.by_route().get(&crate::tests::fixtures::harness_route("app_t")).expect("lane").pending_emits();
    ...
    assert_eq!(lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_t")), Some(1));
    assert_eq!(lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_t")), None);
}
...
fn retiring_a_transaction_drops_its_frame_watermarks() {
    ...
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c1"));
    lanes.push_frame_emit_mark(&crate::tests::fixtures::harness_route("app_t"));
    lanes.retire_transaction(&crate::tests::fixtures::harness_route("app_t"));
    assert_eq!(
        lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_t")),
        None,
...
fn push_pending_emit_allocates_slot_lazily() {
    ...
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c1"));
    assert!(lanes.by_route().contains_key(&crate::tests::fixtures::harness_route("app_t")));
    assert_eq!(
        lanes.by_route().get(&crate::tests::fixtures::harness_route("app_t")).unwrap().pending_emits().len(),
        1
    );
}
...
fn push_pending_emit_accumulates() {
    ...
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c1"));
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c2"));
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c3"));
    let evs = lanes.by_route().get(&crate::tests::fixtures::harness_route("app_t")).unwrap().pending_emits();
    assert_eq!(evs.len(), 3);
...
fn drain_pending_emits_returns_and_clears() {
    ...
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c1"));
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c2"));
    let drained = lanes.drain_pending_emits_for(&crate::tests::fixtures::harness_route("app_t"));
...
fn drain_then_push_starts_fresh() {
    let mut lanes = TxLanes::new();
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c1"));
    let _ = lanes.drain_pending_emits_for(&crate::tests::fixtures::harness_route("app_t"));
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c2"));
    let evs = lanes.by_route().get(&crate::tests::fixtures::harness_route("app_t")).unwrap().pending_emits();
    assert_eq!(evs.len(), 1);
...
fn clear_pending_emits_drops_without_returning() {
    ...
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c1"));
    lanes.push_pending_emit(&crate::tests::fixtures::harness_route("app_t"), dummy_event("c2"));
    lanes.clear_pending_emits_for(&crate::tests::fixtures::harness_route("app_t"));
    assert!(
        lanes
            .by_route()
            .get(&crate::tests::fixtures::harness_route("app_t"))
    ...
    assert!(lanes.drain_pending_emits_for(&crate::tests::fixtures::harness_route("app_t")).is_empty());
}
...
fn sec1_tx_parked_by_app_a_is_invisible_and_untakable_for_app_b() {
    run_async(async {
        ...
        let mut lanes = TxLanes::new();
        let prev = lanes.install_tx_client(&crate::tests::fixtures::harness_route("app_a"), sqlite_tx_conn(&dir).await);
        assert!(prev.is_none(), "tx slot must start empty");

        assert!(
            lanes.has_tx_for(&crate::tests::fixtures::harness_route("app_a")),
            "the owning app must see its own parked tx",
        );
        assert!(
            !lanes.has_tx_for(&crate::tests::fixtures::harness_route("app_b")),
            "SEC-1: app_b must NOT observe app_a's parked tx \
                 (a hit here routes app_b's SQL onto app_a's tx connection)",
        );
        assert!(
            lanes.take_tx_client_for(&crate::tests::fixtures::harness_route("app_b")).is_none(),
        ...
        assert!(
            lanes.take_tx_client_for(&crate::tests::fixtures::harness_route("app_a")).is_some(),
...
fn sec1_frame_watermarks_are_scoped_per_app() {
    run_async(async {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut lanes = TxLanes::new();
        lanes.install_tx_client(&crate::tests::fixtures::harness_route("app_a"), sqlite_tx_conn(&dir).await);

        lanes.push_frame_emit_mark(&crate::tests::fixtures::harness_route("app_a"));
        lanes.push_frame_emit_mark(&crate::tests::fixtures::harness_route("app_a"));
        assert_eq!(
            lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_b")),
            None,
        ...
        lanes.retire_transaction(&crate::tests::fixtures::harness_route("app_b"));
        assert_eq!(
            lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_a")),
            Some(0),
            "SEC-1: app_b's settle must not drop app_a's frame watermarks",
        );
        assert_eq!(lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_a")), Some(0));
        assert_eq!(lanes.pop_frame_emit_mark(&crate::tests::fixtures::harness_route("app_a")), None);
    });
}

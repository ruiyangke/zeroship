...
pub(crate) fn typed_row_to_record(
    columns: &[String],
    row: &[TypedCell],
) -> zeroship_data_sql::value::Map<String, Value> {
...
fn typed_cell_to_value(cell: &TypedCell) -> Value {
    match cell {
        ...
        TypedCell::Integer(n) => Value::Number(zeroship_data_sql::value::Number::from(*n)),
        TypedCell::Real(f) => {
            zeroship_data_sql::value::Number::from_f64(*f).map_or(Value::Null, Value::Number)
        }

...
fn mask_date_year(plaintext: &str) -> String {
    ...
    if bytes.len() < 10 || !bytes[0..4].iter().all(|b| b.is_ascii_digit()) || bytes[4] != b'-' {
        return "***".to_string();
    }
...
mod tests {
    ...
    fn last4_preserves_last_four_alphanumerics() {
        assert_eq!(
            apply_mask_kind(MaskKind::Last4, "123-45-6789"),
            "***-**-6789"
        );
    ...
    fn date_year_preserves_year() {
        assert_eq!(
            apply_mask_kind(MaskKind::DateYear, "1985-04-12"),
            "1985-**-**"
        );
        assert_eq!(
            apply_mask_kind(MaskKind::DateYear, "2026-12-31"),
            "2026-**-**"
        );
    }

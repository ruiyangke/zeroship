...
///
/// THE ONLY NUMBER IN THIS BINARY, and it counts FILES, not findings. 21 gate
/// scripts existed on 2026-08-20; gates are added and deleted by hand, so a
/// drop is a decision somebody made and should record here in the same commit,
/// not an accident to be absorbed. Set at the observed count on purpose: a
/// slack floor here would let the glob half-break unnoticed, which is the
/// precise failure this binary exists to catch one level down.
const GATE_FILE_FLOOR: usize = 21;

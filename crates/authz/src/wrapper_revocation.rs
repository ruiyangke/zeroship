//! Shared signed-token family revocation marker.
//!
//! **`token_revocations`** is the shared token-family revocation primitive
...
// Family marker (`zeroship.token_revocations`).
//
...
/// rejected from here on. The client id namespaces the subject, so a marker
/// for one client does not affect another client.
...
/// Bounded, short-TTL, read-through cache of token-family revocation
/// state, keyed on `(client_id, sub)`.
///
/// Process-local. Stores the family's latest `revoked_after` so the hot path
/// can re-judge `> iat` locally on a hit without a DB round-trip. See the
/// module-level R1d notes for the staleness bound and the negative-caching
/// rationale.
#[derive(Debug)]

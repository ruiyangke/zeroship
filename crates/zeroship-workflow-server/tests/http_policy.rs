...
impl Fixture {
    async fn new(policy: AppPolicy, configured: bool) -> Self {
        ...
        platform.admin.execute("INSERT INTO zeroship.worker_instances(id,ring_key,public_key,advertise_host,advertise_port,status) VALUES($1,$2,$3,'127.0.0.1',8080,'active')", &[&worker.as_str(), &vec![1_u8], &signer.key.verifying_key_bytes().to_vec()]).await.unwrap();
        let service =

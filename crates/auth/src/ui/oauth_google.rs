...
use crate::ui::oauth_stash::{
    clear_google_stash_cookie, parse_google_stash_cookie, set_google_stash_cookie, OAuthStash,
};
...
pub async fn start(
    ...
    resp.header(
        SET_COOKIE,
        set_google_stash_cookie(&cookie_value, cfg.insecure_dev),
    );
    resp.finish()
}
...
pub async fn callback(
    ...
    let cookie_header = req
        ...
        .unwrap_or("");
    let Some(stash_blob) = parse_google_stash_cookie(cookie_header) else {
        audit::emit(
            db.as_ref(),
            &AuditEvent {
    ...
    resp.header(SET_COOKIE, clear_google_stash_cookie(cfg.insecure_dev));
    resp.finish()
}

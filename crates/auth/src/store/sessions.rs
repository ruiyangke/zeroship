//! `zeroship.sessions` CRUD — the `IdP` login session at `auth.zeroship.ai`.
    ...
    let rows = conn
        .query(
            "UPDATE zeroship.sessions \
             ...
             FROM zeroship.users \
             WHERE zeroship.sessions.id = $1 \
               AND zeroship.users.id = zeroship.sessions.user_id \
               AND zeroship.sessions.credential_version = zeroship.users.credential_version \
               AND zeroship.sessions.revoked_at IS NULL \
               AND zeroship.sessions.idle_expires_at > NOW() \
               AND zeroship.sessions.abs_expires_at > NOW() \
             RETURNING zeroship.sessions.id, zeroship.sessions.user_id, auth_method, amr, acr, \
                       zeroship.sessions.credential_version, idle_expires_at, abs_expires_at",
            &[
                &id,
...
pub async fn revoke(conn: &Client, id: uuid::Uuid) -> Result<()> {
    conn.execute(
        "UPDATE zeroship.sessions SET revoked_at = NOW() WHERE id = $1",
        &[&id],
    )
    .await

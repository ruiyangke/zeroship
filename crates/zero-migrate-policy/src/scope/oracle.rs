...
//!
//! - the meet is EXACT: `Objects(meet(a, b))` equals the intersection of
//!   `Objects(a)` and `Objects(b)`.
//! - `a.subset(b)` is true exactly when `Objects(a)` is contained in `Objects(b)`.
//! - the join covers at least the union: `Objects(join(a, b))` contains every
//!   object in `Objects(a)` or `Objects(b)` - a sanctioned over-approximation.
//! - the difference covers at least `Objects(a)` minus `Objects(b)`, or is
//!   rejected outright - NEVER a strict subset (the escalation direction is
//!   provably impossible).
...
/// The bounded universe of concrete object names: every schema object and every
/// schema.table object over short-segment names. (Kept small so the property
/// sweep, which is quadratic in the scope count and linear in the universe size,
/// stays fast.)
...
/// corners of infix-against-infix (`a*a` against itself, `a_*` against `*_a`)
/// first appear.
...
fn pattern_pool() -> Vec<Pattern> {
    ...
    // two-segment patterns (a subset - schema glob x table glob)
    for sg in &glob_pool {
        for tg in &glob_pool {
...
fn oracle_lattice_properties_hold_over_bounded_universe() {
    ...
    for (i, a) in scopes.iter().enumerate() {
        ...
        for (j, b) in scopes.iter().enumerate() {
            ...
            // -- subset() agrees with set containment --------------------------
            let sub = a.subset(b);
            ...
            // -- join covers at least the union (over-approx sanctioned) -------
            let join = a.join(b);
            ...
            // -- difference covers at least the set subtraction, OR rejects -
...
/// The per-segment glob-intersection primitive, proven EXACT over the full glob
...
/// Helper: the EXACT per-segment intersection result as a set of rendered strings.
...
#[test]
fn pinned_a_star_x() {
    // a_* intersected with *_x  ->  { a_x, a_*_x }
    assert_eq!(
...
fn pinned_a_a_shared_overlap() {
    // [H2 FIX] a_a_* intersected with *_a_a. Prefix p = "a_a_", suffix s = "_a_a",
    // base = |p|+|s| = 8, floor = max(4,4) = 4. Overlaps o (p's last o bytes ==
    // s's first o bytes), witness length = base - o:
    //   o=1: p[-1..]="_" == s[..1]="_" MATCH -> "a_a_" then s[1..]="a_a" = "a_a_a_a" (len 7)
    //   o=2: p[-2..]="a_" != s[..2]="_a" - no witness
    //   o=3: p[-3..]="_a_" == s[..3]="_a_" MATCH -> "a_a_" then s[3..]="a" = "a_a_a" (len 5)
    //   o=4: p[-4..]="a_a_" != s[..4]="_a_a" - no witness
...
fn pinned_a_star_a_excludes_a() {
    // [H1] a*a intersected with itself  ->  { a*a }, and must NOT contain "a".
    ...
    // And idempotence: g intersected with itself denotes exactly Objects(g).
...
fn pinned_cross_arity_app_events() {
    // app_* intersected with *.events  ->  app_*.events  (cross-arity: app_* -> app_*.*)
...
fn pinned_disjoint_meet_is_nothing() {
    // staging meet analytics == Nothing (disjoint proper scopes meet to the empty
    // scope, never the universe).
...
fn pinned_default_meet_disjoint_rule_is_nothing() {
    // meet(default_scope, disjoint-rule) == Nothing (the default-scope narrowing case).
...
fn pinned_difference_exclude_counterexample_stays_reject() {
    ...
    // A minus B must be NON-EMPTY (it contains app_tmp_* objects), so the strict
    ...
    // Also the subset direction: A is NOT within B (A would grant app_tmp_* that
    // B forbids).
    assert!(
        !a.subset(&b),

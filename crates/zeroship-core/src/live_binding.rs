...
//! epoch, one wants the database, one wants only existence. What they must
//! agree on is the FROM and the WHERE, so that is what is spelled once. `$1` is
//! always the app id, bound as `text`; a caller adding its own conjunct appends
//! it and numbers its placeholders from `$2`.
//!
...
///
/// The reader here is the worker version feed, which carries each app's binding
/// epochs so a worker can tell that an apply rotated the role a resident
/// isolate narrows with. It projects `b.app_id` and groups in the caller: one
/// statement per app would be one round trip per app on every poll.

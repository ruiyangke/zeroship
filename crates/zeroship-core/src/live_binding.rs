...
//! whole statement because the three readers project different columns: one
//! wants the epoch, one wants the database, one wants only existence. What
//! they must agree on is the FROM and the WHERE, so that is what is spelled
//! once. `$1` is always the app id, bound as `text`; a caller adding its own
//! conjunct appends it and numbers its placeholders from `$2`.
    ...
    fn the_predicate_carries_all_three_liveness_conjuncts() {
        for conjunct in [
            ...
            assert!(
                ...
                "the live-binding predicate must carry {conjunct}: \
                 {LIVE_BINDINGS_FROM_WHERE}"
            );
        }
    }

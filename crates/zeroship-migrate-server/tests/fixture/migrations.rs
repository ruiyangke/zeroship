...
pub(super) fn apply(url: &str) {
    ...
    let mut child = OwnedChild(
        command
            ...
            .spawn()
            .expect("migration fixtures require Node; run through nix develop"),
    );
    ...
    if !status.success() {
        ...
        let mut last_lines = stdout.lines().rev().take(4).collect::<Vec<_>>();
        last_lines.reverse();
        panic!(
...
fn root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("migration-server crate lives under crates/")
        .to_owned()
}

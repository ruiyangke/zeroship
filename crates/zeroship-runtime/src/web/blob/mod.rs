...
//!
//! Blob's implementation lives directly in this file (rather than a
//! nested `blob` submodule) to avoid `clippy::module_inception` (a
//! submodule sharing its parent module's name) — see `file.rs` for
//! the `File` class. This module root also exposes the install entry
//! point used by `init.rs::setup_globals`.
...
fn blob_stream_start_callback(
    ...
    if let Some(enqueue_v) = controller_obj.get(scope, enqueue_key.into())
        && let Ok(enqueue_fn) = v8::Local::<v8::Function>::try_from(enqueue_v)
    {
    ...
    if let Some(close_v) = controller_obj.get(scope, close_key.into())
        && let Ok(close_fn) = v8::Local::<v8::Function>::try_from(close_v)
    {

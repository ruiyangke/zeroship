...
/// the Rust runtime to snapshot, and supplies the optional index fallback.
...
fn host_entry_js(entry: &str) -> Result<String, String> {
    ...
    let prefix = format!("import * as user from {entry};\n");
...
pub(crate) struct PreparedApplication {
    ...
    /// The creator's own entry module, retained so host dispatch can read the
    /// namespace the host entry imported rather than evaluating it a second
    /// time. `None` only when there is no creator module to compile.
    pub creator_entry: Option<v8::Global<v8::Module>>,
...
pub(crate) fn prepare_application(
  ...
  if (!g.setImmediate) {
    ...
    // The host entry imports the creator entry, so the compiled graph already
    // holds it. Read it back from the registry rather than compiling a second
    // copy: host dispatch and request dispatch must see one module instance.
    let creator_entry = crate::modules::registered_module(scope, modules.first());
    ...
    Ok(PreparedApplication { entry, creator_entry, descriptor: runtime_descriptor, promises })
}
...
fn wrap_with_host_entry(
    ...
    if modules.iter().any(|entry| entry.specifier == host_spec) {

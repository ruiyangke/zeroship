...
use crate::backfill::{BackfillProgressEntry, BackfillSpec};
...
use crate::journal::{AppliedEntry, HistoryEvent};
use crate::requirements::DatabaseRequirements;
...
pub trait MigrationBackend {
    ...
    async fn rollback_plan_transactional(
        ...
        cfg: &ExecutorConfig,
        forward: &Migration,
        inverse_steps: &[crate::step::PlanStep],
        applied_by: &str,
    ) -> Result<(), RollbackError>;
    ...
    /// the engine's `apply::plan_precondition`'s plan-wide phase.
    ///
    ...
    /// PG impl delegates to the PostgreSQL backend's `journal_sql::record_baseline` with
    ...
    /// failure; the dialect-neutral [`crate::executor::ApplyError`] is
    ...
    /// [`PlanStep::Backfill`](crate::step::PlanStep::Backfill)) through the
    ...
    /// engine's `MigrationEngine::apply_plan` gate runs

...
by `zeroship_data_engine::orm`; applications use that entry point.
...
bounds, and attribute diagnostics. The crate has no driver or runtime dependency.

Runtime semantics live in `zeroship-data-engine`, so handwritten mappings and
macro-generated mappings use the same codecs and execution path. Usage is in
`crates/zeroship-data-engine/README.md`; compilation contracts and database round

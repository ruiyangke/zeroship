//! Restore gateway cases from this process's actual platform migrations.
...
impl Seed {
    pub(super) fn build() -> Self {
        eprintln!("gateway fixture: preparing the platform database seed");
        let postgres = image()
            .with_env_var("POSTGRES_DB", "gateway_tests")
            .start()
            .expect("prepare the migrated gateway database seed");
        apply_migrations(database_url(&postgres).as_str());
        eprintln!("gateway fixture: platform migrations applied; capturing the seed");
        let roles = dump(
            &postgres,
...
fn root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("gateway crate lives under crates/")
        .to_owned()
}
...
fn apply_migrations(url: &str) {
    ...
    let mut child = OwnedChild(
        command
            ...
            .spawn()
            .expect("gateway database fixtures require Node; run through nix develop"),
    );

...
async fn apply_encryption_on_write(
    ...
    let has_sqlite_binary =
        current_sql_dialect() == query::SqlDialect::Sqlite && schema_has_sqlite_binary_columns(&schema);
    if !has_enc && !has_mask && !has_sqlite_binary {
        return Ok(());
    }
    ...
    if current_sql_dialect() == query::SqlDialect::Sqlite {
        encode_sqlite_binary_doc_with_schema(&schema, doc)?;
    }
    Ok(())
}
...
async fn apply_encryption_on_update(
    ...
    if current_sql_dialect() == query::SqlDialect::Sqlite {
        encode_sqlite_binary_update_with_schema(&schema, patch)?;
    }
    Ok(())
}

...
if [ ${#missing_packages[@]} -gt 0 ]; then
  echo "[publish-packages] publish list names ${#missing_packages[@]} package(s) that do not exist:" >&2
  printf '  %s/package.json\n' "${missing_packages[@]}" >&2
  echo "[publish-packages] the tree moved and this list did not; fix the list" >&2
  exit 1
fi
...
if (bad.length > 0) {
  console.error(`[publish-packages] ${bad.length} dependency(ies) on workspace packages that are not published:`);
  for (const line of bad) console.error(`  ${line}`);
  console.error("[publish-packages] pnpm pack will rewrite these to concrete versions that resolve nowhere");
  process.exit(1);
}
...
echo "[publish-packages] registry: $REGISTRY"
echo "[publish-packages] packages: ${publish_packages[*]}"
echo "[publish-packages] skipped: packages/zeroship-stub (runtime/test shim for the virtual \"zeroship\" module)"

npm ping --registry "$REGISTRY" "${NPM_USERCONFIG_ARGS[@]}" >/dev/null
publisher="$(npm whoami --registry "$REGISTRY" "${NPM_USERCONFIG_ARGS[@]}")"
echo "[publish-packages] authenticated as: $publisher"

echo "[publish-packages] building workspace packages"
